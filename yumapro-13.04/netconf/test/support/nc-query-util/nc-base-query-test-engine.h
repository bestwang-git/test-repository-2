...
protected:
    ...
    {
        FILE* fp;
        fp = session->logQueryStr(queryStr);
        BOOST_TEST_MESSAGE( "open log for query string:\n" << fp );
       ...
       // log the Result
        fp = session->logQueryResult(queryResult);
        BOOST_TEST_MESSAGE( "open log for query result string:\n" << fp );
