                  ...
                  ses_msg_buff_t *buff)
{
    ...
    if (scb->framing11) {
        ...
        if (LOGDEBUG2) {
            log_debug2("\nses_msg: send 1.1 buff:%zd for s:%u\n",
                       buff->bufflen - buff->buffpos, scb->sid);
            if (LOGDEBUG3) {
                trace_buff(buff);
            }
        }
        ...
        if (buff->bufflen > 0) {
            ...
            log_debug2("\nses_msg: skip sending empty 1.1 buffer on s:%d",
                       scb->sid);
        }
    } else {
        if (buff->bufflen > buff->buffstart) {
            ...
            if (LOGDEBUG2) {
                log_debug("\nses_msg: send 1.0 buff:%zd for s:%u\n",
                          buff->bufflen - buff->buffpos, scb->sid);
                if (LOGDEBUG3) {
                    trace_buff(buff);
                }
            }
            res = send_buff(scb->fd, (const char *)buff->buff, 
                            buff->bufflen);
        } else if (LOGDEBUG2) {
            log_debug2("\nses_msg: skip sending empty 1.0 buffer on s:%d",
                       scb->sid);
        }
    }
            ...
            *buff = newbuff;
            ...
            if (LOGDEBUG4) {
                log_debug4("\nses_msg: reused %s buff %p for s %u", 
                           (outbuff) ? "out" : "in", newbuff, scb->sid);
            }
    ...
    ses_msg_send_buffs (ses_cb_t *scb)
{
    ...
    if (retcnt < 0) {
        ...
        if (LOGDEBUG2) {
            log_debug2("\nses wrote %zd of %d bytes on session %d\n", 
                       retcnt, 
                       total, 
                       scb->sid);
        }
...
void ses_msg_dump (const ses_msg_t *msg, const xmlChar *text)
{
    ...
    for (buff = (const ses_msg_buff_t *)dlq_firstEntry(&msg->buffQ);
      ...
      /* Accomodate the new logger. Output first char as "main line"
       * (this serves to anchor header info if configured). Then
       * APPEND all additional characters in a tight loop. */

        i = buff->buffstart;
	if (i < buff->bufflen) {
	    log_write("%c", buff->buff[i]); /* Output the first char only */
	    i++;
	}
        for (; i < buff->bufflen; i++) {
	    log_write_append("%c", buff->buff[i]); /* Append everything else */
        }
