...
/* #define VAL_MEM_DEBUG 1 */
/* #define VAL_WALKER_DEBUG 1 */
/* #define VAL_CACHE_DEBUG 1 */
#endif

#define PREFIX_BUFF_SIZE  64
                ...
                const ncx_num_t *num)
{
    ...
    uint32 len = 0;
    status_t res = ncx_sprintf_num(numbuff, num, btyp, &len);
    if (res != NO_ERR) {
    ...
    dump_extern (const xmlChar *fname)
{
    ...
    FILE *fil = fopen((const char *)fname, "r");
    if (!fil) {
...
{
    int ret = xmlRegexpExec(pattern, strval);
    if (ret==1) {
        return TRUE;
    } else if (ret==0) {
        return FALSE;
                 ...
                 boolean full)
{
    ...
    ncx_btype_t btyp = val->btyp;
    val_value_t *cur = NULL;
    ...
    while (!dlq_empty(&val->indexQ)) {
        val_index_t *in = (val_index_t *)dlq_deque(&val->indexQ);
        m__free(in);
    }

    while (!dlq_empty(&val->dataruleQ)) {
        ncx_backptr_t *ptr = (ncx_backptr_t *)dlq_deque(&val->dataruleQ);
        ncx_free_backptr(ptr);
    }
...
{
    boolean fastpath = ncx_fastpath_enabled();

    /* get the first pair of index nodes to check */
    const val_index_t *c1 = (val_index_t *)dlq_firstEntry(&val1->indexQ);
    const val_index_t *c2 = (val_index_t *)dlq_firstEntry(&val2->indexQ);
...
#ifdef VAL_WALKER_DEBUG
    log_debug3("\nval: walker processing %s:%s for",
               val_get_mod_name(val), val->name);
    if (modname) {
        log_debug3_append(" modname=%s", modname);
    }
    if (name) {
        log_debug3_append(" name=%s", name);
    }
    if (modname==NULL && name==NULL) {
        log_debug3_append(" *");        
    }
    if (configonly) {
        log_debug3_append(" config-only");        
    }
    if (textmode) {
        log_debug3_append(" text-mode");        
    }
#endif

    boolean fnresult = TRUE;
    if (textmode) {
                          ...
                          val_value_t *realval)
{
    ...
    if (!typ_is_simple(virval->btyp)) {
        ...
        typ_template_t *listtyp = typ_get_listtyp(realval->typdef);
        ncx_btype_t btyp = typ_get_basetype(&listtyp->typdef);
                   ...
                   val_value_t *copy)
{
    ...
    if (val->editvars) {
        ...
        copy->editvars->insert_mode = val->editvars->insert_mode;
                         ...
                         status_t *res)
{
...
#ifdef VAL_CACHE_DEBUG
    if (LOGDEBUG4) {
        log_debug4("\nval: caching virtual val %s:%s",
                   val_get_mod_name(val), val->name);
    }
#endif

    getcb_fn_t getcb = (getcb_fn_t)val->getcb;
    ...
    if (val->virtualval != NULL) {
        ...
        double timediff = difftime(timenow, val->cachetime);
        double timerval = 0;
        boolean disable_cache = FALSE;
        ...
        if (scb != NULL) {
            ...
            uint32 deftimeout = ncx_get_vtimeout_value();
...
#ifdef VAL_CACHE_DEBUG
            if (LOGDEBUG4) {
                log_debug4("\nval: refresh virtual val %s", val->name);
            }
#endif
            val_free_value(val->virtualval);
            val->virtualval = NULL;
...
#ifdef VAL_CACHE_DEBUG
        if (LOGDEBUG4) {
            log_debug4("\nval: reusing cached virtual val");
        }
#endif
...
#ifdef VAL_CACHE_DEBUG
        if (LOGDEBUG4) {
            log_debug4("\nval: getting value for virtual cache");
        }
#endif
                ...
                status_t *res)
{
    ...
    copy->last_modified = val->last_modified;
    copy->etag = val->etag;
    ...
    /* copy any active NACM data rules;
     * this should be empty for candidate or PDU source
     * vals, but in case a copy of running is made, this
     * array of data rule back pointers needs to be transferred
     */
    for (i=0; i<VAL_MAX_DATARULES; i++) {
        copy->datarules[i] = val->datarules[i];
    }
...
/********************************************************************
 * Check if value is a child of a ywx:cli-text-block
 *
 * \param val the value node to check
 * \return TRUE if value parent is marked as ywx:cli-text-block
 *********************************************************************/
static boolean in_cli_text_block (const val_value_t *val)
{
    if (val->obj == NULL) {
        return FALSE;
    }

    if (obj_is_leaf(val->obj) &&
        val->parent && val->parent->obj &&
        obj_is_cli_text_block(val->parent->obj)) {
        return TRUE;
    }
...
/********************************************************************
 * Get the next value node to compare
 *
 * \param val the value node to check
 * \param configonly == TRUE if configuration only; FALSE all nodes
 * \return this paraam or the next valid one
 *********************************************************************/
static const val_value_t *
    get_next_node (const val_value_t *val,
                   boolean configonly)
{
    boolean done = FALSE;
    while (val && !done) {
        ...
        if (VAL_IS_DELETED(val)) {
            skip =  TRUE;
        } else if (configonly && val->obj && !obj_get_config_flag(val->obj)) {
            skip = TRUE;
        } else {
            done = TRUE;
        }
...
* FUNCTION set_stamps
* 
* Set the timestamp and etag for a commit on the descendant-or-self
...
*********************************************************************/
static void
    set_stamps (val_value_t *val,
                time_t *timestamp,
                ncx_transaction_id_t txid)
{
    val->last_modified = *timestamp;
    val->etag = txid;

    val_value_t *child = val_get_first_child(val);
    for (; child; child = val_get_next_child(child)) {
        if (obj_is_config(child->obj)) {
            set_stamps(child, timestamp, txid);
        }
    }
...
* 
* Undo val_set_by_default 
* INPUTS:
*     val == value node to change
*********************************************************************/
static void
    clear_default (val_value_t *val)
{
...
status_t
    val_string_ok (typ_def_t *typdef,
                   ...
                   const xmlChar *strval)
{
    return val_string_ok_ex(NULL, typdef, btyp, strval, NULL, TRUE);
...
{
    return val_string_ok_ex(NULL, typdef, btyp, strval, errinfo, TRUE);
}
...
*********************************************************************/
status_t
    val_string_ok_ex (struct ses_cb_t_ *scb,
                      typ_def_t *typdef,
                      ncx_btype_t btyp,
                      const xmlChar *strval,
                      ncx_errinfo_t **errinfo,
                      boolean logerrors)
{
    ...
    case NCX_BT_INSTANCE_ID:
        ...
        /* this may be called while parsing an XML doc */
        if (scb) {
            xpathpcb->reader = scb->reader;
        }
...
{
    assert(typdef && "typdef is NULL!");
    assert(qname && "qname is NULL!");

    if (typ_get_basetype(typdef) != NCX_BT_IDREF) {
        return ERR_NCX_WRONG_TYPE;
    }

    const typ_idref_t *idref = typ_get_cidref(typdef);
    if (!idref) {
        return SET_ERROR(ERR_INTERNAL_VAL);
    }

    /* find the local-name in the prefix:local-name combo */
    const xmlChar *str = qname;
    while (*str && *str != ':') {
        str++;
    }
    if (*str == ':') {
    ...
    const ncx_identity_t *identity = NULL;
                     ...
                     const ncx_identity_t **id)
{
    ...
    if (*str == ':') {
        /* prefix found */
        uint32 prefixlen = (uint32)(str++ - qname);
        xmlChar prebuff[PREFIX_BUFF_SIZE];
        xmlChar *usebuff = prebuff;
        xmlChar *prefixbuff = NULL;
        if (prefixlen >= PREFIX_BUFF_SIZE) {
        ...
        xml_strncpy(usebuff, qname, prefixlen);

        ncx_module_t *impmod = NULL;

        if (mod) {
            if (xml_strcmp(mod->prefix, usebuff)) {
                const ncx_import_t *import =
                    ncx_find_pre_import(mod, usebuff);
                if (import) {
            ...
            prefixnsid = xmlns_find_ns_by_prefix(usebuff);
            if (prefixnsid) {
                const xmlChar *modname = xmlns_get_module(prefixnsid);
                if (modname) {
                       ...
                       boolean logerrors)
{
    ...
    case NCX_BT_STRING:
    case NCX_BT_BINARY:
        res = val_string_ok_ex(NULL, typdef, btyp, simval,
                               errinfo, logerrors);
        break;
    case NCX_BT_INSTANCE_ID:
        res = val_string_ok_ex(NULL, typdef, btyp, simval, errinfo, logerrors);
        break;
    case NCX_BT_UNION:
        {
            val_value_t *unval = val_new_value();
            if (!unval) {
                res = ERR_INTERNAL_MEM;
            } else {
                unval->btyp = NCX_BT_UNION;
                unval->typdef = typdef;
                res = val_union_ok_ex(typdef, simval, unval, 
                                      errinfo, mod, NULL);
    ...
    case NCX_BT_LEAFREF:
        {
             ...
             */
            typ_def_t *realtypdef = typ_get_xref_typdef(typdef);
            if (realtypdef) {
                res = val_simval_ok_max(realtypdef, simval, errinfo,
                                        mod, logerrors);
            } else {
                res = SET_ERROR(ERR_INTERNAL_VAL);
            }
    ...
    case NCX_BT_SLIST:
        {
            typ_template_t *listtyp = typ_get_listtyp(typdef);
            ncx_btype_t listbtyp = typ_get_basetype(&listtyp->typdef);
                  ...
                  val_value_t *retval)
{
    ...
    return val_union_ok_ex(typdef, strval, retval, NULL, NULL, NULL);
...
status_t
    val_union_ok_errinfo (typ_def_t *typdef,
                          ...
                          ncx_errinfo_t **errinfo)
{
    return val_union_ok_ex(typdef, strval, retval, errinfo, NULL, NULL);
...
*    typdef == typ_def_t for the designated union type
*    strval == the value to check against the member typ defs
*            (may be NULL for empty string)
*    retval == pointer to output struct for converted value
*    errinfo == address of error struct (may be NULL)
*    mod == module in progress, if any (may be NULL)
*    match_typdef == address of return matching typdef (may be NULL)
* OUTPUTS:
*   If return NO_ERR:
*   retval->str or retval->num will be set with the converted value
*   *errinfo == error struct on error exit
                     ...
                     typ_def_t **match_typdef)
{
    ...
    while (!done) {
        ...
        ncx_btype_t testbtyp = typ_get_basetype(undef);
        if (testbtyp == NCX_BT_UNION) {
            res = val_union_ok_ex(undef, strval, retval, errinfo, 
                                  mod, match_typdef);
        } else {
...
void
    val_dump_value (val_value_t *val,
                    int32 startindent)
{
    val_dump_value_full(val, startindent, NCX_DEF_INDENT,
                       DUMP_VAL_LOG, ncx_get_display_mode(),
                        FALSE, FALSE, FALSE);
...
{
    val_dump_value_full(val, startindent, NCX_DEF_INDENT,
                        DUMP_VAL_ALT_LOG, ncx_get_display_mode(),
                        FALSE, FALSE, FALSE);
...
{
    val_dump_value_full(val, startindent, NCX_DEF_INDENT,
                        DUMP_VAL_STDOUT, ncx_get_display_mode(),
                        FALSE, FALSE, FALSE);
...
void
    val_dump_value_max (val_value_t *val,
                        ...
                        boolean configonly)
{
    val_dump_value_full(val, startindent, indent_amount,
                        dumpmode, display_mode,
                        with_meta, configonly, FALSE);
}  /* val_dump_value_max */



/********************************************************************
* FUNCTION val_dump_value_full
* 
* Printf the specified val_value_t struct to 
* the logfile, or stdout if none set
* Uses conf file format (see ncx/conf.h)
...
*********************************************************************/
void
    val_dump_value_full (val_value_t *val,
                         int32 startindent,
                         int32 indent_amount,
                         val_dumpvalue_mode_t dumpmode,
                         ncx_display_mode_t display_mode,
                         ...
                         boolean conf_mode)
{
    ...
    if (btyp == NCX_BT_NONE) {
        /* this is an edit node marker or an internal error */
        return;
    }
    ...
    if (conf_mode && val->obj && obj_is_key(val->obj)) {
        return;
    }
    ...
    if (display_mode == NCX_DISPLAY_MODE_XML ||
        ...
        res = json_wr_check_open_file(outputfile, val, startindent,
                                      indent_amount, NULL);
        if (res != NO_ERR) {
    ...
    dumpfn_t dumpfn = NULL;
    dumpfn_t errorfn = NULL;
    indentfn_t indentfn = NULL;
    ...
    case DUMP_VAL_STDOUT:
        ...
        break;
    case DUMP_VAL_LOG:
        dumpfn = log_write_append;
        errorfn = log_error;
	indentfn = log_indent_append;
        break;
    ...
    if (in_cli_text_block(val)) {
        ;   // do not display name
    } else if (display_mode == NCX_DISPLAY_MODE_PLAIN) {
        ...
        if (val->btyp == NCX_BT_EXTERN) {
            (*dumpfn)("%s:%s (extern=%s) ", prefix,
                      (val->name) ? (const char *)val->name : "--",
                      (val->v.fname) ? (const char *)val->v.fname : "--");
        } else if (val->btyp == NCX_BT_INTERN) {
            (*dumpfn)("%s:%s (intern) ", prefix,
                      (val->name) ? (const char *)val->name : "--");
        } else if (dumpmode == DUMP_VAL_ALT_LOG &&
                   !xml_strcmp(val->name, NCX_EL_DATA)) {
            ;  /* skip the name */
        } else {
            /* do not print the prefix if the parent is the same
             * as this node
             */
            if (val->parent && 
                val_get_nsid(val->parent) == val_get_nsid(val)) {
                (*dumpfn)("%s ", (val->name) ? (const char *)val->name : "--");
            } else {
                (*dumpfn)("%s:%s ", prefix,
                          (val->name) ? (const char *)val->name : "--");
    ...
    if (!dlq_empty(&val->indexQ)) {
        uint32 len = 0;
        res = val_get_index_string(NULL, NCX_IFMT_CLI, conf_mode, val,
                                   NULL, &len);
        if (res == NO_ERR) {
            xmlChar *buff = m__getMem(len+1);
            if (buff) {
                res = val_get_index_string(NULL, NCX_IFMT_CLI, conf_mode,
                                           val, buff, &len);
                if (res == NO_ERR) {
                    (dumpfn)("%s ", buff);
                } else {
    ...
    val_idref_t *idref = NULL;
    int32 bump_amount = max(0, indent_amount);
    ...
    case NCX_BT_ENUM:
        if (val->v.enu.name) {
            if (conf_mode || val_need_quotes(val->v.enu.name)) {
    ...
    case NCX_BT_LEAFREF:
        ...
        if (VAL_STR(val)) {
            ...
            /* force unquoted string if this is
             * a string variant for an anyxml node
             * also if this is a cli-text-block string
             */
            if (in_cli_text_block(val) ||
                (dumpmode == DUMP_VAL_ALT_LOG &&
                 !xml_strcmp(val->name, NCX_EL_DATA))) {
                quotes = FALSE;
            } else {
                /* in config-file write mode, force quotes on strings
                 * because a string like 37hello$mac will be parsed
                 * incorrectly as an invalid number
                 */
                if (conf_mode) {
                    quotes = TRUE;
                }
            ...
            if (!conf_mode && val->obj && obj_is_password(val->obj)) {
    ...
    case NCX_BT_BITS:
        if (dlq_empty(&val->v.list.memQ)) {
            ...
            boolean sp_done = FALSE;
            ncx_btype_t lbtyp = val->v.list.btyp;

            (*dumpfn)("\"");

            uint32 listlen = ncx_list_linelen(&val->v.list);
            // !!! get config value instead of 80 !!!
            boolean fits = (listlen < 80 || conf_mode || startindent < 0);
    ...
    case NCX_BT_CASE:    // should not happen
        if (val->obj && obj_is_cli_text_block(val->obj)) {
            (*dumpfn)("[");
        } else {
            (*dumpfn)("{");
        }
        ...
        for (chval = (val_value_t *)dlq_firstEntry(&val->v.childQ);
            ...
            val_dump_value_full(chval, startindent+bump_amount,
                                indent_amount, dumpmode, display_mode,
                                with_meta, configonly, conf_mode);
        }
    ...
    if (with_meta) {
        dlq_hdr_t *metaQ = val_get_metaQ(val);
        if (metaQ && !dlq_empty(metaQ)) {
            ...
            for (metaval = val_get_first_meta(metaQ);
                ...
                val_dump_value_full(metaval, startindent+(2*bump_amount),
                                    indent_amount, dumpmode, display_mode,
                                    with_meta, configonly, conf_mode);
            }
        }
                    ...
                    const xmlChar *valstr)
{
    ...
    return val_set_simval_str(val, typdef, nsid, valname, 
                              valname ? xml_strlen(valname) : 0, valstr);
                        ...
                        const xmlChar *valstr)
{
    ...
    xpath_source_t xpath_source = XP_SRC_INSTANCEID;
    ...
    case NCX_BT_DECIMAL64:
        if (valstr && *valstr) {
            res = ncx_convert_dec64(valstr, NCX_NF_NONE, 
            ...
            if (!VAL_STR(val)) {
                ...
                if (!xpathpcb) {
                    ...
                    res = xpath1_parse_expr(NULL, 
                                            ...
                                            XP_SRC_YANG);
                    if (res == NO_ERR) {
                        obj_template_t *objroot = ncx_get_gen_root();
                        res = xpath1_validate_expr(objroot->tkerr.mod, 
                                                   objroot, 
                                                   xpathpcb);
                    }
    ...
    case NCX_BT_INSTANCE_ID:
        ...
        if (!VAL_STR(val)) {
            ...
            if (!xpathpcb) {
                ...
                res = xpath_yang_parse_path(NULL, NULL, xpath_source,
                                            xpathpcb);
                if (res == NO_ERR) {
                    obj_template_t *leafobj = NULL;
        ...
        break;
    case NCX_BT_IDREF:
        if (valstr == NULL || *valstr == 0) {
            res = ERR_NCX_INVALID_VALUE;
        } else {
            const xmlChar *localname = NULL;
            const ncx_identity_t *identity = NULL;
            xmlns_id_t qname_nsid = 0;
    ...
    case NCX_BT_ENUM:
        if (valstr == NULL || *valstr == 0) {
            ...
            const xmlChar *retname = NULL;
            int32 retval = 0;
            res = val_enum_ok(typdef, valstr, &retval, &retname);
            if (res == NO_ERR) {
                res = ncx_set_enum(retname, retval, &val->v.enu);
            }
        }
        break;
    case NCX_BT_EMPTY:
                     ...
                     status_t  *res)
{
    ...
    if (valname) {
        *res = val_set_simval_str(val, typdef, nsid, valname, 
                                  xml_strlen(valname), valstr);
    } else {
        *res = val_set_simval_str(val, typdef, nsid, NULL, 
                                  0, valstr);
    }
    return val;
...
#define REPLACE_BUFF_LEN 256
        uint32 len = 0;
        xmlChar static_buff[REPLACE_BUFF_LEN];
        xmlChar *usebuff = NULL;
        xmlChar *buffer = NULL;

        res = val_sprintf_simval_nc(NULL, val, &len);
        if (res == NO_ERR) {
            if (len < REPLACE_BUFF_LEN) {
                usebuff = static_buff;
            } else {
            ...
            res = val_sprintf_simval_nc(usebuff, val, &len);
            if (res == NO_ERR) {
                if (copy->btyp == NCX_BT_ANY) {
                    copy->obj = val->obj;
                    res = val_set_simval(copy, val->typdef, copy->nsid,
                                         copy->name, usebuff);
                } else if (typ_is_simple(copy->btyp)) {
                    res = val_set_simval(copy, copy->typdef, copy->nsid,
                                         copy->name, usebuff);
                } else {
                    res = ERR_NCX_INVALID_VALUE;
                }
...
*********************************************************************/
val_value_t *
    val_first_child_match (const val_value_t *parent,
                           const val_value_t *child)
{
    assert(parent);
    assert(child);
    assert(typ_has_children(parent->btyp));

    boolean sys_sorted = (ncx_get_system_sorted() &&
                          obj_is_system_ordered(child->obj));
    val_value_t *val;
    for (val = (val_value_t *)dlq_firstEntry(&parent->v.childQ);
        ...
        if (val->nsid == child->nsid &&
            ...
            if (val->btyp == NCX_BT_LIST) {
                ...
                if (sys_sorted) {
                    int32 ret = val_index_compare(child, val);
                    if (ret == 0) {
                        return val;
                    } else if (ret < 0) {
                        return NULL;
                ...
                if (val->btyp == child->btyp) {
                    ...
                    if (sys_sorted) { 
                        int32 ret = val_compare(child, val);
                        if (ret == 0) {
                            return val;
                        } else if (ret < 0) {
                            return NULL;
...
/********************************************************************
* FUNCTION val_first_child_match_fast
* 
* Get the first instance of the corresponding child node 
* Object pointers must be from the same tree!!!
*
* INPUTS:
*    parent == parent value to check
*    child == child value to find (e.g., from a NETCONF PDU or candidate) 
*    lastmatch == last child to start search from;
*      !!! val_set_canonical_order is required!!! (may be NULL)
*
* RETURNS:
*   pointer to the child if found or NULL if not found
*********************************************************************/
val_value_t *
    val_first_child_match_fast (const val_value_t *parent,
                                const val_value_t *child,
                                val_value_t *lastmatch)
{
    ...
    val_value_t *val;
    if (lastmatch) {
        //val = (val_value_t *)dlq_nextEntry(lastmatch);
        val = lastmatch;
    } else {
        val = (val_value_t *)dlq_firstEntry(&parent->v.childQ);
    }
...
/********************************************************************
* FUNCTION val_find_child_ex
* 
* Find the first instance of the specified child node
* Allow strings that are not z-terminated
*
* INPUTS:
*    parent == parent complex type to check
*    modname == module name; 
...
*                 the first match in any namespace will
*                 be  returned;
*    modnamelen == length of modname
*    childname == name of child node to find
*    childnamelen == length of child name
*
* RETURNS:
*   pointer to the child if found or NULL if not found
*********************************************************************/
val_value_t *
    val_find_child_ex (const val_value_t  *parent,
                       const xmlChar *modname,
                       uint32 modnamelen,
                       const xmlChar *childname,
                       uint32 childnamelen)
{
    ...
    assert( parent && "parent is NULL!" );
    assert( childname && "childname is NULL!" );
    ...
    for (; val != NULL; val = (val_value_t *)dlq_nextEntry(val)) {
        ...
        if (modname && modnamelen) {
            const xmlChar *valmodname = val_get_mod_name(val);
            if (modnamelen != xml_strlen(valmodname) ||
                xml_strncmp(modname, valmodname, modnamelen)) {
                continue;
            }
        }

        if (childnamelen == xml_strlen(val->name) &&
            !xml_strncmp(childname, val->name, childnamelen)) {
            return val;
        }
    }
    return NULL;
                     ...
                     const xmlChar *childname)
{
    ...
    uint32 len = xml_strlen(childname);
    for (val = (val_value_t *)dlq_firstEntry(&parent->v.childQ);
         val != NULL;
         val = (val_value_t *)dlq_nextEntry(val)) {
        if (VAL_IS_DELETED(val)) {
        ...
        if (modname && xml_strcmp(modname, val_get_mod_name(val))) {
            continue;
        }
        if (!xml_strncmp(val->name, childname, len)) {
            return val;
        }
    }
    return NULL;
...
/********************************************************************
* FUNCTION val_match_child_count
* 
* Match the first instance of the specified child node
...
*                 be  returned;
*    childname == name of child node to find
*    matchcount == address of return match count
*
* OUTPUTS:
...
*********************************************************************/
val_value_t *
    val_match_child_count (const val_value_t  *parent,
                           const xmlChar *modname,
                           const xmlChar *childname,
                           uint32 *matchcount)
{
#ifdef DEBUG
    if (!parent || !childname || !matchcount) {
        SET_ERROR(ERR_INTERNAL_PTR);
        return NULL;
    }
#endif
                         ...
                         ncx_xpath_axis_t axis)
{
    ...
    while (val) {
        ...
        if (val_is_virtual(val)) {
            ...
            if (useval == NULL && res != ERR_NCX_SKIPPED) {
                return FALSE;
            }
        } else {
            useval = val;
        ...
        if (res == NO_ERR) {
            fnresult = process_one_valwalker(walkerfn, cookie1, cookie2,
                                         useval, modname, name,
                                         configonly, textmode, &fncalled);
            if (!fnresult) {
                return FALSE;
            }
            ...
            if (!fncalled && dblslash) {
                ...
                for (child = val_get_first_child(useval);
                    ...
                    fnresult = 
                        val_find_all_pfaxis(walkerfn, cookie1, cookie2,
                                            child, modname, name, 
                                            configonly, dblslash,
                                            textmode, axis);
                    if (!fnresult) {
                     ...
                     const val_value_t *val2)
{
    ...
    assert(val1->obj && val1->obj->objtype == OBJ_TYP_LIST);
    assert(val1->obj == val2->obj);
...
int32
    val_compare_max (const val_value_t *val1,
                     ...
                     boolean editing)
{
    return val_compare_max_def(val1, val2, configonly, childonly,
                               editing, TRUE);
...
*               FALSE to compare just the values, so a set by
*               default and value=default are the same value
*    ignore_defaults == TRUE to ignore defaults; expect defaults to
*                be set in both nodes;
*                   == FALSE to compensate for defaults missing in val1;
*                      try to skip past missing default containers and leafs
*                   !!!! TBD: NOT YET IMPLEMENTED FOR FALSE !!!!
* RETURNS:
*   compare result
*     -1: val1 is less than val2 (if complex just different or error)
*      0: val1 is the same as val2
...
*********************************************************************/
int32
    val_compare_max_def (const val_value_t *val1,
                         const val_value_t *val2,
                         boolean configonly,
                         boolean childonly,
                         boolean editing,
    ...
    case NCX_BT_CASE:
        ...
        for (;;) {
            if ((ch1 && VAL_IS_DELETED(ch1)) || 
                (ch2 && VAL_IS_DELETED(ch2)) || configonly) {
                ch1 = get_next_node(ch1, configonly);
                ch2 = get_next_node(ch2, configonly);
            }
            ...
            xmlns_id_t nsid1 = val_get_nsid(ch1);
            xmlns_id_t nsid2 = val_get_nsid(ch1);
            ...
            if (!childonly || typ_is_simple(ch1->btyp)) {
                ...
                ret = val_compare_max_def(ch1, ch2, configonly, 
                                          childonly, editing, ignore_defaults);
                if (ret) {
                    return ret;
                }
...
int32
    val_compare_ex (const val_value_t *val1,
                    ...
                    boolean configonly)
{
    return val_compare_max_def(val1, val2, configonly,
                               FALSE, /* childonly */
                               TRUE,  /* editing */
...
int32
    val_compare (const val_value_t *val1,
                 const val_value_t *val2)
{
    return val_compare_max_def(val1, val2,
                               FALSE, /* configonly */
                               FALSE, /* childonly */
                               TRUE,  /* editing */
                               TRUE); /* ignore_defaults */
...
/********************************************************************
* FUNCTION val_compare_to_string_len
* 
* Compare a val_value_t struct value contents to a string
* Provide a max-length to compare
*
* Handles NCX_CL_BASE and NCX_CL_SIMPLE data classes
* by comparing the simple value.
*
...
*********************************************************************/
int32
    val_compare_to_string_len (const val_value_t *val1,
                               const xmlChar *strval2,
                               uint32 strval2len,
                               status_t *res)
{
#define MYBUFFSIZE  64
    ...
    if (myres != NO_ERR) {
        *res = myres;
        ...
        retval = xml_strncmp(mbuff, strval2, strval2len);
    } else {
        retval = xml_strncmp(buff, strval2, strval2len);
    }
                             ...
                             const val_value_t *val2)
{
    ...
    case NCX_BT_CASE:
        ...
        ch2 = (val_value_t *)dlq_firstEntry(&val2->v.childQ);
        for (;;) {
            ch1 = get_next_node(ch1, TRUE);
            ch2 = get_next_node(ch2, TRUE);
...
/********************************************************************
* FUNCTION val_compare_for_edit
*
* Compare 2 val_value_t struct value contents
* from an editing POV; compare for merge or delete!
*
...
*********************************************************************/
int32
    val_compare_for_edit (const val_value_t *val1,
                          const val_value_t *val2,
                          boolean ismerge)
{
    assert( val1 && "val1 is NULL!");
    ...
    if (typ_is_simple(val1->btyp)) {
        boolean isdef = val_set_by_default(val2);
        if (ismerge) {
            if (isdef) {
                return 1;
            }
            return val_compare(val1, val2);
        } else {
            /* delete operation */
            if (isdef) {
                return 0;
    ...
    switch (btyp) {
    case NCX_BT_LIST:
        // this is redundant because all the child nodes are checked!!!
        ret = index_match(val1, val2);
        if (ret) {
            break;
        } /* else drop though and check values */
    ...
    case NCX_BT_CASE:
        ...
        for (;;) {
            ...
            /* check if both child nodes exist */
            ch2 = (ch1) ? val_first_child_match_fast(val2, ch1, NULL) : NULL;
            if (!ch1) {
                /* equal means NO-OP for merge but not for delete */
                return (ismerge) ? 0 : 1;
            } else if (!ch2) {
                return 1;
            }
            ...
            /* check if they have same value */
            ret = val_compare_for_edit(ch1, ch2, ismerge);
            if (ret) {
                return ret;
            }

            /* get the next child node to check */
            ch1 = (val_value_t *)dlq_nextEntry(ch1);
        }
        /*NOTREACHED*/
        break;
...
*********************************************************************/
status_t
    val_parse_meta (struct ses_cb_t_ *scb,
                    typ_def_t *typdef,
                    xml_attr_t *attr,
                    val_value_t *retval)
{
    ...
    case NCX_BT_BINARY:
    case NCX_BT_INSTANCE_ID:
        res = val_string_ok_ex(scb, typdef, btyp, attrval, NULL, TRUE);
        if (res == NO_ERR) {
...
*   isxml == TRUE to account for XML start and end tags
                     ...
                     boolean isxml)
{
    ...
    case NCX_BT_SLIST:
    case NCX_BT_BITS:
        valsize = ncx_list_linelen(&val->v.list);
        break;
    case NCX_BT_ANY:
    case NCX_BT_CONTAINER:
    case NCX_BT_LIST:
    ...
    uint32 valnamesize = xml_strlen(val->name);
    xmlns_id_t nsid = val_get_nsid(val);

    if (isxml && nsid) {
        /* account for 'foo:' */
        valnamesize += (xml_strlen(xmlns_get_ns_prefix(nsid)) + 1);
    }
    ...
    if (isxml) {
        ...
        totalsize = valsize + valnamesize + 1;
    }
    ...
    val_is_config_data (const val_value_t *val)
{
    ...
    if (val->obj == NULL) {
        return FALSE;
    } else if (obj_is_root(val->obj)) {
        return TRUE;
    } else if (obj_is_data_db(val->obj) && obj_get_config_flag(val->obj)) {
        return TRUE;
    } else {
        return FALSE;
    }
...
*********************************************************************/
val_value_t *
    val_get_virtual_value (ses_cb_t *scb,
                           val_value_t *val,
                           status_t *res)
{
    val_value_t *retval;
    ...
    val_is_default (val_value_t *val)
{
    ...
    case NCX_BT_ENUM:
        ...
        /** note the dummy enum value that will not get used
         ** because the emum names will get compared
         ** by ncx_compare_enums; normally the typdef should
         ** be retrieved and the correct int32 value used
         **/
        res = ncx_set_enum(def, 0, &enu);
...
/********************************************************************
* FUNCTION val_set_child_deleted_flag
* 
...
*********************************************************************/
void
    val_set_child_deleted_flag (val_value_t *val)
{
    assert(val && "val is NULL!");
    val->flags |= VAL_FL_CHILD_DELETED;

} /* val_set_child_deleted_flag */

/********************************************************************
* FUNCTION val_get_child_deleted_flag
* 
...
*********************************************************************/
boolean
    val_get_child_deleted_flag (val_value_t *val)
{
    assert(val && "val is NULL!");
    return (val->flags & VAL_FL_CHILD_DELETED) ? TRUE : FALSE;
...
*     timestamp == new last-modified timestamp to set
*     txid == transaction id for this edit
*     is_delete == TRUE if the descendant nodes of the 'val'
*        node should be skipped  (delete or remove operation)
...
{
    assert(val && "val is NULL!");
    assert(timestamp && "timestamp is NULL!");

    val->flags &= ~(VAL_FL_DIRTY | VAL_FL_CHILD_DELETED);
    clear_default(val);

    val_value_t *parent = val->parent;
    while (parent) {
        parent->flags &= ~VAL_FL_SUBTREE_DIRTY;
        parent->last_modified = *timestamp;
        parent->etag = txid;
        clear_default(parent);
        if (obj_is_root(parent->obj)) {
            parent = NULL;
...
{
    assert(val && "val is NULL!");
    assert(val->obj && "va->objl is NULL!");
    ...
    val_get_first_leaf (val_value_t *val)
{
    ...
    if (obj_is_leafy(val->obj)) {
        ...
        for (child = (val_value_t *)
            ...
            val_value_t *found = val_get_first_leaf(child);
            if (found) {
                return found;
            }
        }
...
val_value_t *
    val_make_from_insertxpcb (val_value_t *sourceval,
                              status_t *res)
{
    assert(sourceval && "sourceval is NULL!");
    ...
    xpath_pcb_t *xpcb = sourceval->editvars->insertxpcb;
    ...
    while (!done && myres == NO_ERR) {
        ...
        const xmlChar *keyname = TK_CUR_VAL(xpcb->tkc);
        ...
        const xmlChar *keystring = TK_CUR_VAL(xpcb->tkc);
        ...
        val_value_t *keyval =
            val_make_string(val_get_nsid(sourceval), keyname, keystring);
        if (!keyval) {
...
{
    assert(val && "val is NULL!");
    return (val->flags & VAL_FL_WITHDEF);
...
/********************************************************************
* FUNCTION val_delete_default_npcon
* 
* Do the internal work to convert an NP-container to its
* subtree of YANG default values
*
* INPUTS:
*    val == val_value_t struct to use
*
...
*********************************************************************/
status_t
    val_delete_default_npcon (val_value_t *val,
                              val_value_t *rootval,
                              val_value_t *cxtval)
{
    assert(val && "val is NULL!");
    ...
    status_t res = val_add_defaults(val, rootval, cxtval, FALSE);
...
/********************************************************************
* FUNCTION val_move_metadata
* 
* Move the attribute fields from one val to another
* for xml_wr purposes
*
* INPUTS:
*    srcval == source val_value_t struct to move from
...
*********************************************************************/
void
    val_move_metadata (val_value_t *srcval,
                       val_value_t *destval)
{
#ifdef DEBUG
    if (srcval == NULL || destval == NULL) {
...
{
    if (val->editvars) {
        log_debug3("\nval: reusing existing editvars for val %s",
                   val->name);
        return NO_ERR;
    }

    val_editvars_t *editvars = m__getObj(val_editvars_t);
    if (!editvars) {
        return ERR_INTERNAL_MEM;
    }
...
/********************************************************************
* FUNCTION val_all_np_containers
* 
* Check if the value tree is all NP containers and nothing else
* This is needed by yangcli to prune these edits from the config
* mode editing process
*
* INPUTS:
*    val == val_value_t data structure to check
*
...
*********************************************************************/
boolean
    val_all_np_containers (val_value_t *val)
{
    assert(val && "val is NULL!");
    ...
    if (obj_is_np_container(val->obj)) {
        val_value_t *childval = val_get_first_child(val);
        for (; childval; childval = val_get_next_child(childval)) {
            boolean ret = val_all_np_containers(childval);
            if (!ret) {
                return FALSE;
            }
        }
...
*********************************************************************/
status_t
    val_sprintf_etag (val_value_t *val,
                      xmlChar *buff,
                      int32 buffsize)
{
    assert(val && "val is NULL!");
    assert(buff && "buff is NULL!");
    assert(buffsize && "buffsize is zero!");

    xmlChar numbuff[NCX_MAX_NUMLEN+1];
    int32 ilen = sprintf((char *)numbuff, "%llu",
                         (unsigned long long)val->etag);
    if (ilen < buffsize) {
        xml_strcpy(buff, numbuff);
        return NO_ERR;
    } else {
...
/********************************************************************
* FUNCTION val_get_last_modified
* 
...
*********************************************************************/
time_t *
    val_get_last_modified (val_value_t *val)
{
    assert(val && "val is NULL!");

    return &val->last_modified;
}
