    ...
    unexpected_error (xpath_pcb_t *pcb)
{
    if (pcb->logerrors) {
        ...
        ncx_print_errormsg(pcb->tkc, pcb->tkerr.mod, ERR_NCX_WRONG_TKTYPE);
    }
...
*    position == XML position value to use
...
static xpath_result_t *
    new_nodeset (xpath_pcb_t *pcb,
                 ...
                 boolean dblslash)
{
    xpath_result_t *result = new_result(pcb, XP_RT_NODESET);
    if (!result) {
        return NULL;
    }
...
{
    (void)cookie1;
    xpath_compwalkerparms_t *parms = (xpath_compwalkerparms_t *)cookie2;
...
#ifdef XPATH1_WALKER_DEBUG
    log_debug3("\nxpath1: enter compare walker for %s:%s (%d)",
               val_get_mod_name(val), val->name, parms->res);
#endif
    ...
    if (val_is_virtual(val)) {
        val_value_t *v_val = val_get_virtual_value(NULL, val, &res);
        if (v_val == NULL) {
    ...
    if (obj_is_password(val->obj)) {
        ...
        if (parms->cmpstring) {
            parms->cmpresult = 
                compare_strings(parms->cmpstring, VAL_STR(useval),
                                parms->exop);
        } else {
            parms->cmpresult = 
                compare_numbers(parms->cmpnum, VAL_STR(useval),
                                parms->exop);
        }
    } else {
        ...
        if (cnt < parms->buffsize) {
            ...
            if (res == NO_ERR) {
                if (parms->cmpstring) {
                    parms->cmpresult = 
                        compare_strings(parms->cmpstring, parms->buffer,
                                        parms->exop);
                } else {
                    parms->cmpresult = 
                        compare_numbers(parms->cmpnum, parms->buffer,
                                        parms->exop);
                }
            } else {
                parms->res = res;
            ...
            /* use a temp buffer */
            xmlChar *buffer = m__getMem(cnt+1);
            if (!buffer) {
            ...
            if (parms->cmpstring) {
                parms->cmpresult = 
                    compare_strings(parms->cmpstring, buffer, parms->exop);
            } else {
                parms->cmpresult = 
                    compare_numbers(parms->cmpnum, buffer, parms->exop);
            }
...
#ifdef XPATH1_WALKER_DEBUG
    log_debug3("\nxpath1: enter top compare walker for %s:%s (%d)",
               val_get_mod_name(val), val->name, parms->res);
#endif
                              ...
                              status_t *res)
{
    *res = NO_ERR;
    ...
    if (val2->restype == XP_RT_BOOLEAN) {
        boolean bool1 = xpath_cvt_boolean(val1);
        boolean bool2 = val2->r.boo;
    ...
    for (resnode = (xpath_resnode_t *)dlq_firstEntry(&val1->r.nodeQ);
        ...
        val_value_t *testval = resnode->node.valptr;
        status_t myres = NO_ERR;

        if (val_is_virtual(testval)) {
            val_value_t *newval = val_get_virtual_value(NULL, testval, &myres);
            if (newval == NULL) {
                if (myres == ERR_NCX_SKIPPED) {
                    myres = NO_ERR;
                } else {
                    *res = myres;
                }
        ...
        case XP_RT_STRING:
            {
                xmlChar *cmpstring = NULL;
                *res = xpath1_stringify_node(pcb, testval, &cmpstring);
                if (*res == NO_ERR) {
                    fnresult = compare_strings(cmpstring, val2->r.str, exop);
                }
                if (cmpstring) {
                    m__free(cmpstring);
                }
        ...
        case XP_RT_NUMBER:
            {
                ...
                if (typ_is_number(testval->btyp)) {
                    myres = ncx_cast_num(&testval->v.num, testval->btyp,
                                         &cmpnum, NCX_BT_FLOAT64);
                    if (myres != NO_ERR) {
                        ncx_set_num_nan(&cmpnum, NCX_BT_FLOAT64);
                    }
                } else if (testval->btyp == NCX_BT_STRING) {
                    myres = ncx_convert_num(VAL_STR(testval), NCX_NF_NONE,
                                            NCX_BT_FLOAT64, &cmpnum);
                    if (myres != NO_ERR) {
                        ncx_set_num_nan(&cmpnum, NCX_BT_FLOAT64);
                    }
                } else {
                ...
                int32 cmpresult = ncx_compare_nums(&cmpnum, &val2->r.num, 
                                                   NCX_BT_FLOAT64);
                     ...
                     status_t *res)
{
    *res = NO_ERR;
    ...
    if (val1->restype == val2->restype) {
        switch (val1->restype) {
        case XP_RT_NODESET:
            return compare_nodesets(pcb, val1, val2, exop, res);
        case XP_RT_NUMBER:
            cmpval = ncx_compare_nums(&val1->r.num, &val2->r.num,
                                      NCX_BT_FLOAT64);
            return convert_compare_result(cmpval, exop);
        case XP_RT_STRING:
            return compare_strings(val1->r.str, val2->r.str, exop);
        case XP_RT_BOOLEAN:
            return compare_booleans(val1->r.boo, val2->r.boo, exop);
            break;
        default:
            *res = SET_ERROR(ERR_INTERNAL_VAL);
            return TRUE;
    ...
    if (val1->restype == XP_RT_NODESET ||
        ...
        return compare_nodeset_to_other(pcb, val1, val2, exop, res);
    }
            ...
            if (result->isval) {
                ...
                if (val) {
                    log_write("%s:%s [L:%u]", 
                              ...
                              val->name, val_get_nest_level(val));
                } else {
                    log_write("NULL");
                }
            } else {
                ...
                if (obj) {
                    log_write("%s:%s [L:%u]", 
                              ...
                              obj_get_name(obj), obj_get_level(obj));
                } else {
                    log_write("NULL");
                }
            }
...
{
    (void)cookie1;
    xpath_stringwalkerparms_t *parms = (xpath_stringwalkerparms_t *)cookie2;

#ifdef XPATH1_WALKER_DEBUG
    log_debug3("\nxpath1: enter stringify walker for %s:%s (%d)",
               val_get_mod_name(val), val->name, parms->res);
#endif
    ...
    if (val_is_virtual(val)) {
        val_value_t *newval = val_get_virtual_value(NULL, val, &res);
        if (newval == NULL) {
            if (res == ERR_NCX_SKIPPED) {
                res = NO_ERR;
            }
                ...
                status_t  *res)
{
    ...
    if (parm->restype == XP_RT_NUMBER) {
        *res = ncx_num_ceiling(&parm->r.num, &result->r.num, NCX_BT_FLOAT64);
                           
    } else {
        ncx_init_num(&num);
        xpath_cvt_number(parm, &num);
        *res = ncx_num_ceiling(&num, &result->r.num, NCX_BT_FLOAT64);
        ...
        *res = ERR_INTERNAL_MEM;
        ...
        if (parm->restype != XP_RT_NODESET) {
            ...
            if (pcb->val) {
                ...
                *res = ncx_cast_num(&tempnum, NCX_BT_UINT32,
                                    &result->r.num, NCX_BT_FLOAT64);
                ...
                status_t  *res)
{
    ...
    result = new_nodeset(pcb, pcb->orig_context.node.objptr,
                         pcb->orig_context.node.valptr, 1, FALSE);
    if (!result) {
        *res = ERR_INTERNAL_MEM;
    } else {
        *res = NO_ERR;
              ...
              status_t  *res)
{
    ...
    if (parm->restype == XP_RT_NUMBER) {
        *res = ncx_num_floor(&parm->r.num, &result->r.num, NCX_BT_FLOAT64);
    } else {
        ncx_init_num(&num);
        xpath_cvt_number(parm, &num);
        *res = ncx_num_floor(&num, &result->r.num, NCX_BT_FLOAT64);
        ncx_clean_num(NCX_BT_FLOAT64, &num);
    }   
             ...
             status_t  *res)
{
    ...
    *res = NO_ERR;
    ...
    if (!result) {
        ...
        *res = ncx_cast_num(&tempnum, NCX_BT_INT64,
                            &result->r.num, NCX_BT_FLOAT64);
                        ...
                        status_t  *res)
{
    ...
    if (parm && parm->restype != XP_RT_STRING) {
        ...
        tempresult= new_nodeset(pcb, pcb->context.node.objptr,
                                pcb->context.node.valptr, 1, FALSE);
              ...
              status_t  *res)
{
    ...
    if (parm->restype == XP_RT_NUMBER) {
        *res = ncx_round_num(&parm->r.num, &result->r.num, NCX_BT_FLOAT64);
    } else {
        ncx_init_num(&num);
        xpath_cvt_number(parm, &num);
        *res = ncx_round_num(&num, &result->r.num, NCX_BT_FLOAT64);
        ncx_clean_num(NCX_BT_FLOAT64, &num);
    }
        ...
        if (pcb->val) {
            ...
            if (!tempresult) {
                *res = ERR_INTERNAL_MEM;
                ...
                tempstr = NULL;
                *res = xpath_cvt_string(pcb, tempresult, &tempstr);
                free_result(pcb, tempresult);
                if (*res == NO_ERR) {
                    len = xml_strlen(tempstr);
                    m__free(tempstr);
    ...
    if (startpos > (slen+1) || (copylenset && copylen <= 0)) {
        ...
        if (copylen >= 1) {
            if (copylen >= NCX_MAX_UINT) {
                ...
                result->r.str = xml_strndup(&str1[startpos-1], maxlen);
            }
        } else {
            result->r.str = xml_strdup(EMPTY_STRING);
        }
                         ...
                         status_t  *res)
{
    ...
    if (result) {
        ...
        if (str) {
            ...
            if (result->r.str) {
                xml_strncpy(result->r.str, str1, retlen);
            }
        } else {
            result->r.str = xml_strdup(EMPTY_STRING);
        }
    ...
    if (parm->restype != XP_RT_NODESET || !pcb->val) {
        ...
        for (resnode = (xpath_resnode_t *)
            ...
            if (val && typ_is_number(val->btyp)) {
                myres = ncx_cast_num(&val->v.num, val->btyp,
                                     &tempnum, NCX_BT_FLOAT64);
                if (myres == NO_ERR) {
                    mysum.d += tempnum.d;
                }
            } else {
                        ...
                        xmlns_id_t *nsid)
{
    ...
    if (pcb->reader) {
        res = xml_get_namespace_id(pcb->reader, prefix, prefixlen,
                                   TRUE, nsid);
    } else {
                    ...
                    status_t *res)
{
    ...
    *res = ERR_INTERNAL_MEM;
    ...
    if (typ_is_string(useval->btyp)) {
        ...
        result = new_result(pcb, XP_RT_NUMBER);
        if (result) {
            *res = ncx_cast_num(&useval->v.num, useval->btyp,
                                &result->r.num, NCX_BT_FLOAT64);
            if (*res != NO_ERR) {
                free_result(pcb, result);
                result = NULL;
        ...
        if (useval->btyp == NCX_BT_BOOLEAN) {
            result = new_result(pcb, XP_RT_BOOLEAN);
            if (result) {
                result->r.boo = VAL_BOOL(useval);
                *res = NO_ERR;
            }
        } else {
            uint32 len = 0;
        ...
        result = new_nodeset(pcb, NULL, useval, 1, FALSE);
        if (result) {
            *res = NO_ERR;
        }
    }
                   ...
                   xpath_result_t *val2)
{
    ...
    if ((!pcb->val && !pcb->obj) || IS_SKIP_MODE(pcb)) {
        return;
    }
...
static boolean
    value_walker_fn (val_value_t *val,
                     ...
                     void *cookie2)
{
    xpath_pcb_t *pcb = (xpath_pcb_t *)cookie1;
    xpath_walkerparms_t *parms = (xpath_walkerparms_t *)cookie2;

#ifdef XPATH1_WALKER_DEBUG
    log_debug3("\nxpath1: enter value walker for %s:%s (%d)",
               val_get_mod_name(val), val->name, parms->res);
#endif
    ...
    xpath_resnode_t *newresnode = new_val_resnode(pcb, position, FALSE, val);
    if (!newresnode) {
        parms->res = ERR_INTERNAL_MEM;
        return FALSE;
    }
                      ...
                      void *cookie2)
{
...
#ifdef XPATH1_WALKER_DEBUG
    log_debug3("\nxpath1: enter object walker for %s:%s (%d)",
               obj_get_mod_name(obj), obj_get_name(obj), parms->res);
#endif
    ...
    xpath_resnode_t *newresnode =
        new_obj_resnode(pcb, ++parms->callcount, FALSE, obj);
    if (!newresnode) {
        parms->res = ERR_INTERNAL_MEM;
        return FALSE;
    }
                      ...
                      boolean textmode)
{
    ...
    const xmlChar *modname = (nsid) ? xmlns_get_module(nsid) : NULL;
    boolean useroot;
    if (pcb->backptrs_mode) {
        useroot = FALSE;
    } else {
    ...
    while (!dlq_empty(&result->r.nodeQ) && res == NO_ERR) {
        ...
        xpath_resnode_t  *findnode = NULL;
        boolean cfgonly = FALSE;
        boolean fnresult = FALSE;

        xpath_resnode_t *resnode =
            (xpath_resnode_t *)dlq_deque(&result->r.nodeQ);
        if (resnode->dblslash) {
            if (pcb->val) {
                testval = resnode->node.valptr;
            ...
            if (pcb->val) {
                ...
                if (keep) {
                    findnode = find_resnode(pcb, &resnodeQ, testobj);
                    if (findnode) {
                        if (resnode->dblslash) {
                            findnode->dblslash = TRUE;
                            findnode->position = 
        ...
        if (resnode->dblslash) {
            ...
            if (pcb->val) {
                ...
                if (testval == pcb->val_docroot) {
                    ...
                    if (keep) {
                        findnode = find_resnode(pcb, &resnodeQ, testval);
                        if (findnode) {
                ...
                if (testobj == pcb->docroot) {
                    ...
                    if (!resnode->dblslash && (modname || name)) {
                        ...
                        findnode = find_resnode(pcb, &resnodeQ, pcb->docroot);
                        if (findnode) {
                            if (resnode->dblslash) {
                                findnode->position = ++position;
                                findnode->dblslash = TRUE;
                       ...
                       ncx_xpath_axis_t axis)
{
    ...
    status_t res = NO_ERR;
    boolean cfgonly = (pcb->flags & XP_FL_CONFIGONLY) ? TRUE : FALSE;
    boolean orself = (axis == XP_AX_DESCENDANT_OR_SELF) ? TRUE : FALSE;
    ...
    while (!dlq_empty(&result->r.nodeQ) && res == NO_ERR) {
        ...
        xpath_resnode_t *resnode =
            (xpath_resnode_t *)dlq_deque(&result->r.nodeQ);
        boolean myorself = (orself || resnode->dblslash) ? TRUE : FALSE;
        ...
        if (pcb->val) {
            ...
            obj_template_t *testobj = resnode->node.objptr;
    ...
    if (!dlq_empty(&resnodeQ)) {
        ...
        if (pcb->logerrors && childname) {
            /* Only generate an error if a local-name was given,
             * not if the '*' wildcard was given (childname == NULL)
                          ...
                          ncx_xpath_axis_t axis)
{
    ...
    boolean orself = (axis == XP_AX_ANCESTOR_OR_SELF) ? TRUE : FALSE;
    ...
    while (!dlq_empty(&result->r.nodeQ) && res == NO_ERR) {
        ...
        boolean fnresult, fncalled = FALSE;
        ...
        if (walkerparms.res != NO_ERR) {
            ...
            xpath_result_t *dummy = new_result(pcb, XP_RT_NODESET);
            if (!dummy) {
                res = ERR_INTERNAL_MEM;
                continue;
            }
            ...
            if (walkerparms.res != NO_ERR) {
                ...
                res = set_nodeset_ancestor(pcb, dummy, nsid, name, 
                                           textmode, axis);
                while (!dlq_empty(&dummy->r.nodeQ)) {
                    xpath_resnode_t *testnode = (xpath_resnode_t *)
                     ...
                     xpath_result_t **result)
{
    ...
    xpath_nodetype_t nodetyp = XP_EXNT_NONE;
    ...
    case TK_TT_NCNAME_STAR:
        ...
        if (!pcb->tkc->cur->nsid) {
            res = check_qname_prefix(pcb, TK_CUR_VAL(pcb->tkc),
    ...
    case TK_TT_MSTRING:
        /* match all nodes in the namespace w/ specified prefix */
        if (!pcb->tkc->cur->nsid) {
            res = check_qname_prefix(pcb, TK_CUR_MOD(pcb->tkc),
    ...
    case TK_TT_TSTRING:
        ...
        if (nodetyp == XP_EXNT_PROC_INST) {
            ...
            tk_type_t nexttyp = tk_next_typ(pcb->tkc);
    ...
    case XP_AX_ANCESTOR_OR_SELF:
        if (!*result) {
            *result = new_nodeset(pcb, pcb->context.node.objptr,
                                  pcb->context.node.valptr, 1, FALSE);
            if (!*result) {
                res = ERR_INTERNAL_MEM;
            }
        ...
        if (res == NO_ERR) {
            res = set_nodeset_ancestor(pcb, *result, nsid, name,
                                       textmode, axis);
        } 
        break;
    ...
    case XP_AX_CHILD:
        ...
        if (res == NO_ERR) {
            res = set_nodeset_child(pcb, *result, nsid, name, textmode, axis);
        }
        break;
    ...
    case XP_AX_PRECEDING_SIBLING:
        ...
        if (res == NO_ERR) {
            res = set_nodeset_pfaxis(pcb, *result, nsid, name,
                                     textmode, axis);
        }        
        break;
    ...
    case XP_AX_PARENT:
        ...
        if (res == NO_ERR) {
            res = set_nodeset_parent(pcb, *result, nsid, name);
        }
        break;
    case XP_AX_SELF:
        ...
        if (res == NO_ERR) {
            res = set_nodeset_self(pcb, *result, nsid, name, textmode);
        }                                  
        break;
    case XP_AX_NONE:
    default:
                     ...
                     xpath_result_t **result)
{
    ...
    status_t res = xpath_parse_token(pcb, TK_TT_LBRACK);
    if (res != NO_ERR) {
        return res;
    }
    ...
    boolean boo = FALSE;
    if ((pcb->val || pcb->obj) && !IS_SKIP_MODE(pcb)) {
        tk_token_t *leftbrack = TK_CUR(pcb->tkc);
        contextset = *result;
        if (!contextset) {
            return SET_ERROR(ERR_INTERNAL_VAL);
...
{
    tk_type_t nexttyp = tk_next_typ(pcb->tkc);
    ncx_xpath_axis_t axis = XP_AX_CHILD;
    status_t res = NO_ERR;
    tk_type_t nexttyp2 = TK_TT_NONE;
    const xmlChar *nextval = NULL;
    ...
    if (nexttyp == TK_TT_DBLFSLASH) {
        ...
        if (!IS_SKIP_MODE(pcb)) {
            if (!*result) {
                *result = new_nodeset(pcb, pcb->context.node.objptr,
                                      pcb->context.node.valptr, 1, TRUE);
                if (!*result) {
        ...
        if (!*result && !IS_SKIP_MODE(pcb)) {
            /* this is the first call */
            *result = new_nodeset(pcb, pcb->docroot, pcb->val_docroot,
                                  1, FALSE);
            if (!*result) {
                return ERR_INTERNAL_MEM;
            }
    ...
    case TK_TT_RANGESEP:
        ...
        if (!IS_SKIP_MODE(pcb)) {
            ...
            if (res == NO_ERR) {
                res = set_nodeset_parent(pcb, *result, 0, NULL);
            }
        }
        return res;
    case TK_TT_ATSIGN:
                         ...
                         status_t *res)
{
    ...
    if (*res == NO_ERR && pcb->backptrs_mode) {
        if (LOGDEBUG2) {
            log_debug2("\nxpath_backptr: add while processing '%s'",
                       pcb->exprstr);
            if (val1) {
                dump_result(pcb, val1, "backptrs");
            }
        }

        if (val1 && val1->restype == XP_RT_NODESET) {
            xpath_resnode_t *resnode;
            for (resnode = (xpath_resnode_t *)dlq_firstEntry(&val1->r.nodeQ);
                 resnode != NULL;
                 resnode = (xpath_resnode_t *)dlq_nextEntry(resnode)) {
                ...
                if (!val1->isval) {
                    obj_template_t *obj = resnode->node.objptr;
                    if (obj) {
                        *res = obj_assign_xpath_backptr(obj, pcb);
                    }
                }
            }
        }
                         ...
                         status_t *res)
{
    ...
    *res = xpath_parse_token(pcb, TK_TT_TSTRING);
    ...
    xpath_result_t *val1 = NULL;
    int32 parmcnt = 0;
    dlq_hdr_t parmQ;
    dlq_createSQue(&parmQ);

    /* find the function in the library */
    const xpath_fncb_t *fncb = NULL;
    ...
    if (fncb) {
        ...
        *res = xpath_parse_token(pcb, TK_TT_LPAREN);
        ...
        tk_type_t nexttyp = tk_next_typ(pcb->tkc);
        boolean done = (nexttyp == TK_TT_RPAREN) ? TRUE : FALSE;
        while (!done && *res == NO_ERR) {
            val1 = parse_expr(pcb, res);
            if (*res == NO_ERR) {
                parmcnt++;
    ...
    while (!dlq_empty(&parmQ)) {
        xpath_result_t *val2 = (xpath_result_t *)dlq_deque(&parmQ);
                        ...
                        status_t *res)
{
    ...
    case TK_TT_QVARBIND:
        *res = xpath_parse_token(pcb, nexttyp);
         ...
         */
        if (*res == NO_ERR && pcb->val && !IS_SKIP_MODE(pcb)) {
            ncx_var_t *varbind = NULL;
            const xmlChar *errstr = NULL;
            if (TK_CUR_TYP(pcb->tkc) == TK_TT_VARBIND) {
                varbind = get_varbind(pcb, NULL, 0, TK_CUR_VAL(pcb->tkc), res);
                errstr = TK_CUR_VAL(pcb->tkc);
            } else {
                varbind = get_varbind(pcb, TK_CUR_MOD(pcb->tkc),
                                      TK_CUR_MODLEN(pcb->tkc), 
            ...
            if (!varbind || *res != NO_ERR) {
                ...
                val1 = cvt_from_value(pcb, varbind->val, res);
            }
        }
        break;
    case TK_TT_LPAREN:
    ...
    case TK_TT_RNUM:
        *res = xpath_parse_token(pcb, nexttyp);
        ...
        if (*res == NO_ERR && !IS_SKIP_MODE(pcb)) {
            ...
            ncx_numfmt_t numfmt = ncx_get_numfmt(TK_CUR_VAL(pcb->tkc));
            if (numfmt == NCX_NF_OCTAL) {
                numfmt = NCX_NF_DEC;
            }
            if (numfmt == NCX_NF_DEC || numfmt == NCX_NF_REAL) {
                *res = ncx_convert_num(TK_CUR_VAL(pcb->tkc), numfmt,
                                       NCX_BT_FLOAT64, &val1->r.num);
            } else if (numfmt == NCX_NF_NONE) {
                *res = ERR_NCX_INVALID_VALUE;
            } else {
                *res = ERR_NCX_WRONG_NUMTYP;
...
static xpath_result_t *
    parse_filter_expr (xpath_pcb_t *pcb,
                       status_t *res)
{
    xpath_result_t *val1 = parse_primary_expr(pcb, res);
    if (*res != NO_ERR) {
        return val1;
    }
    ...
    case TK_TT_TSTRING:
        {
             ...
             */
            tk_type_t nexttyp2 = tk_next_typ2(pcb->tkc);
            const xmlChar *nextval = tk_next_val(pcb->tkc);
                      ...
                      status_t *res)
{
    ...
    xpath_result_t *val1 = parse_union_expr(pcb, res);
        ...
        if (IS_SKIP_MODE(pcb)) {
            ...
            if (pcb->val || pcb->obj) {
                ...
                if (val1->restype == XP_RT_NUMBER) {
                    ...
                    xpath_result_t *result = new_result(pcb, XP_RT_NUMBER);
                    if (!result) {
                        *res = ERR_INTERNAL_MEM;
                        return NULL;
                    }
                               ...
                               status_t *res)
{
    ...
    xpath_result_t *val2 = NULL;
    xpath_result_t *result = NULL;
    xpath_exop_t curop = XP_EXOP_NONE;
    boolean done = FALSE;
    ...
    while (!done && *res == NO_ERR) {
        ...
        if (*res == NO_ERR) {
            ...
            case TK_TT_STAR:
                ...
                break;
            case TK_TT_TSTRING:
                if (match_next_token(pcb, TK_TT_TSTRING, XP_OP_DIV)) {
                    curop = XP_EXOP_DIV;
                } else if (match_next_token(pcb, TK_TT_TSTRING, XP_OP_MOD)) {
                    curop = XP_EXOP_MOD;
                } else {
                    done = TRUE;
                }
            ...
            if (IS_SKIP_MODE(pcb)) {
                ...
                if (val2) {
                    if (pcb->val || pcb->obj) {
                         ...
                         */
                        boolean cmpresult =
                            compare_results(pcb, val2, val1, curop, res);
            ...
            if (IS_SKIP_MODE(pcb)) {
                ...
                if (val2) {
                    if (pcb->val || pcb->obj) {
                         ...
                         */
                        if (pcb->flags & XP_FL_INSTANCEID) {
                            *res = check_instanceid_expr(pcb, val2, val1);
                        }
        ...
        if (*res == NO_ERR) {
            if (IS_SKIP_MODE(pcb)) {
                ...
                if (val2) {
                    if (pcb->val || pcb->obj) {
                        /* val1 holds the 2nd operand
                         * bool1 holds the 1st operand
                         * it is not really needed since this fn would
                         * exit first false val1 returned above
                         */
                        boolean bool2 = xpath_cvt_boolean(val1);

                        result = new_result(pcb, XP_RT_BOOLEAN);
                        if (!result) {
                            *res = ERR_INTERNAL_MEM;
                        } else {
                            boolean ret = (bool1 && bool2) ? TRUE : FALSE;
                            result->r.boo = ret;
                            if (!ret && pcb->val) {
                                SET_SKIP_MODE(pcb);
                                setskipmode = TRUE;
                    ...
                    /* first time through so val2 is NULL */
                    if (val1) {
                        bool1 = xpath_cvt_boolean(val1);
                        if (!bool1 && pcb->val) {
                            SET_SKIP_MODE(pcb);
...
*                     == FALSE if a warning
*    set_backptrs == TRUE if this is the special mode to
*      go through the expression and set obj_template_t
*      back ptrs; used only by the server is ncx_use_xpath_backptrs()
                             ...
                             boolean set_backptrs)
{
    ...
    assert( pcb->tkc && "pcb->tkc is NULL" );
    ...
    pcb->backptrs_mode = set_backptrs;
...
status_t
    xpath1_validate_expr (ncx_module_t *mod,
                          ...
                          xpath_pcb_t *pcb)
{
    return xpath1_validate_expr_ex(mod, obj, pcb, TRUE, FALSE);
}
                          ...
                          xpath_result_t *result)
{
    ...
    for (resnode = (xpath_resnode_t *)
        ...
        /* remove the node and checkif it exists elsewhere
         * either direct duplicate or ancestor node
         */
        dlq_remove(resnode);

        if (xpath1_check_node_exists(pcb, &result->r.nodeQ,
                                     resnode->node.valptr)) {
                                   ...
                                   const val_value_t *val)
{
     ...
     * which means nothing else is needed
     */
    if (find_resnode_slow(pcb, resultQ, 0, pcb->val_docroot->name)) {
        return TRUE;
    }
    ...
    while (val) {
        if (find_resnode_slow(pcb, resultQ, val_get_nsid(val), val->name)) {
            return TRUE;
        }
                           ...
                           xmlChar **str)
{
    ...
    if (val->btyp == NCX_BT_EMPTY) {
        /* force to be an empty string not an element name */
        *str = xml_strdup(EMPTY_STRING);
        if (!*str) {
            return ERR_INTERNAL_MEM;
        }
                                     ...
                                     status_t *res)
{
    ...
    *res = NO_ERR;
    ...
    retval = compare_results(pcb, result, &sresult, XP_EXOP_EQUAL, res);
