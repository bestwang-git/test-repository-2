 ...
 *
 * \param pcb the parser control block in progress|
 * \param gotnum == set to true if a number was parsed

 * \return the status result
 *********************************************************************/
static status_t parse_node_identifier_num (xpath_pcb_t *pcb,
                                           boolean *gotnum)
{
    ...
    *gotnum = FALSE;
    ...
    if (res == NO_ERR && pcb->obj != NULL) {
        res = set_next_objnode( pcb, prefix, nsid, nodename );
    }

    return res;

}  /* parse_node_identifier_num */
...
#define MAX_KEYS   64
    ...
    while (!done) {
        ...
        if ((pcb->flags & XP_FL_INSTANCEID) ||
            ...
            res = parse_node_identifier_num(pcb, &gotnum);
        } else {
            res = parse_node_identifier(pcb);
        }
        if (res != NO_ERR) {
            ...
            if (pcb->obj && pcb->altobj) {
                if (pcb->altobj->objtype != OBJ_TYP_LEAF) {
                    ...
                    if (pcb->logerrors) {
                        ...
                        ncx_mod_exp_err(pcb->tkc, pcb->objmod, res, "leaf");
                    }
                    done = TRUE;
                    continue;
                } else {
                                ...
                                boolean logerrors)
{
    ...
    if (pcb->tkc) {
        ...
        pcb->tkc = tk_tokenize_xpath_string(NULL, pcb->exprstr, 0, 0, &res);
    }

    if (!pcb->tkc || res != NO_ERR) {
        if (pcb->logerrors) {
            log_error("\nError: Invalid path string '%s'", pcb->exprstr);
        }
        pcb->parseres = res;
        return res;
    }
                                    ...
                                    val_value_t **deepest)
{
    ...
    while (!done && res == NO_ERR) {
        ...
        while (!done2 && res == NO_ERR) {
            ...
            objname = TK_CUR_VAL(pcb->tkc);
            if (!xml_strcmp(objname, (const xmlChar *)".") &&
                obj_is_leaf_list(curobj)) {
                leaflist_mode = TRUE;
            } else {
                /* this is expected to be a QName or LCName identifier */
                objprefix = TK_CUR_MOD(pcb->tkc);
            ...
            /* get the key leaf of leaf-list value */
            res = TK_ADV(pcb->tkc);
            if (res != NO_ERR) {
                continue;
            }
            ...
            if (leaflist_mode) {
                /* set the current leaf with the value */
                res = val_set_simval_obj(curtop, curobj, 
                                         TK_CUR_VAL(pcb->tkc));
            } else {
                /* set the new leaf with the value */
                res = val_set_simval_obj(childval, childobj,
                                         TK_CUR_VAL(pcb->tkc));
            }
            if (res != NO_ERR) {
                continue;
