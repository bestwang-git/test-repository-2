 ...
 */
#define NCXMOD_NETCONFD   (const xmlChar *)"netconfd-pro"

#define NCXMOD_NCX        (const xmlChar *)"yuma-ncx"

#define NCXMOD_YUMA_NACM  (const xmlChar *)"yuma-nacm"

#define NCXMOD_IETF_NACM  (const xmlChar *)"ietf-netconf-acm"

#define NCXMOD_WITH_DEFAULTS (const xmlChar *)"ietf-netconf-with-defaults"

#define NCXMOD_YUMAWORKS_ATTRS (const xmlChar *)"yumaworks-attrs"

#define NCXMOD_YUMAWORKS_APP_COMMON (const xmlChar *)"yumaworks-app-common"

#define NCXMOD_YANGCLI (const xmlChar *)"yangcli-pro"
...
#define NCXMOD_IETF_NETCONF_REV (const xmlChar *)"2011-06-01"
...
#define NCXMOD_YANG_API (const xmlChar *)"yumaworks-yang-api"

#define NCXMOD_YUMA_IDS (const xmlChar *)"yumaworks-ids"
...
#define NCXMOD_HOME         "YUMAPRO_HOME"

/* NCX Environment Variable for tools install directory
 * The default is /usr/share/yuma
 */
#define NCXMOD_INSTALL   "YUMAPRO_INSTALL"

/* !! should import this from make !! */
#ifdef FREEBSD
#define NCXMOD_DEFAULT_INSTALL (const xmlChar *)"/usr/local/share/yumapro"
#else
#define NCXMOD_DEFAULT_INSTALL (const xmlChar *)"/usr/share/yumapro"
#endif

/* !! should import this from make !! */
#define NCXMOD_DEFAULT_YUMALIB64 (const xmlChar *)"/usr/lib64/yumapro"

#define NCXMOD_DEFAULT_YUMALIB (const xmlChar *)"/usr/lib/yumapro"


/* !! should import this from make !! */
#define NCXMOD_ETC_DATA (const xmlChar *)"/etc/yumapro"

/* NCX Environment Variable for MODULE search path */
#define NCXMOD_MODPATH      "YUMAPRO_MODPATH"

/* NCX Environment Variable for DATA search path */
#define NCXMOD_DATAPATH      "YUMAPRO_DATAPATH"

/* NCX Environment Variable for SCRIPTS search path */
#define NCXMOD_RUNPATH      "YUMAPRO_RUNPATH"

/* per user yangcli internal data home when $HOME defined */
#define NCXMOD_YUMA_DIR (const xmlChar *)"~/.yumapro"

/* per user yangcli internal data home when $HOME not defined */
#define NCXMOD_TEMP_YUMA_DIR (const xmlChar *)"/tmp/yumapro"

/* Yuma work directory name */
#define NCXMOD_YUMA_DIRNAME    (const xmlChar *)".yumapro"
...
/* directory uses to store rpydata per step of recorded test
 */
#define NCXMOD_UT_RECORDTEST_DIR (const xmlChar *)"/recordtest"
 ...
 *
 * user function callback template to process a backup file
 * during a subtree traversal
 *
 * DESCRIPTION:
 *   Handle the current filename in the subtree traversal
 ...
 *    recoverable error, just log and move on
 *********************************************************************/
typedef status_t (*ncxmod_backup_cbfn_t) (const char *fullspec,
                                          const xmlChar *mtime,
					  void *cookie);
