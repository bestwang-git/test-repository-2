                       ...
                       boolean *wasadded)
{
    ...
    *wasadded = FALSE;
    ...
    case YANG_PT_IMPORT:
        ...
        if (mod->ismod || pcb->top == mod) {
            ...
            if (!loaded && retres == NO_ERR) {
                if (ietfnetconf || pcb->parsemode) {
                    /* do not add to regular module registry
                     * if ietf-netconf or yangcli parse mode 
                     */
                    res = NO_ERR;
                    *wasadded = FALSE;
                } else {
                    if (pcb->diffmode) {
                        res = ncx_add_to_modQ(mod, NULL);
                    } else {
    ...
    mod->flags |= NCX_FL_MOD_DONE;

    if (retres == NO_ERR && mod->ismod && ncx_collapse_submodules_ok()) {
        ncx_collapse_submod_defs(mod);
    }
    return retres;
                              ...
                              boolean isyang)
{
    ...
    if (res != NO_ERR) {
        ...
        if (pcb->parsemode) {
             ...
             * duplicate */
            if (pcb->savemodQ) {
                ncx_add_to_modQ(mod, pcb->savemodQ);
                keepmod = TRUE;
            } else {
                if (pcb->top == mod) {
                    pcb->top = NULL;
