...
/* number of static xpath backptrs to have in each object node */
#define OBJ_NUM_XPATH_BACKPTRS 4
...
/* def is a CLI text block */
#define OBJ_FL_CLI_TEXT_BLOCK      bit1
...
/* object is marked for deletion -- dynamic flag */
#define OBJ_FL_DELETED      bit3

/* object is dirty in the candidate datastore; used by server only */
#define OBJ_FL_CDIRTY        bit4
...
/* bit7 is reused in the server since the 'seen' bit only
 * applies the the yangdiff-pro program
 * object is dirty in the running datastore; used by server only
 */
#define OBJ_FL_RDIRTY       bit7
...
/* object is marked as config set so OBJ_FL_CONFIG is valid */
#define OBJ_FL_CONFSET      bit16

/* object config value */
#define OBJ_FL_CONFIG       bit17

/* object is marked as mandatory set so OBJ_FL_MANDATORY is valid */
...
/* object is tagged as ywx:rpc-root operation container */
#define OBJ_FL_RPC_ROOT     bit31


/********* xflags field in obj_template_t *************/

/* object is a default NP-container */
#define OBJ_FL_DEF_NPCON    bit0
...
typedef struct obj_template_t_ {
    ...
    uint8          xflags;            /* see OBJ_FL_* definitions */
    ncx_error_t    tkerr;
 ...
 * \return status
 *********************************************************************/
extern status_t obj_gen_object_id_error (const obj_template_t *obj, 
                                         xmlChar  **buff);
 ...
 *********************************************************************/
extern boolean
    obj_is_leaf_list (const obj_template_t  *obj);


/********************************************************************
* FUNCTION obj_is_list
* 
...
*********************************************************************/
extern boolean
    obj_is_list (const obj_template_t  *obj);
 ...
 *********************************************************************/
extern boolean
    obj_is_data_node (const obj_template_t *obj);
...
*
* RETURNS:
*   TRUE if object is marked as ywx:rpc-root
*   FALSE if not
*********************************************************************/
extern boolean
    obj_is_rpc_root (const obj_template_t *obj);
 ...
 *********************************************************************/
extern boolean obj_is_dirty (const obj_template_t *obj,
                             ncx_cfg_t cfgid);
 ...
 *********************************************************************/
extern boolean obj_has_ro_descendants (obj_template_t *obj);
...
* uses log_write_level() for writing ... 
*
* INPUTS:
*   level == debug level to use for output
...
/********************************************************************
* FUNCTION obj_is_cli_text_block
*
...
*********************************************************************/
extern boolean
    obj_is_cli_text_block (const obj_template_t *obj);
 ...
 *********************************************************************/
extern ncx_backptr_t *
    obj_next_xpath_ptr (ncx_backptr_t *xptr);
