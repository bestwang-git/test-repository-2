...
uint32
    xml_val_qname_len (xmlns_id_t  nsid,
                       const xmlChar *name)
{
    const xmlChar *pfix = xmlns_get_ns_prefix(nsid);
    if (!pfix) {
        SET_ERROR(ERR_INTERNAL_VAL);   /* catch no namespace error */
                           ...
                           const xmlChar *name)
{
    ...
    uint32 len = xml_strlen(name) + xml_strlen(pfix) + 2;
    if (len > bufflen) {
                      ...
                      val_value_t *val)
{
    ...
    val_value_t *newval = new_string_attr( name, nsid, attrval );
    if (!newval) {
        return ERR_INTERNAL_MEM;
    }
...
status_t
    xml_val_add_cattr (const xmlChar *name,
                       ...
                       val_value_t *val)
{
    xmlChar *str = xml_strdup( cattrval );
    if (!str) {
        return ERR_INTERNAL_MEM;
    }

    status_t res = xml_val_add_attr( name, nsid, str, val );
    if ( NO_ERR != res ) {
        m__free( str );
    }
    return res;
...
* 
* RETURNS:
*   new root struct or NULL if malloc error
*********************************************************************/
val_value_t *
    xml_val_new_root (const xmlChar *name,
                      xmlns_id_t     nsid)
{
    val_value_t *val = val_new_value();
    if (!val) {
...
val_value_t *
    xml_val_new_string (const xmlChar *name,
                        ...
                        xmlChar *strval)
{
    val_value_t *val = new_string_attr(name, nsid, strval);
    if (!val) {
        return NULL;
    }
...
{
    xmlChar *str = xml_strdup(strval);
    if (!str) {
        return NULL;
    }

    val_value_t *val = new_string_attr( name, nsid, str );
    if ( !val ) {
        m__free( str );
        return NULL;
    }
...
/********************************************************************
* FUNCTION xml_val_new_leaflist_string
* 
*  Set up a new string element; reuse the value instead of copying it
*  hand off a malloced string
*
...
*********************************************************************/
val_value_t *
    xml_val_new_leaflist_string (const xmlChar *name,
                                 xmlns_id_t     nsid,
                                 xmlChar *strval)
{
    val_value_t *val = new_string_attr(name, nsid, strval);
