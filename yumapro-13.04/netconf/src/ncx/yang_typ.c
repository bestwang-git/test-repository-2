    ...
    loop_test (typ_def_t *typdef)
{
    ...
    if (typdef->tclass == NCX_CL_NAMED) {
        typ_def_t *testdef = typ_get_parent_typdef(typdef);
        typ_def_t *lastdef = testdef;
                      ...
                      typ_def_t *newdef)
{
    ...
    ncx_btype_t btyp = typ_get_basetype(newdef);
    if (btyp == NCX_BT_UNION) {
        return NO_ERR;
    }

    if (btyp == NCX_BT_NONE) {
        /* this is still an unresolved type such as a union
         * within the same module
         */
        return ERR_NCX_SKIPPED;
    }

    dlq_hdr_t *rangeQ = typ_get_rangeQ_con(newdef);
    ncx_strrest_t strrest = typ_get_strrest(newdef);
     ...
     */
    typ_pattern_t *pat = typ_get_first_pattern(newdef);
    if (pat) {
                      ...
                      typ_def_t *typdef)
{
    ...
    while (testdef && testdef->tclass==NCX_CL_NAMED) {
        typ_def_t *newdef = typ_get_new_named(testdef);
        if (!newdef) {
            testdef = typ_get_parent_typdef(testdef);
            continue;
        }
                      ...
                      typ_def_t *typdef)
{
    ...
    if (retres == NO_ERR) {
        res = insert_yang_enumbit(tkc, mod, enu, typdef, NCX_BT_BITS, pos);
        if (res != NO_ERR) {
            typ_free_enum(enu);
        }
    } else {
                      ...
                      typ_def_t *typdef)
{
    ...
    if (!bitdone) {
        ...
        ncx_mod_missing_err(tkc, mod, "bits", "bit");
    }
                       ...
                       typ_def_t *typdef)
{
    ...
    if (retres == NO_ERR) {
        res = insert_yang_enumbit(tkc, mod, enu, typdef, NCX_BT_ENUM, valdone);
        if (res != NO_ERR) {
            typ_free_enum(enu);
        }
    } else {
                       ...
                       typ_def_t *typdef)
{
    ...
    while (!done) {
        ...
        if (!xml_strcmp(val, YANG_K_TYPE)) {
            ...
            res = yang_typ_consume_type(pcb, tkc, mod, un->typdef);
            if (res != NO_ERR) {
                retres = res;
                if (NEED_EXIT(res)) {
                    typ_free_unionnode(un);
                       ...
                       ncx_btype_t rbtyp)
{
    ...
    case NCX_CL_NAMED:
        ...
        } // else the named type is after this one and not available
        break;
    case NCX_CL_SIMPLE:
                        ...
                        const xmlChar *name)
{
    ...
    if (retres == NO_ERR && defval != NULL && !defvaldone) {
        ...
        if (parent != NULL) {
            log_error("\nError: Union %s '%s' has invalid default value (%s)",
                      (name) ? "type" : 
                              ...
                              const xmlChar *name)
{
    ...
    for (un = (typ_unionnode_t *)dlq_firstEntry(unionQ);
        ...
        if (un->typdef->flags & TYP_FL_RESOLVE_SKIPPED) {
            res = obj_set_named_type(tkc, mod, name, un->typdef,
                                     parent, NULL /* grp */);

            if (res == NO_ERR) {
                res = restriction_test(tkc, mod, un->typdef);
            }
            if (res != NO_ERR) {
                return res;
            }
                  ...
                  grp_template_t *grp)
{
    ...
    /* check the appinfoQ */
    status_t res = ncx_resolve_appinfoQ(pcb, tkc, mod, &typdef->appinfoQ);
    if (NEED_EXIT(res)) {
        return res;
    } else {
        res = NO_ERR;
    ...
    if (res == NO_ERR) {
        ...
        case NCX_BT_BITS:
            {
                ...
                if (typdef->tclass == NCX_CL_SIMPLE) {
                    for (enu = (typ_enum_t *)
                        ...
                        res = ncx_resolve_appinfoQ(pcb, tkc, mod, 
                                                   &enu->appinfoQ);
                        CHK_EXIT(res, retres);
                    }
                    res = NO_ERR;
        ...
        case NCX_BT_IDREF:
            {
                typ_idref_t *idref = typ_get_idref(typdef);
                if (idref) {
    ...
    if (res == NO_ERR && typdef->tclass == NCX_CL_SIMPLE &&  
        btyp == NCX_BT_UNION) {

        typ_def_t *testdef = typ_get_base_typdef(typdef);
    ...
    if (name != NULL) {
        ...
        if (obj && obj->grp && obj->grp->parentgrp) {
            grp_template_t *nextgrp = obj->grp->parentgrp;
            while (nextgrp) {
                        ...
                        const xmlChar *defval)
{
    ...
    if (typdef->flags & TYP_FL_RESOLVE_SKIPPED) {
        res = obj_set_named_type(tkc, mod, name, typdef, obj, NULL /* grp */);
...
#else
    if (!typ_ok(typdef) || typdef->tclass == NCX_CL_BASE ) {
        /* skip this test if typdef malformed or base type */
        return res;
    }
#endif
    ...
    const xmlChar *typname =
        (typdef->typenamestr) ? typdef->typenamestr : NCX_EL_NONE;

    log_debug4("\nyang_typ: resolve type final '%s'", typname);

    /* check default value if any defined */
    if (btyp == NCX_BT_LEAFREF || btyp == NCX_BT_INSTANCE_ID) {
        xpath_pcb_t *pcb = typ_get_leafref_pcb(typdef);
        if (tkc) {
                           ...
                           typ_def_t *intypdef)
{
    ...
    return consume_type(pcb, tkc, mod, intypdef, FALSE);
                               ...
                               obj_template_t *parent)
{
    ...
    for (typ = (typ_template_t *)dlq_firstEntry(typeQ);
        ...
        res = resolve_typedef(pcb, tkc, mod, typ, parent, NULL);
        CHK_EXIT(res, retres);
    }
                                 ...
                                 obj_template_t *obj)
{
    ...
    res = resolve_type_final(tkc, mod, obj, typdef, name, defval);
    return res;
