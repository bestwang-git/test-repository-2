...
static status_t
    add_node_defaults (obj_template_t *chobj,
                       val_value_t *val,
                       val_value_t *rootval,
                       val_value_t *cxtval,
                       boolean scriptmode,
 ...
 *             to a case
 *   rcxt == runstack context (may be NULL)
                  ...
                  ncx_etag_t etag)
{
    ...
    if (LOGDEBUG3) {
        log_debug3("\nadd_default: checking '%s:%s'",
                   obj_get_mod_name(obj), obj_get_name(obj));
    }

    obj_template_t *chobj = obj_first_child(obj);
    for (; chobj != NULL && res == NO_ERR;
         chobj = obj_next_child(chobj)) {
        if (obj_has_name(chobj) && obj_is_config(chobj)) {
            res = add_node_defaults(chobj, val, rootval, cxtval,
                                    scriptmode, rcxt, last_modified, etag);
        }
    }
                    ...
                    uint32  *len)
{
    ...
    if (force_quotes) {
        quotes = TRUE;
    } else if (format==NCX_IFMT_XPATH1 || format==NCX_IFMT_XPATH2) {
        quotes = TRUE;
    } else if (typ_is_string(val->btyp)) {
        quotes = (format==NCX_IFMT_CLI || format==NCX_IFMT_CLI2) ?
    ...
    if (quotes) {
        qchar = (xmlChar)
            ((format==NCX_IFMT_XPATH1 || format==NCX_IFMT_CLI) ?
                         ...
                         uint32  *len)
{
    ...
    *len = 0;
    ...
    if (val->obj->objtype == OBJ_TYP_RPCIO) {
        ...
        name = val->name;
        if (name == NULL) {
            name = (val->obj) ? obj_get_name(val->obj) : NCX_EL_NONE;
        }
    }
    ...
    } else if (format == NCX_IFMT_CLI2) {
        if (!root) {
            if (buff) {
                *buff++ = VAL_CLI_SEPCH;   /*   middle ' ' */
            }
            cnt++;
        }
    } else {
    ...
    if (root && val->obj->objtype == OBJ_TYP_RPCIO) {
        ...
        if (buff) {
            ...
            switch (format) {
            case NCX_IFMT_C:
                sepch = VAL_INST_SEPCH;
                break;
            case NCX_IFMT_CLI2:
                sepch = VAL_INDEX_CLI_SEPCH;
                break;
            default:
                sepch = VAL_XPATH_SEPCH;
            }
            *buff++ = sepch;
        }
        cnt++;
    ...
    if (!dlq_empty(&val->indexQ)) {
        cnt = 0;
        res = val_get_index_string(mhdr, format, FALSE, val, buff, &cnt);
        if (res == NO_ERR) {
            if (buff) {
                buff[cnt] = 0;
            }
...
/********************************************************************
 * FUNCTION add_node_defaults
 * 
 * Check/Add defaults for the specified node and descendant nodes
 * Allow a runstack_context to be passed for val_add_defaults in
 * script mode
 * INPUTS:
 *   obj == top-lvel object to add defaults to
 *   val == the value struct to modify
 *   rootval == the root value for XPath purposes
 *           == NULL to skip when-stmt check
 *   cxtval == the context value for XPath purposes
 ...
 *   scriptmode == TRUE if the value is a script object access
 *              == FALSE for normal val_get_simval access instead
 *   rcxt == runstack_context to use (may be NULL)
 *   last_modified == timestamp to set in new value
 *   etag == entity tag to set in new value
 *
 * OUTPUTS:
                       ...
                       ncx_etag_t etag)                              
{
    ...
    /* check if this node has a false when-stmt associated
     * with it; skip this node if when-stmt(s) are FALSE
     */
    if (rootval) {
        uint32 whencount = 0;
        ...
        if (whencount && !condresult) {
            if (LOGDEBUG3) {
                log_debug3("\nadd_default: skipping false when '%s:%s'",
                           obj_get_mod_name(chobj), obj_get_name(chobj));
            }
            return NO_ERR;
        } else if (whencount && condresult && LOGDEBUG3) {
            log_debug3("\nadd_default: processing true when '%s:%s'",
                       obj_get_mod_name(chobj), obj_get_name(chobj));
        }
    }
    ...
    case OBJ_TYP_ANYXML:
        /* anyxml not allowed to have default-stmt */
        break;
    case OBJ_TYP_LEAF:
        /* always skip key leafs */
        if (obj_is_key(chobj)) {
            break;
        }

        /* first check if this leaf has a default */
        ...
        chval = val_find_child(val, obj_get_mod_name(chobj),
                               obj_get_name(chobj));
        if (!chval) {
            /* add the default leaf */
            res = cli_parse_parm(rcxt, val, chobj, defval, scriptmode);
            if (res==NO_ERR) {
                chval = val_find_child(val, obj_get_mod_name(chobj),
                                       obj_get_name(chobj));
                if (!chval) {
                    ...
                    chval->last_modified = last_modified;
                    chval->etag = etag;
                }
            }
        }
    ...
    case OBJ_TYP_CHOICE:
        ...
        if (casobj) {
             ...
             * or selected case 
             */
            res = add_defaults(val, rootval, cxtval, scriptmode, 
                               casobj, rcxt, last_modified, etag);
        }
        break;
    ...
    case OBJ_TYP_CONTAINER:
        if (obj_is_root(chobj)) {
            /* do not dive into <config> nodes starting from
             * the <rpc> element    */
            break;
        }
        /* else fall through */
    ...
    case OBJ_TYP_CASE:
        ...
        chval = val_find_child(val, obj_get_mod_name(chobj),
                               obj_get_name(chobj));
        if (chval) {
            if (cxtval && typ_has_children(cxtval->btyp)) {
                if (ncx_fastpath_enabled()) {
                    chcxtval =
                        val_first_child_match_fast(cxtval, chval, NULL);
                } else {
                    chcxtval = val_first_child_match(cxtval, chval);
                }
            }
            res = add_defaults(chval, rootval, chcxtval, scriptmode,
                               NULL, rcxt, last_modified, etag);
        } else if (obj_npcon_has_defaults(chobj)) {
            /* this is a default NP container with nested def. leafs */
            val_value_t *npcon = val_new_value();
            if (npcon == NULL) {
                res = ERR_INTERNAL_MEM;
                break;
            }

            val_init_from_template(npcon, chobj);
            npcon->flags |= VAL_FL_DEFSET;
            npcon->last_modified = last_modified;
            npcon->etag = etag;

            val_add_child_sorted(npcon, val);

            if (LOGDEBUG3) {
                log_debug3("\nadd default NP container '%s:%s'",
                           val_get_mod_name(npcon), npcon->name);
            }

            res = add_defaults(npcon, rootval, chcxtval, scriptmode,
                               NULL, rcxt, last_modified, etag);
        }
        if (chobj->objtype == OBJ_TYP_LIST) {
            while (res == NO_ERR && chval) {
                chval = val_find_next_child(val, obj_get_mod_name(chobj),
                                            obj_get_name(chobj), chval);
                if (chval) {
                    if (chcxtval) {
...
status_t 
    val_add_defaults (val_value_t *val,
                      ...
                      boolean scriptmode)
{
    return add_defaults(val, rootval, cxtval, scriptmode,
                        NULL, NULL, 0, 0);
...
/********************************************************************
 * FUNCTION val_add_defaults_ex
 * 
 * add defaults to an initialized complex value
 * Go through the specified value struct and add in any defaults
 * for missing leaf and choice nodes, that have defaults.
 * Use a runstack_contect for adding defaults!!
 *
 * !!! Only the child nodes will be checked for missing defaults
 * !!! The top-level value passed to this function is assumed to
 * !!! be already set
 ...
 *********************************************************************/
status_t 
    val_add_defaults_ex (val_value_t *val,
                         val_value_t *rootval,
                         val_value_t *cxtval,
                         boolean scriptmode,
                         runstack_context_t *rcxt,
                         ...
                         ncx_etag_t etag)
{
    return add_defaults(val, rootval, cxtval, scriptmode, NULL,
                        rcxt, last_modified, etag);

} /* val_add_defaults_ex */


/********************************************************************
 * FUNCTION val_add_node_defaults
 * 
 * Check/Add defaults for the specified node and descendant nodes
 *
 * INPUTS:
 ...
 *********************************************************************/
status_t 
    val_add_node_defaults (obj_template_t *chobj,
                           val_value_t *val,
                           val_value_t *rootval,
                           val_value_t *cxtval,
                           boolean scriptmode,
                           ...
                           ncx_etag_t etag)
{
    return add_node_defaults(chobj, val, rootval, cxtval, scriptmode,
                          ...
                          uint32  *len)
{
    ...
    *len = 0;
    ...
    const val_value_t  *ival = NULL;
    const val_index_t *valin =
        (const val_index_t *)dlq_firstEntry(&val->indexQ);
    if (valin) {
        ival = valin->val;
    } else {
        return SET_ERROR(ERR_INTERNAL_VAL);
     ...
     * start with the left bracket or space
     */
    if (buff) {
        if (format == NCX_IFMT_CLI || format == NCX_IFMT_CLI2) {
            *buff++ = (xmlChar)' ';
        } else {
    ...
    while (ival) {
        ...
        status_t res = get_index_comp(mhdr, format, force_quotes, ival,
                                      buff, &cnt);
        if (res != NO_ERR) {
            return res;
        } else {
        ...
        const val_value_t *nextival = NULL;
    ...
    /* add the closing right bracket or space */
    if (!(format == NCX_IFMT_CLI || format == NCX_IFMT_CLI2)) {
        if (buff) {
            *buff++ = VAL_EINDEX_CH;
            *buff = 0;
                        ...
                        uint32  *whencount)
{
    ...
    *condresult = FALSE;
     ...
     */
    ncx_backptr_t *xptr = obj_first_xpath_ptr(obj);
    for (; xptr; xptr = obj_next_xpath_ptr(xptr)) {
        cnt++;
        res = check_when_stmt(val, valroot, val, 
                              (xpath_pcb_t *)xptr->node, condresult);
        if (res != NO_ERR || !*condresult) {
            if (whencount) {
                *whencount = cnt;
            }
    ...
    val_set_logging_parms (val_value_t *parentval)
{
     ...
     * do not overwrite the value that was passed into ncx_init
     * if the CLI value is set by default
     */
    val = val_find_child(parentval, 
                         val_get_mod_name(parentval),
                         NCX_EL_LOGLEVEL);
    if (val && val->res == NO_ERR && !val_set_by_default(val)) {
        log_set_debug_level
    ...
    /*
     * Process --log-mirroring
     *
     * Set this early so that it does not inhibit debug output to
     * STDOUT or logfile if/when --log-syslog is specified.
    */
     ...
     * Process --log-append
     *
     * Set this before --log=<filename> since it is required when that
     * file is opened.
     */
    val = val_find_child(parentval, val_get_mod_name(parentval),
                         NCX_EL_LOGAPPEND);
    ...
    /* Process --log-backtrace=<max-frames> */
    val = val_find_child(parentval, val_get_mod_name(parentval),
                         NCX_EL_LOGBACKTRACE);

    if (val && (val->res == NO_ERR)) {
        log_set_backtrace(VAL_UINT(val));
    }

    /* Process --log-backtrace-detail */
    val = val_find_child(parentval, val_get_mod_name(parentval),
                         NCX_EL_LOGBT_DETAIL);
   if (val && val->res == NO_ERR) {
	...
	if (ncx_string_in_list((const xmlChar *)"write", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_WRITE);
	}
	if (ncx_string_in_list((const xmlChar *)"error", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_ERROR);
	}
	if (ncx_string_in_list((const xmlChar *)"warn", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_WARN);
	}
	if (ncx_string_in_list((const xmlChar *)"dev0", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEV0);
	}
	if (ncx_string_in_list((const xmlChar *)"dev1", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEV1);
	}
	if (ncx_string_in_list((const xmlChar *)"info", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_INFO);
	}
	if (ncx_string_in_list((const xmlChar *)"debug", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEBUG);
	}
	if (ncx_string_in_list((const xmlChar *)"debug2", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEBUG2);
	}
	if (ncx_string_in_list((const xmlChar *)"debug3", bits)) {
	    level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEBUG3);
	}
	if (ncx_string_in_list((const xmlChar *)"debug4", bits)) {
	...
	if (ncx_string_in_list((const xmlChar *)"custom", bits)) {
	    log_set_custom();
	}
	if (ncx_string_in_list((const xmlChar *)"localtime", bits)) {
	    log_set_localtime();
	}
    }

    /* Process --log-suppress-ctrl */
    val = val_find_child(parentval, val_get_mod_name(parentval),
                         NCX_EL_LOGSUPPRESSCTRL);
   if (val && val->res == NO_ERR) {
...
/********************************************************************
* FUNCTION val_set_message_indent_parm
...
*********************************************************************/
status_t
    val_set_message_indent_parm (val_value_t *parentval)
{
    val_value_t        *val;
    ...
    if (val && val->res == NO_ERR) {
        ncx_set_message_indent(VAL_INT8(val));
    }
...
*********************************************************************/
void
    val_set_home_parms (val_value_t *parentval)
{
#ifdef DEBUG
    if (!parentval) {
        SET_ERROR(ERR_INTERNAL_PTR);
    ...
    val_value_t *val =
        val_find_child(parentval, val_get_mod_name(parentval), NCX_EL_HOME);
    if (val && val->res == NO_ERR) {
        ncxmod_set_home(VAL_STR(val));
    }


    /* get the --yumapro-home parameter */
    val = val_find_child(parentval, val_get_mod_name(parentval),
                         NCX_EL_YUMAPRO_HOME);
    if (val && val->res == NO_ERR) {
        ncxmod_set_yuma_home(VAL_STR(val));
    }
                   ...
                   status_t *res)
{
    ...
    *malloced = FALSE;
    ...
    if (val->obj && obj_is_root(val->obj)) {
        *res = NO_ERR;
        return val;
    }
    ...
    if (testfn) {
        if (!(*testfn)(msg, msg->withdef, TRUE, val)) {
            *res = ERR_NCX_SKIPPED;
            return NULL;
        }
    }
    ...
    val_value_t *useval = (v_val) ? v_val : val;
    ...
    if (useval->btyp == NCX_BT_LEAFREF) {
        ...
        if (realtypdef) {
            ...
            default:
                ...
                if (realval) {
                    *malloced = TRUE;
                    val_move_fields_for_xml(val, realval,
                                            msg->acm_cbfn == NULL);
                    return realval;
                } else {
                    return NULL;
...
*********************************************************************/
status_t
    val_cache_datarule (val_value_t *val,
                        void *rule)
{
    /* check for first empty slot */
    uint32 i = 0;
    for (; i < VAL_MAX_DATARULES; i++) {
        if (val->datarules[i] == NULL) {
            val->datarules[i] = rule;
            return NO_ERR;
        }
    }

    status_t res = NO_ERR;
    ncx_backptr_t *ptr = ncx_new_backptr(rule);
    if (ptr) {
        dlq_enque(ptr, &val->dataruleQ);
    } else {
        res = ERR_INTERNAL_MEM;
    }
    return res;
...
*********************************************************************/
void
    val_clear_datarule (val_value_t *val,
                        const void *rule)
{
    /* check for slot containing this rule back-ptr */
    uint32 i = 0;
    for (; i < VAL_MAX_DATARULES; i++) {
        if (val->datarules[i] == rule) {
            val->datarules[i] = NULL;
            return;
        }
    }

    ncx_backptr_t *ptr = ncx_first_backptr(&val->dataruleQ);
    for (; ptr; ptr = ncx_next_backptr(ptr)) {
        if (ptr->node == rule) {
            ncx_remove_backptr(ptr);
...
*********************************************************************/
boolean
    val_match_datarule (val_value_t *val,
                        void *rule)
{
    /* check for first empty slot */
    uint32 i = 0;
    ...
    ncx_backptr_t *ptr = ncx_find_backptr(&val->dataruleQ, rule);
...
/********************************************************************
* FUNCTION val_find_list_from_keyvalQ
* 
...
*   keyvalQ == Q of yangapi_keyval_t structs representing the
*              string value of each key
*   res == address of return status
*
* OUTPUTS:
...
*********************************************************************/
val_value_t *
    val_find_list_from_keyvalQ (val_value_t *parentval,
                                ...
                                status_t *res)
{
    *res = NO_ERR;
    ...
    obj_template_t *obj = firstmatch->obj;
    if (obj == NULL) {
        *res = ERR_NCX_OPERATION_FAILED;
        return NULL;
    }

    uint32 keycount = obj_key_count(obj);
    if (keycount == 0) {
        *res = ERR_NCX_MISSING_KEY;
        return NULL;
    }

    if (keycount != dlq_count(keyvalQ)) {
        *res = ERR_NCX_MISSING_KEY;
        return NULL;
    }

    /* start dummy list to match against */
    val_value_t *dumlist = val_new_value();
    if (dumlist == NULL) {
        *res = ERR_INTERNAL_MEM;
        return NULL;
    }
    val_init_from_template(dumlist, obj);

    val_value_t *retval = NULL;
    obj_key_t *objkey = obj_first_key(obj);
    yangapi_keyval_t *kv = (yangapi_keyval_t *)dlq_firstEntry(keyvalQ);
    for (; kv != NULL && objkey != NULL && *res == NO_ERR;
         kv = (yangapi_keyval_t *)dlq_nextEntry(kv),
             objkey = obj_next_key(objkey)) {

        val_value_t *dumkey = val_make_simval_obj(objkey->keyobj,
                                                  kv->value, res);
        if (dumkey && *res == NO_ERR) {
            val_add_child(dumkey, dumlist);
        } else {
            val_free_value(dumkey);
            if (*res == NO_ERR) {
                *res = ERR_INTERNAL_MEM;
            }
        }
    ...
    if (*res == NO_ERR) {
        *res = val_gen_index_chain(dumlist->obj, dumlist);
    }

    if (*res == NO_ERR) {
        boolean done = FALSE;
        for (; firstmatch != NULL && !done;
             firstmatch = val_find_next_child(parentval,
                                              val_get_mod_name(firstmatch),
                                              firstmatch->name, firstmatch)) {
            if (val_index_match(firstmatch, dumlist)) {
                retval = firstmatch;
                done = TRUE;
            }
        }
...
/********************************************************************
* FUNCTION val_move_config_false
*
* Look for all the child nodes of the newval and
* any config-false child nodes will moved 
...
*********************************************************************/
status_t
    val_move_config_false (val_value_t *newval,
                           val_value_t *curval)
{
    assert(newval && "newval is NULL!");
    assert(curval && "curval is NULL!");

    if (newval->obj != curval->obj) {
        return ERR_NCX_WRONG_NODETYP;
    }

    if (!obj_has_children(curval->obj)) {
        return NO_ERR;
    }

    val_value_t *curchild = val_get_first_child(curval);
    val_value_t *next_curchild = NULL;
    for (; curchild; curchild = next_curchild) {
        next_curchild = val_get_next_child(curchild);

        if (!obj_is_config(curchild->obj)) {
            val_remove_child(curchild);
            val_add_child_sorted(curchild, newval);
        }
    }
...
/********************************************************************
 * FUNCTION val_add_one_operation_attr
 * 
 ...
 *********************************************************************/
status_t
    val_add_one_operation_attr (val_value_t *val,
                                op_editop_t op)
{
    assert(val && "val is NULL!");

    if (LOGDEBUG3) {
        log_debug3("\nAdding operation=%s attr to %s:%s",
                   op_editop_name(op), val_get_mod_name(val),
                   val->name);
    }

    /* get the string value for the edit operation */
    const xmlChar *editopstr = op_editop_name(op);
    if (editopstr  == NULL) {
        return ERR_NCX_INVALID_VALUE;
    }

    /* get the internal nc:operation object */
    obj_template_t *operobj = ncx_get_nc_operation();
    if (operobj == NULL) {
    ...
    status_t res = val_set_simval(metaval, obj_get_typdef(operobj),
 ...
 *********************************************************************/
val_value_t *
    val_make_config_root (void)
{
    val_value_t *configval = val_new_value();
    if (configval == NULL) {
        return NULL;
    }

    val_value_t *cfgroot = cfg_get_root(NCX_CFGID_RUNNING);
    if (cfgroot) {
        val_init_from_template(configval, cfgroot->obj);
    } else {
        val_init_from_template(configval, ncx_get_gen_root());
        val_set_name(configval, NCX_EL_CONFIG, xml_strlen(NCX_EL_CONFIG));
        val_change_nsid(configval, xmlns_nc_id());
    }
