...
static tk_ent_t tlist [] = {
    ...
    { TK_TT_LBRACK, 1, "[", "left bracket", FL_XPATH | FL_CONF },
    { TK_TT_RBRACK, 1, "]", "right bracket", FL_XPATH | FL_CONF },
    ...
    { TK_TT_LINE, 0, "", "raw line string", FL_CONF},
...
/********************************************************************
* FUNCTION add_lbracket_token
* 
* Allocatate a new left bracket token and add it to the parse chain
*
* INPUTS:
...
*********************************************************************/
static status_t 
    add_lbracket_token (tk_chain_t *tkc)
{
    tk_token_t   *tk;
    ...
    tk = new_token(TK_TT_LBRACK, (const xmlChar *)"[", 1);
    if (!tk) {
        return ERR_INTERNAL_MEM;
    }
    tk->linenum = tkc->linenum;
...
/********************************************************************
* FUNCTION add_rbracket_token
* 
* Allocatate a new right bracket token and add it to the parse chain
*
* INPUTS:
*   tkc == token chain to use
*
...
*********************************************************************/
static status_t 
    add_rbracket_token (tk_chain_t *tkc)
{
    tk_token_t   *tk;
    ...
    tk = new_token(TK_TT_RBRACK, (const xmlChar *)"]", 1);
    if (!tk) {
        return ERR_INTERNAL_MEM;
    }
...
/********************************************************************
* FUNCTION tokenize_conf_line_block
* 
* Handle a [ <raw-lines> ] sequence
*
* Converts escaped chars during this first pass processing
*
* Does not realign whitespace in this function
...
*********************************************************************/
static status_t 
    tokenize_conf_line_block (tk_chain_t *tkc)
{
    /* the bptr is pointing at the '[' char which
     * indicates the start of a raw line block
     * find the end of each line 
     */
    status_t res = add_lbracket_token(tkc);
    if (res != NO_ERR) {
        return res;
    }

    /* skip over last char (1st time: left bracket */
    ++tkc->linenum;
    ++tkc->linepos;
    ++tkc->bptr;

    /* find all the raw lines for this block */
    boolean done = FALSE;    
    while (!done && res == NO_ERR) {

        /* set the start char of the raw line candidate */
        xmlChar *str = tkc->bptr;

        /* find the EOLN */
        boolean nowsp = FALSE;
        while (*str && *str != '\n') {
            if (*str == ']' && !nowsp) {
                /* found right bracket as first non-whsp char on
                 * rest-of-line so treat it as the end of block  */
                uint32 total = (uint32)(str - tkc->bptr);
                tkc->linepos += total;
                tkc->bptr += total;
                res = add_rbracket_token(tkc);

                /* account for the right bracket char */
                tkc->linepos++;
                tkc->bptr++;
        ...
        if (!*str) {
            /* End of buffer reached: error should have found right bracket */
            done = TRUE;
            res = ERR_NCX_UNENDED_BLOCK;
            continue;
        }

        if (nowsp) {
            /* save the non-empty line */
            res = add_new_token(tkc, TK_TT_LINE, str, tkc->linepos);
            if (res != NO_ERR) {
                done = TRUE;
                continue;
            }
    ...
    tokenize_id_string (tk_chain_t *tkc)
{
    ...
    /* check CONF parser stopped on proper end of ID */
...
/********************************************************************
* FUNCTION tk_setup_chain_cli
* 
* Setup a previously allocated chain for a CLI command line
*
* INPUTS
*    tkc == token chain to setup
*    line == CLI buffer to parse; this pointer is just borrowed
*            it will not be freed when the oken chain is freed
*********************************************************************/
void
    tk_setup_chain_cli (tk_chain_t *tkc,
                        xmlChar *line)
{
#ifdef DEBUG
    if (!tkc || !line) {
    ...
    while (!done && res == NO_ERR) {
        ...
        while (*tkc->bptr && res==NO_ERR) { 
            ...
            if (!*tkc->bptr) {
                ...
                /* save newline token for conf file only */
                if (tkc->source == TK_SOURCE_CONF) {
                    tk_token_t *tk = new_token(TK_TT_NEWLINE, NULL, 0);
                    if (!tk) {
                        res = ERR_INTERNAL_MEM;
                        done = TRUE;
                        continue;
                ...
                /* in case this is a conf file, do not record
                 * a newline token for a comment line  */
                if (*tkc->bptr == '\n') {
                    tkc->bptr++;
                }
            } else if (tkc->source == TK_SOURCE_CONF && *tkc->bptr == '[') {
                /* found start of a conf file raw line block */
                res = tokenize_conf_line_block(tkc);
            } else if (tkc->source == TK_SOURCE_YANG &&
                       *tkc->bptr == '/' && tkc->bptr[1] == '*') {
                ...
                tk_type_t ttyp = get_token_id(tkc->bptr, 2, tkc->source);
                if (ttyp != TK_TT_NONE) {
...
*********************************************************************/
boolean
    tk_cur_is_first (const tk_chain_t  *tkc)
{
#ifdef DEBUG
    if (!tkc) {
        SET_ERROR(ERR_INTERNAL_PTR);
    ...
    tk_token_t *tk = (tk_token_t *)dlq_firstEntry(&tkc->tkQ);
    return (tk == tkc->cur);
