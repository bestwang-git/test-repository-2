                  ...
                  grp_template_t *grp)
{
    ...
    ingrp = FALSE;
    if (!list->keystr && obj_is_data_db(obj) &&
                   ...
                   obj_template_t *obj)
{
    ...
    for (must = (xpath_pcb_t *)dlq_firstEntry(mustQ);
        ...
        res = xpath1_validate_expr_ex(mod, obj, must, FALSE, FALSE);
        CHK_EXIT(res, retres);
    }
    return retres;
                  ...
                  obj_template_t *obj)
{
    ...
    return xpath1_validate_expr_ex(mod, context, when, FALSE, FALSE);
                             ...
                             boolean ingrouping)
{
    ...
    status_t res = check_parent(tkc, mod, obj);
    CHK_EXIT(res, retres);

    boolean ingrp = FALSE;
    boolean cfgcheck = obj_get_config_flag_check(obj, &ingrp);

    if (!ingrouping && !obj_is_abstract(obj) && cfgcheck &&
        obj_is_np_container(obj)) {
        if (obj_npcon_has_defaults_slow(obj)) {
            obj->xflags |= OBJ_FL_DEF_NPCON;
        }
    }
         
    if (!ingrouping && !obj_is_abstract(obj) &&
        obj_is_np_container(obj) && cfgcheck &&
                                ...
                                obj_template_t *targobj)
{
    ...
    if (srcobj->when) {
        xptr = ncx_new_backptr(srcobj->when);
        if (xptr) {
    ...
    for (xptr2 = (ncx_backptr_t *)dlq_firstEntry(&srcobj->inherited_whenQ);
         xptr2 != NULL;
         xptr2 = (ncx_backptr_t *)dlq_nextEntry(xptr2)) {

        xptr = ncx_new_backptr(xptr2->node);
        if (xptr) {
            dlq_enque(xptr, &targobj->inherited_whenQ);
        } else {
            return ERR_INTERNAL_MEM;
                 ...
                 dlq_hdr_t *datadefQ)
{
    ...
    if (LOGDEBUG4) {
        obj_dump_child_list(LOG_DEBUG_DEBUG4, datadefQ,
			    NCX_DEF_INDENT, NCX_DEF_INDENT);
    }
        ...
        case OBJ_TYP_LEAF_LIST:
            ...
            if (obj_get_basetype(testobj) == NCX_BT_LEAFREF) {
                ...
                if (!pcbclone) {
                    ...
                    res = xpath_yang_parse_path(tkc, mod, XP_SRC_LEAFREF,
                                                pcbclone);
                    if (res == NO_ERR) {
                        leafobj = NULL;
                        res = xpath_yang_validate_path(mod, testobj, pcbclone, 
                                                       FALSE, &leafobj);
