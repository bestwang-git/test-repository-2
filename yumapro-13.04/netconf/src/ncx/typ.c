...
* RETURNS:
*   pointer to Nth enumdef or NULL if none
*********************************************************************/
typ_enum_t *
    typ_get_enumdef_n (typ_def_t *typdef,
                       uint32 num)
{
#ifdef DEBUG
    if (!typdef) {
    ...
    uint32 count = 0;
    typ_enum_t *en = NULL;
    for (en = (typ_enum_t *)dlq_firstEntry(&typdef->def.simple.valQ);
         en != NULL; en = (typ_enum_t *)dlq_nextEntry(en)) {
        if (count == num) {
            return en;
        }
        count++;
