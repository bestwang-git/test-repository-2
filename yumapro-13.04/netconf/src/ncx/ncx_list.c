...
/********************************************************************
* FUNCTION ncx_list_linelen
* 
* Get the length of the string if the list was printed in 1 line
* with spaces between the entries
*
* INPUTS:
*    list == pointer to ncx_list_t memory
* RETURNS:
...
*********************************************************************/
uint32
    ncx_list_linelen (const ncx_list_t *list)
{
#ifdef DEBUG
    if (!list) {
        SET_ERROR(ERR_INTERNAL_PTR);
    ...
    ncx_btype_t btyp = list->btyp;
    uint32 total = 0;
    const ncx_lmem_t *nextmem = NULL;
    const ncx_lmem_t *listmem;

    for (listmem = (const ncx_lmem_t *)dlq_firstEntry(&list->memQ);
         listmem != NULL; listmem = nextmem) {

        nextmem = (const ncx_lmem_t *)dlq_nextEntry(listmem);

        uint32 len = 0;

        if (typ_is_string(btyp)) {
            if (listmem->val.str) {
                len = xml_strlen(listmem->val.str);
                if (val_need_quotes(listmem->val.str)) {
                    len += 2;
                }
            } else {
                len = 2;
            ...
            status_t res = ncx_sprintf_num(numbuff, &listmem->val.num,
                                           btyp, &len);
            if (res != NO_ERR) {
            ...
            case NCX_BT_ENUM:
                if (listmem->val.enu.name) {
                    len = xml_strlen(listmem->val.enu.name);
                } else {
                    len = 2;
                }
                break;
            ...
            case NCX_BT_BOOLEAN:
                if (listmem->val.boo) {
                    len = xml_strlen(NCX_EL_TRUE);
                } else {
                    len = xml_strlen(NCX_EL_FALSE);
                }
                break;
            default:
        ...
        if (nextmem) {
            len++;  // space between list entries 
        }
...
*********************************************************************/
uint32
    ncx_lmem_count (ncx_list_t *list)
{
#ifdef DEBUG
    if (!list) {
        SET_ERROR(ERR_INTERNAL_PTR);
...
#endif
    return (ncx_lmem_t *)dlq_nextEntry(cur);

}  /* ncx_next_lmem */


/********************************************************************
* FUNCTION ncx_add_strlist
* 
* add a string as a complete list member to an initialized list
* must be type NCX_BT_STRING
*
* INPUTS:
*     liststr == list string to add
*     liststrlen == length od liststr
*     list == ncx_list_t that should already be initialized and
*             will have an lmem added with the value from the string
*
* RETURNS:
*     status
*********************************************************************/
status_t
    ncx_add_strlist (const xmlChar *liststr,
                     uint32 liststrlen,
                     ncx_list_t *list)
{
#ifdef DEBUG
    ...
    if (list->btyp != NCX_BT_STRING) {
        return ERR_NCX_WRONG_TYPE;
    }

    /* get rid of quotes if they are present
     * assuming that well-formed quoted string is present!!
     */
    if (liststrlen >=2 && (*liststr == '"' || *liststr == '\'')) {
        liststr++;
        liststrlen -= 2;
    }

    ncx_lmem_t *lmem = ncx_new_lmem();
    if (!lmem) {
        return ERR_INTERNAL_MEM;
    }

    lmem->val.str = xml_strndup(liststr, liststrlen);
    if (!lmem->val.str) {
        ncx_free_lmem(lmem, NCX_BT_STRING);
        return ERR_INTERNAL_MEM;
    }
...
/********************************************************************
* FUNCTION ncx_get_lmem_strval
* 
* Get the string value from an lmem
* must be type NCX_BT_STRING
*
* INPUTS:
...
*********************************************************************/
const xmlChar *
    ncx_get_lmem_strval (const ncx_lmem_t *lmem)
{
    return lmem->val.str;
