...
static void init_template (obj_template_t *obj)
{
    ...
    dlq_createSQue(&obj->xpath_backptrQ);
                   ...
                   uint32 *matchcount  )
{
    ...
    // the name of the current obj being checked
    const xmlChar *curname = NULL;

    // the name of the current mod being checked
    const xmlChar *curmodulename = NULL;  
                       ...
                       boolean forcexpath)
{
    *retlen = 0;
    ...
    /* hack: using both flags == TRUE to identify that
     * this object string will be used for rpc-error
     * so the input node is not supposed to be present
     */
    boolean skip_rpcio = withmodname && forcexpath;
    ...
    if (skip_rpcio && obj->objtype == OBJ_TYP_RPCIO) {
        return NO_ERR;
    }
...
/********************************************************************
 * Utility function for validating the type of a found child node.
 ...
 *
 * \param ptrQ the Q of ncx_backptr_t to clean
 *********************************************************************/
static void clean_inherited_whenQ( dlq_hdr_t *ptrQ )
{
    while (!dlq_empty(ptrQ)) {
        ncx_backptr_t *ptr = (ncx_backptr_t *)dlq_deque(ptrQ);
...
/********************************************************************
 * Set any XPath backptrs for the specified object
 *
 * \param obj the object to check for any needed xpath_backptrs
 *********************************************************************/
static status_t
    set_xpath_backptrs (obj_template_t *obj)
{
    status_t res = NO_ERR;
    ncx_module_t *mod = obj_get_mod(obj);

    /* check the one when statement for this object */
    if (obj->when) {
#ifdef OBJ_BACKPTR_DEBUG
        if (LOGDEBUG2) {
            log_debug2("\nxpath_backptr: set 'when' for obj '%s:%s'", 
                       obj_get_mod_name(obj), obj_get_name(obj));
        }
#endif
        res = xpath1_validate_expr_ex(mod, obj, obj->when, FALSE, TRUE);
        if (res != NO_ERR) {
            return res;
        }
    }

    /* check all inherited when statements for this object */
    ncx_backptr_t *xptr = (ncx_backptr_t *)
        dlq_firstEntry(&obj->inherited_whenQ);
    for (; xptr != NULL; xptr = (ncx_backptr_t *)dlq_nextEntry(xptr)) {
#ifdef OBJ_BACKPTR_DEBUG
        if (LOGDEBUG2) {
            log_debug2("\nxpath_backptr: set 'inherited-when' for obj '%s:%s'", 
                       obj_get_mod_name(obj), obj_get_name(obj));
        }
#endif

        xpath_pcb_t *pcb = (xpath_pcb_t *)xptr->node;
        res = xpath1_validate_expr_ex(mod, obj, pcb, FALSE, TRUE);
        if (res != NO_ERR) {
            return res;
        }
    }

    /* check all the must statements for this object */
...
#ifdef OBJ_BACKPTR_DEBUG
            if (LOGDEBUG2) {
                log_debug2("\nxpath_backptr: set 'must' for obj '%s:%s'", 
                           obj_get_mod_name(obj), obj_get_name(obj));
            }
#endif

            res = xpath1_validate_expr_ex(mod, obj, must, FALSE, TRUE);
            if (res != NO_ERR) {
                return res;
            }
        }
...
void obj_free_template( obj_template_t *obj )
{
    ...
    while (!dlq_empty(&obj->xpath_backptrQ)) {
        ncx_backptr_t *ptr = (ncx_backptr_t *)dlq_deque(&obj->xpath_backptrQ);
        ncx_free_backptr(ptr);
    }
...
status_t obj_gen_object_id_error (const obj_template_t *obj, xmlChar  **buff)
{
    ...
    *buff = NULL;
    ...
    /* figure out the length of the object ID */
    res = get_object_string(obj, NULL, NULL, 0, TRUE, NULL, &len, TRUE, TRUE);
    if (res != NO_ERR) {
        return res;
    }

    /* get the nc prefix */
    const xmlChar *prefix = xmlns_get_ns_prefix(xmlns_nc_id());
    uint32 plen = xml_strlen(prefix);
    uint32 rlen = xml_strlen(NCX_EL_RPC);

    /* get a buffer to fit the instance ID string + /nc:rpc */
    *buff = (xmlChar *)m__getMem(len + plen + rlen + 3);
    if (!*buff) {
        return ERR_INTERNAL_MEM;
    }
    ...
    *ptr = '/';
    xml_strcpy(ptr+1, prefix);
    ptr[plen+1] = ':';
    xml_strcpy(&ptr[plen+2], NCX_EL_RPC);

    /* get the object ID for real this time */
    res = get_object_string(obj, NULL, &ptr[plen+2+rlen], len+1, TRUE,
                            NULL, &len, TRUE, TRUE);
    if (res != NO_ERR) {
        m__free(*buff);
        *buff = NULL;
    ...
    obj_get_name (const obj_template_t *obj)
{
    ...
    case OBJ_TYP_NONE:
        assert(0 && "object type not set");
        /* fall through if assert disabled */
    default:
    ...
    obj_get_default_case (obj_template_t *obj)
{
    ...
    if (obj->def.choic->defval) {
        dlq_hdr_t *que = obj_get_datadefQ(obj);
        if ( !que ) {
            return NULL;
        }

        uint32 matchcount = 0;
        return find_template(que, obj_get_mod_name(obj),
...
/********************************************************************
* FUNCTION obj_npcon_has_defaults
* 
* Check if the specified NP container has defaults within it
...
*  
* RETURNS:
*   TRUE if NP container and it has default leafs within it
*   or within NP containers within this NP container
*   FALSE if not an NP container or no defaults within it
*********************************************************************/
boolean
    obj_npcon_has_defaults (obj_template_t *obj)
{
    assert(obj && "obj is NULL!");

    return (obj->xflags & OBJ_FL_DEF_NPCON);

}  /* obj_npcon_has_defaults */


/********************************************************************
* FUNCTION obj_npcon_has_defaults_slow
* 
* Check if the specified NP container has defaults within it
* Must be a config object!!!
*
...
*********************************************************************/
boolean
    obj_npcon_has_defaults_slow (obj_template_t *obj)
{
    assert(obj && "obj is NULL!");
    ...
    for ( ; chobj && !test; chobj = obj_next_child(chobj)) {
        ...
        if (obj_is_np_container(chobj)) {
            test = obj_npcon_has_defaults_slow(chobj);
        } else if (obj_get_default(chobj)) {
            test = TRUE;
        }
    }
 ...
 *
 * \param obj the obj_template to check
 * \return TRUE if object is a leaf-list
 *********************************************************************/
boolean obj_is_leaf_list (const obj_template_t  *obj)
{
    assert(obj && "obj is NULL" );
    return (obj->objtype == OBJ_TYP_LEAF_LIST); 
}  /* obj_is_leaf_list */
...
{
    assert(obj && "obj is NULL" );
    return (obj->objtype == OBJ_TYP_LIST); 
}  /* obj_is_list */


/********************************************************************
* FUNCTION obj_is_container
* 
...
*********************************************************************/
boolean
    obj_is_container (const obj_template_t  *obj)
{
    assert(obj && "obj is NULL" );
    return (obj->objtype == OBJ_TYP_CONTAINER); 
}


/********************************************************************
* FUNCTION obj_is_choice
* 
...
*********************************************************************/
boolean
    obj_is_choice (const obj_template_t  *obj)
{
    assert(obj && "obj is NULL" );
    return (obj->objtype == OBJ_TYP_CHOICE); 
}
...
boolean obj_is_data_db (const obj_template_t *obj)
{
    ...
    if (obj_get_nsid(obj) == xmlns_ncx_id()) {
        ;
    } else if (obj_is_abstract(obj) || obj_is_cli(obj)) {
        return FALSE;
    }
 ...
 * 
 * \param obj the obj_template to check
 * \return TRUE if object type is a real data node
 *         FALSE if not
 *********************************************************************/
 ...
 *
 * \param obj the obj_template to check
 * \return TRUE if object is marked as OBJ_FL_RPC_ROOT
 *********************************************************************/
boolean obj_is_rpc_root (const obj_template_t *obj)
{
    //assert(obj && "obj is NULL" );
    return (obj->flags & OBJ_FL_RPC_ROOT) ? TRUE : FALSE;
}   /* obj_is_rpc_root */
...
/********************************************************************
 * Check if object is marked as dirty (edited) for datastore validation
 * purposes
 * \param obj the obj_template to check
 * \param cfgid the configuration ID for the datastore to check
 * \return TRUE if object is marked as dirty; FALSE if not
 *********************************************************************/
                      ...
                      ncx_cfg_t cfgid)
{
    ...
    case NCX_CFGID_RUNNING:
        if (obj->flags & OBJ_FL_RDIRTY) {
            retval = TRUE;
        }
        break;
    case NCX_CFGID_CANDIDATE:
        if (obj->flags & OBJ_FL_CDIRTY) {
            retval = TRUE;
        }
        break;
    default:
...
/********************************************************************
 * Check if there are any accessible read-only child nodes within
 *  the object
 *
 * \param obj the obj_template to check
...
/********************************************************************
 * Check if there are any accessible read-only descendant nodes
 *  within the object
 *
 * \param obj the obj_template to check
 * \return TRUE if there are any accessible read-only children
...
boolean obj_has_ro_descendants (obj_template_t *obj)
{
    ...
    for ( ; childobj ; childobj = obj_next_child(childobj)) {
        ...
        if (obj_has_name(childobj) && obj_is_enabled(childobj) && 
            ...
            if (obj_get_config_flag(childobj)) {
                boolean test = obj_has_ro_descendants(childobj);
                if (test) {
                    return TRUE;
                }
            } else {
    ...
    obj_set_ncx_flags (obj_template_t *obj)
{
    ...
    if (obj->objtype == OBJ_TYP_CONTAINER) {
        ...
        if (ncx_find_const_appinfo(appinfoQ, YWX_PREFIX, NCX_EL_RPC_ROOT)) {
            obj->flags |= OBJ_FL_RPC_ROOT;
        }
    }
    ...
    /* ietf-netconf-acm.yang extensions */
    if (ncx_find_const_appinfo(appinfoQ, NACM_PREFIX, 
                               NCX_EL_DEFAULT_DENY_WRITE)) {
        obj->flags |= OBJ_FL_SECURE;
    }

    if (ncx_find_const_appinfo(appinfoQ, NACM_PREFIX, 
                               NCX_EL_DEFAULT_DENY_ALL)) {
        obj->flags |= OBJ_FL_VERY_SECURE;
    }
    ...
    if (obj->objtype == OBJ_TYP_CONTAINER ||
        obj->objtype == OBJ_TYP_LIST) {

        /* ywx:cli-text-block extension */
        if (ncx_find_const_appinfo(appinfoQ, YWX_PREFIX, 
                                   NCX_EL_CLI_TEXT_BLOCK)) {
            obj->flags |= OBJ_FL_CLI_TEXT_BLOCK;
        }
    }
...
*********************************************************************/
void
    obj_dump_child_list (log_debug_t level,
			 dlq_hdr_t *datadefQ,
                         uint32  startindent,
                         uint32 indent)
{
    ...
    for (obj = (obj_template_t *)dlq_firstEntry(datadefQ);
	  ...
	  log_write_level_append(level, " ");
        }

        log_write_level_append(level, "%s", obj_get_typestr(obj));

        if (obj_has_name(obj)) {
	    log_write_level_append(level, " %s", obj_get_name(obj));
        }

        child_datadefQ = obj_get_datadefQ(obj);
        if (child_datadefQ != NULL) {
	    obj_dump_child_list(level,
				child_datadefQ,
                                startindent+indent,
                                indent);
        }
...
{
    assert( obj && "obj is NULL!" );
    return (obj->flags & OBJ_FL_CLI_TEXT_BLOCK) ? TRUE : FALSE;
    ...
    obj_first_xpath_ptr (obj_template_t *obj)
{
    ...
    ncx_backptr_t *ptr = (ncx_backptr_t *)
        dlq_firstEntry(&obj->inherited_whenQ);
    return ptr;
 ...
 *********************************************************************/
ncx_backptr_t *
    obj_next_xpath_ptr (ncx_backptr_t *ptr)
{
    assert(ptr && "xptr is NULL" );
    ncx_backptr_t *nextptr = (ncx_backptr_t *)dlq_nextEntry(ptr);
    return nextptr;
...
/********************************************************************
* FUNCTION obj_find_xpath_backptr
*
...
*********************************************************************/
boolean
    obj_find_xpath_backptr (obj_template_t *obj,
                            struct xpath_pcb_t_ *xpath)
{
    assert(obj && "obj is NULL!");
    assert(xpath && "xpath is NULL!");

    uint32 idx = 0;
    for (; idx < OBJ_NUM_XPATH_BACKPTRS; idx++) {
        if (obj->xpath_backptr[idx] == xpath) {
            return TRUE;
        }
    }

    ncx_backptr_t *xpathptr =
        ncx_find_backptr(&obj->xpath_backptrQ, xpath);
    return (xpathptr) ? TRUE : FALSE;

}  /* obj_find_xpath_backptr */


/********************************************************************
* FUNCTION obj_assign_xpath_backptr
*
* Set an xpath-expr backptr
* Malloc and initialize a new ncx_backptr_t struct for the
* xpath_backptrQ if no xpath_backptr slots are available
*
* INPUTS:
*   obj == object to set
...
*********************************************************************/
status_t
    obj_assign_xpath_backptr (obj_template_t *obj,
                           struct xpath_pcb_t_ *xpath)
{
    assert(obj && "obj is NULL!");
    assert(xpath && "xpath is NULL!");

    if (obj_find_xpath_backptr(obj, xpath)) {
#ifdef OBJ_BACKPTR_DEBUG
        if (LOGDEBUG3) {
            log_debug3("\nxpath_backptr: already set - skipping %s (%s)",
                       obj_get_name(obj), xpath->exprstr);
        }
#endif
        return NO_ERR;
    }
    ...
    uint32 idx = 0;
    for (; idx < OBJ_NUM_XPATH_BACKPTRS; idx++) {
        if (obj->xpath_backptr[idx] == NULL) {
#ifdef OBJ_BACKPTR_DEBUG
            if (LOGDEBUG3) {
                log_debug3("\nxpath_backptr: add %s (%s) slot %u", 
                           obj_get_name(obj), xpath->exprstr, idx);
            }
#endif
            obj->xpath_backptr[idx] = xpath;
            return NO_ERR;
        }
    }

    ncx_backptr_t *xpathptr = ncx_new_backptr(xpath);
    if (xpathptr == NULL) {
        return ERR_INTERNAL_MEM;
    }
...
#ifdef OBJ_BACKPTR_DEBUG
    if (LOGDEBUG3) {
        log_debug3("\nxpath_backptr: add  %s (%s)", 
                   obj_get_name(obj), xpath->exprstr);
    }
#endif

    dlq_enque(xpathptr, &obj->xpath_backptrQ);
    return NO_ERR;

}  /* obj_assign_xpath_ptr */


/********************************************************************
* FUNCTION obj_clear_xpath_backptr
*
* Clear an xpath-expr backptr
* Free a found ncx_backptr_t struct for the
* xpath_backptrQ or clear an xpath_backptr slot
*
* INPUTS:
*   obj == object to clear
...
*********************************************************************/
void
    obj_clear_xpath_backptr (obj_template_t *obj,
                             struct xpath_pcb_t_ *xpath)
{
    assert(obj && "obj is NULL!");
    assert(xpath && "xpath is NULL!");
...
#ifdef OBJ_BACKPTR_DEBUG
            if (LOGDEBUG3) {
                log_debug3("\nxpath_backptr: clear node xpath '%s' in "
                           "obj '%s' slot %u",
                           xpath->exprstr, obj_get_name(obj), idx);
            }
#endif

            /* found the slot with this Xpath control block ptr */
            obj->xpath_backptr[idx] = NULL;

            /* try to move a queue entry into this slot */
            ncx_backptr_t *moveptr =
                (ncx_backptr_t *)dlq_deque(&obj->xpath_backptrQ);
            if (moveptr) {
                obj->xpath_backptr[idx] = moveptr->node;
                ncx_free_backptr(moveptr);
            } else {
                /* try to move another slot into this slot */
                uint32 moveidx = idx + 1;
                for (; moveidx < OBJ_NUM_XPATH_BACKPTRS; moveidx++) {
                    if (obj->xpath_backptr[moveidx] == NULL) {
                        obj->xpath_backptr[idx] = obj->xpath_backptr[moveidx];
                        obj->xpath_backptr[moveidx] = NULL;
                        return;
                    }
                }
            }
    ...
    ncx_backptr_t *xpathptr =
        (ncx_backptr_t *)dlq_firstEntry(&obj->xpath_backptrQ);
    for (; xpathptr != NULL; 
         xpathptr = (ncx_backptr_t *)dlq_nextEntry(xpathptr)) {

        if (xpathptr->node == xpath) {
#ifdef OBJ_BACKPTR_DEBUG
            if (LOGDEBUG3) {
                log_debug3("\nxpath_backptr: clear node xpath '%s' in "
                  "obj '%s'", xpath->exprstr, obj_get_name(obj));
            }
#endif
            ncx_remove_backptr(xpathptr);
            ncx_free_backptr(xpathptr);
            return;
        }
    }
...
#ifdef OBJ_BACKPTR_DEBUG
    if (LOGDEBUG) {
        log_debug("\nxpath_backptr: clear node xpath '%s' not "
                  "found in obj '%s'", xpath->exprstr, obj_get_name(obj));
    }
#endif
...
/********************************************************************
* FUNCTION obj_set_xpath_backptrs
*
* Check a top-level data node and all its descendants
* to see if they have any XPath expressions that need
* backptrs added
*
* INPUTS:
*   obj == object to check
*********************************************************************/
status_t
    obj_set_xpath_backptrs (obj_template_t *obj)
{
    assert(obj && "obj is NULL!");

    status_t res = NO_ERR;

    if (obj_is_data_node(obj)) {
        res = set_xpath_backptrs(obj);
        if (res != NO_ERR) {
            return res;
        }
    }
    ...
    for (; chobj != NULL; chobj = obj_next_child(chobj)) {
        if (obj_is_config(chobj)) {
            res = obj_set_xpath_backptrs(chobj);
            if (res != NO_ERR) {
                return res;
            }
        }
...
/********************************************************************
* FUNCTION obj_flag_xpath_backptrs_dirty
*
* Check all the xpath backptr records for this object
* and set them to dirty or clean, based on input
*
* INPUTS:
*   obj == object to check
...
*********************************************************************/
void
    obj_flag_xpath_backptrs_dirty (obj_template_t *obj,
                                   ncx_cfg_t cfg_id,
                                   boolean flag)
{
    assert(obj && "obj is NULL!");

    /* set or clear the dirty bit in the object; used by
     * the server datastore validation code commit tests
     * to determine if the tests for this object can be skipped
     */
    switch (cfg_id) {
    case NCX_CFGID_RUNNING:
        if (flag) {
            obj->flags |= OBJ_FL_RDIRTY;
        } else {
            obj->flags &= ~OBJ_FL_RDIRTY;
        }
        break;
    ...
    /* set the XPath back pointers used in individual must/when exprs */
    uint32 idx = 0;
    boolean done = FALSE;
    for (; !done && idx < OBJ_NUM_XPATH_BACKPTRS; idx++) {
        if (obj->xpath_backptr[idx]) {
#ifdef OBJ_BACKPTR_DEBUG
            if (LOGDEBUG2) {
                log_debug2("\nxpath_backptr: flag=%d %s slot %u", 
                           flag, obj_get_name(obj), idx);
            }
#endif

            xpath_pcb_t *pcb = obj->xpath_backptr[idx];
            if (flag) {
                xpath_set_backptrs_dirty(pcb, cfg_id, TRUE);
            } else if (!xpath_check_manual_clear(pcb)) {
                xpath_set_backptrs_dirty(pcb, cfg_id, FALSE);
            } else if (LOGDEBUG2) {
                log_debug2("\nxpath_backptr: skip clear manual-only "
                           "xpath '%s'", pcb->exprstr);
            }
        } else {
            done = TRUE;
    ...
    if (!done) {
        ...
        for (; xpathptr != NULL; 
            ...
            xpath_pcb_t *pcb = (xpath_pcb_t *)xpathptr->node;

#ifdef OBJ_BACKPTR_DEBUG
            if (LOGDEBUG2) {
                log_debug2("\nxpath_backptr: flag=%d %s (%s)", 
                           flag, obj_get_name(obj), pcb->exprstr);
            }
#endif
    ...
    obj_template_t *chobj = obj_first_child(obj);
    for (; chobj; chobj = obj_next_child(chobj)) {
        if (obj_is_data_node(chobj) && obj_is_config(chobj)) {
            obj_flag_xpath_backptrs_dirty(chobj, cfg_id, flag);
        }
    }
...
/********************************************************************
* FUNCTION obj_is_target_template
*
* Check a data node and its descendants
* match the template for NETCONF configuration sources
*
* container
*   choice (1 - N leafs)
*      leaf (type empty)
...
*********************************************************************/
status_t
    obj_is_target_template (obj_template_t *obj)
{
    assert(obj && "obj is NULL!");
    ...
    for (; leafobj; leafobj = obj_next_child_deep(leafobj)) {
        if (leafobj->objtype != OBJ_TYP_LEAF) {
            return FALSE;
        }
        if (obj_get_basetype(leafobj) != NCX_BT_EMPTY) {
            return FALSE;
        }
    }
...
/********************************************************************
* FUNCTION obj_is_supported
*
* Check an RPC node to check if it is supported or not
* It could be disabled at run-time without removing it
*
* INPUTS:
*   obj == object to check
*********************************************************************/
status_t
    obj_is_supported (obj_template_t *obj)
{
    assert(obj && "obj is NULL!");
    if (obj->objtype == OBJ_TYP_RPC) {
        return obj->def.rpc->supported;
    } else {
        return TRUE;
    }
    /*NOTREACHED*/
