...
static cfg_reload_candidate_cb_t reloadfn;
...
static cfg_template_t *
    new_template (const xmlChar *name,
                  ncx_cfg_t cfg_id)
{
    ncx_module_t *mod = ncx_find_module(NCXMOD_NETCONF, NULL);
    if (!mod) {
        SET_ERROR(ERR_INTERNAL_VAL);
        return NULL;
    }

    obj_template_t *cfgobj = ncx_find_object(mod, NCX_EL_CONFIG);
    if (!cfgobj) {
        SET_ERROR(ERR_INTERNAL_VAL);
        return NULL;
    }

    cfg_template_t *cfg = m__getObj(cfg_template_t);
    if (!cfg) {
        return NULL;
    }
    ...
    cfg_update_last_ch_time(cfg, NULL);

    cfg->cfg_id = cfg_id;
    cfg->cfg_state = CFG_ST_INIT;
    // leave cfg->root NULL until load_running_config

    if (cfg_id == NCX_CFGID_STARTUP) {
...
/********************************************************************
* FUNCTION reload_candidate
*
* Reload the candidate config if needed
*
* INPUTS:
*    cfg = Config template to reload
* RETURNS:
*    status
*********************************************************************/
static status_t
    reload_candidate (cfg_template_t *cfg)
{
    /* sec 8.3.5.2 requires a discard-changes
     * when a lock is released on the candidate
     * and there are edits done to the candidate
     */
    status_t res = NO_ERR;
    if (cfg->cfg_id == NCX_CFGID_CANDIDATE) {
        if (cfg_get_dirty_flag(cfg)) {
            log_debug2("\ncfg: unlock dirty candidate forcing reload");
            res = cfg_fill_candidate_from_running();
        } else {
            log_debug2("\ncfg: unlock clean candidate skipping reload");
        }
    }
    ...
    cfg_fill_candidate_from_running (void)
{
    ...
    log_debug("\nFilling candidate config from running config");
...
/********************************************************************
* FUNCTION cfg_clear_running_dirty_flag
*
* Clear the running dirty flag when it is saved to NV-storage
* or loaded into running from startup
*
*********************************************************************/
void
    cfg_clear_running_dirty_flag (void)
{
              ...
              cfg_source_t  lock_src)
{
    ...
    status_t res = cfg_ok_to_lock(cfg);
                ...
                ses_id_t locked_by)
{
    ...
    status_t res = cfg_ok_to_unlock(cfg, locked_by);

    if (res == NO_ERR) {
        cfg->cfg_state = CFG_ST_READY;
        cfg->locked_by = 0;
        cfg->lock_src = CFG_SRC_NONE;
        res = reload_candidate(cfg);
    }
    ...
    cfg_release_locks (ses_id_t sesid)
{
    ...
    for (i=0; i<CFG_NUM_STATIC; i++) {
        ...
        if (cfg && cfg->locked_by == sesid) {
            ...
            status_t res = reload_candidate(cfg);
            if (res != NO_ERR) {
                log_error("\nError: reload candidate failed (%s)",
...
*      !!! This parameter is malloced memory 
*      !!! and freed by this function
*      !!! Do not double free from agt_val/apply_write_val
*
*********************************************************************/
void
                         ...
                         val_value_t *newroot)
{
    ...
    assert ( !cfg->root && "cfg->root already set!" );
                             ...
                             time_t *timestamp)
{
    ...
    if (cfg->root) {
        cfg->root->last_modified = *timestamp;
    }
    tstamp_time2datetime(timestamp, cfg->last_ch_time);
...
*********************************************************************/
void
    cfg_update_stamps (cfg_template_t *source,
                       cfg_template_t *dest)
{
    assert (source && "source is NULL!");
    assert (dest && "dest is NULL!");

    xml_strcpy(dest->last_ch_time, source->last_ch_time);
    dest->last_txid = source->last_txid;
    ...
    cfg_ok_to_partial_lock (const cfg_template_t *cfg)
{
    ...
    case CFG_ST_PLOCK:
        ...
        break;
    case CFG_ST_FLOCK:
        /* config already has a global lock open */
        res = ERR_NCX_LOCK_DENIED;
        break;
    case CFG_ST_NONE:
    case CFG_ST_INIT:
...
/********************************************************************
* FUNCTION cfg_setup_running
*
* Setup the running config root if load_running_config 
...
*********************************************************************/
status_t 
    cfg_setup_running (void)
{
    cfg_template_t  *cfg = cfg_arr[NCX_CFGID_RUNNING];
    if (cfg == NULL) {
...
/********************************************************************
* FUNCTION cfg_register_reload_candidate_cb
*
* Register a callback function to be called anytime the
* cfg_fill_candidate_from_ FOO function is called
*
*********************************************************************/
void
