...
#define CFG_INITIAL_TXID (ncx_transaction_id_t)0

/* this must match the number of enums in ncx_cfg_t */
...
typedef struct cfg_template_t_ {
    ...
    xmlChar        last_ch_time[TSTAMP_MIN_SIZE]; // TBD: remove!
    uint32         flags;
    ses_id_t       locked_by;
...
/* support for server callback anytime the candidate
 * config is reloaded from the running config <discard-changes>
 */
