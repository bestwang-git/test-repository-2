...
static ncx_var_t *
    new_var (const xmlChar *name,
             ...
             status_t *res)
{
    ncx_var_t *var = m__getObj(ncx_var_t);
    if (!var) {
                ...
                dlq_hdr_t *que)
{
    ...
    for (cur = (ncx_var_t *)dlq_firstEntry(que);
        ...
        int ret = xml_strcmp(var->name, cur->name);
        if (ret < 0) {
                ...
                var_type_t vartype)
{
    ...
    for (cur = (ncx_var_t *)dlq_firstEntry(varQ);
        ...
        int ret = xml_strncmp(name, cur->name, namelen);
             ...
             var_type_t vartype)
{
    ...
    ncx_var_t *var = find_var(rcxt, varQ, name, namelen, 0, vartype);
    if (var) {
                      ...
                      val_value_t *new_parm)
{
    ...
    case OBJ_TYP_CASE:
        if (typ_is_string(varval->btyp) &&
             ...
             * varval value 
             */
            targobj = obj_find_child(obj, NULL, VAL_STR(varval));
            if (targobj != NULL &&
        ...
        if (!typ_is_simple(varval->btyp)) {
            ...
            if (!cloneval) {
                ...
                if (!xml_strcmp(obj_get_mod_name(obj), NCXMOD_YANGCLI) &&
                    ...
                    /* the old code moved the children to the 
                     * anyxml container */
                    val_move_children(cloneval, new_parm);
                    val_free_value(cloneval);
...
/********************************************************************
* FUNCTION set_string_val_append
* 
* set a $foo/bar/baz variable
*
* INPUTS:
*   varval == var value node that was found
*   new_parm == value in progress to fill in
*   append_str == string to append to the expanded var value
*
* RETURNS:
*   status
*********************************************************************/
static status_t
    set_string_val_append (val_value_t *varval,
                           val_value_t *new_parm,
                           const xmlChar *append_str)
{
    /* got a string appended to the name $HOME/test/foo */
    if (!typ_is_simple(varval->btyp)) {
        log_error("\nError: cannot append string to complex var '%s'\n",
                  varval->name);
        return ERR_NCX_INVALID_VALUE;
    }

    uint32 varval_len = 0;
    status_t res = val_sprintf_simval_nc(NULL, varval, &varval_len);
    if (res != NO_ERR) {
        log_error("\nError: expand var '%s' failed (%s)\n",
                  varval->name, get_error_string(res));
        return res;
    }

    uint32 append_len = xml_strlen(append_str);
    xmlChar *buff = m__getMem(varval_len + append_len + 1);
    if (buff == NULL) {
        log_error("\nError: malloc failed expanding var '%s'\n",
                  varval->name);
        return ERR_INTERNAL_MEM;
    }

    res = val_sprintf_simval_nc(buff, varval, &varval_len);
    if (res != NO_ERR) {
        log_error("\nError: expand var '%s' failed (%s)\n",
                  varval->name, get_error_string(res));
        m__free(buff);
    ...
    xml_strncpy(&buff[varval_len], append_str, append_len);

    /* replace the new_parm string pointer */
    if (VAL_STR(new_parm)) {
        m__free(VAL_STR(new_parm));
    }
    VAL_STR(new_parm) = buff;

    return NO_ERR;

}  /* set_string_val_append */
...
{
    assert(varQ && "varQ is NULL!");

    while (!dlq_empty(varQ)) {
        ncx_var_t *var = (ncx_var_t *)dlq_deque(varQ);
        var_free(var);
    }
...
{
    assert(name && "name is NULL!");
    return var_set_str(rcxt, name, xml_strlen(name), value, vartype);
                 ...
                 const val_value_t *value)
{
    ...
    status_t res = set_str(rcxt, NULL, name, xml_strlen(name), val, 
                           VAR_TYP_SYSTEM);
    if (res != NO_ERR) {
        val_free_value(val);
    }
                         ...
                         var_type_t vartype)
{
    ...
    obj_template_t *genstr = ncx_get_gen_string();
    if (!genstr) {
        return SET_ERROR(ERR_INTERNAL_VAL);
    }
    ...
    status_t res = val_set_string(val, name, valstr);
    if (res != NO_ERR) {
        val_free_value(val);
        return res;
    }
    ...
    /* save the variable */
    res = set_str(rcxt, NULL, name, xml_strlen(name), val, vartype);
               ...
               var_type_t vartype)
{
    ...
    ncx_var_t *var = find_var(rcxt, NULL, name, namelen, 0, vartype);
                   ...
                   xmlns_id_t  nsid)
{
    ...
    ncx_var_t *var =
        remove_var(NULL, varQ, name, namelen, nsid, VAR_TYP_QUEUE);
    if (var) {
        free_var(var);
        return NO_ERR;
    } else {
...
{
    assert(name && "name is NULL!");
    return var_get_type_str(rcxt, name, xml_strlen(name), globalonly);
                     ...
                     xmlns_id_t  nsid)
{
    ...
    ncx_var_t *var = find_var(NULL, varQ, name, namelen, nsid, VAR_TYP_QUEUE);
    if (var) {
        return var->val;
    } else {
        return NULL;
...
{
    assert(name && "name is NULL!");
    ncx_var_t *var = find_var(NULL, varQ, name, xml_strlen(name), nsid, 
                              VAR_TYP_QUEUE);
    if (var) {
        return var->val;
    } else {
...
{
    assert(name && "name is NULL!");
    return find_var(NULL, varQ, name, xml_strlen(name), nsid, VAR_TYP_QUEUE);
...
{
    assert(name && "name is NULL!");
    ncx_var_t *var = find_var(rcxt, NULL, name, xml_strlen(name), 0,
                              VAR_TYP_LOCAL);
    if (var) {
        return var->val;
    }
                       ...
                       uint32 namelen)
{
    ...
    ncx_var_t *var = find_var(rcxt, NULL, name, namelen, 0, VAR_TYP_LOCAL);
    if (var) {
        return var->val;
    }
    return NULL;
                   ...
                   uint32 *namelen)
{
    ...
    assert(len && "len is NULL!");
    assert(vartype && "vartype is NULL!");
    assert(name && "name is NULL!");
    assert(namelen && "namelen is NULL!");
    ...
    if (isdigit((int)*str)) {
        ...
        *namelen = 1;
        ...
        status_t res = ncx_parse_name(str, namelen);
        if (res != NO_ERR) {
            *len = 0;
            return res;
        }
    ...
    *len = (uint32)(str - line);
    ...
    if (*vartype == VAR_TYP_GLOBAL) {
        ...
        ncx_var_t *testvar =
            find_var(rcxt, NULL, *name, *namelen, 0, VAR_TYP_GLOBAL);
        if (testvar) {
                           ...
                           status_t *res)
{
    ...
    *res = NO_ERR;
    ...
    boolean simtyp = typ_is_simple(obj_get_basetype(obj));
    ...
    if (strval == NULL) {
        if (fillval != NULL) {
            val_value_t *fillcopy = val_clone(fillval);
            if (fillcopy == NULL) {
                *res = ERR_INTERNAL_MEM;
            } else {
                if (simtyp) {
         ...
         */
        xmlChar *sourcefile = ncx_get_source_ex(&strval[1], FALSE, res);
        if (*res == NO_ERR && sourcefile != NULL) {
            xmlChar *fname = ncxmod_find_data_file(sourcefile, TRUE, 
                                                   FALSE, res);
            if (fname) {
         ...
         */
        uint32 val_len = xml_strlen(strval);
        uint32 len = 0;
        ...
        *res = var_check_ref(rcxt, strval, ISRIGHT, &len, &vartype, 
        ...
        if (*res == NO_ERR) {
            ...
            val_value_t *varval = var_get_str(rcxt, name, namelen, vartype);
            if (!varval) {
                *res = ERR_NCX_VAR_NOT_FOUND;
            } else if (val_len == len) {
                /* just got $foo, no extra text */
                *res = set_val_from_var(obj, varval, useval);
            } else if (typ_is_string(useval->btyp)) {
                /* got a string appended to the name
                 * $HOME/test/foo                 */
                const xmlChar *append_str = &strval[len];
                *res = set_string_val_append(varval, useval, append_str);
            }
        }
        ...
        if (simtyp) {
             ...
             */
            *res = val_set_string2(useval, obj_get_name(obj), 
                                   obj_get_typdef(obj), strval, 
                                   xml_strlen(strval)-1); 
        } else if (obj->objtype == OBJ_TYP_ANYXML) {
            *res = val_replace_str(strval, xml_strlen(strval)-1, useval);
        }
    } else if ((*strval == NCX_XML1a_CH) &&
               (strval[1] == NCX_XML1b_CH)) {

        /* this is a bracketed inline XML sequence */
        const xmlChar *str = strval+2;
        ...
        xmlChar *intbuff = xml_strndup(strval+1, (uint32)(str-strval));
        if (!intbuff) {
            *res = ERR_INTERNAL_MEM;
        } else {
        ...
        *res = NO_ERR;
         ...
         */
        *res = val_set_simval(useval, obj_get_typdef(obj), 
                              val_get_nsid(useval), useval->name, strval);
    } else if (obj->objtype == OBJ_TYP_ANYXML) {
        ...
        *res = val_set_simval(useval, typ_get_basetype_typdef(NCX_BT_STRING), 
                              val_get_nsid(useval), useval->name, strval);
    } else {
                          ...
                          status_t *res)
{
    ...
    *res = NO_ERR;
    ...
    if (*strval == NCX_VAR_CH) {
         ...
         * flag an error if variable not found
         */
        *res = var_check_ref(rcxt, strval, ISRIGHT, &len, &vartype, &name, 
                             &namelen);
        if (*res == NO_ERR) {
            const val_value_t *varval =
                var_get_str(rcxt, name, namelen, vartype);
            if (!varval) {
    ...
    if (*strval==NCX_AT_CH) {
         ...
         * find the file with the raw XML data
         */
        xmlChar *fname = ncxmod_find_data_file(&strval[1], TRUE, FALSE, res);
        if (fname) {
            /* hand off the malloced 'fname' to be freed later */
        ...
        *res = val_set_string2(newval, NULL, obj_get_typdef(useobj), 
        ...
        *res = val_set_simval(newval,
        ...
        useobj = ncx_get_gen_string();
        *res = val_set_simval(newval, obj_get_typdef(useobj), 
                              val_get_nsid(newval), newval->name, strval);
    }
    ...
    var_cvt_generic (dlq_hdr_t *varQ)
{
    ...
    for (cur = (ncx_var_t *)dlq_firstEntry(varQ);
        ...
        if (cur->val) {
            status_t res = val_cvt_generic(cur->val);
            if (res != NO_ERR) {
                SET_ERROR(res);
            }
        }
              ...
              xmlns_id_t nsid)
{
    ...
    uint32 namelen = xml_strlen(varname);
    if (namelen == 0) {
        return NULL;
    }

    ncx_var_t *retvar =
        find_var(rcxt, NULL, varname, namelen, nsid, VAR_TYP_LOCAL);
    if (retvar == NULL) {
        retvar = find_var(rcxt, NULL, varname, namelen, nsid, VAR_TYP_GLOBAL);
    }
...
/********************************************************************
* FUNCTION var_get_first
* 
* Find the first local or global variable
*
* INPUTS:
*   rcxt == runstack context to use
*   islocal == TRUE for local variables
...
*********************************************************************/
ncx_var_t *
    var_get_first (runstack_context_t *rcxt,
                   boolean islocal)
{
    assert(rcxt && "rcxt is NULL!");

    dlq_hdr_t *varQ = runstack_get_que(rcxt, !islocal);
    return (ncx_var_t *)dlq_firstEntry(varQ);

}  /* var_get_first */


/********************************************************************
* FUNCTION var_get_next
* 
...
*********************************************************************/
ncx_var_t *
    var_get_next (ncx_var_t *curvar)
{
    assert(curvar && "curvar is NULL!");
    return (ncx_var_t *)dlq_nextEntry(curvar);
...
*********************************************************************/
const xmlChar *
    var_get_name (const ncx_var_t *curvar)
{
    assert(curvar && "curvar is NULL!");
    return curvar->name;
...
*********************************************************************/
extern var_type_t
    var_get_vartype (const ncx_var_t *curvar)
{
    assert(curvar && "curvar is NULL!");
    return curvar->vartype;
