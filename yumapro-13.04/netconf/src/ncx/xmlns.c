...
*    temp == true if temporary NSID, FALSE for permanent
                 ...
                 xmlns_id_t *ns_id)
{
    ...
    /* flag this NSID for removal during resource cleanup */
    rec->ns_temporary = temp;
    ...
    if (LOGDEBUG2) {
        log_debug2("\nxmlns_reg: %s id:%2d mod:%s\turi: %s",
                   temp ? (const xmlChar *)"temporary" : EMPTY_STRING,
                   rec->ns_id, 
                   rec->ns_module ? rec->ns_module : NCX_EL_NONE, 
                   rec->ns_name);
    }
                       ...
                       xmlns_id_t *ns_id)
{
    ...
    return register_ns(ns, pfix, modname, modptr, FALSE, ns_id);

} /* xmlns_register_ns */


/********************************************************************
* FUNCTION xmlns_register_temp_ns
* 
...
*********************************************************************/
status_t 
    xmlns_register_temp_ns (const xmlChar *ns,
                            const xmlChar *pfix,
                            xmlns_id_t *ns_id)
{
#ifdef DEBUG
    ...
    return register_ns(ns, pfix, NULL, NULL, TRUE, ns_id);

} /* xmlns_register_temp_ns */


/********************************************************************
* FUNCTION xmlns_convert_temp_ns
* 
* Register the specified permanent namespace by converting
* the specified temporary NSID to a permanent entry
*
* INPUTS:
*    nsid == namespace id
*    pfix == namespace prefix to use (if none provided but needed)
...
*********************************************************************/
status_t 
    xmlns_convert_temp_ns (xmlns_id_t nsid,
                           const xmlChar *pfix,
                           const xmlChar *modname,
                           void *modptr)
{
    ...
    status_t res = NO_ERR;
    xmlChar *new_pfix = NULL;
    xmlChar *new_mod = NULL;

    /* copy the prefix string */
    if (pfix) {
        new_pfix = xml_strdup(pfix);
        if (!new_pfix) {
    ...
    if (new_pfix) {
        m__free(rec->ns_pfix);
        rec->ns_pfix = new_pfix;
    }

    if (new_mod) {
        m__free(rec->ns_module);
        rec->ns_module = new_mod;
        rec->ns_mod = modptr;
    }

    rec->ns_temporary = FALSE;
    return NO_ERR;
