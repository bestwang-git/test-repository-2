                                  ...
                                  xmlTextReaderPtr  *reader)
{
     ...
     */
    int options = XML_READER_OPTIONS + XML_PARSE_XINCLUDE;
                                ...
                                xmlTextReaderPtr  *reader)
{
     ...
     * be omitted by the parser
     */
    int options = XML_READER_OPTIONS;
                                  ...
                                  xmlTextReaderPtr  reader)
{
     ...
     * be omitted by the parser
     */
    int ret = xmlReaderNewIO(reader, readfn, closefn, context,
...
{
    assert(startnode && "startnode is NULL!");
...
xml_attr_t *
    xml_new_attr (void)
{
    xml_attr_t *attr = m__getObj(xml_attr_t);
    if (!attr) {
        return NULL;
    }
...
{
    assert(attrs && "attrs is NULL!");
    assert(attr_name && "attr_name is NULL!");
    assert(attr_val && "attr_val is NULL!");

    xml_attr_t *attr = xml_new_attr();
    if (!attr) {
...
{
    assert(attrs && "attrs is NULL!");
    assert(attr_qname && "attr_qname is NULL!");
    assert(attr_val && "attr_val is NULL!");
...
int 
    xml_strcmp (const xmlChar *s1, 
                const xmlChar *s2)
{
    assert(s1 && "s1 is NULL!");
    assert(s2 && "s2 is NULL!");
...
*    register_missing == register temporary NSID if valid
*      namespace found but not registered
...
#define PREFIX_BUFF_SIZE 256
    ...
    *retnsid = 0;
    ...
    if (prefix && prefixlen) {
        ...
        ns = xmlTextReaderLookupNamespace(reader, usebuff);
    } else {
        ns = xmlTextReaderLookupNamespace(reader, NULL);
    }
    ...
    if (ns) {
        *retnsid = xmlns_find_ns_by_name(ns);
        ...
        if (*retnsid == 0 && register_missing) {
            res = xmlns_register_temp_ns(ns, usebuff, retnsid);
            if (res != NO_ERR) {
                log_error("\nError: register temporary nsid failed "
...
xmlChar * xml_trim_string (xmlChar *str)
{
    ...
    uint32 len = xml_strlen(str);
    xmlChar *frontp = str - 1;
    xmlChar *endp = str + len;

    /* Move the front and back pointers to address
     * the first non-whitespace characters from
     * each end.
     */
    while ( xml_isspace(*(++frontp)) ) {
        ;
    }
