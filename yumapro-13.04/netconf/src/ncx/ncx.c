...
//#define LOGGER_TEST                  // Exercise logger during init
//#define LOGGER_TEST_VENDOR_REG_EARLY // Exercise logger during init
//#define LOGGER_TEST_VENDOR_REG_LATE  // Exercise logger during init
#endif
...
/* generic leaf-list string object template */
static obj_template_t   *gen_leaflist;

/* YANG insert attribute metaval leaf */
static obj_template_t   *yang_insert;

/* YANG insert attribute metaval leaf */
static obj_template_t   *yang_key;

/* YANG insert attribute metaval leaf */
static obj_template_t   *yang_value;

/* pointer to the NETCONF operation attribute meta-object */
static obj_template_t   *nc_operation;
 ...
 * not YUMAPRO_MODPATH or YUMAPRO_HOME, or YUMAPRO_INSTALL subdirs
 ...
 * controls indentation for protocol message output, not logging
 */
static int8         message_indent;

/* flag to force yang_parse to collapse submodule definitions
 * into the main module when the parse is completed
 * mandatory data nodes; applies to server <load> operation
 */
static boolean      collapse_submodules;

/* flag to indicate to xpath modules whether obj_template_t back-ptrs
 * need to be set when xpath expressions are validated.
 */
static boolean      use_xpath_backptrs;
                  ...
                  void **dptr)
{
    ...
    if (!imp->mod) {
        res = ncxmod_load_module(imp->module, imp->revision, 
                                 pcb->savedevQ, &imp->mod);
...
// ----------------------------------------------------------------------------!
/********************************************************************
* MACRO: NCX_INTERNAL_CLI_STATE
*
*    Output string to stderr reporting unexpected (or expected, if
*    debugging) state.
*
*    OUTPUT:
*    Info string, function name, and line number ...
*    Dump salient CLI (parm) state, and resulting action ...
*
* EXAMPLE:
*
*     MACRO: NCX_INTERNAL_CLI_STATE(Unexpected log-backtrace-level, CONTINUING);
*
*    OUTPUT: CLI [bootstrap_cli@959]: Unexpected log-backtrace-level
*            [parm=93e5310, count=0/0x0, value='(null)'] - CONTINUING
*
*
*********************************************************************/
#define NCX_INTERNAL_CLI_STATE(info_str, action)                        \
  log_internal_err("\nCLI [%s@%d]: " #info_str                          \
		   " [parm=%p, count=%d/0x%x, value='%s'] - %s\n",      \
		   __FUNCTION__, __LINE__,				\
		   parm, (int)parm->count, (int)parm->count,	        \
		   (char *)parm->value, #action)
                   ...
                   boolean logtstamps)
{
...
#ifdef LOGGER_TEST
    log_error("\nError: (1) ERROR TEST entering %s ...", __FUNCTION__);
    log_dev1( "\nError: (2)  DEV1 TEST entering %s ...", __FUNCTION__);
#endif

    /* create bootstrap parm: home */
    if (res == NO_ERR) {
	parm = cli_new_rawparm(NCX_EL_HOME, FALSE);
	  if (parm) {
    ...
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_LOGLEVEL, FALSE);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_LOG, FALSE);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    /* create bootstrap parm: log-backtrace */
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_LOGBACKTRACE, FALSE);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    /* create bootstrap parm: log-backtrace-detail */
    if (res == NO_ERR) {
        parm = cli_new_empty_rawparm(NCX_EL_LOGBT_DETAIL);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    /* create bootstrap parm: log-backtrace-level */
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_LOGBT_LEVEL, TRUE /* XSDLIST */);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    /* create bootstrap parm: log-backtrace-stream */
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_LOGBT_STREAM, TRUE /* XSDLIST */);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    /* create bootstrap parm: log-header */
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_LOGHEADER, TRUE /* XSDLIST */);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    /* create bootstrap parm: log-mirroring */
    if (res == NO_ERR) {
        parm = cli_new_empty_rawparm(NCX_EL_LOGMIRRORING);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
	    log_error("\nError: malloc failed during %s", __FUNCTION__);
            res = ERR_INTERNAL_MEM;
        }
    }

    /* create bootstrap parm: log-stderr */
    if (res == NO_ERR) {
        parm = cli_new_empty_rawparm(NCX_EL_LOGSTDERR);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
	    log_error("\nError: malloc failed during %s", __FUNCTION__);
    ...
    /* create bootstrap parm: log-suppress-ctrl */
    if (res == NO_ERR) {
        parm = cli_new_empty_rawparm(NCX_EL_LOGSUPPRESSCTRL);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
	    log_error("\nError: malloc failed during %s", __FUNCTION__);
    ...
    /* create bootstrap parm: log-syslog */
    if (res == NO_ERR) {
        parm = cli_new_empty_rawparm(NCX_EL_LOGSYSLOG);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
	    log_error("\nError: malloc failed during %s", __FUNCTION__);
    ...
    /* create bootstrap parm: log-vendor */
    if (res == NO_ERR) {
        parm = cli_new_empty_rawparm(NCX_EL_LOGVENDOR);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
	    log_error("\nError: malloc failed during %s", __FUNCTION__);
    ...
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_MODPATH, FALSE);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    /* create bootstrap parm: yumapro-home */
    if (res == NO_ERR) {
        parm = cli_new_rawparm(NCX_EL_YUMAPRO_HOME, FALSE);
        if (parm) {
            dlq_enque(parm, &parmQ);
        } else {
            log_error("\nError: malloc failed");
    ...
    //log_set_debug_level(LOG_DEBUG_DEBUG2); // For cli_parse_raw() debugs ...
    ...
    if (parm && parm->count) {
	    ...
	    //NCX_INTERNAL_CLI_STATE(log-level, CONTINUING);
                ...
                if (logfilename) {
                    ...
                    if (res != NO_ERR) {
                        log_error("\nError: open logfile '%s' failed (%s)",
                                  logfilename, get_error_string(res));
		    }
                    m__free(logfilename);
                }
            } else {
    ...
    /* --log-backtrace[=<max-frame-count>] */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_LOGBACKTRACE, &parmQ);
	if (parm && parm->count) {
	    if (parm->count > 1) {
	        log_error("\nError: Only one log-backtrace parameter allowed");
		res = ERR_NCX_DUP_ENTRY;
	    } else if (parm->value) {
	        //NCX_INTERNAL_CLI_STATE(log-backtrace, CONTINUING);
		ncx_init_num(&testnum);
		res = ncx_decode_num((const xmlChar *)parm->value,
				     NCX_BT_UINT32, &testnum);
		if (res == NO_ERR) {
		    log_set_backtrace(testnum.u);
		} else {
    ...
    /* --log-backtrace-detail */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_LOGBT_DETAIL, &parmQ);
        if (parm && parm->value) {
            log_error("\nError: log-backtrace-detail is empty parameter");
            res = ERR_NCX_INVALID_VALUE;
        }
	if (parm && parm->count) {
	    log_set_backtrace_detail();
	}
    }

    /* --log-backtrace-level="error warn info debug ... debug4 */
    parm = cli_find_rawparm(NCX_EL_LOGBT_LEVEL, &parmQ);
    if (parm && parm->count) {
        if (parm->count > 1) {
	  log_error("\nError: Only one log-backtrace-level param allowed");
	  res = ERR_NCX_DUP_ENTRY;
	} else if (parm->value && parm->ncx_list) {
	    //NCX_INTERNAL_CLI_STATE(log-backtrace-level, CONTINUING);

	    level_mask = 0;

	    if (ncx_string_in_list((const xmlChar *)"write", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_WRITE);
	    }
	    if (ncx_string_in_list((const xmlChar *)"error", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_ERROR);
	    }
	    if (ncx_string_in_list((const xmlChar *)"warn", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_WARN);
	    }
	    if (ncx_string_in_list((const xmlChar *)"dev0", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEV0);
	    }
	    if (ncx_string_in_list((const xmlChar *)"dev1", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEV1);
	    }
	    if (ncx_string_in_list((const xmlChar *)"info", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_INFO);
	    }
	    if (ncx_string_in_list((const xmlChar *)"debug", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEBUG);
	    }
	    if (ncx_string_in_list((const xmlChar *)"debug2", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEBUG2);
	    }
	    if (ncx_string_in_list((const xmlChar *)"debug3", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEBUG3);
	    }
	    if (ncx_string_in_list((const xmlChar *)"debug4", parm->ncx_list)) {
	        level_mask |= log_cvt_debug_level2bit(LOG_DEBUG_DEBUG4);
	    }

	    log_set_backtrace_level_mask(level_mask);
    ...
    parm = cli_find_rawparm(NCX_EL_LOGBT_STREAM, &parmQ);
    if (parm && parm->count) {
        if (parm->count > 1) {
	  log_error("\nError: Only one log-backtrace-stream param allowed");
	  res = ERR_NCX_DUP_ENTRY;
	} else if (parm->value && parm->ncx_list) {
	    //NCX_INTERNAL_CLI_STATE(log-backtrace-stream, CONTINUING);

	    if (ncx_string_in_list((const xmlChar *)"logfile",
				   parm->ncx_list)) {
	        log_set_backtrace_logfile();
	    }
	    if (ncx_string_in_list((const xmlChar *)"stderr",
				   parm->ncx_list)) {
	        log_set_backtrace_stderr();
	    }
	    if (ncx_string_in_list((const xmlChar *)"stdout",
				   parm->ncx_list)) {
	        log_set_backtrace_stdout();
	    }
	    if (ncx_string_in_list((const xmlChar *)"syslog",
				   parm->ncx_list)) {
	        log_set_backtrace_syslog();
	    }
	    if (ncx_string_in_list((const xmlChar *)"vendor",
				   parm->ncx_list)) {
    ...
    parm = cli_find_rawparm(NCX_EL_LOGHEADER, &parmQ);
    if (parm && parm->count) {
        if (parm->count > 1) {
	  log_error("\nError: Only one log-header param allowed");
	  res = ERR_NCX_DUP_ENTRY;
	} else if (parm->value && parm->ncx_list) {
	    //NCX_INTERNAL_CLI_STATE(log-header, CONTINUING);

	    if (ncx_string_in_list((const xmlChar *)"custom",
				   parm->ncx_list)) {
	        log_set_custom();
	    }
	    if (ncx_string_in_list((const xmlChar *)"localtime",
				   parm->ncx_list)) {
	        log_set_localtime();
	    }
        } else {
    ...
    /* --log-mirroring */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_LOGMIRRORING, &parmQ);
        if (parm && parm->value) {
            log_error("\nError: log-mirroring is empty parameter");
            res = ERR_NCX_INVALID_VALUE;
        }
	if (parm && parm->count) {
	    log_set_mirroring();
    ...
    /* --log-stderr */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_LOGSTDERR, &parmQ);
        if (parm && parm->value) {
            log_error("\nError: log-stderr is empty parameter");
            res = ERR_NCX_INVALID_VALUE;
        }
	if (parm && parm->count) {
	    log_set_stderr();
    ...
    /* --log-suppress-ctrl */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_LOGSUPPRESSCTRL, &parmQ);
        if (parm && parm->value) {
            log_error("\nError: log-suppress-ctrl is empty parameter");
            res = ERR_NCX_INVALID_VALUE;
        }
	if (parm && parm->count) {
	    log_set_suppress_ctrl();
	}
    }

    /* --log-syslog */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_LOGSYSLOG, &parmQ);
        if (parm && parm->value) {
            log_error("\nError: log-syslog is empty parameter");
            res = ERR_NCX_INVALID_VALUE;
        }
	if (parm && parm->count) {
	    log_set_syslog();
    ...
    /* --log-vendor */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_LOGVENDOR, &parmQ);
        if (parm && parm->value) {
            log_error("\nError: log-vendor is empty parameter");
            res = ERR_NCX_INVALID_VALUE;
        }
	if (parm && parm->count) {
	    log_set_vendor();
    ...
    /* --yumapro-home=<$YUMAPRO_HOME> */
    if (res == NO_ERR) {
        parm = cli_find_rawparm(NCX_EL_YUMAPRO_HOME, &parmQ);
        if (parm && parm->count) {
            if (parm->count > 1) {
                log_error("\nError: Only one 'yumapro-home' parameter allowed");
                res = ERR_NCX_DUP_ENTRY;
            } else {
                /*** VALIDATE YUMAPRO_HOME ***/
...
#ifdef LOGGER_TEST
    log_error("\nError: (1) ERROR TEST exiting %s ...", __FUNCTION__);
    log_dev1( "\nError: (2)  DEV1 TEST exiting %s ...", __FUNCTION__);
    log_backtrace(LOG_DEBUG_DEV1, "\nTrace: (3)  DEV1 TEST exiting %s ...",
		  __FUNCTION__);
#endif
...
/**
 * \fn init_namespaces
 * \brief Initialize the static namespaces without modues
 * \return status 
 */
static status_t init_namespaces (void)
{
    status_t res;
    xmlns_id_t  nsid = 0;

    /* Initialize the INVALID namespace to help filter handling */
    res = xmlns_register_ns(INVALID_URN, INV_PREFIX, NCX_MODULE, 
                            NULL, &nsid);
    if (res != NO_ERR) {
        return res;
    }
    ...
    res = xmlns_register_ns(WILDCARD_URN, WILDCARD_PREFIX, 
    ...
    res = xmlns_register_ns(YIN_URN, YIN_PREFIX, YIN_MODULE, 
                            NULL, &nsid);
    if (res != NO_ERR) {
        return res;
    }
    ...
    /* Initialize the XSD namespace for yangdump program */
    ...
    res = xmlns_register_ns(NC_WD_ATTR_URN, NC_WD_ATTR_PREFIX,
...
/********************************************************************
* FUNCTION walk_all_identities
* 
* Go through all the identities that
* match the specified base identity
* INPUTS:
*   ident == identity to process
*   base == original identity base to match
*   cbfn == callback function to invoke when a match is found
*   cookie == cookie to pass to callback function
* RETURNS:
*  TRUE to keep going; FALSE to halt
*********************************************************************/
static boolean
    walk_all_identities (ncx_identity_t *ident,
                         ncx_identity_t *base,
                         ...
                         void *cookie)
{
    ...
    boolean cont = TRUE;
    if (ident != base) {
        cont = (*cbfn)(ident, cookie);
        if (!cont) {
            return FALSE;
        }
    }

    ncx_idlink_t *idlink = (ncx_idlink_t *)dlq_firstEntry(&ident->childQ);
    for (; idlink; idlink = (ncx_idlink_t *)dlq_nextEntry(idlink)) {
        cont = walk_all_identities(idlink->identity, base, cbfn, cookie);
        if (!cont) {
            return FALSE;
        }
    }
 ...
 * \param collapse_submods TRUE if OK to move submod defs to main mod
 *                   FALSE if not
 * \param xpath_backptrs TRUE if XPath backptr caching needed;
 *                   FALSE if not; used only by the server
              ...
              char *argv[])
{
    ...
    message_indent = NCX_DEF_MSG_INDENT;

    collapse_submodules = collapse_submods;

    use_xpath_backptrs = xpath_backptrs;

    /* check that the correct version of libxml2 is installed */
    LIBXML_TEST_VERSION;

    /* init internal message id */
    xml_msg_init();

    /* init module library handler */
    status_t res = ncxmod_init();
    if (res != NO_ERR) {
        return res;
    }

#ifdef LOGGER_TEST_VENDOR_REG_EARLY
...
#ifdef LOGGER_TEST_VENDOR_REG_LATE
    /* Test registration callback for vendor logging. Pretend
     * to be vendor code. */
    log_vendor_extern_register_send_fn(log_vendor_test_send_to_syslog);
#endif
...
#ifdef LOGGER_TEST
    log_dev1("\n%s:  DEV1: Returned from bootstrap_cli() ...", __FUNCTION__);
    log_backtrace(LOG_DEBUG_DEV1,
		  "\n%s: TRACE: Returned from bootstrap_cli() ...",
		  __FUNCTION__);
#endif

    if (startmsg) {
        log_write("%s", startmsg);
    }
...
#ifdef LOGGER_TEST
    log_dev1("\n%s:  DEV1: In ncx_init() ...", __FUNCTION__);
    log_backtrace(LOG_DEBUG_DEV1,
		  "\n%s: TRACE: In ncx_init() ...", __FUNCTION__);
#endif

    /* init the static namespaces related to XML and YANG */
    res = init_namespaces();
    if (res != NO_ERR) {
        return res;
    }
    ...
    /* load the yuma-ncx module */
    ncx_module_t  *mod = NULL;
    res = ncxmod_load_module(NCXMOD_NCX, NULL, NULL, &mod);
    if (mod == NULL) {
        return res;
    }
    ...
    gen_leaflist = ncx_find_object(mod, NCX_EL_LEAFLIST);
    if (!gen_leaflist) {
        return ERR_NCX_DEF_NOT_FOUND;
    }
    ...
    /* load the yang-attributes module */
    ncx_module_t  *yangmod = NULL;
    res = ncxmod_load_module(YANG_MODULE, NULL, NULL, &yangmod);
    if (yangmod == NULL) {
        return res;
    }

    yang_insert = ncx_find_object(yangmod, NCX_EL_INSERT);
    if (!yang_insert) {
        return ERR_NCX_DEF_NOT_FOUND;
    }

    yang_key = ncx_find_object(yangmod, NCX_EL_KEY);
    if (!yang_key) {
        return ERR_NCX_DEF_NOT_FOUND;
    }

    yang_value = ncx_find_object(yangmod, NCX_EL_VALUE);
    if (!yang_value) {
        return ERR_NCX_DEF_NOT_FOUND;
    }

#ifdef LOGGER_TEST
    log_dev1("\n%s:  DEV1: Ready to return from ncx_init() ...", __FUNCTION__);
    log_backtrace(LOG_DEBUG_DEV1,
		  "\n%s: TRACE: Ready to return from ncx_init() ...",
		  __FUNCTION__);
#endif
    ...
    ncx_cleanup (void)
{
    ...
    /*
     * At this point, all memory allocations should have been returned ...
     * save one. The logging subsystem retains it's dynamically
     * allocated syslog/vendor output buffers IF --log-syslog OR
     * --log-vendor have been specified. (This is somewhat complicated
     * by the fact that for testing purposes, --log-vendor can redirect
     * it's output via syslog, temporarily or permanently.) For now, treat
     * the allocation of said buffers separately in case the implementation
     * changes in future.
     *
     * In order to report a possible log_error() below, this/these
     * buffer(s) MUST remain allocated. Hence the special logic to
     * account for this possibility. Further note that in existing code,
     * log_error() may be called even AFTER we exit this function ... !!!
     * (See print_error_count())
     */
    free_cnt_to_test = free_cnt;

    if (log_get_syslog_bfr_allocated()) {
        free_cnt_to_test += 1;
    }

    if (log_get_vendor_bfr_allocated()) {
	free_cnt_to_test += 1;
    }

    if (malloc_cnt > free_cnt_to_test) {
        log_error("\n*** Error: memory leak (m:%u f:%u)\n", 
                  malloc_cnt, free_cnt);
    } else if (malloc_cnt < free_cnt_to_test) {
                     ...
                     const xmlChar *revision)
{
    ...
    if (mod == NULL && ncx_sesmodQ != ncx_curQ) {
 ...
 */
obj_template_t *
    ncx_match_rpc (ncx_module_t *mod,
                   const xmlChar *rpcname,
                   uint32 *retcount)
{
    assert ( mod && " param mod is NULL" );
    ...
    *retcount = 0;
    ...
    uint32 cnt = 0;
    obj_template_t *firstfound = NULL;
    uint32 len = xml_strlen(rpcname);
    obj_template_t *rpc;
    for (rpc = (obj_template_t *)dlq_firstEntry(&mod->datadefQ);
         rpc != NULL;
         rpc = (obj_template_t *)dlq_nextEntry(rpc)) {
    ...
    *retcount = 0;
    ...
    if (module) {
        ...
        for (mod = ncx_get_first_module();
            ...
            uint32 tempcnt = 0;
            obj_template_t *rpc = ncx_match_rpc(mod, rpcname, &tempcnt);
            if (rpc) {
                if (firstfound == NULL) {
                    firstfound = rpc;
                }
    ...
    ncx_find_any_object (const xmlChar *objname)
{
    ...
    for (;
        ...
        obj = obj_find_template_top(mod, ncx_get_modname(mod), objname);
        if (obj) {
            return obj;
        }
    }
...
{
    assert ( objname && " param objname is NULL" );
    assert ( retres && " param retres is NULL" );
...
/**
 * \fn ncx_match_any_object_ex
 * \brief Check if an obj_template_t in in any module that
 * matches the object name string; extended parameters
 * \param modname module name of the object to match (may be NULL)
 * \param objname object name to match
 * \param dataonly TRUE to match only data nodes; FALSE to match 
 * any object type
 * \param name_match name match mode enumeration
 * \param alt_names TRUE if alternate names should be checked
 *                  after regular names; FALSE if not
 ...
 */
obj_template_t *
    ncx_match_any_object_ex (const xmlChar *modname,
                             ...
                             status_t *retres)
{
    ...
    for (; mod != NULL; mod =  ncx_get_next_module(mod)) {
        obj = obj_find_template_top_ex(mod, modname, objname,
                                       name_match, alt_names,
                                       dataonly, retres);
        if (obj) {
            return obj;
        }
                                   ...
                                   boolean tempmod)
{
    ...
    assert ( mod->ns && " mod namespace is NULL" );
    ...
    if (!tempmod) {
        ...
        xmlns_id_t nsid = xmlns_find_ns_by_prefix(mod->prefix);
        if (nsid) {
    ...
    boolean tempns = FALSE;
    boolean regns = TRUE;
    xmlns_t *ns = def_reg_find_ns(mod->ns);
    if (ns && ns->ns_temporary) {
        mod->nsid = ns->ns_id;
        tempns = TRUE;
    } else if (ns) {
        regns = FALSE;
        if (tempmod) {
            ...
            ;
                   ...
                   xml_strcmp(mod->name, ns->ns_module) &&
    ...
    if (regns && !isnetconf && res == NO_ERR) {
        const xmlChar *pfix = 
            (mod->xmlprefix) ? mod->xmlprefix : mod->prefix;
        ncx_module_t *usemod = (tempmod) ? NULL : mod; 

        if (tempns && ns) {
            res = xmlns_convert_temp_ns(ns->ns_id, pfix, mod->name, usemod);
        } else {
            res = xmlns_register_ns(mod->ns, pfix, mod->name, usemod,
                                    &mod->nsid);
        }
        if (res != NO_ERR) {
            /* this NS registration failed */
            log_error("\nncx reg: Module '%s' %s namespace '%s' failed (%s)",
                      mod->name, 
                      (tempns) ? "converting temporary" : "registering",
                      mod->ns, get_error_string(res));
        }
    }
 ...
 * \param modQ Q of ncx_module_t * to add to; NULL will pick ncx_curQ
 * \return status of the operation
 */
status_t 
    ...
    ncx_get_first_data_object (ncx_module_t *mod)
{
    ...
    if (!ncx_mod_has_data_nodes(mod, TRUE, TRUE)) {
        return NULL;
    }
...
/**
 * \fn ncx_get_first_data_object_ex
 * \brief Get the first database object in the datadefQs 
 * for the specified module
 * \param mod module to search for the first object 
 * \param config == TRUE for config node FALSE for non-config
 * \return pointer to the first object or NULL if empty Q
 */
obj_template_t *
    ncx_get_first_data_object_pick (ncx_module_t *mod,
                                    boolean config)
{
    assert ( mod && " param mod is NULL");
    ...
    if (config) {
        ...
        if (!ncx_mod_has_data_nodes(mod, FALSE, TRUE)) {
            return NULL;
        }
    }
    ...
    for (obj = (obj_template_t *)dlq_firstEntry(&mod->datadefQ);
        ...
        if (obj_is_config(obj) == config && obj_is_data_db(obj)) {
            return obj;
        }
    }
 ...
 */
obj_template_t *
    ncx_get_next_data_object_same (ncx_module_t *mod,
                                   obj_template_t *curobj)
{
    assert ( mod && " param mod is NULL");
    assert ( curobj && " param curobj is NULL");

    boolean isconfig = obj_is_config(curobj);
    ...
    for (obj = (obj_template_t *)dlq_nextEntry(curobj);
        ...
        if (isconfig == obj_is_config(obj) && obj_is_data_db(obj)) {
            return obj;
        }
    }
 ...
 * \param enum_name enum string value to set
 * \param enum_val enum value number to set
 * \param retenu pointer to return enum variable to fill in 
 * \return status 
 */
status_t
    ncx_set_enum (const xmlChar *enum_name,
                  const int32 enum_val,
                  ncx_enum_t *retenu)
{
    assert ( enum_name && " param enumv_name is NULL" );
    assert ( retenu && " param retenu is NULL" );

    m__free(retenu->dname);
    retenu->dname = NULL;
    retenu->name = enum_name;
    retenu->val = enum_val;
...
/**
 * \fn ncx_get_gen_leaflist
 * \brief Get the object template for the NCX generic leaf-list string
 * \return pointer to generic leaf-list string object template
 */
obj_template_t *
    ncx_get_gen_leaflist (void)
{
    return gen_leaflist;
...
/**
 * \fn ncx_get_yang_insert
 * \brief Get the object template for the YANG insert attribute
 * \return pointer to object template
 */
obj_template_t *
    ncx_get_yang_insert (void)
{
    return yang_insert;
...
/**
 * \fn ncx_get_yang_key
 * \brief Get the object template for the YANG key attribute
 * \return pointer to object template
 */
obj_template_t *
    ncx_get_yang_key (void)
{
    return yang_key;
...
/**
 * \fn ncx_get_yang_value
 * \brief Get the object template for the YANG value attribute
 * \return pointer to object template
 */
obj_template_t *
    ncx_get_yang_value (void)
{
    return yang_value;
    ...
    *res = NO_ERR;
    ...
    if (*p == NCXMOD_PSCHAR) {
        ...
        if (userlen) {
            ...
            if (user == NULL && 
                userlen == xml_strlen((const xmlChar *)USER_HOME) &&
                !xml_strncmp(start, (const xmlChar *)USER_HOME, userlen)) {
                /* special case: no $HOME variable; check if
                 * ncxmod_home variable is set   */
                user = ncxmod_get_userhome(NULL, 0);
            }
        }
...
/********************************************************************
* FUNCTION ncx_get_session_modQ
* 
* RETURNS:
*    que for session-specific loaded modules
*********************************************************************/
dlq_hdr_t *
    ncx_get_session_modQ (void)
{
    return ncx_sesmodQ;
                            ...
                            const xmlChar *line)
{
    ...
    if (tkc && (tkc->source != TK_SOURCE_YANG)) {
        return;
    }
                     ...
                     uint32 buffsize)
{
    ...
    const xmlChar *eval = EMPTY_STRING;

#ifdef EVAL_VERSION
    eval = (const xmlChar *)"-EVAL";
#endif
    ...
    versionlen = xml_strlen(YUMA_VERSION) +
        xml_strlen(numbuff) + xml_strlen(eval) + 1;
    ...
    *str++ = '-';
    ...
    versionlen = xml_strlen((const xmlChar *)SVNVERSION) + xml_strlen(eval);
    if (versionlen >= buffsize) {
        return ERR_BUFF_OVFL;
    }

    str = buffer;
    str += xml_strcpy(str, (const xmlChar *)SVNVERSION);
    xml_strcpy(str, eval);
    ...
    ncx_protocol_enabled (ncx_protocol_t proto)
{
    ...
    case NCX_PROTO_NETCONF11:
        ...
        break;
    case NCX_PROTO_YUMA_YANGAPI:
        ret = (protocols_enabled & NCX_FL_PROTO_YUMA_YANGAPI) ? TRUE : FALSE;
        break;
    case NCX_PROTO_YUMA_CLI:
        ret = (protocols_enabled & NCX_FL_PROTO_YUMA_CLI) ? TRUE : FALSE;
        break;
    default:
        SET_ERROR(ERR_INTERNAL_VAL);
    }
...
/********************************************************************
* FUNCTION ncx_val_protocol_enabled
*
* Check if the specified protocol version is enabled
*
* INPUTS:
...
*********************************************************************/
boolean
    ncx_val_protocol_enabled (ncx_protocol_t proto,
                              uint16 flags)
{
    boolean ret = FALSE;
    switch (proto) {
    case NCX_PROTO_NETCONF10:
        ret = (flags & NCX_FL_PROTO_NETCONF10) ? TRUE : FALSE;
        break;
    case NCX_PROTO_NETCONF11:
        ret = (flags & NCX_FL_PROTO_NETCONF11) ? TRUE : FALSE;
        break;
    case NCX_PROTO_YUMA_YANGAPI:
        ret = (flags & NCX_FL_PROTO_YUMA_YANGAPI) ? TRUE : FALSE;
        break;
    case NCX_PROTO_YUMA_CLI:
        ret = (flags & NCX_FL_PROTO_YUMA_CLI) ? TRUE : FALSE;
        break;
    default:
        SET_ERROR(ERR_INTERNAL_VAL);
    }
    ...
    ncx_set_protocol_enabled (ncx_protocol_t proto)
{
    ...
    case NCX_PROTO_NETCONF11:
        ...
        break;
    case NCX_PROTO_YUMA_YANGAPI:
        protocols_enabled |= NCX_FL_PROTO_YUMA_YANGAPI;
        break;
    case NCX_PROTO_YUMA_CLI:
        protocols_enabled |= NCX_FL_PROTO_YUMA_CLI;
        break;
    default:
        SET_ERROR(ERR_INTERNAL_VAL);
    }
...
/********************************************************************
* FUNCTION ncx_set_val_protocol_enabled
*
* Set the specified protocol version to be enabled
* in a uint16 bits val

* INPUTS:
*   proto == protocol version to enable
*   flags == address of bitmask to set
*
* OUTPUTS:
*   *flags is set with the bit for the protocol
*********************************************************************/
void
    ncx_set_val_protocol_enabled (ncx_protocol_t proto,
                                  uint16 *flags)
{
    switch (proto) {
    case NCX_PROTO_NETCONF10:
        *flags |= NCX_FL_PROTO_NETCONF10;
        break;
    case NCX_PROTO_NETCONF11:
        *flags |= NCX_FL_PROTO_NETCONF11;
        break;
    case NCX_PROTO_YUMA_YANGAPI:
        *flags |= NCX_FL_PROTO_YUMA_YANGAPI;
        break;
    case NCX_PROTO_YUMA_CLI:
        *flags |= NCX_FL_PROTO_YUMA_CLI;
        break;
    default:
        SET_ERROR(ERR_INTERNAL_VAL);
    }
...
/********************************************************************
* FUNCTION ncx_get_protocols_enabled
*
...
*********************************************************************/
const xmlChar *
    ncx_get_protocols_enabled (uint16 flags)
{
    if ((flags & NCX_FL_PROTO_NETCONF10) && 
        (flags & NCX_FL_PROTO_NETCONF11)) {
        return (const xmlChar *)"netconf1.0 netconf1.1";
    }

    if (flags & NCX_FL_PROTO_NETCONF10) {
        return (const xmlChar *)"netconf1.0";
    }

    if (flags & NCX_FL_PROTO_NETCONF11) {
        return (const xmlChar *)"netconf1.1";
    }
    return EMPTY_STRING;
...
/********************************************************************
* FUNCTION ncx_get_message_indent
* 
* Get the protoco9l output indent amount
*
* RETURNS:
*   -1 .. 9 : indent parameter value
*********************************************************************/
int8
    ncx_get_message_indent (void)
{
...
*
* INPUTS:
*   msg_indent == -1 .. 9 : indent parameter value
*********************************************************************/
void
    ncx_set_message_indent (int8 msg_indent)
{
    if (msg_indent < -1) {
        msg_indent = -1;
    } else if (msg_indent > 9) {
        msg_indent = 9;
    }
    message_indent = msg_indent;

}  /* ncx_set_message_indent */


/********************************************************************
* FUNCTION ncx_decode_url_string
* 
...
*********************************************************************/
status_t
    ncx_decode_url_string (const xmlChar *urlstr,
                           uint32 urlstrlen,
                           xmlChar *buffer,
                           uint32 *cnt)
{
...
/**
 * \fn dump_modQ
 * \brief dump module Q contents [name, revision] for debugginh
 * \param modQ == Q of ncx_module_t to use
 * \return none
 */
void
    dump_modQ (dlq_hdr_t *modQ)
{
    if (!LOGDEBUG) {
        return;
    }

    ncx_module_t *testmod = (ncx_module_t *)dlq_firstEntry(modQ);
    for (; testmod != NULL;
         testmod = (ncx_module_t *)dlq_nextEntry(testmod)) {

        log_debug("\n%smodule %s revision %s",
                  testmod->ismod ? EMPTY_STRING : (const xmlChar *)"sub",
                  testmod->name ? testmod->name : EMPTY_STRING,
                  testmod->version ? testmod->version : EMPTY_STRING);
    }
}  /* dump_modQ */


/********************************************************************
* FUNCTION ncx_mod_has_rpcs
* 
...
*********************************************************************/
boolean
    ncx_mod_has_rpcs (ncx_module_t *mod)
{
    assert ( mod && " param mod is NULL" );

    if (mod->flags & NCX_FL_MOD_DONE) {
        if (mod->flags & NCX_FL_MOD_RPC_CHK) {
            return (mod->flags & NCX_FL_MOD_RPC) ? TRUE : FALSE;
        }
    }
    ...
    for (rpc = (obj_template_t *)dlq_firstEntry(&mod->datadefQ);
         rpc != NULL && !found;
         rpc = (obj_template_t *)dlq_nextEntry(rpc)) {
        if (rpc->objtype == OBJ_TYP_RPC && !obj_is_abstract(rpc)) {
            found = TRUE;
        }
    }

    if (mod->flags & NCX_FL_MOD_DONE) {
        mod->flags |= NCX_FL_MOD_RPC_CHK;
        if (found) {
            mod->flags |= NCX_FL_MOD_RPC;
        }
    }
...
/********************************************************************
* FUNCTION ncx_mod_has_data_nodes
* 
...
*********************************************************************/
boolean
    ncx_mod_has_data_nodes (ncx_module_t *mod,
                            boolean check_config,
                            boolean check_operational)
{
    assert ( mod && " param mod is NULL" );

    if (!check_config && !check_operational) {
        log_debug("\nncx_mod_has_data_nodes got empty search request");
        return FALSE;
    }
    ...
    if (mod->flags & NCX_FL_MOD_DONE) {
        if (check_config) {
            if (mod->flags & NCX_FL_MOD_CDATA_CHK) {
                found = (mod->flags & NCX_FL_MOD_CDATA) ? TRUE : FALSE;
            } else {
                checkdone = FALSE;
            }
        }
        if (!found && check_operational) {
            if (mod->flags & NCX_FL_MOD_ODATA_CHK) {
                found = (mod->flags & NCX_FL_MOD_ODATA) ? TRUE : FALSE;
            } else {
                checkdone = FALSE;
            }
        }
    ...
    obj_template_t *data;
    for (data = (obj_template_t *)dlq_firstEntry(&mod->datadefQ);
         data != NULL && !found;
         data = (obj_template_t *)dlq_nextEntry(data)) {
        if (obj_is_data_db(data)) {
            config = obj_is_config(data);
            if (config) {
                if (check_config) {
                    found = TRUE;
                }
            } else if (check_operational) {
                found = TRUE;
            }
        }
    }
    ...
    if (mod->flags & NCX_FL_MOD_DONE) {
        if (found) {
            /* did not check entire list so only know for sure
             * that the type of node found was checked
             */
            if (config && check_config) {
                mod->flags |= NCX_FL_MOD_CDATA_CHK;
                mod->flags |= NCX_FL_MOD_CDATA;
            } else if (!config && check_operational) {
                mod->flags |= NCX_FL_MOD_ODATA_CHK;
                mod->flags |= NCX_FL_MOD_ODATA;
            } // else should not happen
        } else {
            /* checked entire list and did not find any matches */
            if (check_config) {
                mod->flags |= NCX_FL_MOD_CDATA_CHK;
            }
...
/********************************************************************
* FUNCTION ncx_collapse_submodules_ok
* 
* Check if it is OK to collapse submodule definitions
...
*********************************************************************/
boolean
    ncx_collapse_submodules_ok (void)
{
    return collapse_submodules;
}  /* ncx_collapse_submodules_ok */


/********************************************************************
* FUNCTION ncx_collapse_submod_defs
* 
* Go through one module and move all the definitions from the
* sub-module queues to the main module queue.  This is only done
* in the yangcli/netconfd programs to make searches faster and
* support sub-modules correctly;  many functions do not check the
* sub-modules and will ignore definitions in those sub-modules.
*
* This must only be done after all modules are loaded and
* the module mod_done flag is being set to TRUE.
* Should only be called by yang_parse/parse_yang_module
*
* INPUTS:
*    mod == module to check
*********************************************************************/
void
    ncx_collapse_submod_defs (ncx_module_t *mod)
{
    ...
    yang_node_t *node = (yang_node_t *)dlq_firstEntry(&mod->allincQ);
    for (; node; node = (yang_node_t *)dlq_nextEntry(node)) {
        if (node->submod == NULL) {
            SET_ERROR(ERR_INTERNAL_PTR);
            continue;
        }
        ...
        if (LOGDEBUG2) {
            log_debug2("\nMoving definitions from submod '%s' to '%s'",
                       node->submod->name, mod->name);
        }

        /* move all the definitions to the main module */
        dlq_block_enque(&node->submod->typeQ, &mod->typeQ);
        dlq_block_enque(&node->submod->groupingQ, &mod->groupingQ);
        dlq_block_enque(&node->submod->datadefQ, &mod->datadefQ);
        dlq_block_enque(&node->submod->extensionQ, &mod->extensionQ);
        dlq_block_enque(&node->submod->deviationQ, &mod->deviationQ);
        dlq_block_enque(&node->submod->featureQ, &mod->featureQ);
        dlq_block_enque(&node->submod->identityQ, &mod->identityQ);
    }
...
/********************************************************************
* FUNCTION ncx_use_xpath_backptrs
* 
...
*********************************************************************/
boolean
    ncx_use_xpath_backptrs (void)
{
    return use_xpath_backptrs;

}  /* ncx_use_xpath_backptrs */


/********************************************************************
* FUNCTION ncx_set_all_obj_xpath_backptrs
* 
...
*********************************************************************/
status_t
    ncx_set_all_obj_xpath_backptrs (void)
{
    ncx_module_t *mod;
    for (mod = ncx_get_first_module();
         mod != NULL;
        ...
        status_t res = ncx_set_obj_xpath_backptrs(mod);
        if (res != NO_ERR) {
            return res;
        }
    }
...
/********************************************************************
* FUNCTION ncx_set_obj_xpath_backptrs
* 
* Add Xpath back pointers to the data-db objects in the database
* for 1 module
*
...
*********************************************************************/
status_t
    ncx_set_obj_xpath_backptrs (ncx_module_t *mod)
{

    obj_template_t *obj = ncx_get_first_data_object_pick(mod, TRUE);
    for (; obj != NULL; obj = ncx_get_next_data_object_same(mod, obj)) {
        status_t res = obj_set_xpath_backptrs(obj);
        if (res != NO_ERR) {
            return res;
        }
    }
...
*********************************************************************/
ncx_backptr_t *
    ncx_new_backptr (void *node)
{
    ncx_backptr_t *ptr = m__getObj(ncx_backptr_t);
    if (ptr == NULL) {
        return NULL;
    }
    memset(ptr, 0x0, sizeof(ncx_backptr_t));
    ptr->node = node;
    return ptr;
...
/********************************************************************
* FUNCTION ncx_clean_backptrQ
*
* Clean an entire Q of ncx_backptr_t structs
*
* INPUTS:
*   backptrQ == Q of ncx_backptr_t to use
*********************************************************************/
void ncx_clean_backptrQ (dlq_hdr_t *backptrQ)
{
    ...
    while (!dlq_empty(backptrQ)) {
        ncx_backptr_t *backptr = (ncx_backptr_t *)dlq_deque(backptrQ);
 ...
 *
 * \param que the queue of ncx_backptr_t to check
 * \return pointer to first entry or NULL if none
 *********************************************************************/
ncx_backptr_t *
    ncx_first_backptr (dlq_hdr_t *que)
{
    assert(que && "que is NULL" );

    return (ncx_backptr_t *)dlq_firstEntry(que);
 ...
 *
 * \param curptr the current ncx_backptr_t to check
 * \return pointer to next entry or NULL if none
 *********************************************************************/
ncx_backptr_t *
    ncx_next_backptr (ncx_backptr_t *curptr)
{
    assert(curptr && "curptr is NULL" );

    return (ncx_backptr_t *)dlq_nextEntry(curptr);
 ...
 *
 * \param backptr the ncx_backptr_t to check
 * \return pointer to node within backptr
 *********************************************************************/
void *
    ncx_get_backptr_node (ncx_backptr_t *backptr)
{
    assert(backptr && "curptr is NULL" );
 ...
 *********************************************************************/
void
    ncx_remove_backptr (ncx_backptr_t *ptr)
{
 ...
 * \return pointer to the found backptr entry or NULL if not found
 *********************************************************************/
ncx_backptr_t *
    ncx_find_backptr (dlq_hdr_t *que,
                      void *node)
{
    assert(que && "que is NULL" );
    assert(node && "node is NULL" );

    ncx_backptr_t *ptr = (ncx_backptr_t *)dlq_firstEntry(que);
    for (; ptr; ptr = (ncx_backptr_t *)dlq_nextEntry(ptr)) {
        if (ptr->node == node) {
            return ptr;
        }
    }
...
/********************************************************************
* FUNCTION ncx_find_all_identities
* 
* Go through all the modules and find all the identities that
* match the specified base identity
* INPUTS:
...
*********************************************************************/
void
    ncx_find_all_identities (ncx_identity_t *base,
                             ncx_identity_cbfn_t cbfn,
                             void *cookie)
{
    assert(cbfn && "cbfn is NULL!");
    walk_all_identities(base, base, cbfn, cookie);

}  /* ncx_find_all_identities */


/********************************************************************
* FUNCTION ncx_get_nc_operation
* 
* Get the cached operation meta-object template used to contruct
* a metaval for the attribute
*
* RETURNS:
*  pointer to object template; NULL if not available
*********************************************************************/
obj_template_t *
    ncx_get_nc_operation (void)
{
    /* hack: server does not save description-stmts and yangcli does */
    boolean is_server = !save_descr;

    if (is_server && nc_operation) {
        /* OK to cache this pointer in the server only */
        return nc_operation;
    }

    ncx_module_t *ncmod = ncx_find_module(NC_MODULE, NULL);
    if (ncmod == NULL) {
        return NULL;
    }

    obj_template_t *operobj =
        ncx_find_object(ncmod, NC_OPERATION_ATTR_NAME);

    if (is_server) {
        nc_operation = operobj;
    }
...
*********************************************************************/
boolean
    ncx_fastpath_enabled (void)
{
    /* TBD: use is_netconfd parameter from cheetah-perf */
    return !save_descr;
