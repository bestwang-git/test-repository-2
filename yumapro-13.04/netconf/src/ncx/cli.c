                         ...
                         status_t *res)
{
    ...
    *res = NO_ERR;
    ...
    /* need the pad amount for each line in case double quotes
     * get added to the buffer; otherwise copy_argv will write
     * past the end of the buffer when it adds dlb quotes
     * that argv/argv takes out
     */
...
*********************************************************************/
cli_rawparm_t *
    cli_new_rawparm (const xmlChar *name, boolean xsdlist)
{
    cli_rawparm_t  *parm;
                   ...
                   dlq_hdr_t *rawparmQ)
{
    ...
    assert(rawparmQ && "rawparmQ is NULL!");
    ...
    /*
     * Note that debugs in this function are somewhat problematic for
     * several reasons:
     * -- Called from bootstrap_cli() in ncx.c, --log-level has not
     *    yet been parsed or set. See caller function for a temp patch.
     * -- The default debug level passed to bootstrap_cli() is LOG_DEBUG_INFO.
     * -- Even when patched in bootstrap_cli(), the output will be displayed
     *    on the console, since neither --log, --log-syslog, nor --log-vendor
    ...
    status_t res = NO_ERR;
    int32 bufflen = 0;
    char *buff = copy_argv_to_buffer(argc, argv, CLI_MODE_PROGRAM, 
                                     &bufflen, &res);
    if (!buff) {
        return res;
    }
    ...
    while (buffpos < bufflen && res == NO_ERR) {
        ...
        char *parmname = &buff[buffpos];
        char *str = &parmname[1];
        int32 cnt = buffpos + 1;
        while (cnt < bufflen && *str && !isspace((int)*str) && *str != '=') {
            str++;
            cnt++;
        }

        int32 parmnamelen = str - parmname;
        buffpos += parmnamelen;

        char *parmval = NULL;

        /* check if this parameter name is in the parmset def */
        cli_rawparm_t *rawparm = find_rawparm(rawparmQ, parmname, 
                                              parmnamelen);
        if (rawparm) {
            rawparm->count++;
            if (!rawparm->hasvalue) {
        ...
        if ((buffpos < bufflen) &&
            ...
            if (buffpos < bufflen)  {
                ...
                /* terminate string if really getting this parameter */
                if (rawparm) {
                    *str = 0;
                }
        ...
        if (rawparm) {
            ...
            if (rawparm->ncx_list) { /* Necessary ??? */
                ncx_free_list(rawparm->ncx_list);
            }
            rawparm->value = (char *)
	    ...
	    if (rawparm->xsdlist) {
	        ...
	        /*
		 * Set up to parse the xsdlist type string param into
		 * a "proper" xsdlist, represented by an ncx_list_t
		 * structure. This will be used in the second stage of
		 * raw (bootstrap_cli()) parsing by calling the common
		 * checking routines in ncx_list.c.
		 */
		if (LOGDEBUG2) {
		    log_debug2("\n%s: parm (%p) is xsdlist '%s'",
			       __FUNCTION__, (void *)rawparm,
			       (char *)rawparm->value);
		}

		/* Set up ncx_list based structure */
		ncx_list = ncx_new_list(NCX_BT_SLIST);
		if (!ncx_list) {
		    res = ERR_INTERNAL_MEM;
		    continue;
		}

		rawparm->ncx_list = ncx_list;
		/* Parse XSD string into list ... */
		ncx_set_list(NCX_BT_SLIST, (const xmlChar *) rawparm->value,
			     rawparm->ncx_list);
	    }
        }
    }
    ...
    while (buffpos < bufflen && res == NO_ERR) {
        ...
        if (res == NO_ERR) {
            ...
            if (chobj == NULL) {
                ...
                ncx_btype_t btyp = obj_get_basetype(chobj);
                parmval = NULL;
        ...
        if (res != NO_ERR) {
            const char *msg = get_error_string(res);
            xmlChar errbuff[ERRLEN+1];
            errbuff[0] = 0;
            if (parmname != NULL) {
