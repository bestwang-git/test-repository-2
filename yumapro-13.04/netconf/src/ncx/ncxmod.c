...
static xmlChar *ncxmod_backup_dir;

static const xmlChar *ncxmod_start_dir;
                  ...
                  boolean dive)
{
    ...
    while (!dirdone) {
        ...
        if (ep->d_type == DT_REG || ep->d_type == DT_UNKNOWN) {
		    ...
		    log_stdout("%s", (const char *)buff);
                }
                log_stdout("%s", ep->d_name);
                ...
                if (helpmode == HELP_MODE_FULL) {
		    log_write("%s", (const char *)buff);
                }
                log_write("%s", ep->d_name);
                if (helpmode != HELP_MODE_BRIEF) {
                    ;
                }
            }
...
*
* Get the files in the subdir and invoke the callback function
*
* INPUTS:
*    buff == buffer to use for filespec construction
*            at the start it contains the path string to use;
...
*    cookie == parameter for callback function
*    dive == TRUE to list subdirs; FALSE to list just the
*            specified directory
*
* RETURNS:
...
*********************************************************************/
static status_t
    get_subdirs (xmlChar *buff,
                 uint32 bufflen,
                 ncxmod_backup_cbfn_t cbfn,
                 void *cookie,
                 boolean dive)
{
    status_t res = NO_ERR;

    uint32 pathlen = xml_strlen(buff);
    struct dirent *ep = NULL;

    /* try to open the buffer spec as a directory */
    DIR *dp = opendir((const char *)buff);
    if (!dp) {
        return NO_ERR;  /* not done yet */
    }
    ...
    while (!dirdone && res == NO_ERR) {
        ...
        if (ep->d_type == DT_REG || ep->d_type == DT_UNKNOWN) {
            ...
            const xmlChar *fname = (const xmlChar *)ep->d_name;
            uint32 flen = xml_strlen(fname);

            if ((pathlen + flen + 2) >= bufflen) {
                res = ERR_BUFF_OVFL;
                dirdone = TRUE;
            } else {
                xmlChar *start = &buff[pathlen];
                *start = '/';
                xml_strcpy(start+1, fname);

                time_t filetime;
                res = ncxmod_get_file_mtime(buff, &filetime);
                *start = 0;   // reset buff to just dirspec
                if (res == NO_ERR) {
                    xmlChar tbuff[TSTAMP_MIN_SIZE];
                    tstamp_time2datetime(&filetime, tbuff);

                    /* invoke the callback for the file */
                    res = (*cbfn)(ep->d_name, tbuff, cookie);
                }
            }
        }
    }
    ...
    while (!dirdone && res == NO_ERR) {
         ...
         * the dot-char or is named CVS
         */
        uint32 dentlen = xml_strlen((const xmlChar *)ep->d_name); 
            ...
            if (*ep->d_name != '.' && strcmp(ep->d_name, "CVS")) {
                if ((pathlen + dentlen + 2) >= bufflen) {
                    ...
                    *str = '\0';
                    ...
                    res = get_subdirs(buff, bufflen, cbfn, cookie, dive);
...
*   7) default install module location, which is
                 ...
                 ncx_module_t **retmod)
{
    ...
    if (LOGDEBUG2) {
        ...
        if (revision) {
            log_debug2_append(" r:%s", revision);
        }
    } else if (LOGDEBUG) {
        log_debug("\nload_module called for '%s'", modname);
    }

    uint32 modlen = xml_strlen(modname);
    ...
    ncxmod_mode_t mode = determine_mode( modname, modlen );
    ...
    if ((ptyp != YANG_PT_INCLUDE) && 
        ...
        if (testmod) {
            ...
            /*** hack for now, check if this is ietf-netconf, except
             *** if the application is actually searching for this
             *** module   */
            if (!pcb->searchmode && 
                !xml_strcmp(modname, NCXMOD_IETF_NETCONF)) {
    ...
    uint32 bufflen = NCXMOD_MAX_FSPEC_LEN+1;
    xmlChar *buff = m__getMem(bufflen);
    if (!buff) {
        return ERR_INTERNAL_MEM;
    } else {
        *buff = 0;
    ...
    /* 8) start-dir/modules directory */
    if (!done && ncxmod_start_dir) {
        res = check_module_path( ncxmod_start_dir, buff, bufflen, modname, 
                                 revision, pcb, ptyp, FALSE, &done );
    }
                             ...
                             void *cookie)
{
    ...
    if (res != NO_ERR || retmod == NULL) {
        ...
        /* keep going unless there is a fatal error
         * may hit some YANG module that fails to compile
         */
        if (!NEED_EXIT(res)) {
            res = NO_ERR;
        }
    ...
    ncxmod_init (void)
{
    ...
    ncxmod_backup_dir = NULL;
    ncxmod_start_dir = NULL;

    log_debug4("\ncxmod: reading environment variables");

    /* try to get the YUMA_HOME environment variable */
    ncxmod_yuma_home = (const xmlChar *)getenv(NCXMOD_HOME);
    if (ncxmod_yuma_home && !ncxmod_test_subdir(ncxmod_yuma_home)) {
        log_warn("\nWarning: cannot open '%s' directory; Ignoring "
                 "'%s' env-var\n", ncxmod_yuma_home, NCXMOD_HOME);
    ...
    if (ncxmod_env_install && !ncxmod_test_subdir(ncxmod_env_install)) {
        log_warn("\nWarning: cannot open '%s' directory; Ignoring "
                 "'%s' env-var", ncxmod_env_install, NCXMOD_INSTALL);
        ncxmod_env_install = NULL;
    }

    /* try to get the user HOME environment variable */
    ncxmod_home = (const xmlChar *)getenv(USER_HOME);
    if (ncxmod_home && !ncxmod_test_subdir(ncxmod_home)) {
        log_warn("\nWarning: cannot open '%s' directory; Ignoring "
                 "'%s' env-var", ncxmod_home, USER_HOME);
        ncxmod_home = NULL;
    }

    /* try to get the PWD environment variable */
    ncxmod_start_dir = (const xmlChar *)getenv("PWD");
    if (ncxmod_start_dir && !ncxmod_test_subdir(ncxmod_start_dir)) {
        log_warn("\nWarning: cannot open '%s' directory; Ignoring "
                 "'%s' env-var", ncxmod_start_dir, "PWD");
        ncxmod_start_dir = NULL;
    }

    /* try to get the module search path variable */
    ncxmod_mod_path = (const xmlChar *)getenv(NCXMOD_MODPATH);
    /* the MODPATH chain of dirspecs does not have to contain
     * all valid directories, so it is not checked here
     */
    ...
    /* the DATAPATH chain of dirspecs does not have to contain
     * all valid directories, so it is not checked here
     */

    /* try to get the script search path variable */
    ncxmod_run_path = (const xmlChar *)getenv(NCXMOD_RUNPATH);
    /* the RUNPATH chain of dirspecs does not have to contain
     * all valid directories, so it is not checked here
     */
                        ...
                        ncx_module_t **retmod)
{
    ...
    if (!pcb) {
        ...
        ncx_module_t *testmod = NULL;

        pcb->revision = revision;
        pcb->savedevQ = savedevQ;
        pcb->with_submods = TRUE;  // require top to be a module

        res = try_load_module(pcb, YANG_PT_TOP, modname, revision, &testmod);

        if (res == NO_ERR && testmod) {
            testmod->supported = TRUE;
            if (retmod) {
                *retmod = testmod;
            }
...
*   revision == optional revision date of 'modname' to find
*   savedevQ == Q of ncx_save_deviations_t to use, if any
*   savemodQ == Q of ncx_module_t to save any loaded modules
*   retmod == address of return module
*
* OUTPUTS:
*    savemodQ will have entries added for any missing imports
*      and the 'modname' module
*       THIS Q HAS LIVE MALLOCED STRUCTS THAT NEED
*       TO BE FREED LATER
*
*    *retmod == pointer to requested module version
*
                         ...
                         ncx_module_t **retmod)
{
    ...
    if (LOGINFO && res != NO_ERR) {
        if (revision) {
            log_info("\nLoad module '%s', revision '%s' failed (%s)",
                     modname, revision, get_error_string(res));
        } else {
            log_info("\nLoad module '%s' failed (%s)",
                     modname, get_error_string(res));
        }
    }
                        ...
                        const xmlChar *revision)
{
    ...
    status_t res = 
        try_load_module(pcb, YANG_PT_TOP, modname, revision, &retmod);
    ...
    ncxmod_find_all_modules (dlq_hdr_t *resultQ)
{
    ...
    /* 5) start-dir/modules directory */
    if (res == NO_ERR && ncxmod_start_dir) {
        res = search_module_path(ncxmod_start_dir,
                                 buff,
                                 bufflen,
                                 search_subtree_callback,
                                 resultQ);
                           ...
                           dlq_hdr_t *deviationQ)
{
    ...
    if (!pcb) {
        ...
        pcb->with_submods = TRUE;  // require deviations to be a module
                         ...
                         ncx_module_t **retmod)
{
    ...
    yang_node_t *node = yang_find_node(&pcb->failedQ, modname, revision);
    if (node) {
        return node->res;
    }

    const xmlChar *savedrev = pcb->revision;
    pcb->revision = revision;
    pcb->parentparm = parent;

    status_t res = try_load_module(pcb, ptyp, modname, revision, retmod);
...
xmlChar *
    ncxmod_find_data_file (const xmlChar *fname,
                           ...
                           status_t *res)
{
    assert(fname && "fname is NULL!");
    assert(res && "res is NULL!");

    *res = NO_ERR;

    if (LOGDEBUG2) {
        log_debug2("\nNcxmod: Finding %s file (%s)",
                   backup_file ? "backup" : "data", fname);
    }
    ...
    /* check if this is a backup file and the server vendor
     * wants to keep backup files in a special directory     */
    if (backup_file) {
        /* if backup_file specified then that is the only
         * dir to look in    */
        if (ncxmod_backup_dir &&
            test_file(buff, bufflen, ncxmod_backup_dir, NULL, fname)) {
            return buff;
        }
        m__free(buff);
    ...
    /* 8) start-dir/data directory */
    if (ncxmod_start_dir) {
        if (test_file(buff, bufflen, ncxmod_start_dir, NCXMOD_DATA_DIR, 
                      fname)) {
            return buff;
        }
    }
...
*  3) $YUMA_INSTALL/lib
*  4) $YUMA_INSTALL/lib/yumapro
*  5) /usr/lib64/yumapro directory (LIB64 only)
*  6) /usr/lib/yumapro directory
*  7) start-dir/lib
*
* INPUTS:
                          ...
                          status_t *res)
{
    ...
    *res = NO_ERR;
    ...
    /* 4) YUMA_INSTALL/lib/yumapro directory */
    if (ncxmod_env_install) {
        if (test_file(buff, 
                      bufflen, 
                      ncxmod_env_install,
                      (const xmlChar *)"lib/yumapro",
                      fname)) {
            return buff;
        }
    }

    /* 5) start-dir/lib/yumapro directory */
    if (ncxmod_start_dir) {
        if (test_file(buff, 
                      bufflen, 
                      ncxmod_start_dir,
                               ...
                               status_t *res)
{
    ...
    *res = NO_ERR;
    ...
    if (backup_file) {
         ...
         * dir to look in    */
        if (ncxmod_backup_dir && 
            test_file_make(buff, bufflen, ncxmod_backup_dir,
                           NULL, fname) == NO_ERR) {
            return buff;
        }
        m__free(buff);
    ...
    /* 1) try the YUMA_DATAPATH environment variable */
    if (ncxmod_data_path) {
        if (test_pathlist_make(ncxmod_data_path, buff, bufflen)) {
            uint32 pathlen = xml_strlen(buff);
    ...
    /* 7) start-dir directory */
    if (ncxmod_start_dir) {
        if (test_file_make(buff, bufflen, ncxmod_start_dir, NULL, fname)
            == NO_ERR) {
            return buff;
        }
    }
                             ...
                             status_t *res)
{
    ...
    *res = NO_ERR;
    ...
    /* 6) start-dir directory */
    if (ncxmod_start_dir) {
        if (test_file(buff, bufflen, ncxmod_start_dir, NCXMOD_SCRIPT_DIR,
                      fname)) {
            return buff;
        }
    }
    ...
    ncxmod_set_home (const xmlChar *home)
{
    ...
    if (!ncxmod_test_subdir(ncxmod_home_cli)) {
        log_error("\nError: cannot open 'home' directory '%s'\n",
                  ncxmod_home_cli);
        m__free(ncxmod_home_cli);
        ncxmod_home_cli = savehome;
        return;
    ...
    ncxmod_set_yuma_home (const xmlChar *yumahome)
{
    ...
    if (yumahome && *yumahome) {
        ...
        if (!ncxmod_test_subdir(ncxmod_yuma_home_cli)) {
            log_error("\nError: cannot open yuma home directory '%s',"
                      "ignoring new value", ncxmod_yuma_home_cli);
            return;
        }
                            ...
                            boolean logstdout)
{
    ...
    /* 6) start-dir/data directory */
    if (ncxmod_start_dir) {
        pathlen = xml_strlen(ncxmod_start_dir);
        if (pathlen + 6 < bufflen) {
            p = buff;
            p += xml_strcpy(p, ncxmod_start_dir);
            *p++ = NCXMOD_PSCHAR;
            p += xml_strcpy(p, (const xmlChar *)"data");
            *p++ = NCXMOD_PSCHAR;
            *p = '\0';
    ...
    ncxmod_setup_yumadir (void)
{
    ...
    status_t res = NO_ERR;
    DIR *dp = opendir((const char *)ncxmod_yumadir_path);
    if (dp == NULL) {
        /* create a yuma directory with 700 permissions */
        int retcode = mkdir((const char *)ncxmod_yumadir_path, S_IRWXU);
        if (retcode != 0) {
            res = errno_to_status();
        }
    } else {
...
/********************************************************************
* FUNCTION ncxmod_setup_backup_dir
*
...
*********************************************************************/
status_t
    ncxmod_setup_backup_dir (const xmlChar *backup_dir)
{
    m__free(ncxmod_backup_dir);
    status_t res = NO_ERR;
    ncxmod_backup_dir = ncx_get_source(backup_dir, &res);
    if (ncxmod_backup_dir && res == NO_ERR) {
        /* try to open backup directory */
        DIR *dp = opendir((const char *)ncxmod_backup_dir);
        if (dp == NULL) {
            /* create a backups directory with 700 permissions */
            int retcode = mkdir((const char *)ncxmod_backup_dir, S_IRWXU);
            if (retcode != 0) {
                res = errno_to_status();
            }
        } else {
            /* use the existing backup directory */
            (void)closedir(dp);
        }
    }

    if (res != NO_ERR) {
        log_error("\nError: Could not setup backups directory (%s)\n",
                  get_error_string(res));
    }
...
*
* Setup the ~/.yumapro/tmp directory if it does not exist
*
* RETURNS:
*   status
*********************************************************************/
    ...
    ncxmod_setup_tempdir (void)
{
    ...
    if (dp == NULL) {
        ...
        /* use the existing ~/.yumapro/tmp directory */
        (void)closedir(dp);
    }
...
/********************************************************************
* FUNCTION ncxmod_setup_recordtest_dir
*
...
*********************************************************************/
status_t
   ncxmod_setup_recordtest_dir (void)

{
    const xmlChar *tmpdir = NCXMOD_UT_RECORDTEST_DIR;
    xmlChar       *recordtest_dir_path, *str;
    DIR           *dp;
    status_t       res;
    uint32         len;
    int            retcode;
    ...
    /* make the string to use for the yuma temp directory */
    len = xml_strlen(ncxmod_yumadir_path) + xml_strlen(tmpdir) + 1;
    recordtest_dir_path = m__getMem(len);
    if (recordtest_dir_path == NULL) {
        return ERR_INTERNAL_MEM;
    }
    str = recordtest_dir_path;
    str += xml_strcpy(str, ncxmod_yumadir_path);
    xml_strcpy(str, tmpdir);

    /* try to open yuma temp directory */
    dp = opendir((const char *)recordtest_dir_path);
    if (dp == NULL) {
        /* create a yuma temp directory with 700 permissions */
        retcode = mkdir((const char *)recordtest_dir_path, S_IRWXU);
        if (retcode != 0) {
            res = errno_to_status();
        }
    } else {
        /* use the existing ~/.yumapro/recordtest directory */
        (void)closedir(dp);
    }

    m__free(recordtest_dir_path);
    ...
    ncxmod_new_program_tempdir (status_t *res)
{
    ...
    *p++ = NCXMOD_PSCHAR;
    ...
    /* try to open ~/.yumapro/tmp/<progstr> directory */
    dp = opendir((const char *)tempdir_path);
    if (dp == NULL) {
                                ...
                                status_t *res)
{
    ...
    *p++ = NCXMOD_PSCHAR;
     ...
     * try to open ~/.yumapro/tmp/<progstr>/<sidnum> directory */
...
/********************************************************************
* FUNCTION ncxmod_get_file_mtime
*
...
*********************************************************************/
status_t
    ncxmod_get_file_mtime (const xmlChar *filespec,
                           time_t *timebuff)
{
#ifdef DEBUG
    if (filespec == NULL || timebuff == NULL) {
        return SET_ERROR(ERR_INTERNAL_PTR);
    }
#endif
    ...
    memset(&statbuf, 0x0, sizeof(statbuf));
    int ret = stat((const char *)filespec, &statbuf);
    if (ret != 0 || !S_ISREG(statbuf.st_mode)) {
        return ERR_NCX_NOT_FOUND;
    }

    *timebuff = statbuf.st_mtime;
    return NO_ERR;
...
/********************************************************************
* FUNCTION ncxmod_get_backup_files
*
* Get all the available backup files found in the
* specified backup directory if it exist;
...
*********************************************************************/
status_t
    ncxmod_get_backup_files (ncxmod_backup_cbfn_t cbfn,
                             void *cookie)
{
#ifdef DEBUG
    if (cbfn == NULL) {
    ...
    xml_strcpy(buff, ncxmod_backup_dir);
    status_t res = get_subdirs(buff, bufflen, cbfn, cookie, FALSE);
