                    ...
                    int sqlError)
{
	...
	log_error_append("\n   %s:%d",
			 filename, linenum);
	log_error_append("\n   Error %d: %s",
			 status, get_error_string(status));
	log_backtrace(LOG_DEBUG_ERROR, "\n");
	log_error_append("\n");
...
#ifdef ASSERT_ON_SET_ERROR
    assert(0 && "set_error hit!");
#endif
    ...
    get_error_string (status_t res)
{
    ...
    case ERR_NCX_UNENDED_BLOCK:
        return "unended config line block";
    case ERR_NCX_EVAL_VERSION:
        return "not supported in evaluation version";
    case ERR_NCX_RESOURCE_INSTANCE_UNKNOWN:
        return "unknown resource instance";
    case ERR_NCX_UNEXPECTED_INPUT:
        return "input data not expected";
    case ERR_NCX_METHOD_NOT_ALLOWED:
        return "method not allowed";
    case ERR_NCX_QPARAM_NOT_ALLOWED:
        return "query parameter not allowed for method";
    case ERR_NCX_PRECONDITION_FAILED:
        return "edit pre-condition failed";
