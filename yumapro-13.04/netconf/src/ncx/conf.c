               ...
               val_value_t *val)
{
    ...
    const xmlChar *valname = obj_get_name(obj);
    ...
    if (typ_is_simple(btyp)) {
        ...
        res = val_set_simval(val, typdef, nsid, valname, useval);
        if (res != NO_ERR) {
        ...
        while (!done && res==NO_ERR) {
             ...
             */
            tk_type_t nextyp = tk_next_typ(tkc);
            if (nextyp == TK_TT_NEWLINE) {
                /* skip the NEWLINE token */
                (void)adv_tk(tkc);
            } else if ((block_mode && nextyp == TK_TT_RBRACK) ||
                       (!block_mode && nextyp == TK_TT_RBRACE)) {
                ...
                if (block_mode) {
                    /* expecting this to be a raw line */
                    if (TK_CUR_TYP(tkc) == TK_TT_LINE) {
                        val_value_t *lineval = 
                            val_make_string(val_get_nsid(val),
                                            NCX_EL_STRING,
                                            TK_CUR_VAL(tkc));
                        if (!lineval) {
                            ...
                            val_add_child(lineval, val);
                        }
                    } else {
                        /* wrong token type found */
                        res = ERR_NCX_WRONG_TKTYPE;
                        ncx_conf_exp_err(tkc, res, "raw line");
                    }
                } else {
                    ...
                    if (TK_CUR_ID(tkc)) {
                        ...
                        obj_template_t *chobj = 
                            obj_find_child(obj, TK_CUR_MOD(tkc),
                                           TK_CUR_VAL(tkc));
                        if (chobj) {
                            val_value_t *chval = val_new_value();
                            if (!chval) {
                                res = ERR_INTERNAL_MEM;
                                ncx_print_errormsg(tkc, NULL, res);
                            } else {
        ...
        /* expecting a right brace or bracket to finish the complex value */
        if (res == NO_ERR) {
            if (block_mode) {
                res = consume_tk(tkc, TK_TT_RBRACK);
            } else {
                res = consume_tk(tkc, TK_TT_RBRACE);
            }
            if (res != NO_ERR) {
                if (block_mode) {
                    ncx_conf_exp_err(tkc, res, "right bracket");
                }else {
                ...
                boolean keepvals)
{
    ...
    const xmlChar *curmod = TK_CUR_MOD(tkc);
    const xmlChar *curval = TK_CUR_VAL(tkc);
     ...
     */
    if (curmod) {
        modname = xmlns_get_module(xmlns_find_ns_by_prefix(curmod));
        if (modname) {
            curparm = val_find_child(val, modname, curval);
        }
    }  else {
        curparm = val_find_child(val, val_get_mod_name(val), curval);
    }
        
    if (curparm) {
        obj = curparm->obj;
    } else {
        obj = obj_find_child(val->obj, curmod, curval);
    }
    ...
    if (!obj) {
        ...
        if (curmod) {
            ...
            log_error("\nError: parameter '%s' not found", curval);
        }
        ncx_conf_exp_err(tkc, res, "parameter name");
        return res;
    }
               ...
               boolean keepvals)
{
    ...
    while (!done) {
        const xmlChar *name = obj_get_name(val->obj);
        res = match_name(tkc, name);
        if (res == ERR_NCX_EOF) {
            if (LOGDEBUG) {
                                  ...
                                  boolean fileerr)
{
    ...
    const xmlChar *banner = (const xmlChar *)"CLI parameters";
    return conf_parse_val_from_filespec_ex(filespec, banner,
                                           val, keepvals, 
                                           fileerr, &fileopened);
...
/********************************************************************
* FUNCTION conf_parse_from_filespec_ex
* 
* Parse a file as an NCX text config file against
* a specific parmset definition.  Fill in an
* initialized parmset (and could be partially filled in)
...
*   filespec == absolute path or relative path
*               This string is used as-is without adjustment.
*   banner object == string to display in debug-stmt about
...
*   fileerr == TRUE to generate a missing file error
*              FALSE to return NO_ERR instead, if file not found
*   fileopened == address of return file opened status
*
* OUTPUTS:
...
*********************************************************************/
status_t 
    conf_parse_val_from_filespec_ex (const xmlChar *filespec,
...
{
#ifdef DEBUG
    if (!filespec || !banner_object || !val || !fileopened) {
    ...
    *fileopened = FALSE;
    ...
    status_t res = NO_ERR;
    xmlChar *sourcespec = ncx_get_source(filespec, &res);
    if (!sourcespec) {
        return res;
    }

    log_debug("\nAttempting to open .conf file: '%s'",
              sourcespec);

    FILE *fp = fopen((const char *)sourcespec, "r");
    if (!fp) {
        m__free(sourcespec);
        if (fileerr) {
    ...
    *fileopened = TRUE;
    log_debug("\nLoading %s from '%s'", banner_object, sourcespec);
