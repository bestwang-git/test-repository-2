                       ...
                       uint32 *cnt)
{
    *cnt = 0;
    ...
    const xmlChar *startstr = urlpath+1;
    ...
    while (!done) {
        ...
        if (expectnode) {
            ...
            outlen = 0;
            res = ncx_decode_url_string(startstr, inlen, usebuff, &outlen);
            if (res == NO_ERR) {
                ...
                if (targobj == NULL) {
                    ...
                    targobj = ncx_match_any_object(usebuff, match_names,
                                                   alt_naming, &res);
                } else {
            ...
            if (targobj->objtype == OBJ_TYP_RPC) {
                targobj = obj_find_child(targobj, NULL, YANG_K_INPUT);
                if (targobj == NULL) {
            ...
            if (withkeys) {
                ...
                if (objkey != NULL) {
                    expectnode = FALSE;
                } else if (obj_is_leaf_list(targobj)) {
                    /* this is a leaf-list expecting the value to follow */
                    expectnode = FALSE;
                    leaflist_mode = TRUE;
                }
            }
            ...
            const xmlChar *namestr = NULL;
            if (leaflist_mode) {
                namestr = obj_get_name(targobj);
            } else {
                namestr = obj_get_name(objkey->keyobj);
            }
            ...
            if (wildcards && startstr[0] == XP_URL_ESC_WILDCARD &&
                startstr[1] == '/' && startstr[2] != '/') {

                /* skip this key in the output */
                p = &startstr[1];
            } else {
                const xmlChar *out_namestr =
                    (leaflist_mode) ? (const xmlChar *)"." : namestr;

                /* treat whatever string that is present as a key content
                 * account for [foo='val'] predicate first the [foo=' part */
                outtotal += (1 + xml_strlen(out_namestr) + 2);
                if (writeptr != NULL) {
                    *writeptr++ = '[';
                    writeptr += xml_strcpy(writeptr, out_namestr);
                    *writeptr++ = '=';
                    *writeptr++ = '\'';
                }
                ...
                inlen = 0;
                outlen = 0;
                res = decode_url_esc_string(startstr, writeptr,
                                            &inlen, &outlen);
                if (res != NO_ERR) {
                    return res;
                }
            ...
            /* this should not happen for leaf-lists or the extra node
             * will cause an error on the next loop    */
            if (expectnode && *p == '/') {
                outtotal++;
                if (writeptr != NULL) {
                    *writeptr++ = '/';
                           ...
                           xpath_pcb_t **retpcb)
{
    ...
    *retpcb = NULL;
    ...
    pcb->result = xpath1_eval_expr(pcb, startval, root, logerrors, 
                                   configonly, &res);
...
xpath_pcb_t *
    xpath_new_pcb_ex (const xmlChar *xpathstr,
                      ...
                      void *cookie)
{
    xpath_pcb_t *pcb = m__getObj(xpath_pcb_t);
    if (!pcb) {
        return NULL;
    }
    ...
    xpath_clone_pcb (const xpath_pcb_t *srcpcb)
{
    ...
    ncx_set_error(&newpcb->tkerr, srcpcb->tkerr.mod,
                  srcpcb->tkerr.linenum, srcpcb->tkerr.linepos);
    ...
    /* backptrs_mode not copied */
    /* backptrs_flags not copied */
    ...
    /*** FIXME: does the varbindQ need to be cloned?  ***/
...
void xpath_free_pcb (xpath_pcb_t *pcb)
{
    ...
    while (!dlq_empty(&pcb->result_cacheQ)) {
        xpath_result_t *result = (xpath_result_t *)
            dlq_deque(&pcb->result_cacheQ);
        xpath_free_result(result);
    }

    while (!dlq_empty(&pcb->resnode_cacheQ)) {
        xpath_resnode_t *resnode = (xpath_resnode_t *)
                       ...
                       tk_type_t  tktype)
{
    ...
    if (TK_CUR_TYP(pcb->tkc) != tktype) {
        ...
        if (pcb->logerrors) {
            ncx_mod_exp_err(pcb->tkc, pcb->tkerr.mod, res,
                      ...
                      ncx_num_t *num)
{
    ...
    case XP_RT_NODESET:
        if (dlq_empty(&result->r.nodeQ)) {
            ...
            if (result->isval) {
                const xpath_resnode_t *resnode = (const xpath_resnode_t *)
                    dlq_firstEntry(&result->r.nodeQ);
                val_value_t *val = val_get_first_leaf(resnode->node.valptr);
                if (val && typ_is_number(val->btyp)) {
                    res = ncx_cast_num(&val->v.num, val->btyp, num,
                                       NCX_BT_FLOAT64);
                    if (res != NO_ERR) {
                        ncx_set_num_nan(num, NCX_BT_FLOAT64);
                    }
                    ...
                    res = ncx_convert_num(result->r.str, NCX_NF_NONE,
    ...
    case XP_RT_STRING:
        if (result->r.str) {
            ncx_numfmt_t numformat = ncx_get_numfmt(result->r.str);
            switch (numformat) {
            case NCX_NF_DEC:
            case NCX_NF_REAL:
                break;
            ...
            if (res == NO_ERR) {
                ncx_init_num(&testnum);
                res = ncx_convert_num(result->r.str, numformat,
                                      NCX_BT_FLOAT64, &testnum);
                if (res == NO_ERR) {
                    (void)ncx_copy_num(&testnum, num, NCX_BT_FLOAT64);
                } else {
                      ...
                      xmlChar **str)
{
    ...
    *str = NULL;
    ...
    case XP_RT_NODESET:
        ...
        break;
    case XP_RT_NUMBER:
        res = ncx_sprintf_num(NULL, &result->r.num, NCX_BT_FLOAT64, &len);
        if (res != NO_ERR) {
            return res;
        }
...
{
    assert(resnode && "resnode is NULL!");
...
{
    assert(srcresult && "srcresult is NULL!");
    assert(destresult && "destresult is NULL!");
    ...
    dlq_block_enque(&srcresult->r.nodeQ, &destresult->r.nodeQ);
                                 ...
                                 val_value_t *oldval)
{
    ...
    for (resnode = xpath_get_first_resnode(result);
         ...
         */
        val_value_t *valptr = resnode->node.valptr;
        boolean done = FALSE;
        while (!done) {
            if (valptr == oldval) {
                dlq_remove(resnode);
...
xmlChar *
    xpath_convert_url_to_path (const xmlChar *urlpath,
                               ...
                               status_t *res)
{
    assert(urlpath && "urlpath is NULL!");
    assert(res && "res is NULL!");

    xmlChar *buff = NULL;
    uint32   cnt = 0;
    *res = fill_xpath_string(urlpath, NULL, match_names, alt_naming, 
                             wildcards, withkeys, &cnt);
    if (*res == NO_ERR) {
        ...
        *res = fill_xpath_string(urlpath, buff, match_names, alt_naming,
                                 wildcards, withkeys, &cnt);
        if (*res != NO_ERR) {
            m__free(buff);
            buff = NULL;
...
/********************************************************************
* FUNCTION xpath_set_backptrs_dirty
* 
...
*********************************************************************/
void
    xpath_set_backptrs_dirty (xpath_pcb_t *pcb,
                              ncx_cfg_t cfg_id,
                              boolean dirty)
{
    assert(pcb && "pcb is NULL!");
    ...
    case NCX_CFGID_RUNNING:
        bit = XP_FL_BACKPTR_RDIRTY;
        break;
    case NCX_CFGID_CANDIDATE:
        bit = XP_FL_BACKPTR_CDIRTY;
        break;
    case NCX_CFGID_STARTUP:
        bit = XP_FL_BACKPTR_SDIRTY;
        break;
    default:
        SET_ERROR(ERR_INTERNAL_VAL);
        return;
...
/********************************************************************
* FUNCTION xpath_check_backptrs_dirty
* 
...
*********************************************************************/
boolean
    xpath_check_backptrs_dirty (xpath_pcb_t *pcb,
                                ncx_cfg_t cfg_id)

{
    ...
    return (pcb->backptrs_flags & bit) ? TRUE : FALSE;

}  /* xpath_check_backptrs_dirty */


/********************************************************************
* FUNCTION xpath_check_manual_clear
* 
* Check the backptrs_dirty flag in the PCB
* is cleared manual only
*
* INPUTS:
...
*********************************************************************/
boolean
    xpath_check_manual_clear (xpath_pcb_t *pcb)
{
    assert(pcb && "pcb is NULL!");
    return pcb->backptrs_manual_clear;

}  /* xpath_check_manual_clear */


/********************************************************************
* FUNCTION xpath_set_manual_clear
* 
...
*********************************************************************/
void
    xpath_set_manual_clear (xpath_pcb_t *pcb)
{
    assert(pcb && "pcb is NULL!");
    pcb->backptrs_manual_clear = TRUE;

}  /* xpath_set_manual_clear */


/********************************************************************
* FUNCTION xpath_add_to_result
* 
...
*********************************************************************/
status_t
    xpath_add_to_result (xpath_result_t *source,
                         xpath_result_t *target)
{
    assert(source && "source is NULL!");
    assert(target && "target is NULL!");

    if (source->restype != XP_RT_NODESET ||
        target->restype != XP_RT_NODESET) {
        return ERR_NCX_XPATH_NOT_NODESET;
    }

    dlq_block_enque(&source->r.nodeQ, &target->r.nodeQ);
    return NO_ERR;
...
*********************************************************************/
uint32
    xpath_resnode_count (xpath_result_t *result)
{
    assert(result && "result is NULL!");
    ...
    return dlq_count(&result->r.nodeQ);

}  /* xpath_resnode_count */


/********************************************************************
* FUNCTION xpath_discard_resnode
* 
* Get the number of resnodes in the node-set
*
* INPUTS:
...
*********************************************************************/
void
    xpath_discard_resnode (xpath_resnode_t *resnode)
{
    assert(resnode);
