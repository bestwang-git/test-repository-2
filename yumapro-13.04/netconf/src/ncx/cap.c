...
/********************************************************************
* FUNCTION make_mod_urn_nc
*
* Construct and malloc a module capability URN string for ietf-netconf
*/
static xmlChar *
    make_mod_urn_nc (void)
{
    const xmlChar *modname = NCXMOD_IETF_NETCONF;
    const xmlChar *revision = NCXMOD_IETF_NETCONF_REV;
    const xmlChar *namespace = NC_URN;

    /* get the length of the string needed by doing a dry run */
    uint32 len = xml_strlen(namespace);
    len++;   /* '?' char */
...
#if 0  // TEMP: leave features out right now!!!
    len++;   /* & char */
    len += xml_strlen(CAP_FEATURES_EQ);
    len += (feature_count-1);   /* all the commas */
    /* add in all the enabled features length */
    ...
    make_mod_urn (ncx_module_t *mod)
{
    ...
    return make_mod_urn_ex(mod->name, mod->version, mod->ns, mod);
...
status_t
    cap_add_stdval (val_value_t *caplist,
                    cap_stdid_t   capstd)
{
    assert(caplist && "caplist is NULL!");
    assert(capstd < CAP_STDID_LAST_MARKER);

    const xmlChar *pfix = NULL;
    const xmlChar *cap = NULL;
    uint32 len = 0;
    ...
    val_value_t *capval =
        xml_val_new_leaflist_string(NCX_EL_CAPABILITY, xmlns_nc_id(), str);
    if (!capval) {
        m__free(str);
        return ERR_INTERNAL_MEM;
    }
                        ...
                        const xmlChar *uri)
{
    ...
    uint32 caplen = xml_strlen(CAP_URN);
    ...
    for (stdid=CAP_STDID_WRITE_RUNNING;
        ...
        case CAP_STDID_URL:
            ...
            if (!xml_strncmp(str, stdcaps[stdid].cap_name, namelen)) {
                str += namelen;
                if (*str == (xmlChar)'?') {
                    str++;

                    /* first check standard scheme= string */
                    uint32 schemelen = xml_strlen(CAP_SCHEME_EQ);
                    if (!xml_strncmp(str, CAP_SCHEME_EQ, schemelen)) {
                        str += schemelen;
                        if (*str) {
                        ...
                        if (!xml_strncmp(str, CAP_PROTOCOL_EQ, schemelen)) {
                            str += schemelen;
                            if (*str) {
                                return cap_add_url(caplist, str);
                            }
        ...
        case CAP_STDID_WITH_DEFAULTS:
            ...
            if (!xml_strncmp(str, stdcaps[stdid].cap_name,
                ...
                if (*str == (xmlChar)'?') {
                    str++;
                    uint32 basiclen = xml_strlen(CAP_BASIC_EQ);
                    if (!xml_strncmp(str, CAP_BASIC_EQ, basiclen)) {
                        str += basiclen;
                        if (*str) {
                           ...
                           const xmlChar *uri)
{
    ...
    uint32 baselen = (uint32)(qmark-uri);

    status_t res = NO_ERR;
    const xmlChar *module = NULL;
    const xmlChar *revision = NULL;
    const xmlChar *features = NULL;
    const xmlChar *deviations = NULL;

    uint32 modulelen = 0;
    uint32 revisionlen = 0;
    uint32 deviationslen = 0;
    uint32 featureslen = 0;
    boolean curmod = FALSE;
    boolean currev = FALSE;
    boolean curfeat = FALSE;
    boolean curdev = FALSE;
    uint32 parmnamelen = 0;
    uint32 parmvallen = 0;
    boolean usewarning = ncx_warning_enabled(ERR_NCX_RCV_INVALID_MODCAP);

    /* lookup this namespace to see if it is already loaded */
    xmlns_id_t foundnsid = xmlns_find_ns_by_name_str(uri, baselen);

    /* setup the start of the parameter name for each loop iteration */
    const xmlChar *parmname = qmark + 1;
    const xmlChar *parmval = NULL;
    const xmlChar *nextparmname = parmname;
    xmlChar *liststr = NULL;
    xmlChar *commastr = NULL;
    ...
    while (!done) {
        ...
        res = parse_uri_parm(parmname, &parmnamelen, &parmval, 
                             &parmvallen, &nextparmname);
        if (res != NO_ERR) {
            if (usewarning) {
                log_warn("\nWarning: skipping invalid "
                         "parameter syntax (%s) "
                         "in capability URI '%s'", 
                         get_error_string(res), uri);
            }
            if (NEED_EXIT(res)) {
                done = TRUE;
            }
        ...
        if (!xml_strncmp(parmname, YANG_K_MODULE, 
    ...
    if (foundnsid) {
        ...
        if (parmname && xml_strcmp(parmname, NCXMOD_YUMA_NETCONF) &&
            cap->cap_module != NULL && xml_strcmp(parmname, cap->cap_module)) {
            if (usewarning) {
                log_warn("\nWarning: capability base URI mismatch, "
                         "got '%s' not '%s''", cap->cap_module,  parmname);
            }
        }
    }
                 ...
                 const xmlChar *scheme_list)
{
    ...
    if (caplist->cap_schemes) {
        log_info("\nGot extra :url capability, overwriting '%s'",
                 caplist->cap_schemes);
        m__free(caplist->cap_schemes);
    }
                    ...
                    const xmlChar *scheme_list)
{
     ...
     */
    const xmlChar *pfix = CAP_URN;
    const xmlChar *scheme = CAP_SCHEME_EQ;
    const xmlChar *cap = stdcaps[CAP_STDID_URL].cap_name;

    /* get the total length */
    uint32 len = xml_strlen(pfix) + xml_strlen(cap) + 1 +
        xml_strlen(scheme) + xml_strlen(scheme_list) + 1;
                     ...
                     const xmlChar *defstyle)
{
    ...
    const xmlChar *str = defstyle;
    while (*str && *str != '&') {
        str++;
    }
    if (*str) {
        uint32 featureslen = xml_strlen(CAP_SUPPORTED_EQ);
        uint32 basiclen = (uint32)(str - defstyle);
        if (!xml_strncmp(++str, CAP_SUPPORTED_EQ, featureslen)) {
            str += featureslen;
            caplist->cap_supported = xml_strdup(str);
            if (!caplist->cap_supported) {
                return ERR_INTERNAL_MEM;
            }
            caplist->cap_defstyle = xml_strndup(defstyle, basiclen);
            if (!caplist->cap_defstyle) {
                return ERR_INTERNAL_MEM;
            }
        }
...
#define SUPPORTED_BUFFSIZE 64
     ...
     */
    const xmlChar *pfix = CAP_URN;
    const xmlChar *basic = CAP_BASIC_EQ;
    const xmlChar *supported = CAP_SUPPORTED_EQ;
    const xmlChar *cap = stdcaps[CAP_STDID_WITH_DEFAULTS].cap_name;
    xmlChar buffer[SUPPORTED_BUFFSIZE];

    /* figure out what the also-supported parameter value
     * should be and fill the buffer
     */
    ncx_withdefaults_t withdef = ncx_get_withdefaults_enum(defstyle);
                 ...
                 ncx_module_t *mod)
{
    ...
    /* make the cap struct -- reuse function for yangcli for now
     * construct the module URN string then deconstruct it into
     * a cap_rec_t strcut                 */
    xmlChar *str = make_mod_urn(mod);
    if (!str) {
        return ERR_INTERNAL_MEM;
    }
    status_t res = cap_add_module_string(caplist, str);
...
/********************************************************************
* FUNCTION cap_add_netconf_modval
*
* Add a module capability to the list (val_value_t version)
*
* INPUTS:
...
*********************************************************************/
status_t 
    cap_add_netconf_modval (val_value_t *caplist)
{
#ifdef DEBUG
    if (!caplist) {
        return SET_ERROR(ERR_INTERNAL_PTR);
                       ...
                       ncx_save_deviations_t *savedev)
{
    ...
    /* construct the module URN string */
    xmlChar *str = make_devmod_urn(savedev);
    if (!str) {
        return ERR_INTERNAL_MEM;
    }
             ...
             const xmlChar *capuri) 
{
    ...
    uint32 capurilen = xml_strlen(capuri);
    ...
    uint32 len = xml_strlen(CAP_URN);
    if ((capurilen > len+1) && 
        !xml_strncmp(capuri, (const xmlChar *)CAP_URN, len)) {

        /* set str to the 'capability-name:version-number' */
        const xmlChar *str = &capuri[len];    

        /* check all the capability names */
        int i = 1;
        for (; i < CAP_STDID_LAST_MARKER; i++) {
...
*********************************************************************/
const cap_rec_t *
    cap_find_modcap (const cap_list_t *caplist,
                     const xmlChar *modname)
{
    ...
    for (cap = (cap_rec_t *)dlq_firstEntry(&caplist->capQ);
        ...
        if (cap->cap_module && !xml_strcmp(cap->cap_module, modname)) {
            return cap;
        }
    }
    return NULL;
