...
/********************************************************************
* FUNCTION inc_cur_depth
*
* Increment the cur_depth parameter if needed
*
* INPUTS:
*   mhdr == message header to check
*********************************************************************/
static void
    inc_cur_depth (xml_msg_hdr_t *mhdr)
{
    uint32 cur_depth = xml_msg_get_cur_depth(mhdr);
#ifdef XML_WR_DEBUG
    if (LOGDEBUG3) {
        log_debug3("\nxml_wr: inc cur %u to %u", cur_depth, cur_depth+1);
    }
#endif
    xml_msg_set_cur_depth(mhdr, cur_depth+1);
}


/********************************************************************
* FUNCTION dec_cur_depth
*
...
*********************************************************************/
static void
    dec_cur_depth (xml_msg_hdr_t *mhdr)
{
    uint32 cur_depth = xml_msg_get_cur_depth(mhdr);
    if (cur_depth) {
#ifdef XML_WR_DEBUG
        if (LOGDEBUG3) {
            log_debug3("\nxml_wr: dec cur %u to %u", cur_depth, cur_depth-1);
        }
#endif
        xml_msg_set_cur_depth(mhdr, cur_depth-1);
    } else {
...
*********************************************************************/
static boolean
    test_max_depth (xml_msg_hdr_t *mhdr)
{
    uint32 max_depth = xml_msg_get_max_depth(mhdr);
    if (max_depth) {
        uint32 cur_depth = xml_msg_get_cur_depth(mhdr);
        if (cur_depth >= max_depth) {
#ifdef XML_WR_DEBUG
            if (LOGDEBUG3) {
                log_debug3("\nwxml_wr: test max TRUE");
            }
#endif
            return TRUE;
        }
                ...
                int32 indent)
{
    if (indent >= 0) {
        ...
        if (scb->msg_mode) {
            indent_amount = ses_message_indent_count(scb);
        } else {
            indent_amount = ses_indent_count(scb);
        }
        if (indent_amount >= 0) {
                 ...
                 const val_value_t *val)
{
    ...
    return val_fit_oneline(val, SES_LINESIZE(scb), TRUE);
                      ...
                      uint32 list_size)
{
    ...
    const xmlChar *val = xmlns_get_ns_name(nsid);
    if (!val) {
        SET_ERROR(ERR_INTERNAL_VAL);
        return;
    }
    ...
    uint32 idx = 0;
    if (nsid_list) {
        while (idx < list_size && nsid_list[idx]) {
            if (nsid_list[idx] == nsid) {
                log_debug3("\nxml_wr: skip xmlns duplicate (%u=%s)",
                           nsid, val);
                return;
            }
            idx++;
                            ...
                            uint32 *retcount)
{
    ...
    xmlns_id_t xmlns_list[MAX_XMLNS_LIST];
    xmlns_list[0] = 0;
    ...
    *retcount = 0;
    ...
    for (i = 0; i < num_nsids; i++) {
         ...
         * xmlns attribute
         */
        write_xmlns_decl(scb, defpfix, cur_nsid, indent, xmlns_list,
                         MAX_XMLNS_LIST);
    }
...
{
    xmlns_id_t ns_id = xmlns_ns_id();

    xmlns_id_t xmlns_list[MAX_XMLNS_LIST];
    xmlns_list[0] = 0;

    dlq_hdr_t *hdr = dlq_firstEntry(attrQ); 
    for (; hdr != NULL; hdr = dlq_nextEntry(hdr)) {

        const xml_attr_t *attr = NULL;
        val_value_t *val = NULL;
        xmlns_id_t attr_nsid;
        ...
        if (XMLNS_EQ(attr_nsid, ns_id)) {
            ...
            pfix = xml_msg_get_prefix(msg, elem_nsid, attr_nsid,
                                      curelem, &xneeded);
            if (xneeded) {
                write_xmlns_decl(scb, pfix, attr_nsid, indent,
                                 xmlns_list, MAX_XMLNS_LIST);
            }
            ...
            if (val->xpathpcb) {
                ...
                if (xneeded) {
                    write_xmlns_decl(scb, pfix, VAL_IDREF_NSID(val), 
                                     indent, xmlns_list, MAX_XMLNS_LIST);
                }
            }
        }
        ...
        if (isattrq) {
        ...
        } else if (val->btyp == NCX_BT_ENUM) {
            ses_putastr(scb, VAL_ENUM_NAME(val), -1);
        } else if (typ_is_number(val->btyp)) {
            uint32 numbufflen = NCX_MAX_NUMLEN+1;
            xmlChar numbuff[NCX_MAX_NUMLEN+1];
            res = val_sprintf_simval_nc(numbuff, val, &numbufflen);
            if (res != NO_ERR) {
                SET_ERROR(res);
            } else {
                ses_putastr(scb, numbuff, -1);
            }
        } else {
            ...
            if (res != NO_ERR) {
                ...
                xmlChar *buffer = m__getMem(bufferlen+1);
                if (buffer == NULL) {
...
*           == -1 means no newline or indent
*           == 0 means just newline
*   force_xmlns == TRUE for force an xmlns directove for the nsid
*
* RETURNS:
*   none
*********************************************************************/
static void
    begin_elem_val (ses_cb_t *scb,
                    ...
                    boolean force_xmlns)
{
    xmlns_id_t nsid = val->nsid;
    const xpath_pcb_t *xpathpcb = NULL;
    ...
    if (val->parent) {
    ...
    } else if (!XML_MSG_USE_PREFIX(msg) && (val->nsid == xmlns_nc_id())) {
        ...
        if (scb->protocol == NCX_PROTO_YUMA_YANGAPI) {
            parent_nsid = xmlns_nc_id();  // FIXME!!!
        } else {
            parent_nsid = xmlns_nc_id();
        }
    }

    boolean xmlcontent = (val->btyp == NCX_BT_INSTANCE_ID) ||
    ...
    boolean xneeded = FALSE;
    const xmlChar *pfix =
        xml_msg_get_prefix(msg, parent_nsid, nsid, val, &xneeded);
    if (pfix && XML_MSG_USE_PREFIX(msg)) {
        ses_putstr(scb, pfix);
        ses_putchar(scb, ':');
    }

    /* write the element name */
    ses_putstr(scb, val->name);
    ...
    const dlq_hdr_t *attrQ = &val->metaQ;
    if (xneeded || xmlcontent || (attrQ && !dlq_empty(attrQ))) {


        indent = add_indent(scb, indent);
        if (attrQ) {
        ...
        if (xneeded) {
            ...
            write_xmlns_decl(scb, (XML_MSG_USE_PREFIX(msg)) ? pfix : NULL,
                             nsid, indent, NULL, 0);
        }
        ...
        if (xpathpcb) {
            ...
            uint32 retcount = 0;
            status_t res = 
                handle_xpath_start_tag(scb, xpathpcb, indent, &retcount);
    ...
    boolean empty = !val_has_content(val);
    if (empty) {
        ses_putchar(scb, '/');
    }
    ses_putchar(scb, '>');
                             ...
                             val_value_t* out )
{
    ...
    bool xneeded = FALSE;
    const xmlChar *pfix =
        xml_msg_get_prefix(msg, ( (out->parent) ? out->parent->nsid : 0 ),
                                         ...
                                         boolean first )
{
    ...
    uint32 len = xml_strlen_sp( listmem->val.str, &wspace);
                                  ...
                                  int32 indent )
{
    ...
    {
        if ( !first ) {
            if (indent >= 0 && SES_LINELEN(scb) > SES_LINESIZE(scb)) {
                ses_indent(scb, indent);
            } else {
                ses_putchar(scb, ' ');
            }           
                          ...
                          boolean force_xmlns)
{
    ...
    xmlns_id_t out_nsid = val_get_nsid(val);
    ncx_btype_t out_btype = val->btyp;
    val_value_t *out = 
    ...
    /* not tagging default NP containers, just leafs */
    boolean isdefault = FALSE;
    if (typ_is_simple(out_btype)) {
        isdefault = val_is_default(out);
        if (out->obj && !obj_is_key(out->obj)) {
            isdefault = FALSE;
        }
    }
    ...
    if (out_btype == NCX_BT_EMPTY && !VAL_BOOL(out)) {
        /* this is a false (not present) flag */
        ;
    } else if (out_btype == NCX_BT_IDREF) {
        xmlns_id_t parent_nsid =
            (out->parent) ? val_get_nsid(out->parent) : 0;

        /* write a complete QName element */
        xml_wr_qname_elem(scb, msg, out->v.idref.nsid, out->v.idref.name,
                          parent_nsid, out_nsid, out->name,
                          &out->metaQ, FALSE, indent, isdefault);
    } else if (val_has_content(out)) {
        boolean use_empty = test_max_depth(msg);
        boolean do_full = TRUE;
        if (use_empty && !typ_is_simple(out_btype)) {
            do_full = FALSE;
        }

        if (do_full) {
            begin_elem_val(scb, msg, out, indent, force_xmlns);

            int32 new_indent = add_indent(scb, indent);

            /* write the simple value; don't redo ACM check */
            write_check_val(scb, msg, out, new_indent, testfn, FALSE);

            xml_wr_end_elem(scb, msg, out_nsid, out->name, 
                            fit_on_line(scb, out) ? -1 : indent);
        } else {
            xml_wr_empty_elem(scb, msg, val_get_parent_nsid(out),
                              out_nsid, out->name, indent);
        }
    } else {
 ...
 */
static void
    write_child_values(ses_cb_t *scb,
                       xml_msg_hdr_t *msg,
                       val_value_t *out,
                       int32 indent,
                       val_nodetest_fn_t testfn)
{
    ...
    boolean max_depth_mode = FALSE;
    uint32 max_depth = xml_msg_get_max_depth(msg);

    if (max_depth) {
        uint32 cur_depth = xml_msg_get_cur_depth(msg);
    ...
    if (max_depth_mode) {
        ...
        for (chval = val_get_first_child(out); chval; chval = nextval) {

            nextval = val_get_next_child(chval);

            if (typ_is_simple(chval->btyp)) {
                write_full_check_val(scb, msg, chval, indent, testfn, FALSE);
            } else {
                ...
                /*
                xml_wr_empty_elem(scb, msg, val_get_parent_nsid(out),
                                  val_get_nsid(out), out->name, indent);
                */

                /* only generate 1 empty node for lists */
                boolean done = FALSE;
                while (!done && nextval) {
                    if (chval->obj && nextval->obj) {
                        if (chval->obj != nextval->obj) {
                            done = TRUE;
                        }
                    } else {
                        if ((val_get_nsid(chval) != val_get_nsid(nextval)) ||
                            xml_strcmp(chval->name, nextval->name)) {
                            done = TRUE;
                        }
                    }
                    if (!done) {
                        nextval = val_get_next_child(nextval);
                    }
                }
            }
        }
                              ...
                              boolean acmcheck )
{
    ...
    case NCX_BT_BOOLEAN:
         ses_putcstr(scb, (out->v.boo ? NCX_EL_TRUE : NCX_EL_FALSE), indent);
        break;
                  ...
                  int32 indent)
{
    ...
    ses_putchar(scb, '/');
    pfix = NULL;
    if (nsid && XML_MSG_USE_PREFIX(msg)) {
                       ...
                       boolean isdefault)
{
    ...
    if (pfix) {
         ...
         * in the start tag 
         * do not check XML_MSG_USE_PREFIX(msg) here!
         */
        ses_putstr(scb, pfix);
        ses_putchar(scb, XMLNS_SEPCH);
    }
...
*   indent == start indent amount if indent enabled
*   testcb == callback function to use, NULL if not used
*   force_parent_nsid == parent NSID to use, to allow YANG-API
*    to print a data node as the context node and get the
*    xmlns attribute to print
*
* RETURNS:
*   none
*********************************************************************/
void
    xml_wr_max_check_val (ses_cb_t *scb,
                          xml_msg_hdr_t *msg,
                          val_value_t *val,
                          int32  indent,
                          val_nodetest_fn_t testfn,
                          boolean force_xmlns)
{
    ...
    write_full_check_val(scb, msg, val, indent, testfn, force_xmlns);
                           ...
                           val_nodetest_fn_t testfn)
{
    ...
    write_full_check_val(scb, msg, val, indent, testfn, FALSE);
