                           ...
                           size_t len)
{
    ...
    if (LOGDEBUG3 && scb->state != SES_ST_INIT) {
        scb->readbuff[len] = 0;
        log_debug3("\nses: accept buffer (%zu):\n%s\n", 
                   len, scb->readbuff);
    } else if (LOGDEBUG2) {
        log_debug2("\nses: accept buffer (%zu)", len);
    }
#endif

    /* make sure there is a current message */
    status_t res = NO_ERR;
    ses_msg_t *msg = (ses_msg_t *)dlq_lastEntry(&scb->msgQ);
    ...
    /* init local vars */
    const char *endmatch = NC_SSH_END;
    boolean done = FALSE;
    uint32 count = 0;

    /* make sure there is a current buffer to use */
    ses_msg_buff_t *buff = (ses_msg_buff_t *)dlq_lastEntry(&msg->buffQ);
                           ...
                           size_t len)
{
    ...
    if (LOGDEBUG3 && scb->state != SES_ST_INIT) {
        scb->readbuff[len] = 0;
        log_debug3("\nses: accept base:1.1 buffer (%zu):\n%s\n", 
                   len, scb->readbuff);
    } else if (LOGDEBUG2) {
        log_debug2("\nses: accept base:1.1 buffer (%zu)", len);
    }
#endif
...
ses_cb_t *
    ses_new_scb (void)
{
    xmlChar *now = m__getMem(TSTAMP_MIN_SIZE);
    if (now == NULL) {
        return NULL;
    }
    tstamp_datetime(now);

    ses_cb_t *scb = m__getObj(ses_cb_t);
    if (scb == NULL) {
        m__free(now);
        return NULL;
    }
    ...
    scb->msg_indent = ncx_get_message_indent();
    scb->cache_timeout = NCX_DEF_VTIMEOUT;
    scb->warn_xml = TRUE;
    scb->out_encoding = NCX_DEF_ENCODING;

    /* new session expected to use NETCONF10 protocol framing
     * for at least the first message <ncx-connect>
     */
    scb->protocol = NCX_PROTO_NETCONF10;
    return scb;
...
void ses_free_scb (ses_cb_t *scb)
{
    ...
    while (!dlq_empty(&scb->msgQ)) {
        ses_msg_t *msg = (ses_msg_t *)dlq_deque(&scb->msgQ);
                 ...
                 int32 indent)
{
    ...
    while (*str) {
        ...
        case '\\':
            ...
            ses_putchar(scb, '\\');
            break;
       /* note: JSON translators and validators do not encode
        * the forward slash as an escaped char, so this case arm
        * has been removed!!!
        case '/':
            ses_putchar(scb, '\\');
            ses_putchar(scb, '/');
...
/********************************************************************
* FUNCTION ses_message_indent_count
*
...
*********************************************************************/
int32
    ses_message_indent_count (const ses_cb_t *scb)
{
    return scb->msg_indent;
...
void ses_finish_msg (ses_cb_t *scb)
{
    ...
    if (scb->protocol == NCX_PROTO_YUMA_YANGAPI) {
        ;  // nothing to do for YANG-API end of response marker
    } else if (scb->type != SES_TYP_DUMMY &&
        (scb->protocol == NCX_PROTO_NETCONF10 ||
         scb->protocol == NCX_PROTO_NETCONF11 ||
                 ...
                 int len)
{
    ...
    ses_msg_buff_t *buff = msg->curbuff;
    if (buff == NULL) {
    ...
    ses_accept_input (ses_cb_t *scb)
{
    ...
    while (!done) {
        ...
        while (!readdone && res == NO_ERR) {
            ...
            if (scb->rdfn) {
                erragain = FALSE;
                ret = (*scb->rdfn)(scb, (char *)scb->readbuff, 
                                   scb->readbuffsize, &erragain);
            } else {
                ret = read(scb->fd, scb->readbuff, scb->readbuffsize);
            }
        ...
        if (ret == 0) {
            ...
            case NCX_PROTO_NETCONF11:
                ...
                break;
#ifdef WITH_CLI
                /* not used -- this is for raw CLI input
                 * but there is no handler for this input
                 * the yp-shell program uses NETCONF messages
                 */
            ...
            if (res != NO_ERR || 
                ...
                if (LOGDEBUG3) {
                    log_debug3("\nses: bail exit %u:%u (%s)", 
                               ret, scb->readbuffsize, get_error_string(res));
                }
#endif
                done = TRUE;
    ...
    ses_get_transport_name (ses_transport_t transport)
{
    ...
    case SES_TRANSPORT_HTTP:
        return (const xmlChar *)NCX_SERVER_TRANSPORT_HTTP;
    case SES_TRANSPORT_CLI:
        return (const xmlChar *)NCX_SERVER_TRANSPORT_CLI;
    case SES_TRANSPORT_WEBUI:
        return (const xmlChar *)NCX_SERVER_TRANSPORT_WEBUI;
    default:
        SET_ERROR(ERR_INTERNAL_VAL);
        return (const xmlChar *)"none";
    }
                      ...
                      ncx_protocol_t proto)
{
    ...
    if (scb->direct_mode) {
        /* only the yp-shell client will set direct_mode */
        scb->protocol = NCX_PROTO_YUMA_CLI;
...
/********************************************************************
* FUNCTION ses_set_extern_xml_mode
* 
...
*********************************************************************/
void
    ses_set_extern_xml_mode (ses_cb_t *scb)
{
    assert( scb && "scb is NULL!" );
...
/********************************************************************
* FUNCTION ses_clear_extern_xml_mode
* 
...
*********************************************************************/
void
    ses_clear_extern_xml_mode (ses_cb_t *scb)
{
    assert( scb && "scb is NULL!" );

    scb->warn_xml = TRUE;
    scb->keep_xmlns = FALSE;

}  /* ses_clear_extern_xml_mode */


/********************************************************************
* FUNCTION ses_set_out_encoding
* 
*  Set the output encoding
*
* INPUTS:
*    scb == session to check
*    encoding == encoding to est
*********************************************************************/
void
    ses_set_out_encoding (ses_cb_t *scb,
                          ncx_display_mode_t encoding)
{
    assert( scb && "scb is NULL!" );
    scb->out_encoding = encoding;

}  /* ses_set_out_encoding */


/********************************************************************
* FUNCTION ses_get_out_encoding
* 
*  Set the output encoding
*
* INPUTS:
...
*********************************************************************/
ncx_display_mode_t
    ses_get_out_encoding (ses_cb_t *scb)
{
    assert( scb && "scb is NULL!" );
    return scb->out_encoding;

}  /* ses_get_out_encoding */


/********************************************************************
* FUNCTION ses_new_indent_count
*
...
*********************************************************************/
int32
    ses_new_indent_count (boolean indent_in,
                          int32 indent,
                          int32 indent_amount)
{
    if (indent < 0) {
