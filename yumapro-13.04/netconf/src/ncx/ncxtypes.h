...
typedef enum ncx_instfmt_t_ {
    ...
    NCX_IFMT_CLI,       /* CLI syntax used in val_dump_value */
    NCX_IFMT_CLI2       /* CLI syntax used in yangcli config mode */
} ncx_instfmt_t;
...
typedef ncx_transaction_id_t ncx_etag_t;
...
typedef enum ncx_protocol_t_ {
    ...
    NCX_PROTO_NETCONF11,      /* RFC 6241 base:1.1 */
    NCX_PROTO_YUMA_YANGAPI,   /* Yuma YANG-API 1.0 */
    NCX_PROTO_YUMA_CLI        /* Yuma Internal CLI 1.0 */
} ncx_protocol_t;
...
typedef struct ncx_module_t_ {
    ...
    boolean           supported;  /* T: load: F: import only */
...
/* used with various structs to cache back-ptrs */
typedef struct ncx_backptr_t_ {
    dlq_hdr_t    qhdr;
    void        *node;
} ncx_backptr_t;
...
/* user function callback template to traverse all module
 * identities looking for matches for a specified base
 *
 * ncx_identity_cbfn_t
 * 
 *  Run an instrumentation-defined function 
 ...
 */
typedef boolean (*ncx_identity_cbfn_t) (ncx_identity_t *identity,
                                        void *cookie);
