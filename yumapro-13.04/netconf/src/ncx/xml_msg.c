...
{
    assert(msg && "msg is NULL!");
    assert(xneeded && "xneeded is NULL!");
    ...
    *xneeded = FALSE;
    ...
    xmlns_pmap_t *pmap = find_pmap(msg, nsid);
    if (!pmap) {
    ...
    if (!pmap) {
        ...
        *xneeded = TRUE;
    } else if (!XML_MSG_USE_PREFIX(msg) && (parent_nsid != nsid)) {
        /* the prefix needs to be generated again because this
         * is an element or prefixes are not used; If an attribute,
         * then the xmlns attribute may have already been printed
         * since the pmap was found; This will cause invalid XML
         * if same attribute appears twice!
         */
        *xneeded = TRUE;
                                    ...
                                    boolean addncxid )
{
    ...
    if ( addncid ) {
        res = xml_msg_add_ncid_to_prefix_map( msg, attrs, xmlns_nc_id() );
    }

    /* make sure XMLNS decl for NCX is in the map try even if errors in setting
     * up the NETCONF pmap */
    if (addncxid ) {
        status_t retres = xml_msg_add_ncid_to_prefix_map( msg, attrs, 
                                                          xmlns_nc_id() );
...
{
    assert(msg && "msg is NULL!");
    assert(isvalid && "isvalid is NULL!");
                          ...
                          boolean is_output)
{
    assert(msg && "msg is NULL!");
    if (is_output) {
        return msg->output_encoding;
    } else {
        return msg->input_encoding;
    }
    /*NOTREACHED*/
                          ...
                          ncx_display_mode_t encoding)
{
    assert(msg && "msg is NULL!");
    if (is_output) {
        msg->output_encoding = encoding;
    } else {
        msg->input_encoding = encoding;
    }
}  /* xml_msg_set_encoding */
                      ...
                      boolean *test)
{
    ...
    *test = XML_MSG_WITH_ETAGS_TEST(msg);
    return msg->match_etag;
                      ...
                      boolean test_sense)
{
    ...
    XML_MSG_SET_WITH_ETAGS(msg);
    msg->match_etag = etag;
    if (test_sense) {
        XML_MSG_SET_WITH_ETAGS_TEST(msg);
    } else {
        XML_MSG_CLR_WITH_ETAGS_TEST(msg);
    }        
                        ...
                        boolean *test)
{
    ...
    *test = XML_MSG_WITH_TSTAMPS_TEST(msg);
    return msg->match_tstamp;
                        ...
                        boolean test_sense)
{
    ...
    XML_MSG_SET_WITH_TSTAMPS(msg);
    msg->match_tstamp = tstamp;
    if (test_sense) {
        XML_MSG_SET_WITH_TSTAMPS_TEST(msg);
    } else {
        XML_MSG_CLR_WITH_TSTAMPS_TEST(msg);
    }        
