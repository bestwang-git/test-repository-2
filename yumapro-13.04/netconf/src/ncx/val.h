...
/* if set then the VAL_FL_DIRTY flag should also be set;
 * this flags indicates the parent node is dirty because a child
 * was deleted; Otherwise the delete will be interpreted as
 * a 'replace' when the commit from candidate to running is done
 */
...
/* tpe of protocol parameter used for insert operation */
typedef enum val_insert_mode_t_ {
    VAL_INS_MODE_NONE,
    VAL_INS_MODE_KEY,                 /* YANG key attribute */
    VAL_INS_MODE_VALUE,               /* YANG value attribute */
    VAL_INS_MODE_POINT                /* YANG-API point param */
} val_insert_mode_t;
...
typedef struct val_value_t_ {
     ...
     * reference this node
     */
    void  *datarules[VAL_MAX_DATARULES];
