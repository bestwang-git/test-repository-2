...
static mgr_ses_closed_fn_t session_closed_handler;
...
{
#ifdef MGR_IO_DEBUG
    log_debug("\nmgr_io: Enter write_sessions");
#endif

    ses_cb_t *scb = mgr_ses_get_first_outready();
    while (scb) {

#ifdef MGR_IO_DEBUG
        log_debug("\nmgr_io: About to send buffs for scb %p (%d)",
                  scb, SES_MY_SID(scb));
#endif

        status_t res = ses_msg_send_buffs(scb);
        if (res != NO_ERR) {
            if (LOGINFO) {
                mgr_scb_t *mscb = mgr_ses_get_mscb(scb);
                log_info("\nmgr_io write failed; closing session %u (a:%u)", 
                         scb->sid, mscb->agtsid);
            }
                  ...
                  ses_id_t cursid)
{
     ...
     */
    ses_cb_t *scb = def_reg_find_scb(fd);
    if (scb) {
        mgr_scb_t *mscb = mgr_ses_get_mscb(scb);
        if (!mscb) {
            SET_ERROR(ERR_INTERNAL_VAL);
        } else {
            status_t res = ses_accept_input(scb);
            if (res == NO_ERR) {
                ...
                uint32 sid = scb->sid;
                mgr_ses_free_session(scb->sid);
                // invoke call back funtion to yangcli
                if (session_closed_handler != NULL) {
                    (*session_closed_handler)(sid);
                }
...
/********************************************************************
 * FUNCTION mgr_session_close_handler
 *
 * Set the callback function for mgr_session_close_handler
 *
 * INPUTS:
 *   handler == address of the session close handler function
 *********************************************************************/
 void
    mgr_set_ses_closed_handler (mgr_ses_closed_fn_t handler)
 {
    session_closed_handler = handler;

 } /* mgr_session_closed_handler */
...
status_t
    mgr_io_run (void)
{
    mgr_io_state_t state = MGR_IO_ST_INIT;
    ...
    while (!done) {
        ...
        while (!done2) {
             ...
             */
            ret = select(maxrdnum+1, &read_fd_set, &write_fd_set, 
                             NULL, &timeout);
        ...
        if (ret < 0) {
            log_error("\nmgr_io select failed (%s)", strerror(errno));
        ...
        if (LOGDEBUG3) {
            ...
            for (i = 0; i <= maxfd; i++) {
                if (FD_ISSET(i, &active_fd_set)) {
                    log_debug3("\nmgr_io: detect active FD %d", i);
                }
            }
            for (i = 0; i <= maxfd; i++) {
                if (FD_ISSET(i, &read_fd_set)) {
                    log_debug3("\nmgr_io: read detected for FD %d", i);
                }
            }
            log_debug3("\nmgr_io: maxrdnum = %d", maxrdnum);
        }
#endif // MGR_IO_DEBUG
     ...
     * torn down, but the original ncxserver socket needs
     * to be closed now  */
    return NO_ERR;
                            ...
                            boolean *anystdout)
{
    ...
    *anystdout = FALSE;
     ...
     */
    int ret = select(maxrdnum+1, &read_fd_set, &write_fd_set, 
                     NULL, &timeout);
    if (ret > 0) {
        /* normal return with some bytes */
        anyread = TRUE;
