                          ...
                          mgr_rpc_cbfn_t rpyfn)
{
    ...
    xml_attr_t *attr = xml_find_attr_q(&req->attrs, 0, NCX_EL_MESSAGE_ID);
    if (attr) {
        dlq_remove(attr);
        xml_free_attr(attr);
    }
    ...
    status_t res = 
        xml_msg_build_prefix_map(&msg, &req->attrs, FALSE,
                                 (req->data->nsid == xmlns_ncx_id()));

    /* add the message-id attribute */
    if (res == NO_ERR) {
        res = xml_add_attr(&req->attrs, 0, NCX_EL_MESSAGE_ID, req->msg_id);
    }
    ...
    if (res == NO_ERR) {
        ...
        int32 indent = min(ses_message_indent_count(scb), 0);
        ses_start_msg_mode(scb);

        /* start the <rpc> element */
        xml_wr_begin_elem_ex(scb, &msg, 0, nc_id, NCX_EL_RPC, &req->attrs, 
                             ATTRQ, indent, START);
    
        /* send the method and parameters */
        xml_wr_full_val(scb, &msg, req->data, ncx_get_message_indent());

        /* finish the <rpc> element */
        xml_wr_end_elem(scb, &msg, nc_id, NCX_EL_RPC, indent);
                      ...
                      xml_node_t *top)
{
     ...
     */
    rpy->res = mgr_val_parse_reply(scb, rpyobj,
                                   req ? req->rpc : ncx_get_gen_anyxml(),
                                   top, rpy->reply);
    ...
    if (req != NULL) { 
        mgr_rpc_cbfn_t handler = (mgr_rpc_cbfn_t)req->replycb;
        if (handler) {
            (*handler)(scb, req, rpy);
        }
    }
