...
*
* INPUTS:
*     scb == session control block or NULL if N/A
                    ...
                    val_value_t  *retval)
{
    ...
    /* cache the NSIDs of the special attributes */
    xmlns_id_t ncid =  xmlns_nc_id();
    xmlns_id_t yangid =  xmlns_yang_id();
    xmlns_id_t xmlid = xmlns_xml_id();
    xmlns_id_t wdaid = xmlns_wda_id();
    xmlns_id_t ncxid = xmlns_ncx_id();
     ...
     */
    xml_attr_t *attr = xml_get_first_attr(node);
    for (; attr != NULL; attr = xml_next_attr(attr)) {

        boolean keep_xmlns = FALSE;
        if (attr->attr_dname && 
        ...
        status_t res = NO_ERR;
        obj_metadata_t *meta = NULL;
        typ_def_t *metadef = NULL;
        boolean iskey = FALSE;
        ...
        if (keep_xmlns) {
            ;
        } else if (val_match_metaval(attr, ncid, NC_OPERATION_ATTR_NAME)) {
            ...
            continue;
        } else if (val_match_metaval(attr, ncxid, NCX_EL_LAST_MODIFIED)) {
            islastmodified = TRUE;
        } else {
                 ...
                 val_value_t  *retval)
{
    ...
    res2 = parse_metadata(scb, obj, startnode, nserr, retval);
    ...
    case NCX_BT_LIST:
        res = parse_complex(scb, obj, NULL, btyp, startnode, retval);
        break;
    default:
                       ...
                       val_value_t  *retval)
{
    ...
    case NCX_BT_CONTAINER:
    case NCX_BT_LIST:
        res = parse_complex(scb, obj, output, btyp, startnode, retval);
        break;
    default:
