                        ...
                        yang_pcb_t *newpcb)
{
    ...
    if (!cp->firstdone) {
        ...
        if (res == NO_ERR) {
            ...
            ses_putstr(cp->scb, (const xmlChar *)"// ");
            ses_putstr(cp->scb, (const xmlChar *)COPYRIGHT_STRING2);
        } else {
            SET_ERROR(res);
        }
        cp->firstdone = TRUE;
    ...
    main_run (void)
{
    ...
    if (diffparms.versionmode) {
        res = ncx_get_version(versionbuffer, NCX_VERSION_BUFFSIZE);
        if (res == NO_ERR) {
            log_write("%s %s\n", YANGDIFF_PROGNAME, versionbuffer);
        } else {
            SET_ERROR(res);
        }
    }
    if (diffparms.helpmode) {
        help_program_module(YANGDIFF_MOD, YANGDIFF_CONTAINER, 
                       ...
                       yangdiff_diffparms_t  *cp)
{
     ...
     */
    val = val_find_child(valset, YANGDIFF_MOD, YANGDIFF_PARM_CONFIG);
    if (val) {
    ...
    } else if (val_find_child(valset, YANGDIFF_MOD, NCX_EL_NO_CONFIG)) {
