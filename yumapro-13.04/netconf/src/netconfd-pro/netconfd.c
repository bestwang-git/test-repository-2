...
*                                                                   *
*********************************************************************/
#define NETCONFD_MOD       (const xmlChar *)"netconfd-pro"
#define NETCONFD_CLI       (const xmlChar *)"netconfd-pro"
...
static status_t load_base_schema (void)
{
    ...
    res = ncxmod_load_module( NETCONFD_MOD, NULL, NULL, NULL );
    if (res != NO_ERR) {
        return res;
    }
...
#define TESTFEATURE4        (const xmlChar *)"feature4"
    ...
    log_debug2("\nnetconfd-pro: Loading Debug Test Module");
...
{
    log_debug2("\nnetconfd-pro: Loading YANG Modules");
    ...
    /* load in the yumaworks-attrs extension module */
    res = ncxmod_load_module(NCXMOD_YUMAWORKS_ATTRS, NULL, 
                             &profile->agt_savedevQ, NULL);
    if (res != NO_ERR) {
        return res;
    }
...
#define BUFFLEN 256
    ...
    log_init_logfn_va();
    log_set_debug_app(LOG_DEBUG_APP_NETCONFD);
    ...
    len = strlen(START_MSG) + strlen(COPYRIGHT_STRING) + 
        strlen(COPYRIGHT_STRING2 ) + 2;

    if (len < BUFFLEN) {
        strcpy(buff, START_MSG);
        strcat(buff, COPYRIGHT_STRING);
        strcat(buff, COPYRIGHT_STRING2);
    } else {
        return ERR_BUFF_OVFL;
    }

    res = ncx_init( FALSE, /* savestr */
                    dlevel, 
                    TRUE, /* logtstamps */
    ...
    log_debug2("\nnetconfd-pro: Loading Netconf Server Library");
...
{
    if (LOGINFO) {
        xmlChar buff[NCX_VERSION_BUFFSIZE];
        status_t res = ncx_get_version(buff, NCX_VERSION_BUFFSIZE);
        if (res == NO_ERR) {
            log_info("\nRunning netconfd-pro server (%s)\n", buff);
        } else {
            log_info("\nRunning netconfd-pro server\n");
        }
	log_stdout("\n"); /* Ensure one more <return> post log_info() */
	log_flush();
    }
...
static void netconfd_cleanup (void)
{
    ...
    if (LOGINFO) {
        log_info("\nShutting down the netconfd-pro server\n");
    }
...
int main (int argc, char *argv[])
{
	...
	if (!done) {
	    log_cleanup(); /* Restarting: Cleanup logger LAST */
	}
    }

    print_errors();
    print_error_count();
    log_cleanup(); /* Shutdown: Cleanup logger LAST */
