                   ...
                   boolean file_error)
{
    ...
    obj_template_t *varsobj =
        obj_find_template_top(mod, YANGCLI_MOD, YANGCLI_VARS);
    if (varsobj == NULL) {
        return SET_ERROR(ERR_INTERNAL_VAL);
    }

    dlq_hdr_t *que = runstack_get_que(server_cb->runstack_context, ISGLOBAL);
    if (que == NULL) {
        return SET_ERROR(ERR_INTERNAL_VAL);
    }        
    ...
    if (res == NO_ERR && fullspec) {
        ...
        if (res == ERR_XML_READER_START_FAILED) {
            if (file_error) {
                log_error("\nError: User variables file '%s' not found",
                          fullspec);
                res = ERR_FIL_OPEN;
            } else {
                if (LOGDEBUG) {
                    log_debug("\nUser variables file '%s' not found",
                              fullspec);
                }
                res = NO_ERR;
            }
    ...
    /* Save mtime of this file */
    if (res == NO_ERR && file_opened) {
        res = update_def_yangcli_file_mtime (USERVARS_FILE, fullspec);
    }
...
#if 0   // removed val->target -- saving generic uservars still broken
