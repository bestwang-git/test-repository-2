                             ...
                             status_t  *res)
{
    ...
    *res = NO_ERR;
    ...
    if (*res == NO_ERR && sourcefile != NULL) {
        fname = ncxmod_find_data_file(sourcefile, TRUE, FALSE, res);
                ...
                uint32 bufflen)
{
    ...
    boolean cond = runstack_get_cond_state(server_cb->runstack_context);
    uint32 condlen = (cond) ? 0: xml_strlen(FALSE_PROMPT);
    const xmlChar *user = NULL;
    const xmlChar *server = NULL;
    const xmlChar *progname = (const xmlChar *)PROGNAME;
    const xmlChar *clifn = server_cb->cli_fn;
    uint32 clifnlen = (clifn) ? xml_strlen(clifn) + 1 : 0;
    help_mode_t prompt_type = session_cb->prompt_type;

    if (server_cb->program_mode == PROG_MODE_SERVER) {
        progname = YP_PROGNAME;
    }

    uint32 prognamelen = xml_strlen(progname);
    uint32 configlen = 
        (session_cb->config_mode) ? xml_strlen(YANGCLI_CONFIG_MODE) : 0;
    ...
    case MGR_IO_ST_SHUT:
        len = prognamelen + 1 + clifnlen + condlen + 2 + configlen;
        if (len < bufflen) {
            p = buff;
            p += xml_strcpy(p, progname);
            if (clifnlen) {
                *p++ = ':';
            ...
            p += xml_strcpy(p, DEF_PROMPT);
            if (configlen) {
                p += xml_strcpy(p, YANGCLI_CONFIG_MODE);
            }
        } else if (bufflen > prognamelen + 4) {
            p = buff;
            p += xml_strcpy(p, progname);
            if (condlen) {
                p += xml_strcpy(p, FALSE_PROMPT);
            }
            xml_strcpy(p, DEF_PROMPT);
        } else {
    ...
    case MGR_IO_ST_CONN_SHUT:
        p = buff;
        len = xml_strncpy(p, progname, bufflen);
        p += len;
        bufflen -= len;
        ...
        if (session_cb->session_cfg) {
            if (session_cb->def_session) {
                user = session_cb->session_cfg->username;
                server = session_cb->session_cfg->server_addr;
            }
        } else if (prompt_type > HELP_MODE_BRIEF) {
        ...
        /* user@server */
        if (user) {
            len = xml_strncpy(p, user, bufflen);
            p += len;
            bufflen -= len;
            if (bufflen == 0) {
                return;
        ...
        if (server) {
            len = xml_strncpy(p, server, bufflen);
            p += len;
            bufflen -= len;
            if (bufflen == 0) {
                return;
        ...
        /* session name -- print for yangcli only, not yp-shell */
        if (session_cb->session_cfg &&
            get_program_mode() == PROG_MODE_CLIENT) {
            *p++ = '[';
            --bufflen;
            if (bufflen == 0) {
                return;
            ...
            if (session_cb->def_session && prompt_type <= HELP_MODE_NORMAL) {
                *p++ = 'd';
                bufflen -= 1;
            } else {
                len = xml_strncpy(p, session_cb->session_cfg->name, bufflen);
                p += len;
                bufflen -= len;
            }
            if (bufflen == 0) {
        ...
        /* CLI current function name (not config mode) */
        ...
        if (session_cb->config_mode) {
            if (session_cb->config_curobj) {
                len = xml_strncpy(p, YANGCLI_CONFIG_START, bufflen);
                p += len;
                bufflen -= len;
                ...
                case HELP_MODE_BRIEF:
                    usestr = obj_get_name(session_cb->config_curobj);
                    break;
                case HELP_MODE_NORMAL:
                    usestr = session_cb->config_path;
                    break;
                case HELP_MODE_FULL:
                    // change to val instance ID
                    usestr = session_cb->config_path;
                    break;
                default:
                ...
                len = xml_strncpy(p, usestr, bufflen);
                p += len;
                bufflen -= len;
                ...
                len = xml_strncpy(p, YANGCLI_CONFIG_MODE, bufflen);
                p += len;
                bufflen -= len;
            }
        }
    ...
    get_line (server_cb_t *server_cb)
{
    ...
    session_cb_t *session_cb = server_cb->cur_session_cb;    
    if (session_cb == NULL || 
        (server_connected(server_cb) && !session_connected(session_cb))) {

        /* force current session back to the first connected session
         * or the default if none connected               */
    ...
    // needs to be after get_prompt!
    session_cb->returncode = 0;

    if (server_cb->history_line_active) {
        line = (xmlChar *)
            gl_get_line(server_cb->cli_gl, (const char *)prompt,
                        (const char *)server_cb->history_line, -1);
        server_cb->history_line_active = FALSE;
    } else {
        line = (xmlChar *)
            gl_get_line(server_cb->cli_gl, (const char *)prompt, NULL, -1);
    }
    ...
    if (!line) {
        ...
        /* skip ordinary line canceled code due to session dropped
         * detected during get_line_timeout function */
        if (returnstatus == GLR_TIMEOUT) {
            return NULL;
        }
        ...
        switch (returnstatus) {
        case GLR_NEWLINE:
            log_debug2_append("NEWLINE");
            break;
        case GLR_BLOCKED:
            log_debug2_append("BLOCKED");
            break;
        case GLR_SIGNAL:
            log_debug2_append("SIGNAL");
            break;
        case GLR_TIMEOUT:
            log_debug2_append("TIMEOUT");
            break;
        case GLR_FDABORT:
            log_debug2_append("FDABORT");
            break;
        case GLR_EOF:
            log_debug2_append("EOF");
            break;
        case GLR_ERROR:
            log_debug2_append("ERROR");
            break;
        default:
            log_debug2_append("<unknown>");
        }
        log_debug2_append(" (rt:%u errno:%u)",
                          session_cb->returncode,
                          session_cb->errnocode);
    }
              ...
              val_value_t *oldvalset)
{
    ...
    if (obj_is_leaf_list(parm) && server_cb->first_leaflist) {
        if (val_find_child(valset, obj_get_mod_name(parm),
                           obj_get_name(parm))) {
            /* already have at least 1 of these leaf-lists entered
             * so just return and ask if the user wants to add more
             * leaf lists   */
            return NO_ERR;
        }
    }
    ...
    ncx_btype_t btyp = obj_get_basetype(parm);
    status_t res = NO_ERR;
    boolean ispassword = obj_is_password(parm);

    if (obj_is_data_db(parm) && parm->objtype != OBJ_TYP_RPCIO) {
        ...
        log_stdout("\nFilling %s %s %s:", 
                   str, obj_get_typestr(parm), objbuff);
    ...
    const xmlChar *parmname = obj_get_name(parm);
    btyp = obj_get_basetype(parm);
    res = NO_ERR;
    val_value_t *oldparm = NULL;
    const xmlChar *def = NULL;
    boolean done = FALSE;
    xmlChar *start = NULL;
    ...
    while (!done) {
        ...
        if (oldvalset) {
            oldparm = val_find_child(oldvalset, obj_get_mod_name(parm),
                                     parmname);
        }
        ...
        xmlChar *line = get_cmd_line(server_cb, &res);
    ...
    if (!*start) {
        ...
        if (def) {
            ...
            if (btyp==NCX_BT_EMPTY) {
                ...
                val_value_t *newparm = val_clone(oldparm);
                if (!newparm) {
                    res = ERR_INTERNAL_MEM;
                } else {
    ...
    if (res != NO_ERR) {
        ...
        case NCX_BAD_DATA_CHECK:
            ...
            xmlChar *saveline = (start) ? xml_strdup(start) :
            ...
            while (!done) {
                ...
                /* get a line of input from the user */
                xmlChar *line2 = get_cmd_line(server_cb, &res);
                if (!line2) {
                    m__free(saveline);
                    return res;
                }
                ...
                status_t  *res)
{
    ...
    val_value_t *dummy = val_new_value();
    if (!dummy) {
        *res = ERR_INTERNAL_MEM;
        log_error("\nError: malloc failed");
        return NULL;
...
*********************************************************************/
static status_t
    fill_valset_val (server_cb_t *server_cb,
                 obj_template_t *rpc,
                 val_value_t *valset,
                 val_value_t *oldvalset,
                 boolean iswrite,
                 boolean isdelete)
{
    ...
    val_value_t *oldval = NULL;
    val_value_t *firstchoice = NULL;
    obj_template_t *target = NULL;
    boolean done = FALSE;
    uint32 yesnocode = 0;
    status_t res = NO_ERR;
    ...
    obj_template_t *parm = obj_first_child(target);
    for (; parm != NULL && res==NO_ERR;
            ...
            while (!done && res == NO_ERR) {
                ...
                case NO_ERR:
                    ...
                    res = get_yesno(server_cb, YANGCLI_PR_LLIST,
                                    YESNO_NO, &yesnocode);
                    server_cb->first_leaflist = FALSE;
                    if (res == NO_ERR) {
                        switch (yesnocode) {
                        case YESNO_CANCEL:
                            res = ERR_NCX_CANCELED;
        ...
        case OBJ_TYP_RPCIO:
            ...
            res = fill_valset_val(server_cb, parm, val, oldval, iswrite, isdelete);
        ...
        case OBJ_TYP_LIST:
            ...
            while (!done && res == NO_ERR) {
                 ...
                 */
                res = fill_valset_val(server_cb, parm, val, NULL, iswrite, 
                                  isdelete);
                switch (res) {
                case NO_ERR:
...
{
    status_t res;
    res = fill_valset_val (server_cb, rpc, valset, oldvalset, iswrite,isdelete); 

    if (res == NO_ERR && valset) {
       record_new_step_valset (server_cb, rpc, valset);
    }
                      ...
                      help_mode_t mode)
{
    ...
    while (obj) {
        if (obj_is_rpc(obj) && !obj_is_hidden(obj)) {
            if (server_cb->program_mode == PROG_MODE_SERVER) {
                if (is_server_command(obj_get_name(obj))) {
                    if (mode == HELP_MODE_BRIEF) {
                        obj_dump_template(obj, mode, 1, 0);
                    } else {
                        obj_dump_template(obj, mode, 0, 0);
             ...
             boolean iswrite)
{
    ...
    boolean save_getopt = server_cb->get_optional;
    val_value_t *newparm = NULL;
    val_value_t *curparm = NULL;
     ...
     */
    val_value_t *valroot =
        get_instanceid_parm(target, TRUE, TRUE, &targobj, &targval, &res);
    if (res != NO_ERR) {
        if (valroot) {
     ...
     */
    if (targval && targval->btyp == NCX_BT_STRING &&
                           ...
                           boolean iswrite)
{
    ...
    status_t res = NO_ERR;
    val_value_t *curnode = config_content;
    ...
    while (!done) {
        if (curnode->btyp == NCX_BT_LIST) {
            ...
            for (curkey = obj_first_key(curnode->obj);
                ...
                val_value_t *keyval =
                    val_find_child(curnode, obj_get_mod_name(curkey->keyobj),
                                   obj_get_name(curkey->keyobj));
...
{
    session_cb_t *session_cb = server_cb->cur_session_cb;
    val_value_t           *curparm = NULL, *newparm = NULL;
    ...
    status_t               res = NO_ERR;
    ncx_name_match_t       match_names = NCX_MATCH_NONE;
    ...
    *retres = NO_ERR;
    ...
    val_value_t *parm =
        val_find_child(valset, YANGCLI_MOD, NCX_EL_MATCH_NAMES);
    if (parm != NULL) {
        ...
        match_names = session_cb->match_names;
    }

    /* look for the 'alt-names' parameter */
    parm = val_find_child(valset, YANGCLI_MOD, NCX_EL_ALT_NAMES);
    if (parm != NULL) {
        alt_names = VAL_BOOL(parm);
    } else {
        alt_names = session_cb->alt_names;
    }
            ...
            if (parm->btyp == NCX_BT_EXTERN) {
                ...
                curparm = 
                    var_get_script_val_ex(server_cb->runstack_context,
                                          NULL, targobj, NULL, parmval,
                                          ISPARM, (parmval) ? NULL : parm,
                                          &res);
            }
        ...
        if (res == ERR_NCX_SKIPPED) {
            ...
            if (targval == NULL) {
                ...
                *retres = res;
                ...
                /* the empty target node already exists;
                 * do not need to create a newparm leaf because
                 * this will not get used, will not get added to any
                 * payload, and will become leaked memory
                 */
                *retres = res;
                return targval;
            }
                        ...
                        boolean topcontainer)
{
    ...
    case OBJ_TYP_CONTAINER:
        if (!topcontainer) {
            res = val_add_one_operation_attr(val, op);
            break;
        } /* else fall through */
    case OBJ_TYP_CHOICE:
    case OBJ_TYP_CASE:
        {
            ...
            for (childval = val_get_first_child(val);
                ...
                res = val_add_one_operation_attr(childval, op);
                if (res != NO_ERR) {
                    return res;
                }
            }
                          ...
                          const xmlChar *edit_target)
{
    ...
    obj_template_t *insobj = ncx_get_yang_insert();
    if (!insobj) {
        return ERR_NCX_DEF_NOT_FOUND;
    }

    xmlns_id_t yangid = xmlns_yang_id();
    const xmlChar *insopstr = op_insertop_name(insop);

    /* create a value node for the meta-value */
    val_value_t *metaval = val_new_value();
    if (!metaval) {
        return ERR_INTERNAL_MEM;
    }
    val_init_from_template(metaval, insobj);
    val_set_qname(metaval, yangid, YANG_K_INSERT, xml_strlen(YANG_K_INSERT));

    /* set the meta variable value and other fields */
    status_t res =
        val_set_simval(metaval, metaval->typdef, yangid, YANG_K_INSERT,
                       insopstr);
    if (res != NO_ERR) {
    ...
    if (insop == OP_INSOP_BEFORE || insop == OP_INSOP_AFTER) {
        ...
        if (val->obj->objtype==OBJ_TYP_LEAF_LIST) {
            /* expecting the value of the leaf-list that will
             * be used to insert before or after    */
            val_init_from_template(metaval, ncx_get_yang_value());
            val_set_qname(metaval, yangid, YANG_K_VALUE,
                          xml_strlen(YANG_K_VALUE));
        } else {
            /* expecting full-formed XPath predicates */
            val_init_from_template(metaval, ncx_get_yang_key());
            val_set_qname(metaval, yangid, YANG_K_KEY,
                          xml_strlen(YANG_K_KEY));
        }

        /* set the meta variable value and other fields */
        res = val_set_simval(metaval, metaval->typdef,
                             yangid, NULL, edit_target);
        if (res != NO_ERR) {
            val_free_value(metaval);
            return res;
        } else {
             ...
             op_editop_t editop)
{
    ...
    /* init locals */
    status_t res = NO_ERR;
    val_value_t *content = NULL;
    val_value_t *valroot = NULL;
    boolean dofill = TRUE;
    boolean doattr = TRUE;
    boolean topcontainer = FALSE;
    op_defop_t def_editop = session_cb->defop;
    ...
    uint32 timeoutval;
    val_value_t *parm = val_find_child(valset, YANGCLI_MOD, YANGCLI_TIMEOUT);
    if (parm && parm->res == NO_ERR) {
        timeoutval = VAL_UINT(parm);
    } else {
        timeoutval = session_cb->timeout;
    }
    ...
    res = send_edit_config_to_server(server_cb, NULL, valroot, content,
                                     FALSE, timeoutval, def_editop);
    if (res != NO_ERR) {
               ...
               uint32  len)
{
    ...
    if (res == NO_ERR) {
        /* construct an edit-config PDU with default parameters */
        res = send_edit_config_to_server(server_cb, NULL, valroot, content,
                                         FALSE, timeoutval, session_cb->defop);
        if (res != NO_ERR) {
             ...
             uint32  len)
{
    ...
    if (parm && parm->res == NO_ERR) {
        ...
        withdef = session_cb->withdefaults;
    }
                     ...
                     const xmlChar *fname)
{
    ...
    if (res == NO_ERR) {
        ...
        if (glstatus) {
           ...
           if (res == NO_ERR){
              res = update_def_yangcli_file_mtime (HISTORY_FILE,
                                                      fullspec);
           }
           if (fullspec) {
               m__free(fullspec);
                     ...
                     const xmlChar *fname)
{
    ...
    res = NO_ERR;
    xmlChar *fullspec = ncx_get_source(fname, &res);
    res = check_for_saving_def_yangcli_file (HISTORY_FILE, fullspec);
    if (res != NO_ERR) {
        m__free(fullspec);
        return res;
    }
                      ...
                      help_mode_t mode)
{
    ...
    for (notif = (mgr_not_msg_t *)
             dlq_firstEntry(&session_cb->notificationQ);
         notif != NULL && !done;
        ...
        if (eventindex >= startindex) {
            ...
            if (notif->eventType) {
                switch (session_cb->display_mode) {
                       ...
                       int32 maxevents)
{
    ...
    if (maxevents > 0) {
        ...
        while (!dlq_empty(&session_cb->notificationQ) &&
            ...
            notif = (mgr_not_msg_t *)
                dlq_deque(&session_cb->notificationQ);
...
/********************************************************************
 * FUNCTION do_start_rpc_timing (local RPC)
 * 
 * Start RPC timing statistics gathering
 *
 * start-rpc-timing [statfile=filespec] [append=true|false]
 *
 * INPUTS:
 *    server_cb == server control block to use
 *    rpc == RPC method for the history command
 ...
 *********************************************************************/
static status_t
    do_start_rpc_timing (server_cb_t *server_cb,
                         obj_template_t *rpc,
                         const xmlChar *line,
                         uint32  len)
{
    ...
    val_value_t *valset = get_valset(server_cb, rpc, &line[len], &res);
    if (valset && res == NO_ERR) {
        if (session_cb->time_rpcs_stats_active) {
            log_error("\nError: RPC timing statistics already active");
            res = ERR_NCX_IN_USE;
        } else {
            boolean append = FALSE;
            val_value_t *appendval = 
                val_find_child(valset, YANGCLI_MOD, YANGCLI_APPEND);
            if (appendval && appendval->res == NO_ERR) {
                append = VAL_BOOL(appendval);
            }

            const xmlChar *fspec = NULL;
            /* check if the 'statfile' string is set */
            val_value_t *parm = 
                val_find_child(valset, YANGCLI_MOD, YANGCLI_STATFILE);
            if (parm && parm->res == NO_ERR) {
                fspec = VAL_STR(parm);
            } else if (session_cb->time_rpcs_stats_file) {
                fspec = session_cb->time_rpcs_stats_file;
            } else {
                fspec = YANGCLI_DEF_TIME_RPCS_STATS_FILE;
            }
            ...
            if (res == NO_ERR) {
                ...
                session_cb->time_rpcs_fp = 
                    fopen((const char *)fullspec, openstr);
                if (session_cb->time_rpcs_fp == NULL) {
                    log_error("\nError: Could not open RPC timing "
                              "stats file '%s'", fullspec);
                    res = errno_to_status();
                    m__free(fullspec);
                } else {
                    session_cb->time_rpcs = TRUE;
                    session_cb->time_rpcs_stats = TRUE;
                    session_cb->time_rpcs_stats_active = TRUE;
                    session_cb->time_rpcs_stats_file = fullspec;
                    session_cb->echo_replies = FALSE;

                    if (session_cb->time_rpcs_stats_file_rpc) {
                        m__free(session_cb->time_rpcs_stats_file_rpc);
                    }

                    /* pass off memory for fullspec here */
                    session_cb->time_rpcs_stats_file_rpc = fullspec;
                }
            }
        }
    }
...
/********************************************************************
 * FUNCTION do_stop_rpc_timing (local RPC)
 * 
 * Stop RPC timing statistics gathering
 *
 * stop-rpc-timing
 *
 * INPUTS:
 ...
 *********************************************************************/
static status_t
    do_stop_rpc_timing (server_cb_t *server_cb,
                        obj_template_t *rpc,
                        const xmlChar *line,
                        uint32  len)
{
    ...
    status_t res = NO_ERR;
    val_value_t *valset = get_valset(server_cb, rpc, &line[len], &res);
    if ((valset && res == NO_ERR) || res == ERR_NCX_SKIPPED) {
        if (!session_cb->time_rpcs_stats_active) {
            log_error("\nError: RPC timing statistics is not active");
            res = ERR_NCX_OPERATION_FAILED;
        } else {
            fclose(session_cb->time_rpcs_fp);
            session_cb->time_rpcs_fp = NULL;
            session_cb->time_rpcs = get_time_rpcs();
            session_cb->time_rpcs_stats = get_time_rpcs_stats();
            session_cb->time_rpcs_stats_active = FALSE;
            session_cb->echo_replies = get_echo_replies();
            res = NO_ERR;
        }
    }
                           ...
                           uint32  len)
{
    ...
    if (get_program_mode() == PROG_MODE_SERVER) {
        if (!xml_strcmp(rpcname, YANGCLI_CONFIG)) {
            return do_config(server_cb, rpc, line, len);
        } else if (!xml_strcmp(rpcname, YANGCLI_EXIT)) {
            return do_exit(server_cb, rpc, line, len);
        }
        return ERR_NCX_SKIPPED;
    }
    ...
    status_t res = NO_ERR;
    boolean cond = runstack_get_cond_state(server_cb->runstack_context);
    if (!xml_strcmp(rpcname, YANGCLI_AUTO_TEST)) {
        if (cond) {
            res = do_auto_test(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_CONFIG)) {
        if (cond) {
            res = do_config(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_CREATE)) {
        if (cond) {
        ...
        if (cond) {
            res = do_exit(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_GET_LOCKS)) {
        if (cond) {
            res = do_get_locks(server_cb, session_cb, rpc, line, len);
        }
        ...
        if (cond) {
            res = do_release_locks(server_cb, session_cb, rpc, line, len);
        }
    ...
    } else if (!xml_strcmp(rpcname, YANGCLI_START_RPC_TIMING)) {
        if (cond) {
            res = do_start_rpc_timing(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_STOP_RPC_TIMING)) {
        if (cond) {
            res = do_stop_rpc_timing(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_XGET)) {
        if (cond) {
            res = do_xget(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_UPDATE_CONFIG)) {
        if (cond) {
            res = do_update_config(server_cb, rpc, line, len);
        }
                      ...
                      uint32  len)
{
    ...
    if (!xml_strcmp(rpcname, YANGCLI_ALIAS)) {
        ...
        if (cond) {
            res = do_test_suite(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_SESSION)) {
        if (cond) {
            res = do_session(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_SESSION_CFG)) {
        if (cond) {
            res = do_session_cfg(server_cb, rpc, line, len);
        }
     } else if (!xml_strcmp(rpcname, YANGCLI_RECORD_TEST)) {
        if (cond) {
            res = do_record_test(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_SESSIONS_CFG)) {
        if (cond) {
            res = do_sessions_cfg(server_cb, rpc, line, len);
        }
    } else if (!xml_strcmp(rpcname, YANGCLI_SHOW)) {
        if (cond) {
            res = do_show(server_cb, rpc, line, len);
        ...
        if (cond) {
            res = do_start_session(server_cb, rpc, line, len);
        }
        ...
        if (cond) {
            res = do_stop_session(server_cb, rpc, line, len);
        }
                 ...
                 boolean do_command)
{
...
#ifdef EVAL_VERSION
    if (xml_strncmp(line, YANGCLI_QUIT, xml_strlen(YANGCLI_QUIT))) {
        res = check_eval_command_limit();
        if (res != NO_ERR) {
            return res;
        }
    } // else allow the quit command
#endif
    ...
    if (rpc==NULL || !obj_is_rpc(rpc)) {
        if (server_cb->result_name || server_cb->result_filename) {
            res = finish_result_assign(server_cb, session_cb, NULL, useline);
        } else {
            if (res == ERR_NCX_DEF_NOT_FOUND) {
                /* this is an unknown command */
                log_error("\nError: Unrecognized command\n");
            } else if (res == ERR_NCX_AMBIGUOUS_CMD) {
                // !! should be done in finish_result_assign
                log_error("\n");  
            } else {
                log_error("\nError: %s\n", get_error_string(res));
            }
        ...
        if (get_program_mode() == PROG_MODE_SERVER &&
            !is_server_command(obj_get_name(rpc))) {
            log_error("\nError: %s command not allowed\n", obj_get_name(rpc));
            res = ERR_NCX_OPERATION_FAILED;
        } else if (do_command && !is_do_command(obj_get_name(rpc))) {
            log_error("\nError: %s command not allowed\n", obj_get_name(rpc));
            res = ERR_NCX_OPERATION_FAILED;
        } else {
    ...
    if (res == NO_ERR) {
       new_record_step (server_cb, rpc, useline);
    }
                  ...
                  boolean do_save_mode)
{
    ...
    uint32 linelen = xml_strlen(line);
    if (!linelen) {
        return res;
    }
    ...
    /* first check the command keyword to see if it is an alias */
    xmlChar *newline = expand_alias(line, &res);
    if (res == ERR_NCX_SKIPPED) {
        res = NO_ERR;
        useline = line;
    } else if (res == NO_ERR) {
    ...
    ncx_node_t dtyp = NCX_NT_OBJ;
    obj_template_t *rpc =
        (obj_template_t *)parse_def(server_cb, &dtyp, useline, &len, &res);
    if (rpc == NULL || !obj_is_rpc(rpc)) {
        if (server_cb->result_name || server_cb->result_filename) {
            res = finish_result_assign(server_cb, session_cb, NULL, useline);
        } else {
            if (rpc && !obj_is_rpc(rpc)) {
                log_error("\nError: '%s' is not a YANG rpc-stmt\n",
                          obj_get_name(rpc));
    ...
    if (is_yangcli_ns(obj_get_nsid(rpc))) {
        ...
        if (res == ERR_NCX_SKIPPED) {
            if (do_command && !is_do_command(obj_get_name(rpc))) {
    ...
    /* the 'do' command mode does not allow server commands to be used
     * at this time!!
     */
    if (do_command) {
        log_error("\nError: %s command not allowed\n", obj_get_name(rpc));
    ...
    /* only the 'commit' command is allowed in server mode, which
     * is called automatically by do_save() in yangcli_save.c
     */
    if (session_cb->command_mode == CMD_MODE_NORMAL && 
        get_program_mode() == PROG_MODE_SERVER && !do_save_mode) {
        log_error("\nError: %s command not allowed\n", obj_get_name(rpc));
        if (newline) {
            m__free(newline);
        }
    ...
    obj_template_t *input = NULL;
    val_value_t *reqdata =
        xml_val_new_struct(obj_get_name(rpc), obj_get_nsid(rpc));
    if (!reqdata) {
        log_error("\nError allocating a new RPC request\n");
        res = ERR_INTERNAL_MEM;
    } else {
    ...
    if (res == NO_ERR && input) {
        ...
        if (res == NO_ERR) {
            boolean dofill = TRUE;
            if (!session_cb->script_input &&
                runstack_level(server_cb->runstack_context) > 0) {
                dofill = FALSE;
            }
            if (interactive_mode() && dofill) {
         ...
         */
        if (res == NO_ERR) {
            /* check if the valset itself came from an external
             * file, so it might have metadata in it
             */
            val_move_metadata(valset, reqdata);
            val_value_t *parm = val_get_first_child(valset);
            while (parm) {
    ...
    if (res != NO_ERR) {
        ...
        session_cb->state = MGR_IO_ST_CONN_CLOSEWAIT;
    } else {
        session_cb->state = MGR_IO_ST_CONN_RPYWAIT;
    }
...
{
    assert(server_cb && "server_cb is NULL!");
    assert(runscript && "runscript is NULL!");
    ...
    obj_template_t *rpc = ncx_find_object(get_yangcli_mod(), YANGCLI_RUN);
    if (!rpc) {
        return ERR_NCX_DEF_NOT_FOUND;
    }

    /* create a dummy command line 'script <runscipt-text>' */
    uint32 linelen = xml_strlen(runscript) + xml_strlen(NCX_EL_SCRIPT) + 1;
    xmlChar *line = m__getMem(linelen+1);
    if (!line) {
        return ERR_INTERNAL_MEM;
    }
    xmlChar *p = line;
    ...
    *p++ = ' ';
    ...
    status_t res = do_run(server_cb, rpc, line, 0);
...
{
    assert(server_cb && "server_cb is NULL!");
    assert(runcommand && "runcommand is NULL!");
     ...
     */
    xmlChar *copystring = xml_strdup(runcommand);
    if (!copystring) {
        return ERR_INTERNAL_MEM;
    }
    ...
    switch (session_cb->state) {
    case MGR_IO_ST_IDLE:
        res = top_command(server_cb, copystring, FALSE);
        break;
    case MGR_IO_ST_CONN_IDLE:
        res = conn_command(server_cb, copystring, FALSE, FALSE);
        break;
    default:
        res = ERR_NCX_OPERATION_FAILED;
    }
                    ...
                    const xmlChar *runcommand)
{
    ...
    switch (session_cb->state) {
    case MGR_IO_ST_IDLE:
        res = top_command(server_cb, copystring, TRUE);
        break;
    case MGR_IO_ST_CONN_IDLE:
        res = conn_command(server_cb, copystring, TRUE, FALSE);
        break;
    default:
        res = ERR_NCX_OPERATION_FAILED;
    }

    server_cb->cur_session_cb = save_session_cb;
                  ...
                  status_t *res)
{
    ...
    /* init locals */
    xmlChar *clibuff = server_cb->clibuff;
    xmlChar *str = NULL;
    int total = 0;
    int maxlen = YANGCLI_BUFFLEN;
    int len = 0;
    boolean done = FALSE;
    xmlChar *start = clibuff;

    log_flush(); /* Flush buffered log output if we're about to suspend */
    ...
    if (*res == NO_ERR) {
        ...
        update_yangcli_param_change_flag (HISTORY_FILE, TRUE);
        return clibuff;
    } else {
        return NULL;
    }
                ...
                boolean startupmode)
{
    ...
    status_t res = NO_ERR;
    boolean tcp = FALSE;
    val_value_t *connect_valset = get_connect_valset();
   ...
   /* Check if it is to connect a named session */
    res = connect_named_session(valset, server_cb);
    if (res != ERR_NCX_SKIPPED) {
       if (valset != NULL) {
           val_free_value(valset);
       }
    ...
    /* Check if it is to connect-all sessions */
    res = connect_all_sessions(valset, server_cb);
    if (res != ERR_NCX_SKIPPED) {
        if (valset != NULL) {
            val_free_value(valset);
        }
    ...
    /* Handle default session connection */
    session_cb = find_session_cb(server_cb, NCX_EL_DEFAULT);
    set_cur_session_cb(server_cb, session_cb);

    /* Check if this session-name has already connected */
    ...
    /* make sure not set to ERR_NCX_SKIPPED */
    res = NO_ERR;

    /* make sure the 3 required parms are set */
    boolean s1 = val_find_child(valset, YANGCLI_MOD, 
                                YANGCLI_SERVER) ? TRUE : FALSE;
    boolean s2 = val_find_child(valset, YANGCLI_MOD,
                                YANGCLI_USER) ? TRUE : FALSE;
    boolean s3 = val_find_child(valset, YANGCLI_MOD,
                                YANGCLI_PASSWORD) ? TRUE : FALSE;

    /* check the transport parameter */
    val_value_t *testval = 
        val_find_child(valset, YANGCLI_MOD, YANGCLI_TRANSPORT);
    if (testval != NULL && 
        testval->res == NO_ERR && 
    ...
    if (interactive_mode()) {
        if (startupmode && s1 && s2 && (s3 || tcp)) {
            if (LOGDEBUG3) {
                log_debug3("\nyangcli-pro: CLI direct connect mode");
            }
        } else {
    ...
    if (s1 && s2 && (s3 || tcp)) {
        ...
        /* make sure the default session flag and other state vars gets reset */
        clean_session_cb_vars(server_cb->cur_session_cb);

        /* start the session -- don't know if connect worked yet */
        res = create_session(server_cb, server_cb->cur_session_cb, NULL);

        if (res == NO_ERR) {
            update_session_cb_vars(server_cb->cur_session_cb);
        }
               ...
               status_t *retres)
{
    ...
    *len = 0;
     ...
     */
    if (prefix) {
        xmlns_id_t nsid = xmlns_find_ns_by_prefix(prefix);
        if (nsid) {
        ...
        if (def == NULL) {
            /* 2) try an exact match in the NETCONF module */
            def = try_parse_def(server_cb, NULL, NCXMOD_NETCONF, defname, dtyp);
        }

        if (def == NULL) {
            /* 3) try an exact match in the default module
             ...
             */
            const xmlChar *defmod = get_default_module();
            if (defmod != NULL) {
                def = try_parse_def(server_cb, NULL, defmod, defname, dtyp);
            }
        }

        /* 4) if not found, try any server advertised module */
        ...
        /* 5) try any of the manager-loaded modules */
    ...
    if (res == NO_ERR && def == NULL && get_autocomp()) {
        ...
        if (prefix != NULL) {
            ...
            uint32 tempcount = 0;
            ncx_module_t *ncmod = get_netconf_mod(server_cb);
            switch (*dtyp) {
            case NCX_NT_NONE:
            case NCX_NT_OBJ:
                def = NULL;
                if (ncmod) {
                    /* favor the NETCONF module if there are multiple
                     * partial matches -- force full names or prefixes
                     * if the NETCONF commands mask any custom commands
                     * from yangcli or the server                */
                    def = ncx_match_any_rpc_mod(ncmod, defname, &tempcount);
                    lastmatch = def;
                }
                ...
                status_t  *res)
{
    *res = NO_ERR;
    ...
    if (*res == NO_ERR && !valset) {
        ...
        if (!valset) {
            *res = ERR_INTERNAL_MEM;
            ...
            val_init_from_template(valset, obj);
            *res = val_add_defaults_ex(valset, NULL, NULL, SCRIPTMODE,
                                       server_cb->runstack_context, 0, 0);
        }
    }
    ...
    if (runstack_level(server_cb->runstack_context) > 0 &&
        !session_cb->script_input) {
        dofill = FALSE;
    }

    if (*res==NO_ERR && dofill && interactive_mode()) {
        *res = fill_valset(server_cb, rpc, valset, NULL, TRUE, FALSE);
    }
                    ...
                    unsigned long num)
{
    ...
    int glstatus = gl_lookup_history(server_cb->cli_gl, num, &history_line);
                           ...
                           const xmlChar *line)
{
    ...
    for (curindex = history_range.newest;
        ...
        int glstatus = gl_lookup_history(server_cb->cli_gl, curindex,
                                         &history_line);
        if (glstatus == 0) {
            continue;
        }
                    ...
                    session_cfg_t *session_cfg)
{
    if (LOGDEBUG) {
        if (session_cfg) {
            log_debug("\nConnect attempt with session-cfg '%s'\n",
                      session_cfg->name);
        } else if (server_cb->program_mode == PROG_MODE_CLIENT
            ...
            val_dump_value_max(server_cb->connect_valset,
                               ...
                               FALSE, FALSE);
            log_debug("\n");
        } else if (server_cb->program_mode == PROG_MODE_SERVER) {
            log_debug("\nServer connect attempt to local server\n");
        }
    }
    ...
    while (!dlq_empty(&session_cb->searchresultQ)) {
        ncxmod_search_result_t *searchresult = 
            (ncxmod_search_result_t *)dlq_deque(&session_cb->searchresultQ);
    ...
    val_value_t *val = NULL;
    const xmlChar *username = NULL;
    if (server_cb->program_mode == PROG_MODE_SERVER) {
        username = (const xmlChar *)getenv("USER");
    } else if (session_cfg) {
        username = session_cfg->username;
    } else if (server_cb->connect_valset) {
    ...
    const xmlChar *server = NULL;
    if (server_cb->program_mode == PROG_MODE_SERVER) {
        server = (const xmlChar *)"localhost";
    } else if (session_cfg) {
        server = session_cfg->server_addr;
    } else if (server_cb->connect_valset) {
        val = val_find_child(server_cb->connect_valset,
                             YANGCLI_MOD, YANGCLI_SERVER);
        if (val && val->res == NO_ERR) {
    ...
    uint16 port = 0;
    boolean portbydefault = FALSE;
    if (server_cb->program_mode == PROG_MODE_SERVER) {
        ;
    } else if (session_cfg) {
        port = session_cfg->server_port;
        portbydefault = (port == 830);
    } else if (server_cb->connect_valset) {
        val = val_find_child(server_cb->connect_valset,
    ...
    if (server_cb->program_mode == PROG_MODE_SERVER) {
        ...
        publickey = session_cfg->public_key;
    } else if (server_cb->connect_valset) {
        val = val_find_child(server_cb->connect_valset,
    ...
    if (server_cb->program_mode == PROG_MODE_SERVER) {
        ...
        privatekey = session_cfg->private_key;
    } else if (server_cb->connect_valset) {
        val = val_find_child(server_cb->connect_valset,
    ...
    if (server_cb->program_mode == PROG_MODE_CLIENT) {
        if (server_cb->connect_valset) {
            ...
            if (val != NULL && val->res == NO_ERR && 
                !xml_strcmp(VAL_ENUM_NAME(val), NCX_EL_TCP)) {

                /* this is the tail-f direct NETCONF over TCP 
                 * port 2023 protocol or a specified port    */
                tcp = TRUE;
        ...
        /* SERVER mode for yp-shell: run NETCONF base:1.1 over TCP */
        tcp = TRUE;
    }

    if (server_cb->program_mode == PROG_MODE_CLIENT) {
        log_info("\nyangcli-pro: Starting NETCONF session for %s on "
                 "%s over %s on port %u", username, server,
                 tcp ? NCX_EL_TCP : NCX_EL_SSH,
                 port ? port : NCX_NCSSH_PORT);
    } else {
        log_debug("\nyangcli-pro: Starting NETCONF session for %s on "
                  "%s over %s on port %u", username, server,
                  tcp ? NCX_EL_TCP : NCX_EL_SSH,
    ...
    session_cb->state = MGR_IO_ST_CONNECT;

    /* direct_mode is a direct connection to the server <ncx-connect>
     * socket, instead of an external socket like port 22 or 830
     */
    boolean direct_mode = (server_cb->program_mode == PROG_MODE_SERVER) ?
        TRUE : FALSE;

    boolean default_keys = FALSE;
    if (publickey && !strcmp(publickey, YANGCLI_DEF_PUBLIC_KEY)) {
        default_keys = TRUE;
    }
    if (!default_keys && privatekey &&
        !strcmp(privatekey, YANGCLI_DEF_PRIVATE_KEY)) {
        default_keys = TRUE;
    }
     ...
     */
    status_t res = 
        mgr_ses_new_session(username, password,
                            publickey, privatekey,
                            server, port,
                            (tcp) ? SES_TRANSPORT_TCP : SES_TRANSPORT_SSH,
                            server_cb->temp_progcb,
                            &session_cb->mysid,
                            xpath_getvar_fn,
                            (session_cfg) ? NULL : server_cb->connect_valset,
                            (session_cfg) ? session_cfg->server_protocols : 0,
                            session_cb, direct_mode, default_keys);
                            
    if (res == NO_ERR) {
        startedsession = TRUE;
        session_cb->state = MGR_IO_ST_CONN_START;
        log_debug("\nyangcli-pro: Start session %d OK for server '%s'", 
                  session_cb->mysid, server_cb->name);

        res = mgr_set_getvar_fn(session_cb->mysid, xpath_getvar_fn);
        if (res != NO_ERR) {
            log_error("\nError: Could not set XPath variable callback");
        }
    }

    /* only create a new session_cfg if the current one is NULL
     * or replace the default session with updated parameters
     * so the prompt will be right and 'session-cfg save' will
     * save the correct parameters as well
     */
    if (res == NO_ERR && 
        ((session_cb->session_cfg == NULL) || session_cb->def_session)) {

        if (session_cb->session_cfg == NULL) {
            /* this session_cb is connecting as default for the first time */
            if (session_cfg) {
                session_cb->session_cfg = session_cfg;
                session_cb->session_cfg_new = FALSE;
            } else {
                /* get the NETCONF protcols enabled */
                uint16 flags = 0;
                if (ncx_protocol_enabled(NCX_PROTO_NETCONF10)) {
                    ncx_set_val_protocol_enabled(NCX_PROTO_NETCONF10, &flags);
                }
                if (ncx_protocol_enabled(NCX_PROTO_NETCONF11)) {
                    ncx_set_val_protocol_enabled(NCX_PROTO_NETCONF11, &flags);
                }

                /* create a new session_cfg struct */
                session_cb->session_cfg = 
                    new_session_cfg_cli(NULL,  /* default name */
                                        server, username, password,
                                        publickey, privatekey, port, 
                                        flags, &res);
                if (session_cb->session_cfg) {
                    session_cb->session_cfg_new = TRUE;
                }
            }
        } else {
            /* this is another default session starting */
            free_session_cfg(session_cb->session_cfg);
            session_cb->session_cfg = NULL;
            if (session_cfg) {
                session_cb->session_cfg = session_cfg;
                session_cb->session_cfg_new = FALSE;
            } else {
    ...
    if (res == NO_ERR) {
        ...
        log_info("\nyangcli-pro: Start session failed for user %s on "
                 "%s (%s)\n", 
                 username, server, get_error_string(res));
        session_cb->state = MGR_IO_ST_IDLE;
    }
                                ...
                                op_defop_t def_editop)
{
    ...
    val_value_t *target =
        xml_val_new_flag(session_cb->default_target, obj_get_nsid(child));
    if (!target) {
        if (freeroot) {
            val_free_value(valroot);
        } else {
    ...
    if (!(def_editop == OP_DEFOP_NOT_USED ||
        ...
        res = NO_ERR;
        const xmlChar *defopstr = op_defop_name(def_editop);
    ...
    if (session_cb->testoption != OP_TESTOP_NONE) {
        ...
        parm = val_make_simval_obj(child,
                                   op_testop_name(session_cb->testoption),
                                   &res);
        if (!parm) {
            if (freeroot) {
                val_free_value(valroot);
    ...
    if (session_cb->erroption != OP_ERROP_NONE) {
        ...
        res = NO_ERR;
        parm = val_make_simval_obj(child,
                                   op_errop_name(session_cb->erroption),
                                   &res);
        if (!parm) {
            if (freeroot) {
                val_free_value(valroot);
     ...
     */
    if (!content_complete && 
        (config_content->btyp == NCX_BT_CHOICE ||
         ...
         **** THIS IS ONLY CODE THAT REMOVES THE config_content
         **** FROM ITS PARENT BEFORE FREEING IT
         **** COUPLED TO fill_content_from_choice ABOVE !!!
         */
