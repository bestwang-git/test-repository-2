...
/********************************************************************
* FUNCTION do_yang_elem_case
* 
*   Generate the element node for a case object type
*
* INPUTS:
*    mod == module conversion in progress
*    obj == case object to convert to XSD element
*    top == struct parent to contain child nodes for each type
*
* OUTPUTS:
*    top->v.childQ has entries added for simple elements
*
* RETURNS:
*   status
*********************************************************************/
static status_t
    do_yang_elem_case (ncx_module_t *mod,
                       obj_template_t *obj,
                       val_value_t *top)
{
    obj_case_t     *cas = obj->def.cas;
    xmlns_id_t xsd_id = xmlns_xs_id();
    status_t res = NO_ERR;

    /* next level is sequence */
    val_value_t *seq = xml_val_new_struct(XSD_SEQUENCE, xsd_id);
    if (!seq) {
        return ERR_INTERNAL_MEM;
    } else {
    ...
    val_value_t *annot = xsd_make_obj_annotation(obj, &res);
    if (res != NO_ERR) {
        return res;
    } else if (annot) {
    ...
    obj_template_t *child = (obj_template_t *)dlq_firstEntry(cas->datadefQ);
    for (; child != NULL && res==NO_ERR;
         child = (obj_template_t *)dlq_nextEntry(child)) {
        res = do_yang_elem_btype(mod, child, NULL, FALSE, seq);
    }
    if (res == NO_ERR) {
                         ...
                         val_value_t *val)
{
    ...
    for (casobj = (obj_template_t *)dlq_firstEntry(choic->caseQ);
        ...
        res = do_yang_elem_case(mod, casobj, top);
    }
...

{
    ...
    case OBJ_TYP_CHOICE:
        ...
        break;
    case OBJ_TYP_CASE:
        res = do_yang_elem_case(mod, obj, val);
        break;
    case OBJ_TYP_USES:
