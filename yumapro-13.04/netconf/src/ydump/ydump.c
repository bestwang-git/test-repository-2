                       ...
                       yangdump_cvtparms_t  *cp)
{
    ...
    } else if (val_find_child(valset, YANGDUMP_MOD, NCX_EL_NO_CONFIG)) {
    ...
    /* format parameter */
    val = val_find_child(valset, YANGDUMP_MOD, YANGDUMP_PARM_FORMAT);
    if (val && val->res == NO_ERR) {
    ...
    /* indent parameter */
    val = val_find_child(valset, YANGDUMP_MOD, YANGDUMP_PARM_INDENT);
    if (val && val->res == NO_ERR) {
    ...
    /* subdirs parameter */
    val = val_find_child(valset, YANGDUMP_MOD, NCX_EL_SUBDIRS);
    if (val && val->res == NO_ERR) {
    ...
    /* objview parameter */
    val = val_find_child(valset, YANGDUMP_MOD, YANGDUMP_PARM_OBJVIEW);
    if (val && val->res == NO_ERR) {
    ...
    /* output parameter */
    val = val_find_child(valset, YANGDUMP_MOD, YANGDUMP_PARM_OUTPUT);
    if (val && val->res == NO_ERR) {
    ...
    val = val_find_child(valset, YANGDUMP_MOD, NCX_EL_SHOW_ERRORS);
    if (val && val->res == NO_ERR) {
        cp->showerrorsmode = TRUE;
    }

    /* simurls parameter */
    val = val_find_child(valset, YANGDUMP_MOD, YANGDUMP_PARM_SIMURLS);
    if (val && val->res == NO_ERR) {
        cp->simurls = TRUE;
    }

    /* stats parameter */
    val = val_find_child(valset, YANGDUMP_MOD, YANGDUMP_PARM_STATS);
    if (val && val->res == NO_ERR) {
    ...
    /* subtree parameter */
    cp->subtreecount = val_instance_count(valset, NULL, 
    ...
    /* version parameter */
    val = val_find_child(valset, YANGDUMP_MOD, NCX_EL_VERSION);
    if (val && val->res == NO_ERR) {
        cp->versionmode = TRUE;
    }
    ...
    if (res == NO_ERR && pcb->top) {
        ...
        case NCX_CVTTYP_XSD:
            if (ncx_any_dependency_errors(pcb->top)) {
                ...
                res = xsd_convert_module(pcb, pcb->top, cp, &val, &attrs);
                if (res != NO_ERR) {
                    pr_err(res);
                } else {
                    if (cp->defnames || (cp->output && cp->output_isdir)) {
                        ...
                        /* output to the specified file */
                        res = 
                            xml_wr_check_open_file(stdout, val, &attrs,
                                                   DOCMODE, WITHHDR, TRUE,
                                                   0, cp->indent, NULL);
                    }
                }
                ...
                yangdump_cvtparms_t *cvtparms)
{
     ...
     */
    res = ncx_init(TRUE, LOG_DEBUG_INFO, FALSE, FALSE, FALSE, NULL,
                   argc, argv);
    ...
    if (res == NO_ERR) {
        ...
        res = ncxmod_load_module(YANGDUMP_MOD, NULL, NULL, NULL);
    }

    /* Initialize the Notifications namespace */
    if (res == NO_ERR) {
        res = xmlns_register_ns(NCN_URN, NCN_PREFIX, NCX_MODULE, 
                                NULL, &nsid);
    }
    ...
    if (res == NO_ERR && cvtparms->format == NCX_CVTTYP_XSD) {
        /* load in the NETCONF module for conversion namespace info */
        res = ncxmod_load_module(NC_MODULE, NULL, NULL, NULL);
    }
