                       ...
                       boolean uprotomode)
{
    ...
    if (cp->format != NCX_CVTTYP_YC && obj_is_leafy(obj) &&
        obj_get_basetype(obj) != NCX_BT_UNION) {
        /* generate code for newval and curval access vars */
        ses_indent(scb, indent);

        write_c_objtype_max(scb, obj, objnameQ, 0, TRUE, FALSE,
                            FALSE, FALSE, cp->isuser, FALSE, TRUE);
        ses_putstr(scb, (const xmlChar *)"newval_val = (newval) ? ");
        write_c_val_macro_type(scb, obj);
        ses_putstr(scb, (const xmlChar *)"(newval) : 0;");

        ses_indent(scb, indent);
        write_c_objtype_max(scb, obj, objnameQ, 0, TRUE, FALSE,
                            FALSE, FALSE, cp->isuser, FALSE, TRUE);
        ses_putstr(scb, (const xmlChar *)"curval_val = (curval) ? ");
        write_c_val_macro_type(scb, obj);
        ses_putstr(scb, (const xmlChar *)"(curval) : 0;");
    }

    if (cp->format == NCX_CVTTYP_YC && keycount) {
        ses_putstr_indent(scb, (const xmlChar *)"val_value_t *lastkey = NULL;",
                          indent);
        /* add the key local variables */
        write_c_key_vars(scb, obj, objnameQ, PARM_ERRORVAL, keycount, indent);
    }
                   ...
                   boolean uprotomode)
{
    ...
    ses_putchar(scb, '\n');
    ses_putstr_indent(scb, 
                      (const xmlChar *)"if (!agt_notifications_enabled()) {",
                      indent+extraindent);
    ses_putstr_indent(scb,
                      (const xmlChar *)"log_debug2(\"\\nSkipping <",
                      indent+indent+extraindent);
    ses_putstr(scb, obj_get_name(notifobj));
    ses_putstr(scb, (const xmlChar *)"> notification; disabled\");");
    ses_putstr_indent(scb,
                      (const xmlChar *)"return;", indent+indent+extraindent);
    ses_putstr_indent(scb, (const xmlChar *)"}\n", indent+extraindent);
