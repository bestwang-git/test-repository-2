...
#define AGT_NOT_SEQID_MOD   (const xmlChar *)"notifications"
                       ...
                       boolean checkfilter)
{
    ...
    ses_total_stats_t *totalstats = ses_get_total_stats();
    status_t res = NO_ERR;
    ...
    if (!notif->msg) {
        ...
        val_value_t *eventTime = 
            val_make_simval_obj(eventTimeobj, notif->eventTime, &res);
        if (!eventTime) {
        ...
        val_value_t *eventType = val_new_value();
        if (!eventType) {
            log_error("\nError: malloc failed: cannot send notification");
            val_free_value(topval);
            return ERR_INTERNAL_MEM;
        ...
        while (!dlq_empty(&notif->payloadQ)) {
            val_value_t *payloadval = 
                (val_value_t *)dlq_deque(&notif->payloadQ);
        ...
        if (checkfilter && profile->agt_notif_sequence_id) { 
            ...
            val_value_t *sequenceid = 
                val_make_simval_obj(sequenceidobj, numbuff, &res);
            if (!sequenceid) {
    ...
    if (checkfilter && sub->filterval) {
        ...
        case OP_FILTER_SUBTREE:
            filterpassed = 
                agt_tree_test_filter(&msghdr, sub->scb, sub->filterval,
                                     notif->msg);
            break;
        case OP_FILTER_XPATH:
            filterpassed = 
                agt_xpath_test_filter(&msghdr, sub->scb, sub->selectval,
                                      notif->msg);
            break;
        case OP_FILTER_NONE:
        default:
    ...
    if (filterpassed) {
        ...
        ses_start_msg_mode(sub->scb);
        xml_wr_full_val(sub->scb, &msghdr, notif->msg, 
                        ses_message_indent_count(sub->scb));
        ses_finish_msg(sub->scb);
        ses_stop_msg_mode(sub->scb);
        ...
        if (LOGDEBUG2) {
            ...
            if (notif->msg) {
                val_dump_value(notif->msg, ses_indent_count(sub->scb));
            }
            log_debug2("\n");
        }
    }
    ...
    agt_not_init (void)
{
    ...
    if (!agt_profile->agt_use_notifications) {
        log_debug2("\nagt_not: Skipping init1: notifications disabled");
        return NO_ERR;
    }

    /* load the notifications module */
    log_debug2("\nagt_not: Loading notifications module");
    status_t res = ncxmod_load_module(AGT_NOT_MODULE1, NULL, 
    ...
    agt_not_init2 (void)
{
    ...
    agt_profile_t *agt_profile = agt_get_profile();
    if (!agt_profile->agt_use_notifications) {
        log_debug2("\nagt_not: Skipping init2: notifications disabled");
        return NO_ERR;
    }
    ...
    agt_not_cleanup (void)
{
    if (agt_not_init_done) {
        ...
        agt_profile_t *agt_profile = agt_get_profile();
        if (!agt_profile->agt_use_notifications) {
            log_debug2("\nagt_not: Skipping cleanup: notifications disabled");
        } else {
        ...
        while (!dlq_empty(&subscriptionQ)) {
            agt_not_subscription_t *sub =
                (agt_not_subscription_t *)dlq_deque(&subscriptionQ);
            free_subscription(sub);
        }
        ...
        while (!dlq_empty(&notificationQ)) {
            agt_not_msg_t *msg =
                (agt_not_msg_t *)dlq_deque(&notificationQ);
            agt_not_free_notification(msg);
        }
    ...
    agt_not_queue_notification (agt_not_msg_t *notif)
{
    ...
    if (!profile->agt_use_notifications) {
        if (LOGWARN) {
            log_warn("\nWarning: event '%s:%s' dropped because "
                     "notifications disabled",
                     (notif->notobj) ? 
                     obj_get_mod_name(notif->notobj) : (const xmlChar *)"??",
                     (notif->notobj) ? 
