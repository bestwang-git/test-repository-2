    ...
    agt_top_dispatch_msg (ses_cb_t **ppscb)
{
    ...
    ses_total_stats_t *myagttotals = ses_get_total_stats();
    ...
    /* get the first node */
    status_t res = 
        agt_xml_consume_node(scb, &top, NCX_LAYER_TRANSPORT, NULL);
    if (res != NO_ERR) {
        scb->stats.inBadRpcs++;
    ...
    if (top.nodetyp==XML_NT_START || top.nodetyp==XML_NT_EMPTY) {
        ...
        top_handler_t handler = top_find_handler(top.module, top.elname);
        if (handler) {
            /* call the handler */
            (*handler)(scb, &top);
        } else {
    ...
    if (res != NO_ERR) {
        ...
        *ppscb=NULL;
        ...
        /* session was closed */
        agt_ses_kill_session(scb, scb->killedbysid, scb->termreason);
        /* set the supplied ptr to ptr to scb to NULL so that the 
         * caller of this function knows that it was deallotcated */
        *ppscb=NULL;
    }
