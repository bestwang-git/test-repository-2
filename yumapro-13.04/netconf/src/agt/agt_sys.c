...
static uint32 sysCapabilityChangeCount;
                         ...
                         val_value_t  *dstval)
{
    ...
    if (cbmode == GETCB_GET_VALUE) {
        ...
        const xmlChar *loglevelstr =
            log_get_debug_level_string(loglevel);
        if (loglevelstr == NULL) {
            res = ERR_NCX_OPERATION_FAILED;
        } else {
            res = val_set_simval_obj(dstval, dstval->obj, loglevelstr);
        }
    } else {
    ...
    send_sysStartup (void)
{
    ...
    if (!profile->agt_use_notifications) {
        log_debug2("\nDropping <sysStartup>  - notifications disabled");
        return;
    }

    if (!profile->agt_yuma_system_notifs) {
        log_debug2("\nDropping <sysStartup> notification: yuma disabled");
        return;
    }

    log_debug("\nagt_sys: generating <sysStartup> notification");

    agt_not_msg_t *not = agt_not_new_notification(sysStartupobj);
    if (!not) {
    ...
    agt_sys_init (void)
{
    ...
    /* load the latest version of the system module */
    res = ncxmod_load_module(AGT_SYS_MODULE, NULL, 
                             &agt_profile->agt_savedevQ, &sysmod);
    if (res != NO_ERR) {
        return res;
    }

    /* find the object definition for the system element */
    systemobj = ncx_find_object(sysmod, system_N_system);

    if (!systemobj) {
        return SET_ERROR(ERR_NCX_DEF_NOT_FOUND);
    }

    sysStartupobj = ncx_find_object(sysmod, system_N_sysStartup);
    if (!sysStartupobj) {
        return SET_ERROR(ERR_NCX_DEF_NOT_FOUND);
    }

    sysConfigChangeobj = ncx_find_object(sysmod, system_N_sysConfigChange);
    if (!sysConfigChangeobj) {
        return SET_ERROR(ERR_NCX_DEF_NOT_FOUND);
    }

    sysCapabilityChangeobj = ncx_find_object(sysmod,
                                             system_N_sysCapabilityChange);
    if (!sysCapabilityChangeobj) {
        return SET_ERROR(ERR_NCX_DEF_NOT_FOUND);
    }

    sysSessionStartobj = ncx_find_object(sysmod, system_N_sysSessionStart);
    if (!sysSessionStartobj) {
        return SET_ERROR(ERR_NCX_DEF_NOT_FOUND);
    }

    sysSessionEndobj = ncx_find_object(sysmod, system_N_sysSessionEnd);
    if (!sysSessionEndobj) {
        return SET_ERROR(ERR_NCX_DEF_NOT_FOUND);
    }

    sysConfirmedCommitobj = ncx_find_object(sysmod, 
    ...
    agt_sys_init2 (void)
{
    ...
    p = buffer;
    p += xml_strcpy(p, NCXMOD_NETCONFD);
    *p++ = ' ';

    res = ncx_get_version(p, 256 - (xml_strlen(NCXMOD_NETCONFD)+1));
    if (res == NO_ERR) {
    ...
    agt_sys_send_sysSessionStart (const ses_cb_t *scb)
{
    agt_profile_t *profile = agt_get_profile();
    if (!profile->agt_use_notifications) {
        log_debug2("\nDropping <sysSessionStart>  - notifications disabled");
        return;
    }

    if (scb->protocol == NCX_PROTO_YUMA_YANGAPI) {
        log_debug2("\nagt_sys: skipping <sysSessionStart> "
                   "notification for YANG-API session");
        return;
    }

    if (profile->agt_ietf_system_notifs) {
        y_ietf_netconf_notifications_netconf_session_start_send 
            (scb->username, scb->sid, scb->peeraddr);
    }
    ...
    if (profile->agt_yuma_system_notifs) {
        ...
        agt_not_msg_t *not = agt_not_new_notification(sysSessionStartobj);
        if (!not) {
                                ...
                                ses_id_t killedby)
{
    ...
    agt_profile_t *profile = agt_get_profile();
    if (!profile->agt_use_notifications) {
        log_debug2("\nDropping <sysSessionEnd>  - notifications disabled");
        return;
    }

    if (scb->protocol == NCX_PROTO_YUMA_YANGAPI) {
        log_debug2("\nagt_sys: skipping <sysSessionEnd> "
                   "notification for YANG-API session");
        return;
    }
    ...
    if (profile->agt_ietf_system_notifs) {
        y_ietf_netconf_notifications_netconf_session_end_send 
            (scb->username, scb->sid, scb->peeraddr, killedby,
             getTermReasonStr(termreason));
    }

    if (profile->agt_yuma_system_notifs) {
        log_debug("\nagt_sys: generating <sysSessionEnd> notification");

        agt_not_msg_t *not = agt_not_new_notification(sysSessionEndobj);
        if (!not) {
        ...
        const xmlChar *termreasonstr = getTermReasonStr(termreason);
                                  ...
                                  dlq_hdr_t *auditrecQ)
{
    ...
    assert(auditrecQ && "scb is NULL!");

    agt_profile_t *profile = agt_get_profile();
    if (!profile->agt_use_notifications) {
        log_debug2("\nDropping <sysConfigChange>  - notifications disabled");
        return;
    }

    if (profile->agt_ietf_system_notifs) {
        /* the server does not generate edits for the startup config
         * this function will only be called to report edits to
         * running config for now;
         * just generate the 'running' datastore 
         */
        y_ietf_netconf_notifications_netconf_config_change_send 
            (scb->username, scb->sid, scb->peeraddr,
             NCX_EL_RUNNING, auditrecQ);
    }
    ...
    if (profile->agt_yuma_system_notifs) {
        ...
        agt_not_msg_t *not = 
            agt_not_new_notification(sysConfigChangeobj);
        if (!not) {
            log_error("\nError: malloc failed; cannot "
        ...
        obj_template_t *listobj =
            obj_find_child(sysConfigChangeobj, AGT_SYS_MODULE,
                           system_N_edit);
        if (listobj == NULL) {
            ...
            for (auditrec = (agt_cfg_audit_rec_t *)dlq_firstEntry(auditrecQ);
                ...
                if (listval == NULL) {
                    ...
                    status_t  res = NO_ERR;
                    val_value_t *leafval = 
                        agt_make_leaf(listobj, system_N_target,
                                      auditrec->target, &res);
                    if (leafval) {
                    ...
                    leafval = agt_make_leaf(listobj, system_N_operation,
...
{
    sysCapabilityChangeCount++;

    assert(capstr && "capstr is NULL!");

    agt_profile_t *profile = agt_get_profile();
    if (!profile->agt_use_notifications) {
        log_debug2("\nDropping <sysCapabilityChange> - "
                   "notifications disabled");
        return;
    }

    if (profile->agt_ietf_system_notifs) {
        /* the server does not generate capability changes
         * for modified capabilities.  There are no client
         * operations to enable/disable features independently
         * of the YANG module being loaded
         */
        const xmlChar *username = changed_by ? changed_by->username : NULL;
        const xmlChar *srcaddr = changed_by ? changed_by->peeraddr : NULL;
        ses_id_t sesid = changed_by ? changed_by->sid : 0;

        cap_change_t cap_change = 
            is_add ? CAP_CHANGE_ADD : CAP_CHANGE_DELETE;

        y_ietf_netconf_notifications_netconf_capability_change_send 
            (username, sesid, srcaddr, cap_change, capstr);
    }
    ...
    if (profile->agt_yuma_system_notifs) {
        ...
        obj_template_t *changedbyobj = 
            obj_find_child(sysCapabilityChangeobj,
                           AGT_SYS_MODULE,
                           system_N_changed_by);
        if (!changedbyobj) {
            SET_ERROR(ERR_NCX_DEF_NOT_FOUND);
        ...
        agt_not_msg_t *not =
            agt_not_new_notification(sysCapabilityChangeobj);
        if (!not) {
            log_error("\nError: malloc failed; cannot "
        ...
        val_value_t *changedbyval = val_new_value();
        if (!changedbyval) {
            log_error("\nError: malloc failed; cannot "
                      "send <sysCapabilityChange>");
        ...
        val_value_t *leafval = NULL;
        status_t res = NO_ERR;
        if (changed_by) {
            /* add userName */
            if (changed_by->username) {
                leafval = agt_make_leaf(changedbyobj, system_N_userName,
                                        changed_by->username, &res);
                if (leafval) {
            ...
            leafval = agt_make_leaf(changedbyobj, NCX_EL_SERVER,
                                    NULL, &res);
            if (leafval) {
                val_add_child(leafval, changedbyval);
            } else {
                                     ...
                                     uint32 timeout)
{
    agt_profile_t *profile = agt_get_profile();
    if (!profile->agt_use_notifications) {
        log_debug2("\nDropping <sysConfirmedCommit> - "
                   "notifications disabled");
        return;
    }

    const xmlChar *eventstr = ncx_get_confirm_event_str(event);
    if (!eventstr) {
        SET_ERROR(ERR_INTERNAL_VAL);
        return;
    }
    ...
    if (profile->agt_ietf_system_notifs) {
        const xmlChar *username = scb ? scb->username : NULL;
        const xmlChar *srcaddr = scb ? scb->peeraddr : NULL;
        ses_id_t sesid = scb ? scb->sid : 0;

        y_ietf_netconf_notifications_netconf_confirmed_commit_send 
            (username, sesid, srcaddr, eventstr, event, timeout);
    }

    if (profile->agt_yuma_system_notifs) {
        if (LOGDEBUG) {
            log_debug("\nagt_sys: generating <sysConfirmedCommit> "
                      "notification (%s)", eventstr);
        }

        agt_not_msg_t *not = 
            agt_not_new_notification(sysConfirmedCommitobj);
        if (!not) {
            log_error("\nError: malloc failed; cannot "
        ...
        status_t res = NO_ERR;
        val_value_t *leafval = 
            agt_make_leaf(sysConfirmedCommitobj,
...
/********************************************************************
* FUNCTION agt_sys_get_capability_changes
*
...
*********************************************************************/
uint32
    agt_sys_get_capability_changes (void)
{
    return sysCapabilityChangeCount;
}
