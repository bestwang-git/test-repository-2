                 ...
                 boolean *keepempty)
{
    *keepempty = FALSE;
     ...
     * !!! xml parser is used.
     */
    xmlns_id_t wildid = xmlns_wildcard_id();
     ...
     */
    boolean anycon = FALSE;
    boolean anysel = FALSE;
    boolean test = FALSE;

    /* check if this is a real or a virtual value */
    val_value_t *virtualval = NULL;
    val_value_t *useval = NULL;
    if (val_is_virtual(curval)) {
        virtualval = val_get_virtual_value(scb, curval, &res);
        if (virtualval == NULL) {
            return res;
        } else {
            useval = virtualval;
     ...
     * set is rejected
     */
    val_value_t *filchild = val_get_first_child(filval);
    val_value_t *curchild = NULL;
    for (; filchild != NULL; filchild = val_get_next_child(filchild)) {
        ...
        test = FALSE;
        for (curchild = val_first_child_qname(useval, filchild->nsid,
                                              filchild->name);
             curchild != NULL && !test;
             curchild = val_next_child_qname(useval, filchild->nsid,
                                             filchild->name, curchild)) {

            /* check access control for notifications only */
            if (isnotif && scb &&
                !agt_acm_val_read_allowed(msg, scb->username, 
                                          curchild)) {
            ...
            test = content_match_test(scb, VAL_STR(filchild), curchild);
        }
    ...
    for (filchild = val_get_first_child(filval);
         ...
         */
        if (!getop && !agt_check_config(msg, ses_withdef(scb), 
                                        TRUE, filchild)) {
            continue;
        }

        /* if filchild is a selection node, make sure it is
         * not a duplicate; if so skip this node
         */
        if (filchild->btyp == NCX_BT_EMPTY) {
            val_value_t *testval = val_get_first_child(filval);
            boolean skip = FALSE;
            for (; !skip && testval && (testval != filchild); 
                 testval = val_get_next_child(testval)) {

                if (testval->btyp != NCX_BT_EMPTY) {
                    /* not a selection node */
                    continue;
                }

                if (xml_strcmp(testval->name, filchild->name)) {
                    /* not the same name selection node */
                    continue;
                }

                if (testval->nsid == 0) {
                    /* this node is a wildcard-NS so force the original
                     * selection node to be a wildcard-NS and skip this
                     * node instead   */
                    filchild->nsid = 0;
                    skip = TRUE;
                } else if (filchild->nsid == 0) {
                    /* the previous found node is a wildcard-NS
                     * so skip this node   */
                    skip = TRUE;
                } else if (testval->nsid == filchild->nsid) {
                    /* the previous found node is the same NS
                     * so skip this node   */
                    skip = TRUE;
                }  // else different NS, continue
            }

            if (skip) {
                log_debug2("\nagt_tree: Skipping duplicate selection "
                           "node '%s'", filchild->name);
                continue;
            }
        }
        ...
        for (curchild = val_first_child_qname(useval, filchild->nsid,
            ...
            ncx_filptr_t *filptr = NULL;

            /* check access control if scb is non-NULL */
            if (isnotif && scb && 
                !agt_acm_val_read_allowed(msg, scb->username, curchild)) {
                continue;
            }
            ...
            case NCX_BT_CONTAINER:
                 ...
                 */
                boolean mykeepempty = FALSE;
                res = process_val(msg, scb, getop, isnotif,
                                  filchild, curchild, filptr, 
                                  &mykeepempty);
                if (res != NO_ERR) {
                    return res;
                }
            ...
            if (filptr && 
                 ...
                 * get filtered out
                 */
                val_index_t *valindex = val_get_first_index(filptr->node);
                for (; valindex != NULL;
...
static void
    output_node (ses_cb_t *scb, 
                 ...
                 boolean getop)
{
    int32 indentamount = ses_indent_count(scb);
    ...
    for (filptr = (ncx_filptr_t *)dlq_firstEntry(&parent->childQ);
        ...
        val_value_t *val = filptr->node;
        xmlns_id_t valnsid = obj_get_nsid(val->obj);
        xmlns_id_t parentnsid = 0;
        if (val->parent) {
        ...
        if (dlq_empty(&filptr->childQ)) {
            if (getop) {
                xml_wr_full_check_val(scb, &msg->mhdr, val, 
             ...
             */
            boolean retval = agt_acm_val_read_allowed(&msg->mhdr,
                                                      scb->username, val);
            ...
            if (retval) {
                xml_wr_begin_elem_ex(scb, &msg->mhdr, parentnsid,
                                     valnsid, val->name, &val->metaQ, 
                                     FALSE, indent, FALSE);
                ...
                xml_wr_end_elem(scb, &msg->mhdr, valnsid, val->name, 
                                indent);
            }
        }
    }
                      ...
                      int32 indent)
{
    ...
    if (LOGDEBUG2) {
        log_debug2("\n "); /* Note: the extra space prevents logger
			      from completely discarding the output. */
        for (i=0; i<indent; i++) {
	    log_debug2_append(" ");
         }

        log_debug2_append("filptr: %u:%s %s",
                   filptr->node->nsid,
                   filptr->node->name,
                          ...
                          val_value_t *topval)
{
    ...
    case NCX_BT_CONTAINER:
        ...
        keepempty = FALSE;        
        res = process_val(msghdr, scb, TRUE, TRUE, filter, 
                          topval, top, &keepempty);
        if (res != NO_ERR || dlq_empty(&top->childQ)) {
            /* ignore keepempty because the result will
             * be the same w/NULL return, just faster
             */
