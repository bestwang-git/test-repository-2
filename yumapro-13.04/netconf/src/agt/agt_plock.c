        ...
        xml_node_t *methnode)
{
    ...
    if (res != NO_ERR) {
        ...
        if (res == ERR_NCX_LOCK_DENIED) {
            /* need to get the session ID that has the lock */
            const xmlChar *locktime = NULL;
            ses_id_t sid = 0;
            parmtyp = NCX_NT_UINT32_PTR;
            parm = &sid;
            status_t res2 = 
                cfg_get_global_lock_info(running, &sid, &locktime);
            if (res2 != NO_ERR) {
                log_error("\nError: failed to get global lock info (%s)",
                          get_error_string(res2));
            }
        }
        agt_record_error(scb, &msg->mhdr, NCX_LAYER_OPERATION, res,
                         methnode, parmtyp, parm, NCX_NT_NONE, NULL);
        return res;
    }
