...
/********************************************************************
* FUNCTION check_max_sessions
*
* Check if max-sessions parameter has been reached
*
* RETURNS:
*    TRUE if max-sessions reached; FALSE if limit not reached
*********************************************************************/
static boolean
    check_max_sessions (void)
{
    agt_profile_t *profile = agt_get_profile();
    if (profile->agt_max_sessions == 0) {
        return FALSE;
    }

    uint32 last = (next_sesid) ? next_sesid : AGT_SES_MAX_SESSIONS;
    uint32 count = 0;
    uint32 i = 1;
    for (; i < last; i++) {
        if (agtses[i]) {
            if (++count >= profile->agt_max_sessions) {
                return TRUE;
            }
        }
    }
...
#ifdef WITH_CLI
    //agt_cli_proto_free_cb(scb->cli_cb);
#endif
                           ...
                           ses_term_reason_t termreason)
{
    ...
    case SES_ST_SHUTDOWN_REQ:
        agt_ses_kill_session(scb, killedby, termreason);
        break;
    case SES_ST_IN_MSG:
    ...
    agt_ses_process_first_ready (void)
{
    ...
    if (scb->reader) {
        /* reset the xmlreader */
        res = xml_reset_reader_for_session(ses_read_cb, NULL, scb, 
                                           scb->reader);
    } else {
        res = xml_get_reader_for_session(ses_read_cb, NULL, scb, 
                                         &scb->reader);
    }
    ...
    agt_ses_check_timeouts (void)
{
     ...
     */
    boolean no_timeouts =
        (agt_profile->agt_idle_timeout == 0 &&
         agt_profile->agt_hello_timeout == 0);

    if (no_timeouts && !agt_ncx_cc_active()) {
        return;
    }
    ...
    double timediff = difftime(timenow, last_timeout_check);
    ...
    for (i=1; i < last; i++) {
         ...
         */
        if (agt_profile->agt_idle_timeout > 0 && scb->active 
#ifndef EVAL_VERSION
            // just let a normal session stay up if notifs are active
            && !scb->notif_active
#endif
            ) {
#ifdef EVAL_VERSION
            /* use the total time this session has been active
             * not the last rpc time   */
            timediff = difftime(timenow, scb->hello_time);
#else
