                  ...
                  xml_node_t *methnode)
{
     ...
     */
    xml_msg_set_cfgid(&msg->mhdr, NCX_CFGID_RUNNING);
                         ...
                         xml_node_t *methnode)
{
    ...
    status_t res =
        agt_get_cfg_from_parm(NCX_EL_SOURCE, msg, methnode, &source);
    if (res != NO_ERR) {
        return res;  /* error already recorded */
    } 
    ...
    /* check the with-defaults parameter */
    status_t res2 = agt_set_with_defaults(scb, msg, methnode);
    ...
    boolean empty_callback = FALSE;
    val_value_t *testval =
        val_find_child(msg->rpc_input, y_yuma_time_filter_M_yuma_time_filter,
                       IF_MODIFIED_SINCE);
    if (testval != NULL && testval->res == NO_ERR) {
        boolean isneg = FALSE;
        xmlChar *utcstr;
     ...
     * by the agt_rpc module
     */
    xml_msg_set_cfgid(&msg->mhdr, source->cfg_id);
    msg->rpc_user1 = source;
    msg->rpc_data_type = RPC_DATA_STD;
    if (empty_callback) {
        msg->rpc_datacb = agt_output_empty;
                        ...
                        xml_node_t *methnode)
{
    ...
    edit_parms_t *editparms = (edit_parms_t *)msg->rpc_user1;
    op_editop_t defop = editparms->defop;
    op_testop_t testop = editparms->testop;
    cfg_template_t *target = editparms->target;
    val_value_t *urlval = editparms->urlval;
    val_value_t *srcval = editparms->srcval;
    ...
    status_t res = agt_val_apply_write(scb, msg, target, srcval, defop);
                          ...
                          xml_node_t *methnode)
{
   ...
   res = validate_copy_source( scb, msg, methnode, &srccfg, 
    ...
    if ( NO_ERR == res && (srcval || srcurlval) && destcfg && 
         destcfg->cfg_id != NCX_CFGID_STARTUP) {

        val_value_t *useval = (srcval) ? srcval : srcurlval;
        ...
        if (res == NO_ERR) {
            ...
            res = agt_val_validate_write( scb, msg, destcfg, useval, 
                                          OP_EDITOP_REPLACE );
                ...
                case NCX_CFGID_RUNNING:
                    {
                        ...
                        if (cfg_get_dirty_flag(candidate)) {
                            res = cfg_fill_candidate_from_running();
                        } else if (LOGDEBUG2) {
                            log_debug2("\nagt_ncx: skip copy R to C, not dirty");
                        }
                    }
                    break;
                case NCX_CFGID_STARTUP:
                          ...
                          xml_node_t *methnode)
{
    ...
    if ( !destfile ) {
        ...
        destfile = ncxmod_find_data_file( startspec, FALSE, FALSE, &res );
        if ( !destfile ) {
                           ...
                           xml_node_t *methnode)
{
     ...
     */
    if (VAL_UINT(val) == SES_MY_SID(scb)
    ...
    if (res == NO_ERR) {
        if (!xml_strcmp(child->name, NCX_EL_RUNNING)) {
            ...
            if (urlspec || res == NO_ERR) {
                urlfilename =
                    ncxmod_find_data_file(urlspec, FALSE, FALSE, &res);
                  ...
                  cfg_template_t *cfg)
{
    ...
    /* output to the specified file or STDOUT */
    status_t res = xml_wr_check_file(filespec, cfg->root, &attrs, XMLMODE,
                                     WITHHDR, TRUE, 0, profile->agt_indent,
                                     agt_check_save);
                   ...
                   xml_node_t *methnode)
{
    status_t res = NO_ERR;
    boolean errdone = FALSE;
    boolean timeout_extended = FALSE;
    val_value_t *errval = NULL;
    ...
    if (commit_cb.cc_active) {
        ...
        if (confirmedval != NULL) {
            ...
            if (res == NO_ERR) {
                agt_sys_send_sysConfirmedCommit(scb, NCX_CC_EVENT_COMPLETE, 0);
        ...
        if (confirmedval != NULL) {
            ...
            if (persistval != NULL) {
                ...
                commit_cb.cc_persist_id = xml_strdup(VAL_STR(persistval));
                if (commit_cb.cc_persist_id == NULL) {
                    res = ERR_INTERNAL_MEM;
                }
            }
     ...
     */ 
    if (res == NO_ERR) {
        res = agt_ncx_internal_commit(scb, msg,
                                      !timeout_extended,
                                      save_nvstore,
                                      commit_cb.cc_ses_id,
                            ...
                            xml_node_t *methnode)
{
    ...
    val_value_t *persistidval = NULL;
    val_value_t *errval = NULL;
    const xmlChar *cc_persistid = NULL;
    ...
    if (res == NO_ERR) {
        ...
        if (persistidval == NULL && cc_persistid == NULL) {
            ...
            errobj = obj_find_child(msg->rpc_input->obj,
                                    val_get_mod_name(msg->rpc_input),
                                    NCX_EL_PERSIST_ID);
        } else if (persistidval != NULL && cc_persistid == NULL) {
    ...
    if (res != NO_ERR) {
        ncx_node_t parmtyp = NCX_NT_NONE;
        void *parm = NULL;
        if (errval) {
            parm = errval;
            parmtyp = NCX_NT_VAL;
                           ...
                           xml_node_t *methnode)
{
    ...
    if (!candidate) {
        ...
        log_debug2("\nagt_ncx: skip discard-changes, candidate not dirty");
    }
                          ...
                          xml_node_t *methnode)
{
    ...
    msg->rpc_txcb = 
        agt_cfg_new_transaction(NCX_CFGID_RUNNING, AGT_CFG_EDIT_TYPE_FULL,
                                FALSE, /* rootcheck */
                                FALSE, /* is_validate */
                                TRUE, /* is_rollback: load_running also */
                                &res);
    if (msg->rpc_txcb == NULL || res != NO_ERR) {
        if (res == NO_ERR) {
            res = ERR_NCX_OPERATION_FAILED;
                 ...
                 xml_node_t *methnode)
{
    ...
    boolean errdone = FALSE;
    boolean sil_loaded = FALSE;
    boolean module_added = FALSE;
    agt_profile_t *agt_profile = agt_get_profile();

    /* mandatory module name */
    val_value_t *modval =
        val_find_child(msg->rpc_input, AGT_SYS_MODULE, NCX_EL_MODULE);
    ...
    val_value_t *revval =
        val_find_child(msg->rpc_input, AGT_SYS_MODULE, NCX_EL_REVISION);
    ...
    if (mod == NULL) {
        ...
        ncx_module_t *testmod =
            ncx_find_module(VAL_STR(modval), VAL_STR(revval));
        if (testmod != NULL) {
            mod = testmod;
        } else {
    ...
    if (res == NO_ERR && mod && module_added) {
        ...
        /*** FIXME: !!! add any imported modules loaded ***/
    }
    ...
    if (res == NO_ERR && mod && module_added && ncx_use_xpath_backptrs() ) {
        /* add the module Xpath back pointers */
        res = ncx_set_obj_xpath_backptrs(mod);
        /*** FIXME: !!! add any imported modules loaded ***/
    ...
    if (res == NO_ERR && mod && module_added) {
        ...
        xmlChar *moduri = cap_make_moduri(mod);
        if (!moduri) {
            res = ERR_INTERNAL_MEM;
        } else {
                    ...
                    xml_node_t *methnode)
{
    ...
    log_write("\n\n**************"
              ...
              "on session %u at %s\n\n",
              scb->username, SES_MY_SID(scb), timebuff);
...
/********************************************************************
* FUNCTION backup_validate
*
* backup : validate params callback
*
* INPUTS:
*    see agt/agt_rpc.h
* RETURNS:
                     ...
                     xml_node_t *methnode)
{
    ...
    if (agt_backups_enabled()) {
        testval = val_find_child(msg->rpc_input, AGT_YWSYS_MODULE,
                                 NCX_EL_FILENAME);
        if (testval != NULL && testval->res == NO_ERR) {
            filename = VAL_STR(testval);
        } else if (testval) {
            res = testval->res;
        } else {
            res = ERR_NCX_MISSING_PARM;
        ...
        if (filename != NULL && res == NO_ERR) {
            xmlChar *srcfile = ncxmod_find_data_file(filename, FALSE, 
                                                     TRUE, &res);
            if (srcfile && res == NO_ERR) {
                res = ERR_NCX_DATA_EXISTS;
            } else {
                res = NO_ERR;
                msg->rpc_user1 = VAL_STR(testval);
            }
            m__free(srcfile);
        }
    } else {
    ...
    if (res != NO_ERR) {
        agt_record_error(scb, &msg->mhdr, NCX_LAYER_OPERATION, res, methnode,
                         testval ? NCX_NT_VAL : NCX_NT_NONE, testval,
                         testval ? NCX_NT_VAL : NCX_NT_NONE, testval);
    }
...
/********************************************************************
* FUNCTION backup_invoke
*
* backup : invoke callback
* 
* INPUTS:
*    see agt/agt_rpc.h
* RETURNS:
...
*********************************************************************/
static status_t 
    backup_invoke (ses_cb_t *scb,
                   rpc_msg_t *msg,
                   xml_node_t *methnode)
{
    status_t res = NO_ERR;

    const xmlChar *filename = (const xmlChar *)msg->rpc_user1;

    xmlChar *srcfile = ncxmod_make_data_filespec(filename, TRUE, &res);
    if (srcfile && res == NO_ERR) {
        // save the backup
        cfg_template_t *running = cfg_get_config_id(NCX_CFGID_RUNNING);
        res = write_config(srcfile, running);
    }

    if (res == NO_ERR) {
        /* add to the backup file list in state monitoring module */
        res = agt_state_add_backup_file(filename);
    }
...
/********************************************************************
* FUNCTION delete_backup_validate
*
* delete-backup : validate params callback
*
* INPUTS:
*    see agt/agt_rpc.h
* RETURNS:
...
*********************************************************************/
static status_t 
    delete_backup_validate (ses_cb_t *scb,
                            rpc_msg_t *msg,
                            xml_node_t *methnode)
{
    status_t res = NO_ERR;
    ...
    if (agt_backups_enabled()) {
        ...
        if (filename != NULL && res == NO_ERR) {
            ...
            if (srcfile != NULL && res == NO_ERR) {
                msg->rpc_user1 = srcfile;  // malloced memory
                msg->rpc_user2 = filename; // const pointer
            } else {
                m__free(srcfile);
            }
        }
...
} /* delete_backup_validate */


/********************************************************************
* FUNCTION delete_backup_invoke
*
* delete-backup : invoke callback
* 
* INPUTS:
*    see agt/agt_rpc.h
* RETURNS:
...
*********************************************************************/
static status_t 
    delete_backup_invoke (ses_cb_t *scb,
                          rpc_msg_t *msg,
                          xml_node_t *methnode)
{
    status_t res = NO_ERR;

    xmlChar *filespec = (xmlChar *)msg->rpc_user1;
    const xmlChar *filename = (xmlChar *)msg->rpc_user2;

    // delete the backup
    int ret = remove((const char *)filespec);
    if (ret) {
        res = errno_to_status();
    }

    if (res == NO_ERR) {
        /* remove from netconf monitoring module */
        agt_state_delete_backup_file(filename);
    }
    ...
    m__free(filespec);  // live memory malloced in validate fn

    return res;

} /* delete_backup_invoke */


/********************************************************************
* FUNCTION restore_validate
*
* restore : validate params callback
*
* INPUTS:
*    see agt/agt_rpc.h
* RETURNS:
                      ...
                      xml_node_t *methnode)
{
    ...
    if (agt_backups_enabled()) {
        /* check global lock on running config */
        cfg_template_t *running = cfg_get_config_id(NCX_CFGID_RUNNING);
        res = cfg_ok_to_write(running, SES_MY_SID(scb));

        /* check already a confirmed-commit in progress */
        if (res == NO_ERR && agt_ncx_cc_active()) {
            res = ERR_NCX_IN_USE_COMMIT;
        }
...
/********************************************************************
* FUNCTION restore_invoke
*
* restore : invoke callback
* 
* INPUTS:
*    see agt/agt_rpc.h
* RETURNS:
...
*********************************************************************/
static status_t 
    restore_invoke (ses_cb_t *scb,
                    rpc_msg_t *msg,
                    xml_node_t *methnode)
{
    status_t res = NO_ERR;

    xmlChar *filename = (xmlChar *)msg->rpc_user1;

    // restore the backup
    cfg_template_t *running = cfg_get_config_id(NCX_CFGID_RUNNING);

    res = agt_ncx_load_backup(filename, running, SES_MY_SID(scb));
    ...
    register_nc_callbacks (void)
{
    ...
    if (profile->agt_yumaworks_system) {
        ...
        /* backup extension */
        res = agt_rpc_register_method(AGT_YWSYS_MODULE,
                                      NCX_EL_BACKUP,
                                      AGT_RPC_PH_VALIDATE,  
                                      backup_validate);
        if (res != NO_ERR) {
    ...
    unregister_nc_callbacks (void)
{
    ...
    if (profile->agt_yumaworks_system) {
        /* backup extension */
        agt_rpc_unregister_method(AGT_YWSYS_MODULE, NCX_EL_BACKUP);

        /* delete-backup extension */
        agt_rpc_unregister_method(AGT_YWSYS_MODULE, NCX_EL_DELETE_BACKUP);

        /* restore extension */
        agt_rpc_unregister_method(AGT_YWSYS_MODULE, NCX_EL_RESTORE);
    }
                      ...
                      boolean bkup)
{
    ...
    xmlChar *filebuffer = NULL;
    cfg_template_t *startup = NULL;
    val_value_t *copystartup = NULL;
    status_t res = ERR_NCX_OPERATION_NOT_SUPPORTED;
    agt_profile_t *profile = agt_get_profile();
...
{
    status_t res =
        agt_rpc_load_config_file(filespec, cfg,
                                 FALSE, /* isload */
                                 use_sid);
    return res;
                                     ...
                                     ncx_confirm_event_t event)
{
    ...
    agt_sys_send_sysConfirmedCommit(scb, event, 0);
...
/********************************************************************
* FUNCTION agt_ncx_internal_commit
*
* Apply a commit from candidate to running
* Internal access for YANG-API
*
* All validation should have already been done
*
* INPUTS:
...
*   rollback_sid == session ID to use for the load-backup in case
*                   the commit fails and the undo fails
*   backup_source == address of backup filename string var
*   errdone == address of return error done flag
*
* OUTPUTS:
*   *backup_source set to malloced file spec  if it was NULL
*    or else the string already set will be used for the backup target
...
*********************************************************************/
status_t
    agt_ncx_internal_commit (ses_cb_t  *scb,
                             rpc_msg_t *msg,
                             ...
                             boolean *errdone)
{
    ...
    *errdone = FALSE;
    ...
    if (*backup_source == NULL) {
        ...
        xmlChar *fname =
            ncxmod_find_data_file(NCX_DEF_BACKUP_FILE, FALSE, FALSE, &res);
        if (fname) {
            ...
            *backup_source = 
            ...
            res = NO_ERR;
            *backup_source = 
                ncxmod_make_data_filespec(NCX_DEF_BACKUP_FILE, FALSE, &res);
        }
    }

    /* attempt to save the backup, if a file name is available */
    if (res == NO_ERR && write_backup) {
        res = write_config(*backup_source, running);
    }
    ...
    if (res == NO_ERR) {
        ...
        if (res != NO_ERR) {
            *errdone = TRUE;
            ...
            if (msg->rpc_txcb->rollback_res != NO_ERR) {
                ...
                status_t res2 = 
                    agt_ncx_load_backup(*backup_source, running,
                                        rollback_sid);
                if (res2 != NO_ERR) {
                    res = res2;
                    *errdone = FALSE;
        ...
        } else if (msg->rpc_txcb &&
                   (msg->rpc_txcb->fill_candidate_failed ||
                    !msg->rpc_txcb->is_commit)) {
            res = cfg_fill_candidate_from_running();
        }
    }
