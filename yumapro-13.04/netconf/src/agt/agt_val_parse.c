    ...
    get_editop (const xml_node_t  *node)
{
    const xml_attr_t *attr =
        xml_find_ro_attr(node, xmlns_nc_id(), NC_OPERATION_ATTR_NAME);
    if (!attr) {
                    ...
                    obj_template_t *obj)
{
    ...
    boolean ret = obj_get_config_flag2(obj, &setflag);
...
{
    /* init local vars */
    const xml_node_t *errnode = startnode;
    status_t res = NO_ERR;
    boolean done = FALSE;
    boolean getstrend = FALSE;
    ...
    case XML_NT_START:
        ...
        break;
    case XML_NT_EMPTY:
        /* treat this 'any' as an 'empty' data type */
                    ...
                    val_value_t  *retval)
{
    ...
    if (retval->editop == OP_EDITOP_DELETE ||
        ...
        res = xml_node_match(startnode, obj_get_nsid(obj), NULL, 
                             XML_NT_START); 
    }
    ...
    if (res == NO_ERR && !empty) {
        ...
        case XML_NT_STRING:
            /* get the non-whitespace string here */
            res = val_union_ok_ex(obj_get_typdef(obj), valnode.simval, 
                                  retval, &errinfo, NULL, &match_typdef);
            if (res != NO_ERR) {
                badval = valnode.simval;
            }
            break;
        case XML_NT_END:
            stopnow = TRUE;
            res = val_union_ok_ex(obj_get_typdef(obj), EMPTY_STRING, 
                                  retval, &errinfo, NULL, &match_typdef);
            if (res != NO_ERR) {
                badval = EMPTY_STRING;
            }
        ...
        if (res == NO_ERR) {
            typ_def_t *usedef = (match_typdef) ? match_typdef : retval->typdef;
            const xmlChar *usestr = stopnow ? EMPTY_STRING : valnode.simval;
            res = val_set_simval(retval, usedef, retval->nsid,
                                 retval->name, usestr);
        }
    ...
    for (attr = xml_get_first_attr(node);
        ...
        if (val_match_metaval(attr, ncid, NC_OPERATION_ATTR_NAME)) {
            ...
            if (!retval->editvars->insertstr) {
                retval->editvars->insert_mode = VAL_INS_MODE_KEY;
            ...
            if (!retval->editvars->insertstr) {
                retval->editvars->insert_mode = VAL_INS_MODE_VALUE;
                retval->editvars->insertstr = xml_strdup(attr->attr_val);
                if (!retval->editvars->insertstr) {
                    res = ERR_INTERNAL_MEM;
                } else {
        ...
        if (metadef) {
            ...
            if (!metaval) {
                ...
                /* make sure some object is set */
                val_init_from_template(metaval, ncx_get_gen_string());

                /* parse the attribute string against the typdef */
                res = val_parse_meta(scb, metadef, attr, metaval);
                if (res == NO_ERR) {
        ...
        if (res != NO_ERR) {
            if (retval->obj == NULL) {
                agt_record_attr_error(scb, msg, NCX_LAYER_OPERATION, res,  
                                      attr, node, NULL, NCX_NT_OBJ, obj);
            } else {
                agt_record_attr_error(scb, msg, NCX_LAYER_OPERATION, res,  
                                      attr, node, NULL, NCX_NT_VAL, retval);
            }
            CHK_EXIT(res, retres);
        }
    }
                         ...
                         val_value_t  *val)
{
    ...
    if (res == NO_ERR && val->editvars && val->editvars->insertstr) {
        if (val->editvars->insert_mode == VAL_INS_MODE_KEY) {
                    ...
                    val_value_t  *retval)
{
    ...
    if (retval->obj == NULL) {
         ...
         * wants to record an error -- leave the node NULL so 
         * the obj node will get used instead!!
         */
        //retval->obj = ncx_get_gen_anyxml();
    }
