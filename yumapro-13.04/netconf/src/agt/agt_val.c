...
/********************************************************************
 * FUNCTION cvt_editop_ex
 * Determine the effective edit (if OP_EDITOP_COMMIT)
 * Do not convert COMMIT to REPLACE same as cvt_editop
 *
 * \param editop the edit operation to check
 * \param newnode the newval
 * \param curnode the curval
 ...
 * \return the converted editoperation
 *********************************************************************/
static op_editop_t cvt_editop_ex ( op_editop_t editop,
                                   const val_value_t *newval,
                                   const val_value_t *curval)
{
    op_editop_t retop = editop;
                         ...
                         val_value_t *curnode)
{
    ...
    if (usenode && usenode->parent == NULL &&
        usenode->obj && !obj_is_root(usenode->obj)) {
        log_debug2("\nagt_val: skip audit-record for deleted node");
        return;
    }
    ...
    if (msg && msg->rpc_txcb) {
        ...
        tstamp_time2datetime(&msg->rpc_txcb->timestamp, tbuff);
        got_time = TRUE;
    } else {
        log_debug("\nagt_val: no transaction record for commit");
                             ...
                             val_value_t *curnode)
{
    ...
    for (newchild = val_get_first_child(newnode);
        ...
        if (curnode != NULL) {
            curchild = val_first_child_match_fast(curnode, newchild, NULL);
        } else {
            curchild = NULL;
        }
    ...
    for (curchild = val_get_first_child(curnode);
        ...
        newchild = val_first_child_match_fast(newnode, curchild, NULL);
        if (newchild == NULL) {
                          ...
                          boolean indelete)
{
    ...
    while (!done) {
        ...
        if (cbset != NULL && cbset->cbfn[cbtyp] != NULL) {
            ...
            if (LOGDEBUG2) {
                log_debug2("\nFound %s user callback for %s:%s:%s",
                           agt_cbtype_name(cbtyp),
                           op_editop_name(editop),
                           val_get_mod_name(val),
                           val->name);
                   ...
                   val_value_t *parentnode)
{
    ...
    agt_cfg_undo_rec_t *undo = agt_cfg_new_undorec();
    if (!undo) {
        return NULL;
    }

    /* save a copy of the current value in case it gets modified
     * in a merge operation;; done for leafs and NP-containers
     * set-by-default only
     */
    if (curnode && curnode->obj &&
        (obj_is_leafy(curnode->obj) ||
         (obj_is_np_container(curnode->obj) && 
          obj_npcon_has_defaults(curnode->obj)))) {
    ...
    undo->editop = cvt_editop_ex(editop, newnode, curnode);
    ...
    if (editop != OP_EDITOP_LOAD && ncx_use_xpath_backptrs()) {
        val_value_t *useval = newnode ? newnode : curnode;
        ncx_cfg_t cfg_id = msg->rpc_txcb->cfg_id;
        if (useval && useval->obj) {
            obj_flag_xpath_backptrs_dirty(useval->obj, cfg_id, TRUE);
        }
    }
...
*********************************************************************/
static void
    finish_extra_deletes (agt_cfg_transaction_t *txcb,
                          agt_cfg_undo_rec_t *undo)
{

    boolean is_running = (txcb->cfg_id == NCX_CFGID_RUNNING);
    ...
    while (!dlq_empty(&undo->extra_deleteQ)) {
        ...
        if (nodeptr && nodeptr->node) {
            ...
            if (is_running) {
                val_clear_dirty_flag(nodeptr->node, &txcb->timestamp,
                                     txcb->txid, TRUE);
            } else {
                       ...
                       cfg_template_t *target)
{
    ...
    op_editop_t editop = editop = newval->editop;
    ...
    case OBJ_TYP_LEAF_LIST:
        ...
        if (newval->editvars->insert_mode == VAL_INS_MODE_POINT) {
        ...
        if (obj_is_system_ordered(newval->obj)) {
            ...
            agt_record_error(scb, &msg->mhdr, NCX_LAYER_CONTENT, res,
                             NULL, NCX_NT_STRING, newval->editvars->insertstr,
                             NCX_NT_VAL, newval);
            return res;
        }
        ...
        if (newval->editvars->insert_mode == VAL_INS_MODE_POINT) {
            xpath_result_t *result =
                agt_get_result_from_xpath(newval->editvars->insertxpcb,
                                          target->root, &res);
            if (result && res == NO_ERR) {
                newval->editvars->insertval =
                    agt_get_first_result_val(result);
                if (newval->editvars->insertval == NULL) {
             ...
             */
            val_value_t *simval =
                val_make_simval_obj(newval->obj,
                                    newval->editvars->insertstr, &res);
            if (res == NO_ERR && simval) {
                val_value_t *testval =
                    val_first_child_match_fast(curparent, simval, NULL);
                if (!testval) {
    ...
    case OBJ_TYP_LIST:
        ...
        if (newval->editvars->insertxpcb->validateres != NO_ERR) {
        ...
        } else if (newval->editvars->insert_mode == VAL_INS_MODE_POINT) {
            xpath_result_t *result =
                agt_get_result_from_xpath(newval->editvars->insertxpcb,
                                          target->root, &res);
            if (result && res == NO_ERR) {
            ...
            if (res == NO_ERR) {
                 ...
                 */
                val_value_t *testval =
                    val_make_from_insertxpcb(newval, &res);

                if (res == NO_ERR && testval) {
                    val_set_canonical_order(testval);
                    val_value_t *insertval =
                        val_first_child_match_fast(curparent,  testval, NULL);
                    if (!insertval) {
    ...
    default:
        if (newval->editvars && newval->editvars->insertop != OP_INSOP_NONE) {
            res = ERR_NCX_UNEXPECTED_INSERT_ATTRS;
            agt_record_error(scb, &msg->mhdr, NCX_LAYER_CONTENT, res,
                             NULL, newval->editvars->insertstr ? 
                          ...
                          val_value_t *runval)
{
    ...
    if (!val_get_child_deleted_flag(candval)) {
        if (LOGDEBUG3) {
            log_debug3("\ncheck_commit_deletes: skip %s ", runval->name);
        }
        return NO_ERR;
    }
    ...
    dlq_hdr_t *deleteQ = &msg->rpc_txcb->deleteQ;
    ...
    for (; curval != NULL; curval = val_get_next_child(curval)) {
        ...
        if (val_set_by_default(curval)) {
            /* only considered a delete if the node was set by
             * the client, not by server or schema default
             */
            continue;
        }

        /* check only database config nodes */
        if (obj_is_config(curval->obj)) {
            /* check if node deleted in source */
            val_value_t *matchval =
                val_first_child_match_fast(candval, curval, lastmatch);
            if (!matchval) {
                res = commit_delete_allowed(scb, msg, curval, TRUE);
                if (res == NO_ERR) {
                    ncx_backptr_t *backptr = ncx_new_backptr(curval);
                    if (backptr) {
                        dlq_enque(backptr, deleteQ);
                    } else {
                    ...
                    return res;      /* commit over; errors recorded */
                }
            } else {
                /* check if the matchval should be cached */
                if (sys_sorted && obj_is_system_ordered(matchval->obj)) {
                    lastmatch = matchval;
                } else if (lastmatch) {
                    if (lastmatch->obj != matchval->obj) {
                        lastmatch = matchval;
                    }
                } else {
                    lastmatch = matchval;
                     ...
                     boolean      *done)
{
    ...
    if (*done || (newval && obj_is_root(newval->obj))) {
        ...
        if (applyhere && val_get_child_deleted_flag(newval)) {
            applyhere = FALSE;
        }
        *done = applyhere;
        ...
        *done = TRUE;
        ...
        if (msg->rpc_txcb && cur_editop == OP_EDITOP_REPLACE &&
            msg->rpc_txcb->is_rollback) {
            /* this is a rollback, not a load_running_config */
            rollback = TRUE;

            /* for now trying to just add defaults instead of
             * trying to compare subtrees with missing nodes
             */
            if (!add_defs_done) {
                if (newval && newval->parent && newval->parent->obj &&
                    obj_is_root(newval->parent->obj)) {
                    /* add for top-level nodes only
                     * that way this will not be repreated for
                     * the descendant nodes
                     */
                    res = agt_set_val_defaults(newval, (target) ?
                                               target->root : NULL,
                                               curval);
                    if (res != NO_ERR) {
                        log_error("\nError: add defaults for rollback failed");
                    } else {
                        add_defs_done = TRUE;
                    }
                }
        ...
        applyhere = agt_apply_this_node_ex(cur_editop, newval, curval,
                                           !rollback);
        *done = applyhere;
    }

    if (res == NO_ERR && applyhere && newval) {
        ...
        if (obj_is_root(newval->obj)) {
            ...
            res = agt_set_val_defaults(newval, (target) ? target->root : NULL,
                                       curval);
            if (res != NO_ERR) {
    ...
    if (res == NO_ERR && applyhere) {
        if (LOGDEBUG) {
            val_value_t *logval = NULL;
            if (curval) {
                logval = curval;
            } else if (newval) {
                logval = newval;
        ...
        if (curval && val_is_virtual(curval)) {
             ...
             * node contents changed  !!!! WRONG !!!! */
            /* !!! The timstamp and lat txid are only updated
             * !!! once when the commit is done          */
            //cfg_update_last_ch_time(target, NULL);
            return NO_ERR;
        }
        ...
        case OP_EDITOP_COMMIT:
         ...
         case OP_EDITOP_CREATE:
             if (newval && curval) {
                 /* there must be a default leaf or NP-container */
                 undo = add_undo_node(msg, cur_editop, newval, 
                                      newval_marker, curval, NULL, parent);
                 if (undo == NULL) {
                     /* no detailed rpc-error is recorded here!!!
                 ...
                 if (obj_is_leafy(newval->obj)) {
                     undo->apply_res = res = val_merge(newval, curval);
                     undo->edit_action = AGT_CFG_EDIT_ACTION_SET;
                     restore_newnode(undo);
                 } else {
                     assert(val_set_by_default(curval) &&
                            "expecting default NP-container!");

                     /* treat this as a replace on the def NP-con */
                     val_insert_child(newval, curval, parent);
                     VAL_MARK_DELETED(curval);
                     undo->edit_action = AGT_CFG_EDIT_ACTION_REPLACE;
                     undo->free_curnode = TRUE;
         ...
         case OP_EDITOP_REMOVE:
             ...
             if (curval->parent) {
                 ...
                 val_set_child_deleted_flag(curval->parent);
             }
             ...
             } else if (obj_npcon_has_defaults(curval->obj)) {
                 /* convert this NP-container to its default value */
                 res = val_delete_default_npcon(curval,
 ...
 *********************************************************************/
static status_t
    invoke_simval_cb_validate (op_editop_t editop,
                               ses_cb_t  *scb,
                               rpc_msg_t  *msg,
                               val_value_t  *newval,
                               val_value_t  *curval,
                               ...
                               cfg_template_t *target)
{
    ...
    res = check_insert_attr(scb, msg, newval, curparent, target);
    if (res != NO_ERR) {
...
} /* invoke_simval_cb_apply_commit_check */
                      ...
                      boolean done)
{
    ...
    if (retres == NO_ERR && !done && curparent) {
        boolean sys_sorted = ncx_get_system_sorted();
        val_value_t      *chval, *curch, *nextch, *lastmatch = NULL;
        for (chval = val_get_first_child(curparent);
            ...
            if (curval) {
                curch = val_first_child_match_fast(curval, chval, lastmatch);
            } else {
                curch = NULL;
            }
            ...
            CHK_EXIT(res, retres);
            if (retres == NO_ERR && curch) {
                /* check if the curch should be cached */
                if (sys_sorted && obj_is_system_ordered(curch->obj)) {
                    lastmatch = curch;
                } else if (lastmatch) {
                    if (lastmatch->obj != curch->obj) {
                        lastmatch = curch;
                    }
                } else {
                    lastmatch = curch;
                                 ...
                                 val_value_t *curparent )
{
    ...
    if (editop == OP_EDITOP_COMMIT && !is_root && !val_dirty_subtree(useval)) {
        if (LOGDEBUG2) {
            log_debug2("\nSkipping unaffected %s in %s commit for %s:%s",
                       obj_get_typestr(useval->obj), agt_cbtype_name(cbtyp),
                       val_get_mod_name(useval), useval->name);
        }
        return NO_ERR;
    }

    if (LOGDEBUG3) {
        log_debug3("\nStart %s in %s commit for %s:%s",
                   obj_get_typestr(useval->obj), agt_cbtype_name(cbtyp),
                   val_get_mod_name(useval), useval->name);
    }
    ...
    if (cbtyp == AGT_CB_VALIDATE) {
        ...
        /* check if this is a load from startup and defaults need
         * to be added to top-level config nodes          */
        if (editop == OP_EDITOP_LOAD && newval && newval->parent &&
            newval->parent->obj && obj_is_root(newval->parent->obj)) {
                 ...
                 agt_cfg_undo_rec_t *undo)
{
    ...
    if (undo->editop == OP_EDITOP_LOAD) {
        ...
        val_clear_dirty_flag(undo->newnode, &txcb->timestamp,
                             txcb->txid, FALSE);
        undo->newnode = NULL;
        return;
    }
    ...
    boolean isrunning = (txcb->cfg_id == NCX_CFGID_RUNNING);
    boolean iscommit = txcb->is_commit;

    /* clear the object and XPath back pointers if needed
     * leave the dirty bits set in the candidate config so
     * the root-check code will skip all but the altered objects
     */
    if (undo->editop != OP_EDITOP_LOAD &&
        ncx_use_xpath_backptrs() && isrunning) {
        val_value_t *useval = undo->newnode ? undo->newnode : undo->curnode;
        if (useval && useval->obj) {
            ncx_cfg_t cfg_id = txcb->cfg_id;
            obj_flag_xpath_backptrs_dirty(useval->obj, cfg_id, FALSE);
        }
    }
    ...
    if (undo->curnode) {
        if (isrunning) {
            val_clear_dirty_flag(undo->curnode, &txcb->timestamp,
                                 txcb->txid, is_delete);
        } else {
    ...
    if (isrunning) {
        /* if the editvars are cleaned in the candidate then all
         * the insert info will be lost and the insert operation
         * will not work             */
    ...
    if (undo->newnode_marker) {
        ...
        if (isrunning && iscommit && undo->newnode && !is_delete) {
            /* need to clone the newnode and put it back
             * into the candidate config
             */
            status_t res = NO_ERR;
            val_value_t *cloneval =
                val_clone_config_data(undo->newnode, &res);
            if (cloneval) {
                doremove = FALSE;
                val_swap_child(cloneval, undo->newnode_marker);
                val_clear_dirty_flag(cloneval, &txcb->timestamp,
                                     txcb->txid, is_delete);
            } else {
                log_error("\nError: fill candidate for node '%s:%s' "
                          "failed (%s)",
                          val_get_mod_name(undo->newnode),
                          undo->newnode->name,
                          get_error_string(res));
                txcb->fill_candidate_failed = TRUE;
            }
        }
        if (doremove) {
    ...
    finish_extra_deletes(txcb, undo);
                   ...
                   cfg_template_t *target)
{
    ...
    /* if the source is from a PDU, then it will get cleaned up when
                    ...
                    cfg_template_t *target)
{
     ...
     */
    cfg_update_last_ch_time(target, &txcb->timestamp);
                      ...
                      cfg_template_t *target)
{
    ...
    for (; undo != NULL; undo = (agt_cfg_undo_rec_t *)dlq_nextEntry(undo)) {
        ...
        if (undo->editop != OP_EDITOP_LOAD && ncx_use_xpath_backptrs()) {
            val_value_t *useval =
                undo->newnode ? undo->newnode : undo->curnode;
            if (useval && useval->obj) {
                ncx_cfg_t cfg_id = msg->rpc_txcb->cfg_id;
                obj_flag_xpath_backptrs_dirty(useval->obj, cfg_id, FALSE);
            }
                     ...
                     val_value_t *curparent)
{
    ...
    if (LOGDEBUG2) {
        const xmlChar *targstr = (target) ? target->name : NCX_EL_NONE;
        ses_id_t sid = (scb) ? SES_MY_SID(scb) : 0;

        if (editop == OP_EDITOP_DELETE) {
            log_debug2("\n\n***** start commit deletes on %s for "
                       "session %d, transaction %llu *****\n",
                       targstr, sid, (unsigned long long)txcb->txid);
        } else {
            log_debug2("\n\n***** start %s phase on %s for "
                       "session %d, transaction %llu *****\n",
                       agt_cbtype_name(cbtyp), targstr, sid,
                       (unsigned long long)txcb->txid);
        }
    }
                          ...
                          val_value_t *runval)
{
    ...
    if (msg->rpc_txcb->is_commit) {
        dlq_hdr_t *deleteQ = &msg->rpc_txcb->deleteQ;
        while (res == NO_ERR && !dlq_empty(deleteQ)) {
            ncx_backptr_t *backptr =
                (ncx_backptr_t *)dlq_deque(deleteQ);
            val_value_t *curval = ncx_get_backptr_node(backptr);
         ...
         * if the matching node is not in the candidate,
         * then delete that node in the running config as well
         * cannot use val->flags because the confirmed-commit
         * or regular rollback candidate will not have edit flags set
         */
        for (curval = val_get_first_child(runval);
            ...
            if (obj_is_config(curval->obj)) {
                /* check if node deleted in source */
                val_value_t *matchval =
                    val_first_child_match_fast(candval, curval, NULL);
                if (!matchval) {
                    /* prevent the agt_val code from ignoring this node */
                    val_set_dirty_flag(curval);
                           ...
                           uint32 uninum)
{
    ...
    if (resnode == NULL) {
        if (LOGDEBUG3) {
            log_debug3("\nunique_chk: %s:%s skip empty resnode list # %u (%s)", 
    ...
    for (; resnode && retres == NO_ERR; 
        ...
        if (res == NO_ERR) {
            ...
            unique_set_t *uset = new_unique_set();
            if (uset == NULL) {
                retres = ERR_INTERNAL_MEM;
                continue;
            }
    ...
    while (!dlq_empty(&usetQ)) {
        unique_set_t *uset = (unique_set_t *)dlq_deque(&usetQ);
...
*   layer == NCX layer calling this function (for error purposes only)
*   dbcheck == TRUE if checking a datastore; FALSE if checking other
*      
* OUTPUTS:
*   if msg not NULL:
*      msg->msg_errQ may have rpc_err_rec_t structs added to it 
...
*********************************************************************/
static status_t 
    instance_check (agt_cfg_transaction_t *txcb,
                    ses_cb_t *scb,
                    xml_msg_hdr_t *msg,
                    obj_template_t *obj,
                    val_value_t *val,
                    ...
                    boolean dbcheck)
{
    ...
    /* check if the object is dirty and this test can be skipped */
    if (dbcheck && txcb && scb && scb->type != SES_TYP_DUMMY) {
        ncx_cfg_t cfg_id;
        if (txcb->is_commit) {
            cfg_id = NCX_CFGID_CANDIDATE;
        } else {
            cfg_id = txcb->cfg_id;
        }
        if (ncx_use_xpath_backptrs() && !obj_is_dirty(obj, cfg_id)) {
            if (LOGDEBUG2) {
                log_debug2("\nSkipping instance_check for '%s', not dirty",
                           obj_get_name(obj));
            }
            return NO_ERR;
        }
...
*********************************************************************/
static status_t 
    choice_check_agt (agt_cfg_transaction_t *txcb,
                      ses_cb_t  *scb,
                      xml_msg_hdr_t *msg,
                      ...
                      boolean dbcheck)
{
    ...
    for (; testobj != NULL; testobj = obj_next_child(testobj)) {
        res = instance_check(txcb, scb, msg, testobj, val, valroot,
                             layer, dbcheck);
        CHK_EXIT(res, retres);
                                  ...
                                  val_value_t *curval )
{
    ...
    for ( ; must; must = (xpath_pcb_t *)dlq_nextEntry(must)) {
        ...
        xpath_result_t *result =
            xpath1_eval_expr(must, curval, root, FALSE, TRUE, &res);
        if ( !result || res != NO_ERR ) {
                            ...
                            val_value_t *root )
{
   ...
   /* first get all the instances of this object if needed */
    if (ct->result) {
        ...
        if (txcb->start_bad) {
            if (LOGDEBUG3) {
                log_debug3("\nNot reusing bad-startup XPath result for %s",
                           ct->objpcb->exprstr);
            }
            force_clean = TRUE;
        } else if (ct->result_txid && ct->result_txid == txcb->txid) {
            if (LOGDEBUG3) {
                log_debug3("\nReusing XPath result for %s (same txid)",
                           ct->objpcb->exprstr);
            }
        } else if (ct->obj && !obj_is_dirty(ct->obj, cfg_id)) {
            if (LOGDEBUG3) {
                log_debug3("\nReusing XPath result for %s (obj clean)",
                           ct->objpcb->exprstr);
            }
        } else {
            if (LOGDEBUG3) {
                log_debug3("\nNot reusing dirty XPath result for %s",
                           ct->objpcb->exprstr);
            }
            force_clean = TRUE;
        }
...
*              == FALSE to just check the current level
*   dbcheck == TRUE if checking a datastore; FALSE if checking other
* OUTPUTS:
*   if msg not NULL:
*      msg->msg_errQ may have rpc_err_rec_t structs added to it 
...
*********************************************************************/
static status_t 
    run_instance_check (agt_cfg_transaction_t *txcb,
                        ses_cb_t *scb,
                        xml_msg_hdr_t *msg,
                        val_value_t *valset,
                        val_value_t *valroot,
                        ...
                        boolean dbcheck)
{
    ...
    if (LOGDEBUG4) {
        log_debug4("\nrun_instance_check: %s:%s start", 
                   obj_get_mod_name(valset->obj), valset->name);
    }
    ...
    if (all_levels && val_child_cnt(valset)) {
        ...
        for (; chval != NULL; chval = val_get_next_child(chval)) {
            ...
            if (!(obj_is_root(chval->obj) || obj_is_leafy(chval->obj))) {
                ...
                res = run_instance_check(txcb, scb, msg, chval, valroot, 
                                         layer, all_levels, dbcheck);
                CHK_EXIT(res, retres);
            }
        }
    }
    ...
    for (; chobj != NULL; chobj = obj_next_child(chobj)) {
        ...
        if (chobj->objtype == OBJ_TYP_CHOICE) {
            res = choice_check_agt(txcb, scb, msg, chobj, valset, 
                                   valroot, layer, dbcheck);
        } else {
            res = instance_check(txcb, scb, msg, chobj, valset, valroot,
                                 layer, dbcheck);
        }
...
*********************************************************************/
static status_t 
    run_obj_commit_tests (agt_cfg_transaction_t *txcb,
                          ...
                          uint32 testmask)
{
    ...
    if (testflags & AGT_TEST_INSTANCE_MASK) {
        ...
        if (ct) {
            res = run_instance_check(txcb, scb, msghdr, val, root,
                                     NCX_LAYER_CONTENT, FALSE, TRUE);
            CHK_EXIT(res, retres);
        } else {
            ...
            for (; mod != NULL; mod = ncx_get_next_module(mod)) {
                obj_template_t *obj =
                    ncx_get_first_data_object_pick(mod, TRUE);
                for (; obj != NULL; 
                     obj = ncx_get_next_data_object_same(mod, obj)) {
                    ...
                    if (obj->objtype == OBJ_TYP_CHOICE) {
                        res = choice_check_agt(txcb, scb, msghdr, obj, val,
                                               root, NCX_LAYER_CONTENT, TRUE);
                    } else {
                        res = instance_check(txcb, scb, msghdr, obj, val,
                                             root, NCX_LAYER_CONTENT, TRUE);
                    }
                    CHK_EXIT(res, retres);
                }
...
status_t 
    agt_val_instance_check (ses_cb_t *scb,
                            ...
                            ncx_layer_t layer)
{
    return run_instance_check(NULL, scb, msg, valset, valroot, layer,
                              TRUE, FALSE);
                        ...
                        val_value_t *root)
{
     ...
     * during editing  */
    res = run_obj_commit_tests(txcb, profile, scb, msghdr, NULL, root, root,
    ...
    for (; ct != NULL; ct = (agt_cfg_commit_test_t *)dlq_nextEntry(ct)) {
        ...
        for (; resnode != NULL; resnode = xpath_get_next_resnode(resnode)) {
            ...
            res = run_obj_commit_tests(txcb, profile, scb, msghdr, ct,
                                       valnode, root, tests);
            if (res != NO_ERR) {
                valnode->res = res;
...
#define ALLOW_SKIP_EMPTY_COMMIT 1
    ...
    if (!cfg_get_dirty_flag(source)) {
        ...
        if (profile->agt_has_startup) {
             ...
             */
            if (LOGDEBUG) {
                log_debug("\nSkipping apply commit, candidate not dirty");
            }
        } else {
            ...
            if (profile->agt_usestartup && dlq_empty(&target->load_errQ)) {
                /* no startup errors and startup was attempted */
                if (LOGINFO) {
                    log_info("\nSkipping commit, candidate not dirty");
                }
            } else {
                /* startup errors or --no-startup used */
                if (LOGINFO) {
                    log_debug("\nSkipping commit, but saving running "
                              "to NV-storage due to load errors or "
                              "--no-startup");
                }
...
/********************************************************************
* FUNCTION agt_val_clean_cached_results
* 
* clear the xpath_backptr dirty flags and object dirty flags
* in all the commit_test record; this is done for discard-changes
* on the candidate
*
*********************************************************************/
void
    agt_val_clean_cached_results (void)
{
    agt_profile_t *profile = agt_get_profile();
    ...
    for (; ct != NULL; ct = (agt_cfg_commit_test_t *)dlq_nextEntry(ct)) {
        if (ct->obj) {
            obj_flag_xpath_backptrs_dirty(ct->obj, NCX_CFGID_RUNNING, FALSE);
            obj_flag_xpath_backptrs_dirty(ct->obj, NCX_CFGID_CANDIDATE, FALSE);
        }
        if (ct->result) {
            xpath_free_result(ct->result);
            ct->result = NULL;
