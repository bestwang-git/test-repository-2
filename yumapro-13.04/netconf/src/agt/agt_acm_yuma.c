    ...
    new_acm_cache (void)
{
    ...
    acm_cache->mode = agt_acm_get_acmode();
                              ...
                              const obj_template_t *rpcobj)
{
     ...
     * ncx:secure or nacm:very-secure and
     * deny access if so
     */
                     ...
                     boolean *done)
{
    ...
    *done = FALSE;
    ...
    for (rpcrule = val_find_child(rulesval, 
        ...
        modname = val_find_child(rpcrule, AGT_ACM_MODULE,
        ...
        rpcname = val_find_child(rpcrule, AGT_ACM_MODULE, nacm_N_rpcName);
        if (!rpcname) {
            res = SET_ERROR(ERR_INTERNAL_VAL);
            continue;
        }
        ...
        if (xml_strcmp(obj_get_name(rpcobj), VAL_STR(rpcname))) {
            continue;
        }

        /* check if this is the right module */
        if (xml_strcmp(obj_get_mod_name(rpcobj), VAL_STR(modname))) {
            /* deal with the ietf-netconf == yuma-netconf hack */
            if (xml_strcmp(VAL_STR(modname), NCXMOD_IETF_NETCONF) || 
                xml_strcmp(obj_get_mod_name(rpcobj), NCXMOD_YUMA_NETCONF)) {
                continue;
            }
        }
         ...
         */
        done2 = FALSE;
        granted = check_access_bit(rpcrule, nacm_E_allowedRights_exec,
                                   usergroups, &done2);
        if (done2) {
            *done = TRUE;
        } else {
                            ...
                            op_editop_t editop)
{
    ...
    if (!xml_strcmp(access, nacm_E_allowedRights_write)) {
        iswrite = TRUE;
        logfn = (agt_acm_get_log_writes()) ? log_debug2 : log_noop;
    } else {
        iswrite = FALSE;
        logfn = (agt_acm_get_log_reads()) ? log_debug4 : log_noop;
    }
    ...
    case AGT_CB_COMMIT:
        ...
        case OP_EDITOP_CREATE:
            if (newval != NULL && VAL_BOOL(newval)) {
                if (agt_acm_get_acmode() != AGT_ACMOD_ENFORCING) {
    ...
    agt_acm_yuma_init1 (void)
{
    ...
    res = ncxmod_load_module(AGT_ACM_MODULE, NULL, 
                             &profile->agt_savedevQ, &nacmmod);
    if (res != NO_ERR) {
        return res;
    }
    ...
    agt_acm_yuma_init2 (void)
{
    ...
    if (added) {
        ...
        res = agt_set_val_defaults(nacmval, nacmval->parent, NULL);
        if (res != NO_ERR) {
            return res;
        }
    }
                              ...
                              const obj_template_t *rpcobj)
{
    ...
    log_debug2("\nagt_acm_yuma: %s (%s)", retval ? "PERMIT" : "DENY",
               substr ? substr : NCX_EL_NONE);

    return retval;

}   /* agt_acm_yuma_rpc_allowed */
                                ...
                                const obj_template_t *notifobj)
{
    val_value_t             *nacmroot, *rulesval;
    agt_acm_usergroups_t    *usergroups;
    logfn_t logfn = (agt_acm_get_log_reads()) ? log_debug2 : log_noop;
                                    ...
                                    op_editop_t editop)
{
    ...
    return valnode_access_allowed(msg->acm_cache, user, val, newval, curval,
                                  nacm_E_allowedRights_write, editop);

} /* agt_acm_yuma_write_allowed */
...
*
*********************************************************************/
void agt_acm_yuma_clear_session_cache (ses_cb_t *scb)
{
    free_acm_cache(scb->acm_cache);
    scb->acm_cache = NULL;
} /* agt_acm_yuma_clear_session_cache */
...
*
*********************************************************************/
void agt_acm_yuma_invalidate_session_cache (ses_cb_t *scb)
{
    agt_acm_cache_t *cache = scb->acm_cache;
    if (cache) {
        cache->flags &= ~FL_ACM_CACHE_VALID;
    }

} /* agt_acm_yuma_invalidate_session_cache */
...
*********************************************************************/
boolean agt_acm_yuma_session_cache_valid (const ses_cb_t *scb)
{
    agt_acm_cache_t *cache = scb->acm_cache;
    if (cache) {
        return (cache->flags & FL_ACM_CACHE_VALID) ? TRUE : FALSE;
    }
    return FALSE;
} /* agt_acm_yuma_session_cache_valid */

/* END file agt_acm_yuma.c */
