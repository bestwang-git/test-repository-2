                       ...
                       agt_profile_t *agt_profile)
{
    ...
    /* --home and --yumapro-home parameters */
    val_set_home_parms(valset);
    ...
    if (val && val->res == NO_ERR) {
        ...
        if (!val_set_by_default(val)) {
            log_warn("\nWarning: eventlog-size CLI parameter ignored "
...
#ifdef EVAL_VERSION
        if (!val_set_by_default(val)) {
            log_warn("\nWarning: idle-timeout CLI parameter ignored "
                     "in evaluation version\n");
        }
#else
    ...
    /* max-sessions param */
    val = val_find_child(valset, AGT_CLI_MODULE, AGT_CLI_MAX_SESSIONS);
    if (val && val->res == NO_ERR) {
#ifdef EVAL_VERSION
        if (!val_set_by_default(val)) {
            log_warn("\nWarning: max-sessions CLI parameter ignored "
                     "in evaluation version\n");
        }
#else
    ...
    /* get with-notifications param */
    val = val_find_child(valset, AGT_CLI_MODULE, NCX_EL_WITH_NOTIFICATIONS);
    if (val && val->res == NO_ERR) {
        agt_profile->agt_use_notifications = VAL_BOOL(val);
    }
    ...
    /* yangapi-server-url param */
    val = val_find_child(valset, AGT_CLI_MODULE, NCX_EL_YANGAPI_SERVER_URL);
    if (val && val->res == NO_ERR) {
        agt_profile->agt_yangapi_server_url = VAL_STR(val);
    }
                           ...
                           help_mode_t *showhelpmode)
{
    ...
    *showhelpmode = HELP_MODE_NONE;
    ...
    ncx_module_t *mod = ncx_find_module(AGT_CLI_MODULE, NULL);
    if (mod) {
    ...
    if (argc > 0) {
        valset = cli_parse(NULL, argc, argv, obj, FULLTEST, 
                           PLAINMODE, TRUE, CLI_MODE_PROGRAM, &res);
        if (res != NO_ERR) {
            if (valset) {
    ...
    if (valset != NULL) {
        ...
        val = val_find_child(valset, AGT_CLI_MODULE, NCX_EL_CONFIG);
        if (val) {
            if (val->res == NO_ERR) {
#ifdef EVAL_VERSION
                log_warn("\nWarning: Ignoring --config CLI parameter: "
                         "not supported in evaluation version\n");
#else
        ...
        } else if (val_find_child(valset, AGT_CLI_MODULE, 
                                  NCX_EL_NO_CONFIG)) {
            log_info("\nSkipping default config file because "
                     "--no-config specified");
        } else {
#ifdef EVAL_VERSION
            log_warn("\nWarning: Skipping default CLI .conf load: "
                     "not supported in evaluation version\n");
#else
            FILE *fp = fopen((const char *)AGT_DEF_CONF_FILE, "r");
            if (fp != NULL) {
                fclose(fp);
        ...
        res = val_set_message_indent_parm(valset);
        if (res != NO_ERR) {
            val_free_value(valset);
            return res;
        }
        ...
        val = val_find_child(valset, AGT_CLI_MODULE, NCX_EL_MATCH_NAMES);
        if (val && val->res == NO_ERR) {
            ncx_name_match_t match_type = 
                ncx_get_name_match_enum(VAL_ENUM_NAME(val));
            if (match_type == NCX_MATCH_NONE) {
                val_free_value(valset);
                return ERR_NCX_INVALID_VALUE;
            }
            agt_profile->agt_match_names = match_type;
        }

        /* get the alt-names parameter */
        val = val_find_child(valset, AGT_CLI_MODULE, NCX_EL_ALT_NAMES);
        if (val && val->res == NO_ERR) {
            agt_profile->agt_alt_names = VAL_BOOL(val);
        }

        /* get the wildcard-keys parameter */
        val = val_find_child(valset, AGT_CLI_MODULE, NCX_EL_WILDCARD_KEYS);
        if (val && val->res == NO_ERR) {
            agt_profile->agt_wildcards = VAL_BOOL(val);
        }

        /* check the system-notifications param */
        val = val_find_child(valset, AGT_CLI_MODULE, 
                             NCX_EL_SYSTEM_NOTIFICATIONS);
        if (val && val->res == NO_ERR) {
            ncx_list_t *bits = &VAL_BITS(val);
            if (ncx_string_in_list((const xmlChar *)"ietf", bits)) {
                agt_profile->agt_ietf_system_notifs = TRUE;
            } else {
                agt_profile->agt_ietf_system_notifs = FALSE;
            }
            if (ncx_string_in_list((const xmlChar *)"yuma", bits)) {
                agt_profile->agt_yuma_system_notifs = TRUE;
            } else {
