                    ...
                    int32 indent)
{
    ...
    dlq_hdr_t descendantQ;
    dlq_createSQue(&descendantQ);

    int32 indentamount = ses_indent_count(scb);

    /* find the top-level node for this result node */
    val_value_t *topval = curval;
    while (topval->parent && topval->parent != ceilingval) {
        topval = topval->parent;
    }

    /* this code should not be hit for matching siblings
     * if the first node removed from the node set is a sibling
     * of other nodes in the resultQ, then it will be generated
     * out of order; now if is inserted in correct order with
     * other siblings (see below) instead of recursing to get here
     */
    if (topval == curval) {
    ...
    for (testnode = (xpath_resnode_t *)dlq_firstEntry(resultQ);
        ...
        if (xpath1_check_node_exists(pcb, &dummyQ, testnode->node.valptr)) {
            dlq_remove(testnode);
     ...
     */
    boolean dowrite =
        agt_acm_val_read_allowed(&msg->mhdr, scb->username, topval);
     ...
     */
    if (dowrite) {
        xml_wr_begin_elem_ex(scb, &msg->mhdr, ceilingval->nsid, topval->nsid,
                             topval->name, &topval->metaQ, FALSE, 
                             indent, FALSE);
    }
    ...
    if (val_has_index(topval)) {
        ...
        for (valindex = val_get_first_index(topval);
            ...
            if (dowrite) {
                xml_wr_full_val(scb, &msg->mhdr, valindex->val, indent);
                if (valindex->val == curval) {
                    curval_done = TRUE;
                }
            }

            testnode = find_val_resnode(&descendantQ, valindex->val);
            if (testnode) {
                dlq_remove(testnode);
    ...
    /* check if it's time to output the curval */
    boolean do_curval = (curval->parent == topval) ? TRUE : FALSE;
    ...
    for (testnode = (xpath_resnode_t *)dlq_firstEntry(&descendantQ);
        ...
        if (testnode->node.valptr->parent == topval) {
            ...
            dlq_remove(testnode);
            if (dowrite) {
                if (do_curval && !curval_done) {
                    int ret = val_compare(curval, testnode->node.valptr);
                    if (ret < 0) {
                        if (getop) {
                            xml_wr_full_val(scb, &msg->mhdr, curval, indent);
                        } else {
                            xml_wr_full_check_val(scb, &msg->mhdr, curval,
                                                  indent, agt_check_config);
                        }
    ...
    if (dowrite && !curval_done) {
        /* move the ceiling closer to the curval and try again
         * with the current value
         */
        output_resnode(scb, msg, pcb, &descendantQ, curval, 
                       topval, getop, indent);
    }
    ...
    if (dowrite) {
        xml_wr_end_elem(scb, &msg->mhdr, topval->nsid,
                        topval->name, indent);
    }
...
*
* Output the result nodeset in NETCONF format
* Collapse all the subtrees together whenever possible
* Add key leafs in lists if the filter left them out
*
* INPUTS:
                   ...
                   int32 indent)
{
    ...
    while (resnode) {
        val_value_t *curval = resnode->node.valptr;
        ...
        if (curval == pcb->val_docroot) {
            if (getop) {
                ...
                xml_wr_check_val(scb, &msg->mhdr, curval, indent, 
                                 agt_check_config);
            }
        ...
        output_resnode(scb, msg, pcb, &result->r.nodeQ, curval, 
                       pcb->val_docroot, getop, indent);
                             ...
                             int32 indent)
{
    ...
    val_value_t *selectval = msg->rpc_filter.op_filter;

    xpath_result_t *result =
        xpath1_eval_xmlexpr(scb->reader, selectval->xpathpcb,
                            cfg->root, cfg->root, FALSE,
                            !getop, &res);
    ...
    if (result && (res == NO_ERR) && 
        ...
        /* output filter */
        output_result(scb, msg, selectval->xpathpcb, result, getop, indent);
    }
...

{
     ...
     */
    boolean readallowed =
        agt_acm_val_read_allowed(msghdr, scb->username, val);
    ...
    status_t res = NO_ERR;
    boolean getop = TRUE;   /* do not skip config=fale nodes */
     ...
     */
    xpath_result_t *result =
        xpath1_eval_xmlexpr(scb->reader, selectval->xpathpcb, val, val,
                            FALSE, !getop, &res);

    boolean retval = FALSE;
    if (result && (res == NO_ERR) && (result->restype == XP_RT_NODESET)) {
